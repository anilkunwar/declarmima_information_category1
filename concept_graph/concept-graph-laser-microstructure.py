...
def generate_edge_explanations(nx_graph: nx.Graph, tokenizer, model, backend_type: str, 
    ...
    Also stores a formatted multi-line version for better tooltip readability.
    """
    ...
    for idx, (u, v, data) in enumerate(edges_to_process):
        ...
        try:
            if backend_type == "ollama":
                ...
                try:
                    ...
                    with torch.no_grad():
                        outputs = model.generate(inputs.input_ids, max_new_tokens=120, temperature=0.3, 
        ...
        # Store raw explanation
        nx_graph[u][v]['explanation'] = explanation

        # Store formatted explanation with newlines for tooltips (max ~50 chars per line)
        formatted = _format_tooltip_text(explanation, max_line_length=45)
        nx_graph[u][v]['explanation_formatted'] = formatted
...
def _format_tooltip_text(text: str, max_line_length: int = 45) -> str:
    """Format long text into multiple lines for better tooltip readability."""
    if not text:
        return text
    words = text.split()
    lines = []
...
def get_edge_explanations_df(nx_graph: nx.Graph) -> pd.DataFrame:
    """Extract all edge explanations into a downloadable DataFrame."""
    rows = []
    for u, v, data in nx_graph.edges(data=True):
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis",
    ...
    Handles large node counts by using unique IDs and remainder branchvalues to avoid rendering issues.
    """
    if not labels or len(labels) < 2:
        st.info("Not enough categories for sunburst chart.")
        return

    # Plotly Sunburst requires unique labels. If duplicates exist, append index.
    # Also, large datasets with branchvalues='total' can hit numerical precision issues [^6^].
    # We use 'remainder' for large datasets and deduplicate labels with unique IDs.

    n_items = len(labels)
    use_remainder = n_items > 50  # Switch to remainder for large datasets to avoid blank charts

    # Create unique IDs for each label to prevent duplicate label issues [^5^]
    unique_ids = []
    seen = {}
    ...
    # Map parent labels to unique IDs
    parent_ids = []
    for p in parents:
        if p == "":
            parent_ids.append("")
        else:
            # Find the first occurrence of this parent
            for i, lab in enumerate(labels):
                if lab == p:
    ...
    fig = go.Figure(go.Sunburst(
        ...
        parents=parent_ids,
        values=values,
        ids=unique_ids,  # Explicit IDs prevent duplicate label issues [^5^]
    ...
    if use_remainder:
        st.caption("💡 Using 'remainder' branch mode for large dataset stability.")
...
def render_graph_pyvis_custom(nx_graph, concept_abstract_map, physics_enabled=True, 
    ...
    for u, v in nx_graph.edges():
        ...
        # Edge explanation tooltip with formatted newlines
        explanation = nx_graph[u][v].get('explanation_formatted', '')
        if not explanation:
            explanation = nx_graph[u][v].get('explanation', '')
            explanation = _format_tooltip_text(explanation, max_line_length=40)
        title_text = f"<b>🔗 {u} ↔ {v}</b><br>━━━━━━━━━━━━<br>Weight: {w:.2f}<br>Type: {edge_type}"
        if explanation:
            title_text += f"<br>━━━━━━━━━━━━<br><b>💡 Explanation:</b><br>{explanation}"
...
def render_graph_plotly_white(nx_graph, concept_abstract_map, cmap_name="viridis", custom_labels=None,
    ...
    for u, v in nx_graph.edges():
        ...
        explanation = nx_graph[u][v].get('explanation_formatted', '')
        if not explanation:
            explanation = _format_tooltip_text(nx_graph[u][v].get('explanation', ''), max_line_length=40)
        hover_info = f"<b>{u} ↔ {v}</b><br>━━━━━━━━<br>Weight: {w:.2f}<br>Type: {edge_type}"
        if explanation:
            hover_info += f"<br>━━━━━━━━<br><b>💡 Explanation:</b><br>{explanation}"
    ...
    if len(nx_graph.edges()) > 0:
        ...
        for i, (u, v, w) in enumerate(edge_list[:15], 1):
            ...
            if explanation:
                ...
                formatted_exp = _format_tooltip_text(explanation, max_line_length=60)
                for line in formatted_exp.split('<br>'):
                    st.caption(f"&nbsp;&nbsp;&nbsp;&nbsp;💡 *{line}*")
    ...
    if st.session_state.analysis_data is not None:
        ...
        with viz_tab:
            ...
            with st.expander("📈 Research Domain Hierarchy (Sunburst)", expanded=False):
                ...
                with col2:
                    sun_label_size = st.slider("Label size", 20, 50, st.session_state.get('sunburst_label_size', 12), key="sun_label_live")
                with col3:
            ...
            if st.session_state.get('radar_enabled', True):
                with st.expander("📡 Concept Radar Chart (Multi-Dimensional Comparison)", expanded=False):
                    ...
                    with col2:
                        radar_font = st.slider("Font size", 8, 50, 20, key="radar_font_live")
                    with col3:
                        radar_linewidth = st.slider("Line width", 1, 15, 5, key="radar_lw_live")
                    col4, col5, col6 = st.columns(3)
                    with col4:
        ...
        with distill_tab:
            ...
            with col2:
                ...
                top_n_distill = st.slider("Show Top N", 2, min(100, len(distill_df)), min(20, len(distill_df)), key="top_n_distill")
            with col1:
        ...
        with export_tab:
            ...
            # Edge Explanations Download Section
            exp_df = get_edge_explanations_df(nx_graph)
            if not exp_df.empty:
                with st.expander("💡 Edge Explanations", expanded=True):
                    st.markdown(f"**{len(exp_df)}** edges have explanations")
                    st.dataframe(exp_df[['source', 'target', 'edge_type', 'explanation']], use_container_width=True)

                    col1, col2 = st.columns(2)
                    with col1:
                        csv_exp = exp_df.to_csv(index=False).encode('utf-8')
                        st.download_button("📄 Download Explanations (CSV)", data=csv_exp, 
                                          file_name="edge_explanations.csv", mime="text/csv", key="exp_csv")
                    with col2:
                        json_exp = exp_df.to_json(orient='records', indent=2).encode('utf-8')
                        st.download_button("📄 Download Explanations (JSON)", data=json_exp,
                                          file_name="edge_explanations.json", mime="application/json", key="exp_json")

                    # Text-based download with formatted newlines
                    text_lines = ["DECLARMIMA Edge Explanations", "=" * 50, ""]
                    for _, row in exp_df.iterrows():
                        text_lines.append(f"Source: {row['source']}")
                        text_lines.append(f"Target: {row['target']}")
                        text_lines.append(f"Type: {row['edge_type']}")
                        text_lines.append(f"Weight: {row['weight']:.3f}")
                        text_lines.append(f"Explanation: {row['explanation']}")
                        text_lines.append("-" * 40)
                        text_lines.append("")
                    text_content = "\n".join(text_lines)
                    st.download_button("📝 Download Explanations (Text)", data=text_content.encode('utf-8'),
                                      file_name="edge_explanations.txt", mime="text/plain", key="exp_txt")
