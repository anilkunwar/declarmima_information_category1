...
""")
    ...
    if st.button("🚀 Analyze Abstracts", type="primary", use_container_width=True):
        ...
        # Initialize progress bar with 0.0 (not 0)
        ...
        try:
            ...
            # Step 4: Embeddings - Get correct embedding dimension
            ...
            def _training_progress(epoch, loss):
                # Map epoch (0 to TRAIN_EPOCHS) to progress (0.50 to 0.80)
                progress_value = 0.50 + (epoch / TRAIN_EPOCHS) * 0.30
                # Clamp to valid range [0.0, 1.0] to prevent overflow
            ...
            # Step 6: Scoring - Pass node_features to scoring function
            with st.status("📈 Scoring novel directions..."):
                ...
                # Pass node_features (384-dim) as second arg, final_emb as third
            ...
            # Step 7: LLM curation - WITH FIXES
            with st.status("✍️ Generating hypotheses..."):
                # Get max_hypotheses from sidebar
                ...
                directions_df = generate_research_directions(
                    ...
                    tokenizer, 
                    llm_model,
                    max_hypotheses=max_hyp  # Pass the limit parameter
                )
            ...
            # ✅ FIX: Interactive graph - COMPLETE REWRITE FOR RELIABLE RENDERING
            st.subheader("🌐 Concept Graph")
            
            # Check if graph has nodes AND edges before rendering
            if len(nx_graph.nodes()) == 0:
                ...
                st.warning("⚠️ Graph has nodes but no edges — switching to semantic-only fallback")
                # Build semantic-only graph as fallback
                ...
                if nx_graph.number_of_edges() == 0:
                    st.info("💡 Showing text-based graph summary instead...")
                    # Show text summary
                    st.markdown(f"- **Nodes**: {len(nx_graph.nodes())}")
                    st.markdown(f"- **Edges**: 0 (no semantic connections found)")
                else:
                    st.success(f"✅ Fallback graph: {nx_graph.number_of_edges()} edges created")
            
            if len(nx_graph.nodes()) > 0:
                try:
                    # ✅ FIX: Limit graph size for browser rendering
                    if len(nx_graph.nodes()) > 80:
                        st.warning("📊 Graph too large — sampling top 80 nodes by degree")
                        # Sample nodes by degree centrality
                        degrees = dict(nx_graph.degree())
                        top_nodes = sorted(degrees.keys(), key=lambda x: degrees[x], reverse=True)[:80]
                        nx_graph = nx_graph.subgraph(top_nodes).copy()
                    
                    # ✅ FIX: Use PyVis with inline CDN resources for Streamlit compatibility
                    net = Network(
                        height="650px", 
                        width="100%", 
                        bgcolor="#1e1e1e", 
                        ...
                        select_menu=True,
                        notebook=False,
                        cdn_resources='in_line'  # ✅ CRITICAL: Inline JS/CSS for Streamlit
                    )
                    
                    # ✅ FIX: Use stable physics parameters (not -80000 gravity!)
                    net.barnes_hut(
                        gravity=-2000,      # ✅ Reduced from -80000
                        spring_length=150,   # ✅ Added spring length for stability
                    ...
                    # ✅ FIX: Generate HTML directly (no file I/O needed)
                    html_content = net.generate_html()
                    
                    # ✅ FIX: Use st.components.v1.html() NOT iframe() for raw HTML content
                    ...
                    # ✅ EXTRA FIX: Offer download of HTML file as alternative
                    try:
                        net_fallback = Network(height="600px", width="100%", cdn_resources='in_line')
                        net_fallback.from_nx(nx_graph)
                        html_download = net_fallback.generate_html()
                        st.download_button(
            ...
            # Always clean up memory after pipeline completes or fails
            gc.collect()
            if DEVICE.type == 'cuda':
                torch.cuda.empty_cache()
    ...
    **🔍 If graph shows black screen:**
    1. ✅ Code now uses `st.components.v1.html()` instead of `iframe()` — this fixes 90% of blank graph issues
    2. ✅ PyVis configured with `cdn_resources='in_line'` to avoid CDN blocking
    3. ✅ Physics engine tuned with `gravity=-2000` (not -80000) for stable rendering
    4. ✅ Graph size limited to 80 nodes to prevent browser crashes
    5. Check browser console for JavaScript errors (F12 → Console tab)
    6. Try refreshing the page after analysis completes
    7. Use the "Download Interactive Graph" button to open in a new tab
    """)
