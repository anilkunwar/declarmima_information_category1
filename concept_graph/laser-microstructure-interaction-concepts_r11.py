...
st.set_page_config(
    ...
    menu_items={
        ...
        'About': "Laser & Alloy Microstructure Research Direction Generator"
    }
)
...
def load_lightweight_llm():
    ...
    try:
        ...
        # Memory optimization: use float16 on CUDA, float32 on CPU
        torch_dtype = torch.float16 if DEVICE.type == 'cuda' else torch.float32
        
        # Try to load with device_map for automatic GPU/CPU placement
        ...
        # Fallback: load on CPU if GPU fails
        tokenizer = AutoTokenizer.from_pretrained(LLM_NAME, trust_remote_code=True)
        model = AutoModelForCausalLM.from_pretrained(
            LLM_NAME, 
...
def compute_research_direction_scores(
    ...
    # Filter candidate pairs BEFORE computing GNN scores
    candidate_pairs = []
    ...
    with torch.no_grad():
        ...
        # Correct pair feature concatenation
...
# ==========================================
# STEP 7: LLM CURATION OF RESEARCH DIRECTIONS (FIXED)
# ==========================================
def generate_research_directions(top_pairs_df, tokenizer, model, max_hypotheses=10):
    """Generate LLM-curated research hypotheses for top-scoring concept pairs
    
    FIXED: Added error handling, memory cleanup, progress tracking, and limit parameter
    """
...
Avoid generic statements. Focus on laser-matter interaction, solidification, or phase transformation mechanisms."""
    ...
    # Create progress bar for hypothesis generation
    ...
    for idx in range(total_rows):
        try:
            ...
            # CRITICAL: Clear memory after each iteration to prevent OOM
            del inputs, outputs
            if DEVICE.type == 'cuda':
...
# ==========================================
# VISUALIZATION MODULES: PyVis AND Plotly
# ==========================================

def render_graph_pyvis(nx_graph, concept_abstract_map, embed_model=None):
    """Render concept graph using PyVis with Streamlit-compatible settings"""
    ...
    # Initialize PyVis network with inline CDN resources
    net = Network(
        height="650px", 
        width="100%", 
        bgcolor="#1e1e1e", 
        ...
        select_menu=True,
        notebook=False,
        cdn_resources='in_line'  # Critical for Streamlit compatibility
    )
    ...
    # Render using st.components.v1.html (NOT iframe)
...
def render_graph_plotly(nx_graph, concept_abstract_map):
    """Render concept graph using Plotly (more Streamlit-stable alternative)"""
    ...
    # Compute layout with spring algorithm
    ...
    for node in nx_graph.nodes():
        ...
        node_size.append(max(8, min(40, deg * 3 + 10)))
    ...
    fig = go.Figure(data=[edge_trace, node_trace],
                   layout=go.Layout(
                       ...
                       annotations=[dict(
                           text="Drag nodes to rearrange • Hover for details • Scroll to zoom",
                           showarrow=False,
...
def render_graph_fallback(nx_graph, concept_abstract_map):
    """Fallback text-based graph summary when visualizations fail"""
...
def main():
    ...
    with st.sidebar:
        ...
        # ✅ NEW: Visualization selector dropdown
        ...
        viz_backend = st.selectbox(
            ...
            index=0,
            help="PyVis: Rich interactive network (may have rendering issues in some browsers)\nPlotly: More stable, zoomable plot\nText: Simple fallback view"
        )
        ...
        max_hyp = st.slider("Max hypotheses to generate", 1, 20, 10, 
                           help="Reduce this number if experiencing crashes or slow performance")
...
""")
    ...
    if st.button("🚀 Analyze Abstracts", type="primary", use_container_width=True):
        ...
        try:
            ...
            with st.status("✍️ Generating hypotheses..."):
                ...
                st.write(f"📝 Generating up to {max_hyp} hypotheses...")
            ...
            # ✅ DUAL VISUALIZATION SELECTOR IMPLEMENTATION
            st.subheader("🌐 Concept Graph")
            
            # Get selected visualization backend
            ...
            if len(nx_graph.nodes()) == 0:
                ...
                st.warning("⚠️ Graph has nodes but no edges — building semantic-only fallback")
                nx_graph = build_semantic_only_graph(
                    list(nx_graph.nodes()), 
                    embed_model, 
                    similarity_threshold=0.65
                )
                if nx_graph.number_of_edges() == 0:
                    st.info("💡 Showing text-based graph summary...")
                    render_graph_fallback(nx_graph, concept_abstract_map)
                else:
                    st.success(f"✅ Fallback graph: {nx_graph.number_of_edges()} edges created")
                    # Continue to render with selected backend
                    if selected_viz == "PyVis (Interactive Network)":
                        render_graph_pyvis(nx_graph, concept_abstract_map, embed_model)
                    elif selected_viz == "Plotly (Stable Plot)":
                        render_graph_plotly(nx_graph, concept_abstract_map)
                    else:
                        render_graph_fallback(nx_graph, concept_abstract_map)
            else:
                # Render with selected visualization backend
                try:
                    if selected_viz == "PyVis (Interactive Network)":
                        st.info("🎨 Rendering with PyVis... (drag nodes, hover for details)")
                        render_graph_pyvis(nx_graph, concept_abstract_map, embed_model)
                        
                    elif selected_viz == "Plotly (Stable Plot)":
                        st.info("🎨 Rendering with Plotly... (zoom, pan, hover for details)")
                        render_graph_plotly(nx_graph, concept_abstract_map)
                    ...
                    st.warning(f"⚠️ {selected_viz} visualization failed: {viz_error}")
                    st.info("💡 Falling back to text summary...")
                    render_graph_fallback(nx_graph, concept_abstract_map)
                    
                    # Offer to retry with alternative backend
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("🔄 Retry with Plotly", key="retry_plotly"):
                            try:
                                render_graph_plotly(nx_graph, concept_abstract_map)
                            except Exception as e:
                                st.error(f"Plotly also failed: {e}")
                    with col2:
                        if st.button("🔄 Retry with PyVis", key="retry_pyvis"):
                            try:
                                render_graph_pyvis(nx_graph, concept_abstract_map, embed_model)
                            except Exception as e:
                                st.error(f"PyVis also failed: {e}")
            ...
            st.error(f"❌ CUDA Out of Memory: {e}")
            st.info("💡 Try these fixes:\n1. Reduce 'Max hypotheses' slider in sidebar\n2. Close other GPU applications\n3. Switch to CPU mode by setting CUDA_VISIBLE_DEVICES=-1")
    ...
    # Footer with visualization tips
    st.markdown("---")
    st.markdown("""
    ...
    **🎨 Visualization Options:**
    - 🕸️ **PyVis**: Rich interactive network with physics simulation, node dragging, and hover details. Best for exploration. May have rendering issues in some browsers/Streamlit deployments.
    - 📊 **Plotly**: Stable, zoomable plot with pan/zoom controls. More reliable in constrained environments. Less interactive but always renders.
    - 📝 **Text Summary**: Fallback view showing node/edge counts and top connections. Always available.
    
    **🔧 Technical:** Qwen2.5-0.5B + Sentence-BERT (384-dim) + Pure PyTorch GraphSAGE | All local processing
    
    **⚠️ If experiencing crashes:**
    1. Reduce "Max hypotheses" in sidebar (start with 5)
    ...
    **🔍 If graph shows issues:**
    1. Try switching visualization backend in the sidebar dropdown
    2. For PyVis: Check browser console for JavaScript errors (F12 → Console)
    3. For Plotly: Use the zoom/pan controls if graph appears crowded
    4. Use the "Download" buttons to open graphs in a new browser tab
    5. Fall back to "Text Summary" for guaranteed visibility
    """)
