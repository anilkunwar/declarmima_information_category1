...
# -*- coding: utf-8 -*-
"""
DECLARMIMA: Alloy Microstructure Concept Graph with DGL Backend Support
========================================================================================
✅ DGL INTEGRATION ADDED:
   - UnifiedGraphSAGE: automatic fallback between DGL optimized and PyTorch sparse
   - Heterogeneous graph ready architecture
   - Optimized CUDA message passing via DGL kernels
   - Toggle in UI to switch between DGL and native sparse backends
...
Run: streamlit run declarmima_concept_graph_dgl.py
"""
...
# ==========================================
# STEP 5: UNIFIED GRAPHSAGE (DGL + SPARSE)
# ==========================================
class UnifiedGraphSAGE(nn.Module):
    """GraphSAGE supporting both DGL optimized and native PyTorch sparse backends"""
    def __init__(self, in_dim: int, hidden_dim: int = GNN_HIDDEN_DIM, use_dgl: bool = False):
        super().__init__()
        self.use_dgl = use_dgl and DGL_AVAILABLE
        self._backend = 'dgl' if self.use_dgl else 'sparse'

        if self.use_dgl:
            self.conv1 = dglnn.SAGEConv(in_dim, hidden_dim, aggregator_type='mean')
            self.conv2 = dglnn.SAGEConv(hidden_dim, hidden_dim, aggregator_type='mean')
        else:
    ...
    def forward(self, graph_obj, adj_values, num_nodes, h, pos_u, pos_v, neg_u, neg_v):
        if self.use_dgl:
            g = graph_obj
            h = self.conv1(g, h)
            h = F.relu(h)
            ...
            h = self.lin2(torch.sparse.mm(A, h1) * deg_inv.unsqueeze(1))
...
def train_gnn(node_features, nx_graph, concept_to_id, pos_pairs, neg_pairs, 
    ...
    graph_obj = adj_indices
    if use_dgl and DGL_AVAILABLE:
        try:
            g = dgl.graph((src_adj, dst_adj), num_nodes=num_nodes, device=target_device)
            g = dgl.add_self_loop(g)
            if node_features.numel() > 0:
                g.ndata['feat'] = node_features.to(target_device)
            graph_obj = g
            adj_values = None  # Not needed for DGL in this unified interface
        except Exception as e:
            st.warning(f"⚠️ DGL graph construction failed: {e}. Falling back to sparse.")
            use_dgl = False

    model = UnifiedGraphSAGE(in_dim=in_dim, hidden_dim=GNN_HIDDEN_DIM, use_dgl=use_dgl).to(target_device)
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        try:
            if len(neg_pairs) == 0:
                pos_out, _, _ = model(graph_obj, adj_values, num_nodes, node_features, pos_u, pos_v, pos_u[:1], pos_v[:1])
                loss = criterion(pos_out, torch.ones_like(pos_out)) * 0.5
            else:
                pos_out, neg_out, _ = model(graph_obj, adj_values, num_nodes, node_features, pos_u, pos_v, neg_u, neg_v)
            ...
            if "no kernel image" in str(e).lower() or "cuda error" in str(e).lower():
                ...
                if use_dgl and DGL_AVAILABLE and isinstance(graph_obj, dgl.DGLGraph):
                    graph_obj = graph_obj.to(cpu_device)
                else:
                    adj_indices = adj_indices.to(cpu_device)
                    adj_values = adj_values.to(cpu_device) if adj_values is not None else None
    ...
    with torch.no_grad():
        _, _, final_embeddings = model(
            graph_obj, adj_values, num_nodes,
    ...
    return model, final_embeddings.cpu(), graph_obj, adj_values
...
def compute_research_direction_scores(
    ...
    ridge, embed_model, d_prev_dict, graph_obj, adj_values,
    n_samples=3000
):
    ...
    model.eval()
    with torch.no_grad():
        # Handle both DGL and sparse backends
        if DGL_AVAILABLE and hasattr(model, 'use_dgl') and model.use_dgl and isinstance(graph_obj, dgl.DGLGraph):
            graph_obj = graph_obj.to(DEVICE)
            node_feat = node_features.to(DEVICE)
            _, _, h2 = model(
                graph_obj, adj_values, n_concepts,
                node_feat, u_tensor, v_tensor, u_tensor, v_tensor
            )
        else:
            _, _, h2 = model(
                graph_obj, adj_values, n_concepts,
...
def render_graph_pyvis_custom(nx_graph, concept_abstract_map, physics_enabled=True, 
    ...
    for node in nx_graph.nodes():
        ...
        # Convert numpy scalar to native int for JSON serialization
    ...
    for u, v in nx_graph.edges():
        ...
        # Convert all numpy scalars to native Python float/int
...
# ==========================================
# STREAMLIT UI: SIDEBAR WITH DGL CONTROLS
# ==========================================
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # 🕸️ DGL Backend toggle
        st.subheader("🕸️ Graph Neural Network Backend")
        use_dgl = False
        if DGL_AVAILABLE:
            use_dgl = st.toggle(
                "Use DGL (Deep Graph Library) backend",
                value=True,
                help="DGL provides optimized message passing and heterogeneous graph support. Falls back to PyTorch sparse if unchecked or unavailable."
            )
            st.caption(f"✅ DGL {dgl.__version__} available")
        else:
            st.warning("⚠️ DGL not installed")
            st.code("pip install dgl -f https://data.dgl.ai/wheels/torch-2.4/cu121/repo.html")
        st.session_state['use_dgl'] = use_dgl
        ...
        if DGL_AVAILABLE:
            st.caption(f"🕸️ DGL: {dgl.__version__}")
        else:
            st.caption("🕸️ DGL: not installed")
...
def main():
    ...
    st.caption("Physics-informed digital twins for multicomponent alloys • Dual LLM backend: HF Transformers or Ollama • DGL GraphSAGE support")
...
""")
        ...
        try:
            ...
            with st.status("🕸️ Building concept graph..."):
                ...
                if declarmima_edges > 0:
                    st.success(f"🎯 {declarmina_edges} edges aligned with DECLARMIMA goals")
            ...
            # Step 5: Train GNN (with DGL support)
            ...
            with st.status("🤖 Training GraphSAGE..."):
                use_dgl = st.session_state.get('use_dgl', False)
                if use_dgl and DGL_AVAILABLE:
                    st.write("🕸️ Using DGL backend for optimized message passing")
                else:
                    st.write("🔢 Using PyTorch sparse backend")

                gnn_model, final_emb, graph_obj, adj_values = train_gnn(
                    node_features, nx_graph, concept_to_id, pos_pairs, neg_pairs, 
                    _training_progress, use_dgl=use_dgl
                )
                st.success(f"✅ GNN training complete ({gnn_model._backend} backend)")
            ...
            with st.expander("📊 Graph Diagnostics", expanded=len(valid_concepts) < 30):
                ...
                # Show backend info
                col1, col2 = st.columns(2)
                col1.metric("GNN Backend", gnn_model._backend.upper())
                if st.session_state.get('use_declarmima', True):
                    ...
                    declarmima_edges = sum(1 for _, _, d in nx_graph.edges(data=True) if d.get('edge_type') == 'declarmina_aligned')
                    col2.metric("DECLARMIMA Concepts", declarmima_nodes)
                    col1.metric("Aligned Edges", declarmina_edges)
    ...
    st.markdown("""
    **🕸️ DGL Integration:**
    - ✅ **UnifiedGraphSAGE**: Automatic fallback between DGL optimized and PyTorch sparse backends
    - ✅ **Optimized message passing**: DGL CUDA kernels for large graphs
    - ✅ **Heterogeneous ready**: Architecture supports multi-type edges (co-occurrence, semantic, bridge, DECLARMIMA-aligned)
    - ✅ **Toggle in UI**: Switch between DGL and native sparse without code changes
    - ✅ **Graceful fallback**: If DGL fails, automatically falls back to PyTorch sparse
    ...
    **🔧 Technical:** Qwen2.5-0.5B + Sentence-BERT (384-dim) + Unified GraphSAGE (DGL/PyTorch) | Local processing
