...
# This prevents AttributeError when DGL is not installed
if TYPE_CHECKING:
    import dgl
...
def get_dgl_info() -> Dict[str, Any]:
    """Gather DGL installation and backend information."""
    info = {
    ...
    try:
        ...
        if torch.cuda.is_available():
            try:
                # Test GPU tensor creation with DGL
                test_g = dgl.graph(([0, 1], [1, 2])).to('cuda')
                info["cuda_support"] = True
                info["gpu_test_passed"] = True
            except Exception as e:
                info["cuda_support"] = True  # DGL compiled with CUDA but runtime issue
...
def check_dgl_cuda_compatibility() -> Tuple[bool, str]:
    """Check if DGL is properly installed with CUDA support"""
    if not DGL_AVAILABLE:
...
class DGLGraphSAGE(nn.Module):
    """GraphSAGE using DGL's optimized implementation with fallback support"""
...
def train_gnn_pytorch(node_features, nx_graph, concept_to_id, pos_pairs, neg_pairs, progress_callback=None):
    """Original PyTorch sparse tensor GraphSAGE training (fallback)"""
...
def train_gnn(node_features, nx_graph, concept_to_id, pos_pairs, neg_pairs, progress_callback=None, use_dgl: bool = True):
    """Unified GNN training with DGL preference and PyTorch fallback"""
...
def render_graph_pyvis_custom(nx_graph, concept_abstract_map, physics_enabled=True, 
    ...
    # ✅ CRITICAL FIX: Use 'remote' instead of 'in_line' to prevent massive HTML size
    # 'in_line' embeds ~5MB of vis.js/css inline, causing OOM on download & session reset
    net = Network(
    ...
    # ✅ CRITICAL FIX: Safe download generation with memory management
    try:
        # Convert to bytes to avoid Streamlit string serialization OOM
        ...
        # Clear large strings from memory immediately after creating download button
...
# ==========================================
# STREAMLIT UI: SIDEBAR WITH DGL OPTIONS
# ==========================================
def render_sidebar():
    with st.sidebar:
...
""")
    ...
    **🎨 White Theme Visualization:**
    ...
    - ✅ **FIXED:** PyVis download no longer crashes session (uses CDN + bytes encoding)
