...
# -*- coding: utf-8 -*-
"""
DECLARMIMA ENHANCED: Alloy Microstructure Concept Graph
========================================================
✅ NEW ENHANCEMENTS (FULLY IMPLEMENTED):
- Edge explanations: LLM-generated descriptions for graph edges, shown in tooltips & table.
- Distillation metrics: adjustable Top N concepts with metric selection.
- Radar chart: customizable font size, line width, colormap, opacity, metric selection.
- Sunburst chart: customizable label size, colormap, truncation, value display.
- Chord diagram: Plotly-based (no extra deps) + optional HoloViews/Bokeh with dropdown.
- All customisation controls exposed in sidebar and per-chart expanders.
"""
...
# Optional imports for enhanced projections & chord
try:
    import umap
    UMAP_AVAILABLE = True
except ImportError:
...
# ==========================================
# 🎨 50+ COLORMAP REGISTRY (unchanged)
# ==========================================
SUPPORTED_COLORMAPS = {
...
# ==========================================
# 🔧 CUDA & DGL COMPATIBILITY (unchanged)
# ==========================================
...
# ==========================================
st.set_page_config(page_title="DECLARMIMA: Alloy Microstructure Concept Graph (Enhanced)", page_icon="🔬", layout="wide", initial_sidebar_state="expanded")
...
# ==========================================
# MODEL REGISTRY & CONSTANTS (unchanged)
# ==========================================
LOCAL_LLM_OPTIONS = {
...
# ==========================================
# 🔧 CUDA-SAFE MODEL LOADING (unchanged)
# ==========================================
...
# ==========================================
# CONCEPT DISTILLATION EFFICIENCY (enhanced with top N)
# ==========================================
...
# ==========================================
# DECLARMIMA: PROPOSAL-BASED CONCEPT EXTRACTION (unchanged)
# ==========================================
...
# ==========================================
# SEMANTIC CLUSTERING & CONCEPT ABSTRACTION (unchanged)
# ==========================================
...
# ==========================================
# MATHEMATICAL VALIDATIONS (unchanged)
# ==========================================
...
# ==========================================
# STEP 1-2: CONCEPT EXTRACTION & METRICS (unchanged)
# ==========================================
...
# ==========================================
# STEP 3: HYBRID CONCEPT GRAPH CONSTRUCTION (with edge explanation generation)
# ==========================================
def generate_edge_explanation(u: str, v: str, weight: float, edge_type: str, tokenizer, model, backend_type: str) -> str:
    """Generate a short scientific explanation for why two concepts are linked."""
    prompt = f"""Explain in 1-2 short sentences why the concepts "{u}" and "{v}" are scientifically related in the context of laser-microstructure interaction in multicomponent alloys (DECLARMIMA). Focus on physical mechanisms, process-structure-property links, or digital twin modeling. Keep concise and specific.
Response:"""
    try:
        if backend_type == "ollama":
            client = ollama.Client(host=st.session_state.get('ollama_host', 'http://localhost:11434'))
            response = client.chat(model=model, messages=[{"role": "user", "content": prompt}], options={"temperature": 0.2, "num_predict": 80})
            explanation = response.get('message', {}).get('content', '').strip() if isinstance(response, dict) else getattr(response, 'message', {}).get('content', '').strip()
        else:
            inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=200).to(DEVICE)
            with torch.no_grad():
                outputs = model.generate(inputs.input_ids, max_new_tokens=80, temperature=0.2, do_sample=True, pad_token_id=tokenizer.eos_token_id)
            explanation = tokenizer.decode(outputs[0][inputs.input_ids.shape[1]:], skip_special_tokens=True).strip()
        if not explanation:
            explanation = f"Potential synergy between {u} and {v} in laser additive manufacturing processes."
        return explanation[:200]  # keep short
    except Exception:
        return f"Relation: {u} ↔ {v} (weight {weight:.2f}, type {edge_type})"

def build_hybrid_graph(all_concepts, valid_concepts, concept_to_id, embed_model=None, config=None, proposal_embedding=None, tokenizer=None, llm_model=None, backend_type=None):
    ...
    # Generate explanations for edges (optional, may be skipped if LLM not loaded)
    if tokenizer is not None and llm_model is not None and backend_type is not None:
        st.write("💡 Generating edge explanations (may take a moment)...")
        for u, v, data in nx_graph.edges(data=True):
            if 'explanation' not in data:
                expl = generate_edge_explanation(u, v, data['weight'], data.get('edge_type', 'cooccurrence'), tokenizer, llm_model, backend_type)
...
def build_semantic_only_graph(valid_concepts, embed_model, similarity_threshold=0.75):
    ...
    try:
        ...
        # Add dummy explanations for semantic graph (since we don't have LLM here)
        for u, v in nx_graph.edges():
            nx_graph[u][v]['explanation'] = f"Semantic similarity between '{u}' and '{v}' based on embedding cosine similarity."
    except Exception as e:
...
def build_concept_graph(all_concepts, concept_to_id, embed_model=None, config=None, proposal_embedding=None, tokenizer=None, llm_model=None, backend_type=None):
    if config is None: config = get_adaptive_config(len(all_concepts))
    ...
    return build_hybrid_graph(all_concepts, valid_concepts, concept_to_id, embed_model, config, proposal_embedding, tokenizer, llm_model, backend_type)
...
# ==========================================
# STEP 4: SEMANTIC NODE EMBEDDINGS (unchanged)
# ==========================================
...
# ==========================================
# 🔧 DGL GRAPH CONVERSION UTILITIES (unchanged)
# ==========================================
...
# ==========================================
# 🔧 DGL-BASED GRAPHSAGE IMPLEMENTATION (unchanged)
# ==========================================
...
# ==========================================
# 🔧 CUDA-SAFE TRAINING LOOP (unchanged)
# ==========================================
...
# ==========================================
# STEP 6: MICROSTRUCTURE QUANTIFICATION & SCORING (unchanged)
# ==========================================
...
# ==========================================
# STEP 7: LLM CURATION OF RESEARCH DIRECTIONS (unchanged)
# ==========================================
...
# ==========================================
# ENHANCED QUANTITATIVE GRAPH METRICS (unchanged)
# ==========================================
...
# ==========================================
# CATEGORICAL SUNBURST CHART (enhanced with customizations)
# ==========================================
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis",
                          ...
                          max_label_length=30, show_percent=True):
    if not labels:
    ...
    fig = go.Figure(go.Sunburst(
        ...
        textinfo="label+percent entry" if show_percent else "label",
...
# ==========================================
# RADAR CHART (enhanced with full customization)
# ==========================================
def render_radar_chart(concept_scores_df: pd.DataFrame, top_k: int = 15,
                       ...
                       font_size=12, line_width=2, fill_opacity=0.3,
                       cmap_name="viridis", show_legend=True,
                       metric_rename: Dict[str, str] = None):
    ...
    categories = metrics
    if metric_rename:
        categories = [metric_rename.get(m, m) for m in metrics]
    colors = get_colormap_colors(cmap_name, len(top_concepts))
    fig = go.Figure()
    for idx, (_, row) in enumerate(top_concepts.iterrows()):
        concept = row['concept']
        values = [normalized.loc[row.name, m] for m in metrics]
        values += values[:1]
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 1], tickfont=dict(size=font_size-2)),
...
# ==========================================
# CHORD DIAGRAM (Plotly-based, with top N and customizations)
# ==========================================
def render_chord_diagram_plotly(nx_graph: nx.Graph, valid_concepts: List[str], top_n: int = 15,
                                cmap_name="viridis", edge_opacity=0.6, edge_thickness_scale=1.0,
                                label_size=10, label_rotation=0, threshold: float = 0.0):
    """
    Draw a chord diagram using Plotly. Based on adjacency matrix.
    Only include top_n nodes by degree.
    """
    if len(valid_concepts) < 3:
        st.info("Not enough nodes for chord diagram.")
        return
    # Subgraph of top N nodes by degree
    degrees = dict(nx_graph.degree())
    top_nodes = sorted(degrees.keys(), key=lambda x: degrees[x], reverse=True)[:min(top_n, len(valid_concepts))]
    subgraph = nx_graph.subgraph(top_nodes).copy()
    n = len(top_nodes)
    if n < 2:
        st.info("Need at least 2 nodes for chord diagram.")
        return
    ...
    for u, v, data in subgraph.edges(data=True):
        ...
        if weight > threshold:
            ...
            adj[i, j] = weight
            adj[j, i] = weight
    # Prepare chord diagram via Plotly (circle layout)
    # Compute angular positions
    angles = np.linspace(0, 2*np.pi, n, endpoint=False)
    # Normalize weights
    total_weight = adj.sum()
    if total_weight == 0:
        st.info("No edges with weight above threshold.")
        return
    # Use go.Sankey or custom chord? We'll implement a custom chord diagram using arcs and ribbons.
    # Simpler: use a layout with arcs and Scatterpolar? Instead, let's use Plotly's `go.Chord`? Not available.
    # Alternative: Use `plotly.graph_objects.Sankey`? Not appropriate.
    # We'll implement a simple chord diagram using lines with curvature.
    # Since Plotly lacks built-in chord, we use a workaround: for each edge, draw a curved line between points on the circle.
    fig = go.Figure()
    # Draw nodes as markers on circle
    colors = get_colormap_colors(cmap_name, n)
    # Node positions on circle
    circle_x = np.cos(angles)
    circle_y = np.sin(angles)
    # Add node traces
    node_text = [f"{node}<br>Degree: {degrees[node]}" for node in top_nodes]
    fig.add_trace(go.Scatter(x=circle_x, y=circle_y, mode='markers+text', text=top_nodes, textposition="middle center",
                             textfont=dict(size=label_size), marker=dict(size=20, color=colors, line=dict(width=1, color='white')),
                             hovertemplate='%{text}<extra></extra>', name='Concepts'))
    # Add chord edges (curved lines)
    for i in range(n):
        for j in range(i+1, n):
            if adj[i, j] > 0:
                weight = adj[i, j]
                # Draw a quadratic Bezier curve from angle[i] to angle[j]
                # Control point at (0,0) gives a chord inside circle
                # Parameterize t from 0 to 1
                t_vals = np.linspace(0, 1, 50)
                x_start, y_start = circle_x[i], circle_y[i]
                x_end, y_end = circle_x[j], circle_y[j]
                # Quadratic Bezier: B(t) = (1-t)^2 P0 + 2(1-t)t P1 + t^2 P2, with P1 = (0,0)
                x_curve = (1-t_vals)**2 * x_start + 2*(1-t_vals)*t_vals * 0 + t_vals**2 * x_end
                y_curve = (1-t_vals)**2 * y_start + 2*(1-t_vals)*t_vals * 0 + t_vals**2 * y_end
                linewidth = np.clip(weight * edge_thickness_scale, 0.5, 8)
                fig.add_trace(go.Scatter(x=x_curve, y=y_curve, mode='lines', line=dict(width=linewidth, color=colors[i], opacity=edge_opacity),
                                         hoverinfo='text', hovertext=f"{top_nodes[i]} ↔ {top_nodes[j]}<br>Weight: {weight:.2f}",
                                         showlegend=False))
    fig.update_layout(
        title=f"Chord Diagram: Top {n} Concepts by Degree",
        xaxis=dict(visible=False, range=[-1.2, 1.2]),
        yaxis=dict(visible=False, range=[-1.2, 1.2]),
        width=700, height=700,
        plot_bgcolor='white',
        annotations=[dict(text="🔗 Edge thickness = connection strength", showarrow=False, xref="paper", yref="paper", x=0.5, y=-0.05, font=dict(size=10))]
    )
    st.plotly_chart(fig, use_container_width=True)

def render_chord_diagram_holoviews(nx_graph: nx.Graph, valid_concepts: List[str], top_n: int = 15,
                                   cmap_name="viridis", edge_opacity=0.6, edge_thickness_scale=1.0,
                                   label_size=10):
    if not HOLOVIEWS_AVAILABLE:
        st.warning("HoloViews+Bokeh not installed. Install with: pip install holoviews bokeh")
        return
    # Build adjacency matrix for top N
    degrees = dict(nx_graph.degree())
    top_nodes = sorted(degrees.keys(), key=lambda x: degrees[x], reverse=True)[:min(top_n, len(valid_concepts))]
    n = len(top_nodes)
    if n < 2:
    ...
    for u, v, data in nx_graph.edges(data=True):
        if u in node_to_idx and v in node_to_idx:
            ...
            adj[i, j] = weight
            adj[j, i] = weight
    # Convert to HoloViews Chord object
    # Create edges list: (source_idx, target_idx, weight)
    edges = []
    for i in range(n):
        for j in range(i+1, n):
            if adj[i, j] > 0:
    ...
    nodes_df = pd.DataFrame({'index': range(n), 'name': top_nodes})
    chord = hv.Chord((edges_df, nodes_df), label='Concept Connections')
    # Apply styling
    chord.opts(
        opts.Chord(
            labels='name', label_text_font_size=f"{label_size}pt",
            edge_color='value', edge_line_width=hv.dim('value').norm() * edge_thickness_scale + 0.5,
            cmap=cmap_name, edge_alpha=edge_opacity, node_color='index', node_size=15,
            title=f"Chord Diagram: Top {top_n} Concepts",
            width=700, height=700, bgcolor='white'
        )
    )
    # Render with bokeh
...
# ==========================================
# ENHANCED PYVIS GRAPH WITH CATEGORY COLORS & SAFE DOWNLOAD (unchanged)
# ==========================================
...
def render_graph_pyvis_custom(nx_graph, concept_abstract_map, physics_enabled=True, 
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        # Add explanation to tooltip if available
        title = f"{node}\nDegree: {degree}\nFrequency: {freq}"
        # Also show edge explanations in tooltips when hovering over edges (handled below)
    ...
    for u, v, data in nx_graph.edges(data=True):
        ...
        net.add_edge(u, v, value=float(np.clip(w, 0.5, 5)), 
                     ...
                     label=label, title=f"weight: {w:.2f}<br>type: {edge_type}<br><br><b>Explanation:</b> {explanation}")
...
def render_graph_plotly_white(nx_graph, concept_abstract_map, cmap_name="viridis", custom_labels=None,
    ...
    for u, v, data in nx_graph.edges(data=True):
        ...
        w = data.get('weight', 1); edge_type = data.get('edge_type', 'unknown')
        expl = data.get('explanation', 'No explanation')
        hover_info = f"{u} ↔ {v}<br>Weight: {w:.2f}<br>Type: {edge_type}<br><b>Explanation:</b> {expl}"
...
# ==========================================
# ENHANCED EXPORT & POST-PROCESSING (unchanged)
# ==========================================
...
# ==========================================
# STREAMLIT UI & PIPELINE ORCHESTRATION (updated with new controls)
# ==========================================
def render_sidebar():
    with st.sidebar:
        ...
        # New radar customization options
        with st.expander("🔧 Radar Chart Settings"):
            st.slider("Radar font size", 8, 16, 12, key="radar_font_size")
            st.slider("Radar line width", 1, 5, 2, key="radar_line_width")
            st.slider("Radar fill opacity", 0.1, 0.8, 0.3, key="radar_opacity")
            st.selectbox("Radar colormap", options=list(SUPPORTED_COLORMAPS.keys()), index=0, key="radar_cmap")
            st.checkbox("Show legend on radar", True, key="radar_legend")
        # Chord diagram settings
        st.subheader("🎵 Chord Diagram (Top N Concepts)")
        st.selectbox("Chord library", options=["Plotly (built-in)", "HoloViews+Bokeh (if installed)"], index=0, key="chord_library")
        st.slider("Top N for chord diagram", 5, 50, 15, key="chord_top_n")
        st.slider("Chord edge thickness scale", 0.5, 3.0, 1.0, key="chord_thickness")
        st.slider("Chord edge opacity", 0.2, 1.0, 0.6, key="chord_opacity")
        st.slider("Chord label size", 8, 16, 10, key="chord_label_size")
        st.number_input("Chord weight threshold (min weight)", 0.0, 10.0, 0.0, step=0.1, key="chord_threshold")
        # Graph node limit
...
def main():
    st.title("🔬 DECLARMIMA: Laser-Microstructure Interaction Analyzer (Enhanced)")
    st.caption("Physics-informed digital twins for multicomponent alloys • Dual LLM backend • Edge explanations • Chord diagrams • Full customization")
    render_sidebar()
...
""")
    ...
    if st.button("🚀 Analyze Abstracts", type="primary", use_container_width=True):
        ...
        try:
            with status:
                ...
                with st.status("🕸️ Building concept graph with edge explanations..."):
                    nx_graph = build_concept_graph(all_concepts, concept_to_id, embed_model, config, proposal_embedding, tokenizer, llm_model, backend_type if tokenizer is not None else None)
    ...
    if st.session_state.analysis_data is not None:
        ...
        viz_tab, distill_tab, valid_tab, edge_explanations_tab, export_tab = st.tabs(["🎨 Visualization", "📊 Distillation Metrics", "📐 Mathematical Validation", "💡 Edge Explanations", "📥 Export"])
        with viz_tab:
            ...
            if st.session_state.get('radar_enabled', True):
                with st.expander("📡 Concept Radar Chart (Multi-Dimensional Comparison)", expanded=False):
                    ...
                    render_radar_chart(
                        ...
                        font_size=st.session_state.get('radar_font_size', 12),
                        line_width=st.session_state.get('radar_line_width', 2),
                        fill_opacity=st.session_state.get('radar_opacity', 0.3),
                        cmap_name=st.session_state.get('radar_cmap', 'viridis'),
                        show_legend=st.session_state.get('radar_legend', True)
                    )

            with st.expander("🎵 Chord Diagram (Top N Concepts)", expanded=False):
                chord_lib = st.session_state.get('chord_library', 'Plotly (built-in)')
                top_n_chord = st.session_state.get('chord_top_n', 15)
                if chord_lib == 'Plotly (built-in)' or not HOLOVIEWS_AVAILABLE:
                    render_chord_diagram_plotly(
                        nx_graph, valid_concepts, top_n=top_n_chord,
                        cmap_name=cmap, edge_opacity=st.session_state.get('chord_opacity', 0.6),
                        edge_thickness_scale=st.session_state.get('chord_thickness', 1.0),
                        label_size=st.session_state.get('chord_label_size', 10),
                        threshold=st.session_state.get('chord_threshold', 0.0)
                    )
                else:
                    render_chord_diagram_holoviews(
                        nx_graph, valid_concepts, top_n=top_n_chord,
                        cmap_name=cmap, edge_opacity=st.session_state.get('chord_opacity', 0.6),
                        edge_thickness_scale=st.session_state.get('chord_thickness', 1.0),
                        label_size=st.session_state.get('chord_label_size', 10)
        ...
        with distill_tab:
            ...
            distill_df = compute_concept_distillation(valid_concepts, concept_abstract_map, abstracts)
            top_n_distill = st.slider("Show Top N Concepts", 5, 50, 20, key="top_n_distill_slider")
            display_metrics = st.multiselect("Select metrics to display", distill_df.columns.tolist(), default=distill_df.columns.tolist())
            st.dataframe(distill_df.head(top_n_distill)[display_metrics], use_container_width=True)
            st.markdown("**📈 Top Distilled Concepts:**")
            st.bar_chart(distill_df.set_index('concept')[['distillation_efficiency']].head(top_n_distill))
        ...
        with edge_explanations_tab:
            st.subheader("💡 Edge Explanations (LLM-generated)")
            if nx_graph.number_of_edges() == 0:
                ...
                for u, v, data in nx_graph.edges(data=True):
                    edge_data.append({
                        "Source Concept": u,
                        "Target Concept": v,
                        "Edge Type": data.get('edge_type', 'unknown'),
                        "Weight": data.get('weight', 1.0),
                        "Explanation": data.get('explanation', 'Not generated (LLM not available).')
                    })
                edge_df = pd.DataFrame(edge_data).sort_values("Weight", ascending=False)
                st.dataframe(edge_df, use_container_width=True)
                st.download_button("📥 Download Edge Explanations (CSV)", data=edge_df.to_csv(index=False), file_name="edge_explanations.csv", mime="text/csv")
        with export_tab:
