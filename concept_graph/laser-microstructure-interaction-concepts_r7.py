...
def compute_research_direction_scores(
    ...
    ✅ FIX: Now accepts node_features (384-dim) for model inference,
    not final_emb (128-dim) which caused the shape mismatch.
    """
    ...
    model.eval()
    with torch.no_grad():
        # ✅ FIX: Use node_features (384-dim) instead of final_emb (128-dim)
        # The model expects original embeddings as input, not transformed GNN outputs
        _, _, h2 = model(
            adj_indices, adj_values, n_concepts,
            node_features.to(DEVICE),  # ✅ CORRECT: 384-dim input
    ...
    if st.button("🚀 Analyze Abstracts", type="primary", use_container_width=True):
        ...
        try:
            ...
            with st.status("📈 Scoring novel directions..."):
                ...
                # ✅ FIX: Pass node_features (384-dim) for model inference
                top_scores = compute_research_direction_scores(
                    gnn_model,
                    node_features,  # ✅ ADD: original 384-dim embeddings
                    final_emb,      # Keep for reference if needed elsewhere
