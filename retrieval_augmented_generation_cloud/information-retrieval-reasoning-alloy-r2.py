...
class AlloyPropertyExtractor:
    ...
    def _extract_composition_data(self, text: str) -> List[AlloyProperty]:
        ...
        for match in ALLOY_COMPOSITION_PATTERNS["atomic_percent"].finditer(text):
            ...
            prop = AlloyProperty(
                name=f"composition_{element}", value=value, unit="at.%",
                context_snippet=context, extraction_confidence=0.9, property_type="composition"
            )
            self._normalize_property_units(prop)
            properties.append(prop)
        for match in ALLOY_COMPOSITION_PATTERNS["weight_percent"].finditer(text):
            ...
            context = text[max(0, match.start()-100):min(len(text), match.end()+100)]
            prop = AlloyProperty(
                name=f"composition_{element}", value=value, unit="wt.%",
                context_snippet=context, extraction_confidence=0.9, property_type="composition"
            )
            self._normalize_property_units(prop)
            properties.append(prop)
...
class AlloyVisualizationEngine:
    ...
    def create_composition_pie_chart(composition_data: List[Dict], alloy_name: str, title: str = None) -> go.Figure:
        ...
        fig = go.Figure(data=[go.Pie(
            labels=elements, values=percentages, hole=0.3,
    ...
    def create_microstructure_radar_chart(microstructure_metrics: Dict[str, float],
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            fill='toself', name='Microstructure Profile',
        ...
        fig.update_layout(
            ...
            height=450, margin=dict(t=50, b=20, l=20, r=20)
        )
        return fig
    ...
    def create_phase_fraction_bar_chart(phase_data: List[Dict],
        ...
        if stacked:
            fig = px.bar(
                df, x='sample', y='fraction', color='phase',
                title=title,
        ...
        fig.update_layout(
            ...
            xaxis_title="Sample", yaxis_title="Volume Fraction (%)"
        )
        return fig
    ...
    def create_alloy_property_comparison_chart(alloy_properties: List[Dict],
        ...
        fig = px.bar(
            df, x='alloy_name', y=property_name, color='alloy_system',
        ...
        fig.update_layout(
            xaxis_title="Alloy Composition", yaxis_title=property_name.replace('_', ' ').title(),
            showlegend=True, height=400, margin=dict(t=50, b=60, l=40, r=20)
        )
        return fig
    ...
    def create_grain_size_distribution(grain_data: List[Dict],
        ...
        if 'alloy_system' in df.columns:
            for system in df['alloy_system'].unique():
                ...
                fig.add_trace(go.Box(y=system_data, name=system), row=1, col=2)
        fig.update_layout(
            title=title, height=400, margin=dict(t=50, b=40, l=40, r=20), showlegend=False
        )
    ...
    def create_phase_evolution_chart(phase_data: List[Dict],
        ...
        df = pd.DataFrame(phase_data)
        fig = px.line(
            df, x=temperature_col, y='fraction', color='phase',
            title=title,
        ...
        fig.update_layout(
            height=400, margin=dict(t=50, b=40, l=40, r=20),
            xaxis_title="Temperature (K)", yaxis_title="Phase Fraction (%)"
        )
        return fig
...
class AlloyPhysicsValidator:
    ...
    def check_property_bounds(self, property_name: str, value: float, unit: str) -> Dict:
        ...
        if value < bounds["min"] or value > bounds["max"]:
            violation = {
                "type": "bound_violation", "property": property_name,
                "value": value, "unit": unit, "bounds": bounds,
    ...
    def full_validation(self, fused_properties: Dict[str, FusedAlloyProperty]) -> Dict:
        results = {"total_checks": 0, "passed": 0, "failed": 0, "violations": [], "warnings": []}
...
# =============================================
# SESSION STATE & UTILITIES - ROBUST VERSION
# =============================================
...
def initialize_session_state():
    """Initialize ALL session state variables with defaults to prevent AttributeError"""
    defaults = {
        # File processing
        "processed_files": set(),
        "vectorstore": None,
        ...
        # Visualization settings - EXPANDED WITH ALL REQUIRED ATTRIBUTES
        # Defaults now match UI options to prevent selection errors
        "viz_chart_type": "bar",
        "viz_property_focus": "All Properties",
        "viz_alloy_focus": "All Alloys",
        "viz_phase_focus": "All Phases",
    }
    
    # Initialize each key if not already present
...
def _load_ollama_model(model_key: str):
    ...
    try:
        client = ollama.Client(host=safe_get_session_state("ollama_host", "http://localhost:11434"))
        ...
        if model_tag not in model_names:
            ...
            return None, None, safe_get_session_state("ollama_host"), "ollama"
    except Exception as conn_err:
        st.error(f"❌ Connection Error: {conn_err}")
        return None, None, safe_get_session_state("ollama_host"), "ollama"
    return None, model_tag, safe_get_session_state("ollama_host"), "ollama"
...
def load_and_chunk_alloy_documents(uploaded_files: List) -> List[Document]:
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            cached_meta = safe_get_session_state("metadata_cache").get(uploaded_file.name, file_hash)
            if cached_meta:
                ...
                safe_get_session_state("metadata_cache").set(uploaded_file.name, bib_meta, file_hash)
            ...
            for i, chunk in enumerate(chunks):
                chunk.metadata.update({
                    ...
                    **extract_alloy_metadata(chunk.page_content, uploaded_file.name),
                    ...
                    "citation_display": bib_meta.format_citation(safe_get_session_state('citation_style', 'apa')),
                })
...
# =============================================
# VISUALIZATION UI COMPONENTS - ROBUST VERSION
# =============================================
def render_viz_control_panel(fused_properties: Dict[str, FusedAlloyProperty], retrieved_docs: List[Document]):
    ...
    # Defensive initialization to ensure keys exist before access
    if "viz_chart_type" not in st.session_state:
        st.session_state.viz_chart_type = "bar"
    if "viz_alloy_focus" not in st.session_state:
        st.session_state.viz_alloy_focus = "All Alloys"
    if "viz_phase_focus" not in st.session_state:
        st.session_state.viz_phase_focus = "All Phases"
    if "viz_property_focus" not in st.session_state:
        st.session_state.viz_property_focus = "All Properties"
    ...
    # Filters with safe access pattern for defaults
    col1, col2, col3 = st.columns(3)
    with col1:
        alloy_options = ["All Alloys"] + list(set(ALLOY_SYSTEMS.keys()))
        st.session_state.viz_alloy_focus = st.selectbox("Alloy System Filter", options=alloy_options, key="viz_alloy", index=0 if st.session_state.viz_alloy_focus not in alloy_options else alloy_options.index(st.session_state.viz_alloy_focus))
    with col2:
        phase_options = ["All Phases"] + list(ALLOY_STRUCTURE_TYPES.keys())
        st.session_state.viz_phase_focus = st.selectbox("Phase Filter", options=phase_options, key="viz_phase", index=0 if st.session_state.viz_phase_focus not in phase_options else phase_options.index(st.session_state.viz_phase_focus))
    with col3:
        # Build options dynamically if fused properties exist
        current_prop_opts = ["All Properties"] + [p.replace('_', ' ').title() for p in fused_properties.keys()]
        st.session_state.viz_property_focus = st.selectbox("Property Focus", options=current_prop_opts, key="viz_prop", index=0 if st.session_state.viz_property_focus not in current_prop_opts else current_prop_opts.index(st.session_state.viz_property_focus))
        
    # Generate visualization based on selection
    viz_engine = AlloyVisualizationEngine()
    
    # Use safe_get to ensure values are read safely
    current_alloy = safe_get_session_state("viz_alloy_focus", "All Alloys")
    current_phase = safe_get_session_state("viz_phase_focus", "All Phases")
    current_prop = safe_get_session_state("viz_property_focus", "All Properties")
    ...
    if viz_type == "Composition Pie Chart":
        ...
        if composition_data:
            alloy_name = current_alloy if current_alloy != "All Alloys" else "Multicomponent Alloy"
        ...
        for prop_name, prop in fused_properties.items():
            if prop.fused_value is not None and isinstance(prop.fused_value, (int, float)):
                ...
                if current_prop == "All Properties" or current_prop == display_name:
                    microstructure_metrics[display_name] = prop.fused_value
        if microstructure_metrics:
            title = f"Microstructure Profile"
            if current_alloy != "All Alloys":
                title += f" - {current_alloy}"
            if current_phase != "All Phases":
                title += f" ({current_phase})"
            fig = viz_engine.create_microstructure_radar_chart(microstructure_metrics, title=title)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Insufficient microstructure data for radar chart.")
        ...
        if alloy_properties:
            # Ensure property name exists
            prop_name_to_use = [k for k, v in fused_properties.items() if k.replace('_', ' ').title() == current_prop]
            if not prop_name_to_use:
                prop_name_to_use = list(fused_properties.keys())[0] if fused_properties else None
            if prop_name_to_use:
                fig = viz_engine.create_alloy_property_comparison_chart(alloy_properties, prop_name_to_use[0])
                st.plotly_chart(fig, use_container_width=True)
        else:
        ...
        st.info("Phase evolution chart requires temperature-dependent data.")
    ...
    with st.sidebar:
        ...
        if backend_option == "Ollama (if installed)" or is_ollama_model(model_choice):
            st.session_state.ollama_host = st.text_input(
                "🌐 Ollama Host", value=safe_get_session_state("ollama_host", "http://localhost:11434"),
...
<li>📊 Use visualization panel for interactive charts</li>
        ...
        if backend_option == "Ollama (if installed)" and OLLAMA_AVAILABLE:
            try:
                client = ollama.Client(host=safe_get_session_state("ollama_host"))
                models = client.list()
...
def process_documents(uploaded_files):
    if not uploaded_files:
        return False
    new_files = [f for f in uploaded_files if f.name not in safe_get_session_state("processed_files")]
    if not new_files:
...
def render_chat_interface():
    ...
    if st.session_state.llm_tokenizer is None and st.session_state.llm_model_choice:
        ...
        with st.spinner(f"Loading {st.session_state.llm_model_choice}..."):
            result = load_local_llm(st.session_state.llm_model_choice, use_4bit=safe_get_session_state('use_4bit_quantization', True))
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            ...
            if message.get("sources") and safe_get_session_state("show_sources"):
            ...
            if message.get("fusion_metadata") and safe_get_session_state("enable_alloy_fusion"):
            ...
            # Physics validation
            fused_props = message["fusion_metadata"].get("fused_properties", {}) if message.get("fusion_metadata") else {}
            if fused_props:
    ...
    if prompt := st.chat_input("Ask about alloy composition, phase structure, grain size, or compare microstructures..."):
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("🔍 Retrieving, fusing alloy microstructure data, and generating..."):
                try:
                    # Use safe access for filter logic to prevent crashes if state is inconsistent
                    viz_alloy = safe_get_session_state("viz_alloy_focus", "All Alloys")
                    viz_phase = safe_get_session_state("viz_phase_focus", "All Phases")
                    viz_prop = safe_get_session_state("viz_property_focus", "All Properties")
                    
                    if safe_get_session_state("enable_alloy_fusion"):
                        answer, retrieved_docs, relevance, metadata = retrieve_and_answer_with_alloy_fusion(
                            ...
                            query=prompt,
                            k=safe_get_session_state("max_retrieved_chunks", 4),
                            enable_fusion=True,
                            alloy_filter=viz_alloy if viz_alloy != "All Alloys" else None,
                            phase_filter=[viz_phase] if viz_phase != "All Phases" else None,
                            property_filter=[viz_prop.lower().replace(' ', '_')] if viz_prop != "All Properties" else None
                        )
                    else:
                        answer, retrieved_docs, relevance, metadata = retrieve_and_answer_with_alloy_fusion(
                            vectorstore=st.session_state.vectorstore,
                    ...
                    message_dict = {
                        "role": "assistant",
                        "content": answer,
                        "sources": retrieved_docs if safe_get_session_state("show_sources") else None,
                        "relevance": relevance
                    }
                    if safe_get_session_state("enable_alloy_fusion"):
                    ...
                    if safe_get_session_state("enable_alloy_fusion") and metadata.get("fusion_enabled") and metadata.get("fused_properties"):
...
<br><span class="fusion-badge">🧮 Physics validation: Thermodynamic bounds, composition checks</span>
    ...
    if st.session_state.llm_model_choice and not is_ollama_model(st.session_state.llm_model_choice):
        mem_info = estimate_model_memory(st.session_state.llm_model_choice, safe_get_session_state('use_4bit_quantization', True))
        available_vram = get_available_gpu_memory()
...
<ul><li>Using 4-bit quantization (already enabled)</li><li>Selecting a smaller model</li><li>Using Ollama backend for better memory management</li></ul></div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        if safe_get_session_state("processing_complete"):
        ...
        if st.session_state.processed_files:
            if st.button("🗑️ Clear All", use_container_width=True):
                ...
                st.rerun()
    with col2:
        if safe_get_session_state("processing_complete") and st.session_state.vectorstore:
