...
try:
    ...
    CROSSREF_AVAILABLE = False  # crossrefapi is often not installed; set False to avoid import error
except ImportError:
...
def extract_metadata_from_pdf_text(text: str, filename: str) -> BibliographicMetadata:
    ...
    # FIXED: properly escaped regex patterns with \n
    title_patterns = [
        re.compile(r'(?:^|\n)([A-Z][^.\n]{20,150}(?:\.[^A-Z]|$))'),
    ...
    def _build_reasoning_prompt(self, retrieved_docs, query, consensus_data, contradictions, claims) -> str:
        ...
        if consensus_data:
            ...
            for cons in consensus_data[:3]:
                consensus_text += f"- {cons['entity']} ({cons['domain']}): {cons['mean']:.2f} ± {cons['std']:.2f} {cons['unit']} across {cons['doc_count']} papers (n={cons['value_count']})\n"
        if contradictions:
            contradiction_text = "\nDetected Contradictions:\n"
            for contr in contradictions[:3]:
                contradiction_text += f"- {contr['entity']}: {Path(contr['doc_a']).stem}={contr['mean_a']:.2f} vs {Path(contr['doc_b']).stem}={contr['mean_b']:.2f} (ratio {contr['ratio']:.1f}x, {contr['severity']})\n"
        if claims:
...
class VisualizationEngine:
    ...
    def plot_chord_cooccurrence(self, top_n: int = 14) -> go.Figure:
        ...
        for i in range(len(entities)):
            for j in range(i + 1, len(entities)):
                if mat[i][j] > 0:
                    fig.add_trace(go.Scatterpolar(r=[0.2, 0.6, 0.2], theta=[np.degrees(angles[i]), np.degrees((angles[i] + angles[j]) / 2), np.degrees(angles[j])], mode='lines', line=dict(color='rgba(100,100,100,0.3)', width=min(mat[i][j], 3)), showlegend=False, hoverinfo='skip'))
        fig.update_layout(polar=dict(radialaxis=dict(visible=False), angularaxis=dict(visible=False)), title="Entity Co-occurrence Chord Diagram (Document-level)", height=700, width=700)
        return fig
    ...
    def plot_methods_sunburst(self) -> go.Figure:
        ...
        if df.empty:
            ...
            return fig
        fig = px.sunburst(df, path=["domain", "category", "subcategory", "entity"], values="value", color="doc_count", color_continuous_scale="Blues", title="Hierarchical Methods Taxonomy<br><sub>Experimental → Microscopy/Spectroscopy | Computational → FEM/MD/DFT/Phase-Field/ML</sub>")
        return fig
    ...
    def plot_materials_sunburst(self) -> go.Figure:
        ...
        if df.empty:
            ...
            return fig
        fig = px.sunburst(df, path=["domain", "category", "subcategory", "entity"], values="value", color="doc_count", color_continuous_scale="Greens", title="Material System Hierarchy<br><sub>Pure → Binary → Ternary → HEA → Compound → Polymer</sub>")
        return fig
    ...
    def plot_topics_sunburst(self) -> go.Figure:
        ...
        if df.empty:
            ...
            return fig
        fig = px.sunburst(df, path=["topic"], values="count", color="count", color_continuous_scale="Oranges", title="Study Topics Distribution<br><sub>Laser, Microstructure, Interface, AM, Simulation...</sub>")
        return fig

    def plot_document_radar(self) -> go.Figure:
        categories = ["Laser Parameters", "Materials", "Exp. Methods", "Simulation", "Phenomena", "Properties"]
        cat_map = {"Laser Parameters": ["PARAMETER"], "Materials": ["MATERIAL"], "Exp. Methods": ["METHOD:Experimental"], "Simulation": ["METHOD:Computational"], "Phenomena": ["PHENOMENON"], "Properties": ["PARAMETER:Outcome"]}
        fig = go.Figure()
        for doc_id in self.graph.documents:
            values = []
        ...
        fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, max(5, max([max(t.r) for t in fig.data] or [5]))])), showlegend=True, title="Document Coverage Profiles (Radar)")
        return fig
    ...
    def plot_consensus_waterfall(self, top_n: int = 10) -> go.Figure:
        ...
        fig.update_layout(title="Cross-Document Consensus Waterfall<br><sub>Green = strong consensus (≥3 docs), Blue = emerging</sub>", yaxis_title="Mean Value", xaxis_tickangle=-45, height=500)
        return fig
    ...
    def plot_temporal_timeline(self) -> go.Figure:
        ...
        if df.empty:
            ...
            return fig
        fig = px.scatter(df, x="year", y="topic", color="doc", symbol="doc", title="Research Topic Timeline by Document", labels={"year": "Publication Year", "topic": "Topic"}, height=500)
    ...
    def plot_entity_treemap(self) -> go.Figure:
        ...
        if df.empty:
            ...
            return fig
        fig = px.treemap(df, path=["domain", "category", "subcategory", "entity"], values="value", color="docs", color_continuous_scale="Viridis", title="Hierarchical Entity Treemap")
        return fig
...
def initialize_session_state():
    defaults = {
        ...
        "llm_device_or_host": None, "llm_backend_type": None, "embeddings": None,
        "processing_complete": False, "laser_domain_boost": True, "show_sources": True,
        "citation_style": "apa", "max_retrieved_chunks": 6, "use_4bit_quantization": True,
        "ollama_host": "http://localhost:11434", "metadata_cache": metadata_cache,
        "knowledge_graph": None, "reasoning_mode": True, "show_reasoning_chain": True,
        "cross_doc_consensus": True, "feedback_map": {}, "precision_recall": None,
        "show_network": False, "selected_entity": None, "plot_code": "", "last_plot_fig": None,
        "reasoning_chain": None, "visualization_engine": None,
    }
...
def get_available_gpu_memory() -> Optional[float]:
    if not torch.cuda.is_available():
        return None
    try:
        return torch.cuda.get_device_properties(0).total_memory / (1024 ** 3) - torch.cuda.memory_reserved(0) / (1024 ** 3)
    except:
        return None
...
def generate_local_response_transformers(tokenizer, model, device: str, prompt: str, backend_name: str) -> str:
    try:
        if "Qwen" in backend_name or "qwen" in backend_name.lower():
            messages = [{"role": "system", "content": "You are an expert in laser-microstructure interaction research. Synthesize evidence across multiple papers rigorously."}, {"role": "user", "content": prompt}]
...
def retrieve_and_answer(vectorstore, graph: EnhancedCrossDocumentKnowledgeGraph, tokenizer, model, device_or_host: str, backend: str, backend_type: str, query: str, k: int = None, score_threshold: float = None) -> Tuple[str, List[Document], float, Dict[str, Any], Optional[ReasoningChain]]:
    ...
    def llm_generate(prompt: str) -> str:
        return generate_local_response(tokenizer=tokenizer, model_or_tag=model, device_or_host=device_or_host, prompt=prompt, backend=backend, backend_type=backend_type)
...
def plot_consensus_chart(consensus_data: List[Dict]) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=entities, y=means, error_y=dict(type='data', array=stds, visible=True), name='Consensus value', text=[f"{m:.2f} ± {s:.2f} ({u})" for m, s, u in zip(means, stds, units)], hoverinfo='text'))
...
def render_contradiction_table(contradictions: List[Dict]) -> pd.DataFrame:
    ...
    for c in contradictions:
        rows.append({"Entity": c['entity'], "Doc A": Path(c['doc_a']).stem, "Mean A": f"{c['mean_a']:.3f}", "Doc B": Path(c['doc_b']).stem, "Mean B": f"{c['mean_b']:.3f}", "Ratio": f"{c['ratio']:.1f}x", "Severity": c['severity']})
    return pd.DataFrame(rows)

def extract_equations_from_text(text: str) -> List[str]:
    # FIXED: unterminated string literal originally here; now properly closed.
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.knowledge_graph and st.session_state.knowledge_graph.entities:
            ...
            if entity_options:
                st.session_state.selected_entity = st.sidebar.selectbox("Explore entity consensus", options=entity_options, format_func=lambda x: x, key="selected_entity_select")
            else:
...
def render_document_uploader():
    ...
    uploaded_files = st.file_uploader("Select PDF or TXT files about laser processing, multicomponent alloys, additive manufacturing, etc.", type=["pdf", "txt"], accept_multiple_files=True, help="Documents will be processed with semantic section detection and cross-document entity linking.")
    ...
    if prompt := st.chat_input("Ask a cross-document scientific question..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking across documents..."):
                ...
                st.markdown(answer)
                st.session_state.messages.append({"role": "assistant", "content": answer, "sources": retrieved_docs, "reasoning_meta": reasoning_meta, "reasoning_chain": chain})
...
def main():
    st.set_page_config(page_title="🔬 Laser Microstructure RAG + Cross-Doc Reasoning", page_icon="🔬", layout="wide", initial_sidebar_state="expanded")
    st.markdown("""<style>
...
.contradiction-badge { display: inline-block; background: #fee2e2; color: #991b1b; padding: 0.2rem 0.6rem; border-radius: 0.25rem; font-size: 0.85rem; margin: 0.1rem 0.2rem 0.1rem 0; }
    ...
    with col2:
        if st.session_state.processing_complete and st.session_state.vectorstore:
            ...
            st.markdown("""<div class="info-card"><h3>👋 Welcome to Cross-Document Scientific Reasoning!</h3><p>This assistant goes beyond simple retrieval:</p><ul><li><strong>Semantic Chunking:</strong> Preserves Abstract/Methods/Results/Discussion structure</li><li><strong>Entity Extraction:</strong> Identifies materials, parameters, methods automatically</li><li><strong>Cross-Document Alignment:</strong> Links the same entity across different papers</li><li><strong>Consensus Detection:</strong> Statistically aggregates values reported in multiple papers</li><li><strong>Contradiction Flagging:</strong> Highlights when papers disagree significantly</li><li><strong>Multi-Hop Retrieval:</strong> Follows entity links to find related evidence</li><li><strong>Uncertainty Calibration:</strong> Explicit confidence levels in every answer</li></ul><p><strong>Getting started:</strong></p><ol><li>Upload 2+ PDF/TXT papers on multicomponent alloys or laser processing</li><li>Enable "Cross-document reasoning" in sidebar</li><li>Ask comparative or synthesizing questions</li><li>Expand "🧠 Reasoning Chain" to see the logical steps</li></ol></div>""", unsafe_allow_html=True)
            st.markdown("**Try asking:**")
            demo_qs = ["What is the effect of laser power on interfacial IMC thickness in Sn‑Ag‑Cu/Cu joints?", "Do these papers agree on the optimal hatch distance for defect‑free LPBF of Al‑Cr‑Fe‑Ni alloys?", "Summarize the phase‑field models used for simulating selective laser melting of multicomponent alloys.", "How does the composition of high entropy alloys affect their thermal conductivity during laser processing?"]
            for q in demo_qs:
    ...
    if st.session_state.knowledge_graph and st.session_state.processing_complete:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Top Entities & Consensus", "🕸️ Knowledge Graphs", "☀️ Hierarchical Sunbursts", "📡 Document Profiles", "⚡ Contradictions"])
        with tab1:
            c1, c2 = st.columns(2)
            with c1:
                summary = st.session_state.knowledge_graph.get_knowledge_summary()
                if summary['top_entities']:
                    fig = px.bar(x=[x[0] for x in summary['top_entities']], y=[x[1] for x in summary['top_entities']], labels={'x': 'Entity', 'y': 'Occurrences'}, title="Top Entities by Frequency", color=[x[1] for x in summary['top_entities']], color_continuous_scale="Viridis")
