...
# -*- coding: utf-8 -*-
"""
LASER MICROSTRUCTURE RAG CHATBOT - ENHANCED WITH PHYSICS-AWARE EVALUATION
========================================================================================
...
• Fixed StreamlitDuplicateElementKey for fusion property selectors
• Enhanced logging, error recovery, and session state management
• Expanded UI with detailed tooltips, expanders, and robust fallback mechanisms
...
For visualization metrics: pip install scikit-image opencv-python
"""
...
# Suppress non-critical warnings to keep console clean
warnings.filterwarnings("ignore", category=UserWarning)
warnings.filterwarnings("ignore", category=FutureWarning)

# Configure logging for detailed runtime tracking
logging.basicConfig(
    level=logging.INFO, 
    format='%(asctime)s - %(levelname)s - [%(name)s] - %(message)s',
    handlers=[logging.StreamHandler(sys.stdout)]
)
logger = logging.getLogger("LaserMicrostructureRAG")
...
try:
    ...
    logger.warning("Ollama library not found. Install via: pip install ollama")
...
try:
    ...
    logger.warning(f"pdf2doi not available: {e}")
...
try:
    ...
    logger.warning("Crossref API not available. Install via: pip install crossrefapi")
...
class RetrievalEvaluator:
    ...
    def compute_context_relevance(self, query: str, retrieved_chunks: List[Document]) -> float:
        ...
        try:
            ...
            logger.error(f"Context relevance computation failed: {e}")
            return 0.0
...
class PhysicsFaithfulnessChecker:
    ...
    def check_faithfulness_to_context(self, llm_answer: str, retrieved_chunks: List[Document]) -> Dict:
        ...
        if not SKLEARN_AVAILABLE or not retrieved_chunks:
            return {
                "faithfulness_score": 0.5, "max_context_similarity": 0.0,
                "mean_context_similarity": 0.0, "keyword_overlap": 0.0,
                "hallucinated_numbers": [], "is_faithful": False
            }
        try:
...
class MicrostructureComparator:
    ...
    def load_field_data(self, file_path: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.error(f"Failed to load field data: {e}")
            return None
    ...
    def compute_morphological_metrics(self, binary_image: np.ndarray) -> Dict:
        ...
        try:
            ...
            logger.error(f"Morphological metric computation failed: {e}")
            return {"n_grains": 0, "avg_grain_size": 0, "grain_size_std": 0, "interface_density": 0, "phase_fraction": 0}
...
def run_benchmark_evaluation(vectorstore, evaluator: RetrievalEvaluator, k: int = 5):
    ...
    for bench in DECLARMIMA_BENCHMARK_QUERIES:
        ...
        results.append({
            "query": query, "category": bench["category"],
            "recall@5": metrics.recall_at_k.get(5, 0),
            "precision@5": metrics.precision_at_k.get(5, 0),
            "mrr": metrics.mrr, "context_relevance": metrics.context_relevance
        })
...
class StructuredDataLoader:
    """Load and chunk structured simulation data for RAG KB"""
    def load_csv_dataset(self, file_path: str, description: str = "") -> List[Document]:
        ...
        documents.append(Document(page_content=global_desc, metadata={"source": file_path, "type": "csv_global", "chunk_index": 0}))
        desc = df.describe()
        for col in df.select_dtypes(include=[np.number]).columns:
            chunk_text = f"Column '{col}' statistics: "
            chunk_text += f"mean={desc.loc['mean', col]:.4f}, std={desc.loc['std', col]:.4f}, "
            chunk_text += f"min={desc.loc['min', col]:.4f}, max={desc.loc['max', col]:.4f}, "
            chunk_text += f"median={df[col].median():.4f}. "
            chunk_text += f"Sample values: {', '.join([f'{v:.4f}' for v in df[col].dropna().head(5)])}"
            documents.append(Document(page_content=chunk_text, metadata={"source": file_path, "type": "csv_column", "column": col, "chunk_index": len(documents)}))
        return documents
    ...
    def load_tdb_thermodynamic_database(self, file_path: str) -> List[Document]:
        ...
        try:
            ...
            logger.error(f"TDB load failed: {e}")
            return []
        documents = []
        ...
        for block in phase_blocks[1:]:
            ...
            documents.append(Document(page_content=chunk_text, metadata={"source": file_path, "type": "tdb_phase", "phase": phase_name, "chunk_index": len(documents)}))
        ...
        documents.insert(0, Document(page_content=system_chunk, metadata={"source": file_path, "type": "tdb_system", "chunk_index": 0}))
        return documents
...
class ExtractedProperty:
    ...
    def _normalize_property_name(self, name: str) -> str:
        synonym_map = {
            ...
            "wavelength": "wavelength", "laser wavelength": "wavelength", "repetition rate": "repetition_rate",
            "pulse frequency": "repetition_rate", "spot size": "spot_size", "beam diameter": "spot_size",
            "fluence": "fluence", "laser fluence": "fluence", "yield strength": "yield_strength",
            "ys": "yield_strength", "ultimate tensile strength": "ultimate_tensile_strength",
            "uts": "ultimate_tensile_strength", "tensile strength": "ultimate_tensile_strength",
            "elongation": "elongation_at_break", "elongation at break": "elongation_at_break",
            "hardness": "hardness", "microhardness": "hardness", "vickers hardness": "hardness",
            "hv": "hardness", "rockwell hardness": "hardness", "brinell hardness": "hardness",
            "young modulus": "elastic_modulus", "elastic modulus": "elastic_modulus",
            "shear modulus": "shear_modulus", "bulk modulus": "bulk_modulus", "fracture toughness": "fracture_toughness",
            "fatigue strength": "fatigue_strength", "creep resistance": "creep_resistance",
        }
...
class DocumentFusionRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "chunk_id": self.chunk_id, "citation": self.bibliographic_citation,
            "material": self.material_system, "method": self.processing_method,
            "properties": [p.to_dict() for p in self.extracted_properties],
            "topics": self.laser_topics, "conditions": self.experimental_conditions
        }
...
class FusedPropertyEntry:
    ...
    def to_comparison_row(self) -> Dict[str, Any]:
        return {
            "property": self.property_name, "value": self.fused_value, "unit": self.unit,
            ...
            "sources": len(self.sources), "confidence": self.fusion_confidence.value,
...
class FusionEfficiencyMetrics:
    ...
    def compute_overall(self) -> float:
        ...
        self.overall_fusion_efficiency = sum(components) / total_weight if total_weight > 0 else 0.0
        return self.overall_fusion_efficiency
...
class BibliographicMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "year": self.year, "volume": self.volume, "issue": self.issue,
            "pages": self.pages, "publisher": self.publisher,
            "extraction_method": self.extraction_method, "confidence": self.confidence,
            "citation_apa": self.format_citation("apa"), "citation_doi": self.format_citation("doi"),
...
def extract_metadata_from_pdf_file(pdf_path: str, filename: str) -> BibliographicMetadata:
    ...
    if PYPDF2_AVAILABLE:
        try:
            ...
            field_mapping = {'/Title': 'title', '/Author': 'authors', '/CreationDate': 'year', '/Subject': 'journal', '/Keywords': 'keywords'}
...
class MultiDocumentPropertyExtractor:
    UNIT_CONVERSIONS = {
        "nm": {"factor": 1e-9, "base": "m"}, "μm": {"factor": 1e-6, "base": "m"}, "um": {"factor": 1e-6, "base": "m"},
        "mm": {"factor": 1e-3, "base": "m"}, "fs": {"factor": 1e-15, "base": "s"}, "ps": {"factor": 1e-12, "base": "s"},
        "ns": {"factor": 1e-9, "base": "s"}, "J/cm²": {"factor": 1e4, "base": "J/m²"}, "J/cm2": {"factor": 1e4, "base": "J/m²"},
        "mJ/cm²": {"factor": 10, "base": "J/m²"}, "MPa": {"factor": 1e6, "base": "Pa"}, "GPa": {"factor": 1e9, "base": "Pa"},
        "kN": {"factor": 1e3, "base": "N"}, "HV": {"factor": 1, "base": "HV"}, "HRC": {"factor": 1, "base": "HRC"}, "HB": {"factor": 1, "base": "HB"},
    }
    MATERIAL_SYNONYMS = {
    ...
    def extract_properties_from_chunk(self, chunk_text: str, chunk_metadata: Dict[str, Any]) -> DocumentFusionRecord:
        record = DocumentFusionRecord(
            ...
            laser_topics=chunk_metadata.get('laser_topics', []), experimental_conditions=chunk_metadata.get('parameters_found', {}),
            material_system=self._detect_material_system(chunk_text), processing_method=self._detect_processing_method(chunk_text)
        )
    ...
    def _detect_processing_method(self, text: str) -> Optional[str]:
        ...
        methods = [
            ...
            ("laser ablation", "laser_ablation"), ("aging", "aging_treatment"), ("annealing", "annealing"),
            ("heat treatment", "heat_treatment"), ("surface texturing", "surface_texturing"), ("lipss", "lipss_formation"),
    ...
    def _extract_from_tables(self, text: str) -> List[ExtractedProperty]:
        properties = []
        if r'\begin{tabular}' in text or r'\begin{table}' in text: properties.extend(self._parse_latex_table(text))
        elif '|' in text and re.search(r'\|\s*[-:]+\s*\|', text): properties.extend(self._parse_markdown_table(text))
        elif self._detect_plain_text_table(text): properties.extend(self._parse_plain_text_table(text))
        return properties
    ...
    def _parse_latex_table(self, latex_text: str) -> List[ExtractedProperty]:
        ...
        if not table_match: return properties
        ...
        for row in rows:
            ...
            else: data_rows.append(cells)
        if not header_row or len(data_rows) == 0: return properties
        ...
        for row in data_rows:
            if len(row) <= max(property_cols, default=-1): continue
            row_conditions = {}
            ...
            for prop_col in property_cols:
                if prop_col >= len(row) or not row[prop_col].strip(): continue
                ...
                if parsed:
                    prop = ExtractedProperty(name=prop_name, value=parsed['value'], unit=parsed['unit'],
    ...
    def _parse_markdown_table(self, text: str) -> List[ExtractedProperty]:
        ...
        if len(lines) < 3: return properties
        ...
        for line in lines[data_start:]:
            ...
            for header, value in row_data.items():
                ...
                if any(kw in header.lower() for kw in ['strength', 'threshold', 'duration', 'fluence', 'elongation', 'hardness', 'modulus', 'yield', 'tensile', 'hv', 'hrc', 'hb']):
                    ...
                    if parsed:
                        prop = ExtractedProperty(name=header, value=parsed['value'], unit=parsed['unit'],
        ...
        for line in lines:
            ...
            if len(numeric_tokens) >= 2:
                ...
                if len(tokens) >= 3:
                    ...
                    if value_match:
                        try:
                            prop = ExtractedProperty(name=prop_name, value=float(value_match.group(1)),
    ...
    def _extract_inline_properties(self, text: str) -> List[ExtractedProperty]:
        ...
        for match in self.property_pattern.finditer(text):
            ...
            if len(groups) >= 5 and groups[1]:
                ...
                prop = ExtractedProperty(name=prop_name, value=numeric_value if numeric_value is not None else value_str,
    ...
    def _parse_property_value(self, raw_value: str, prop_name: str) -> Optional[Dict[str, Any]]:
        if not raw_value or raw_value.strip() in ['-', '.', '', 'N/A', 'n/a', 'NA', 'na', '--', '...']: return None
        ...
        if numeric_value is not None: result["value"] = numeric_value
        else: result["value"] = raw_value.strip() if raw_value.strip() else None
        return result if result["value"] is not None else None

    def _safe_parse_numeric(self, value_str: str) -> Optional[float]:
        if not value_str: return None
        cleaned = value_str.strip()
        if cleaned in ['.', '-', '--', '...', 'N/A', 'n/a', 'NA', 'na', 'null', 'None', '']: return None
        if '-' in cleaned and not cleaned.startswith('-'):
            parts = cleaned.split('-')
            if len(parts) == 2: cleaned = parts[0].strip()
...
class MultiDocumentFusionEngine:
    ...
    def _fuse_property_group(self, prop_name: str, properties: List[ExtractedProperty]) -> Optional[FusedPropertyEntry]:
        ...
        fused = FusedPropertyEntry(
            property_name=prop_name, fused_value=None,
    ...
    def _compute_fusion_metrics(self, fusion_records: List[DocumentFusionRecord],
        ...
        if fused_properties:
            ...
            if uncertainties: metrics.average_uncertainty_magnitude = np.mean(uncertainties)
            else: metrics.average_uncertainty_magnitude = 0.1
        else:
    ...
    def _estimate_answer_specificity(self, query: str, fused_props: Dict[str, FusedPropertyEntry]) -> float:
        if not fused_props:
            ...
            if any(kw in query_lower for kw in ['compare', 'versus', 'vs', 'difference', 'threshold', 'strength', 'hardness']): return 0.5
            return 0.3
        query_lower = query.lower()
    ...
    def generate_comparison_table(self, fused_properties: Dict[str, FusedPropertyEntry], format: str = "markdown") -> str:
        ...
        if format == "markdown": return self._generate_markdown_table(fused_properties)
        elif format == "latex": return self._generate_latex_table(fused_properties)
        elif format == "html": return self._generate_html_table(fused_properties)
        else: return self._generate_plain_text_table(fused_properties)
    ...
    def _generate_latex_table(self, fused_props: Dict[str, FusedPropertyEntry]) -> str:
        lines = [r"\begin{tabular}{|l|c|c|c|c|c|}", r"\hline", r"\textbf{Property} & \textbf{Value} & \textbf{Unit} & \textbf{Range} & \textbf{Sources} & \textbf{Confidence} \\", r"\hline"]
        for prop_name, entry in sorted(fused_props.items()):
            if entry.fused_value is not None and isinstance(entry.fused_value, (int, float)):
                value_str = f"{entry.fused_value:.3f}"
                if entry.standard_deviation is not None: value_str = f"{value_str} \\pm {entry.standard_deviation:.3f}"
            else: value_str = str(entry.fused_value) if entry.fused_value is not None else "--"
            range_str = f"{entry.value_range[0]:.2f}--{entry.value_range[1]:.2f}" if entry.value_range else "--"
...
def initialize_session_state():
    defaults = {
        ...
        "inference_backend": "Hugging Face Transformers", "llm_device_or_host": None, "llm_backend_type": None
    }
...
def load_local_embeddings():
    try:
        embeddings = HuggingFaceEmbeddings(model_name=LOCAL_EMBEDDING_MODEL, model_kwargs={'device': 'cpu'}, encode_kwargs={'normalize_embeddings': True})
        return embeddings
    except Exception as e:
...
- Available VRAM: {f'{available_vram:.1f}GB' if available_vram else 'N/A (CPU)'}""")
    ...
    if quantization_config: model_kwargs["quantization_config"] = quantization_config
    ...
    try:
        ...
        with torch.no_grad():
            outputs = model.generate(inputs, max_new_tokens=LASER_DOMAIN_CONFIG["max_new_tokens"],
                                     temperature=LASER_DOMAIN_CONFIG["temperature"], do_sample=(LASER_DOMAIN_CONFIG["temperature"] > 0),
...
def render_sidebar():
    with st.sidebar:
        ...
        st.session_state.laser_domain_boost = st.checkbox("Boost laser-topic relevance", value=True, help="Prioritize chunks containing laser-specific keywords")
        st.session_state.show_sources = st.checkbox("Show source citations", value=True, help="Display which documents chunks came from")
        st.session_state.enable_multi_doc_fusion = st.checkbox("🔗 Enable Multi-Document Fusion", value=True, help="Enable cross-document property extraction and comparison")
        st.session_state.debug_extraction = st.checkbox("🐛 Debug Property Extraction", value=False, help="Show extracted properties in UI for diagnosis")
        st.session_state.evaluation_mode = st.checkbox("📊 Enable Evaluation Mode", value=False, help="Show retrieval metrics and physics validation")
        ...
        st.session_state.citation_style = st.selectbox("Citation display style", options=["apa", "doi", "full", "short"], index=0,
                                                       format_func=lambda x: {"apa": "APA: FirstAuthor et al., Journal, Year", "doi": "DOI: 10.xxxx/xxxxx", "full": "Full: Author (Year). Title. Journal, Vol(Issue), Pages", "short": "Short: [FirstAuthor Year] or [DOI]"}[x],
                                                       help="How citations appear in responses and source lists")
        st.session_state.max_retrieved_chunks = st.slider("Chunks to retrieve", min_value=2, max_value=8, value=4, help="More chunks = more context but slower responses")
        st.markdown("---")
...
<li>📊 Enable evaluation mode for metrics</li>
        ...
        if PDF2DOI_AVAILABLE: st.success("✅ pdf2doi: Available for DOI lookup")
        else: st.info("ℹ️ pdf2doi: Install with `pip install pdf2doi` for enhanced DOI extraction")
        if CROSSREF_AVAILABLE: st.success("✅ Crossref API: Available for metadata enrichment")
        else: st.info("ℹ️ Crossref: Install with `pip install crossrefapi` for journal/author lookup")
        if backend_option == "Ollama (if installed)" and OLLAMA_AVAILABLE:
            ...
            except: st.error("❌ Cannot connect to Ollama")
...
def render_fusion_metrics_panel(fusion_metadata: Dict[str, Any]):
    ...
    with st.expander("📊 Information Fusion Efficiency", expanded=True):
        ...
        if fused_props:
            ...
            if conflicts: st.warning(f"⚠️ {len(conflicts)} property(ies) have low-confidence fusion: {', '.join(conflicts[:3])}")
...
def render_comparison_table_in_chat(comparison_table: Optional[str], fused_properties: Dict, unique_key_suffix: str = ""):
    """Fixed to prevent StreamlitDuplicateElementKey by accepting a dynamic suffix"""
    ...
    with st.expander("📋 Property Comparison Table", expanded=False):
        ...
        if fused_properties:
            # Dynamically generate unique key to avoid duplicate key errors across chat messages
            selectbox_key = f"fusion_prop_select_{unique_key_suffix}" if unique_key_suffix else "fusion_prop_select_global"
            selected_prop = st.selectbox("🔍 Explore property details:", options=["Select a property..."] + list(fused_properties.keys()), key=selectbox_key)
...
def render_evaluation_dashboard(vectorstore, embed_model):
    ...
    with tabs[0]:
        ...
        test_query = st.text_input("Enter test query:", "What is the Gibbs free energy for FCC Fe-Cr at 843K?", key="eval_query")
    ...
    with tabs[1]:
        ...
        sample_answer = st.text_area("Paste LLM answer to validate:", height=200, key="sample_answer")
...
def render_chat_interface():
    ...
    if not has_model:
        ...
        return
    # Iterate with index to pass unique suffix for UI keys
                    ...
                    for i, src in enumerate(message["sources"], 1):
                        ...
                        if bib and any(bib.get(k) for k in ['doi', 'authors', 'journal', 'year']):
                            with st.expander("🔍 Bibliographic Details"):
                                ...
                                if bib.get('arxiv_id'): st.markdown(f"**arXiv:** `{bib['arxiv_id']}`")
                                ...
                                if bib.get('volume'):
                                    ...
                                    if bib.get('issue'): vol_str += f"({bib['issue']})"
            ...
            with st.spinner("🔍 Retrieving, fusing multi-document data, and generating..."):
                try:
                    ...
                    if relevance > 0:
                        ...
                        if fusion_eff is not None: st.caption(f"📊 Response relevance: {relevance:.2f}/1.0 | Fusion efficiency: {fusion_eff:.2f}/1.0")
                        else: st.caption(f"📊 Response relevance: {relevance:.2f}/1.0")
                except Exception as e:
...
<br><span class="fusion-badge">📊 Evaluation mode: Retrieval metrics, physics validation, benchmark suite</span>
    ...
    with col1:
        ...
        if st.session_state.processing_complete:
            ...
            if st.session_state.all_chunks:
                ...
                if citation: st.markdown(f'<span class="citation-badge">📝 Sample citation: {citation}</span>', unsafe_allow_html=True)
        elif uploaded_files:
    ...
    with col2:
        if st.session_state.processing_complete and st.session_state.vectorstore:
            if st.session_state.evaluation_mode:
                ...
                render_evaluation_dashboard(st.session_state.vectorstore, load_local_embeddings())
                st.divider()
...
<li>📊 <strong>Retrieval quality metrics</strong> (Recall@k, Precision@k, MRR)</li>
<li>🧮 <strong>Physics validation</strong> (thermodynamic consistency, numerical bounds)</li>
<li>🎯 <strong>Benchmark suite</strong> for DECLARMIMA domain</li></ul>
...
<li>📈 Evaluation dashboard with trends</li></ul>
...
<li>Enable "Evaluation Mode" for metrics dashboard</li><li>Start asking technical questions!</li></ol></div>""", unsafe_allow_html=True)
            ...
            st.info("📊 Processing complete to access chat interface")
    render_footer()
