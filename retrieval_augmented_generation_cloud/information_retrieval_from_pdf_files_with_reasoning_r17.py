...
========================================================================================
UPGRADED VERSION (CODE 16+): Enhanced Salience Awareness & Publication-Quality Visualizations
...
# =============================================
# UPGRADED: 
#   - Multicomponent alloy added as core pillar
#   - Semantic similarity detection for core pillar relatives
#   - 50+ related concepts automatically boosted via embedding similarity
# =============================================
...
class FullTextConceptExtractor:
    ...
    def __init__(self, embed_model, proposal_text: str = None):
        ...
        # =====================================================================
        # ENHANCED CORE PILLARS: Now includes MULTICOMPONENT ALLOY
        # =====================================================================
        self.core_pillars = {
            "laser": 1.00,
            "microstructure": 1.00,
        ...
        # =====================================================================
        # ENHANCED DOMAIN SEEDS: Expanded multicomponent alloy family
        # =====================================================================
        self.domain_seeds = {
            ...
            # Multicomponent alloy family seeds
        ...
        # =====================================================================
        # NEW: Semantic similarity cache for core pillar relatives
        # =====================================================================
        self._pillar_embeddings: Dict[str, np.ndarray] = {}
        self._semantic_boost_threshold = 0.72  # cosine similarity threshold
        self._semantic_boost_factor = 0.35     # boost amount for semantically related
    ...
    def _compute_semantic_pillar_embeddings(self):
        """Pre-compute embeddings for core pillars to detect semantic relatives."""
    ...
    def _get_semantic_boost(self, concept: str, concept_embedding: np.ndarray) -> float:
        """Compute semantic similarity boost if concept is related to core pillars."""
    ...
    def extract_concepts(self, chunks: List[Document], min_salience: float = 0.42) -> Tuple[List[str], Dict[str, Dict]]:
        ...
        # Pre-compute candidate embeddings for semantic boost
        try:
        ...
        for idx, concept in enumerate(candidates):
            ...
            # NEW: Semantic similarity boost for pillar relatives
    ...
    def _extract_candidates(self, chunks: List[Document]) -> List[str]:
        ...
        for chunk in chunks:
            ...
            # NEW: Detect multicomponent alloy related phrases
            mca_patterns = [
...
class EnhancedCrossDocumentKnowledgeGraph:
    def __init__(self):
        ...
        # Store rich metadata including salience
...
# =============================================
# UPGRADED with:
#   - 50+ colormaps (jet, turbo, inferno, rainbow, etc.)
#   - Customizable fonts for all labels
#   - UMAP, t-SNE, PCA dimensionality reduction
#   - Bokeh + HoloViews chord diagrams
#   - Enhanced PyVis networks
...
class PublicationQualityVisualizationEngine:
    ...
    # 50+ Colormap options for publication quality
    COLORMAP_OPTIONS = {
    ...
    def __init__(self, graph: EnhancedCrossDocumentKnowledgeGraph,
        ...
        # Apply global matplotlib settings for publication quality
    ...
    def _get_domain_color(self, domain: str, colormap: Optional[str] = None, index: int = 0, total: int = 1) -> str:
        """Get color for a domain, optionally from a colormap."""
    ...
    # =====================================================================
    # STATIC KNOWLEDGE NETWORK (FIXED matplotlib NameError)
    # =====================================================================
    def plot_static_knowledge_network(self, top_n: int = 25, figsize: Tuple[int, int] = (14, 12),
        ...
        FIXED: Uses mcolors instead of matplotlib.colors to avoid NameError.
        """
        G = nx.Graph()
        ...
        # Color entities by domain using colormap
        ...
        for node in ent_nodes:
            ...
            # FIXED: Use mcolors instead of matplotlib.colors
    ...
    # =====================================================================
    # CHORD DIAGRAM (Plotly version - salience aware)
    # =====================================================================
    def plot_chord_cooccurrence(self, top_n: int = 14, colormap: Optional[str] = None) -> go.Figure:
    ...
    # =====================================================================
    # BOKEH + HOLOVIEWS CHORD DIAGRAM
    # =====================================================================
    def plot_bokeh_chord(self, top_n: int = 20, colormap: str = "Category20", 
                         width: int = 800, height: int = 800) -> Optional[Any]:
        """Create an interactive Bokeh/HoloViews chord diagram.

        Returns a Bokeh figure or None if libraries not available.
        """
        if not BOKEH_AVAILABLE:
            st.warning("Bokeh not installed. Run: pip install bokeh")
            return None
        ...
        for (i, j), w in zip(edge_list, edge_weights):
            ...
            # Simple straight lines for now, can be enhanced
    ...
    # =====================================================================
    def plot_holoviews_chord(self, top_n: int = 20) -> Optional[Any]:
        """Create a HoloViews chord diagram if available."""
        if not HOLOVIEWS_AVAILABLE:
            st.warning("HoloViews not installed. Run: pip install holoviews")
            return None
    ...
    def plot_methods_sunburst(self, top_n_per_category: int = 20, colormap: Optional[str] = None) -> go.Figure:
    ...
    def plot_materials_sunburst(self, top_n_per_category: int = 20, colormap: Optional[str] = None) -> go.Figure:
    ...
    # =====================================================================
    def _get_entity_embeddings(self, embedding_fn: Callable, top_n: int = 80) -> Tuple[List[str], np.ndarray, List[str]]:
        """Get embeddings for top N entities by salience."""
        ent_counts = Counter({k: len(v) for k, v in self.graph.entities.items()})
        scored = [(ent, self.get_salience(ent) * ent_counts.get(ent, 1)) for ent in self.graph.entities.keys()]
    ...
    def plot_entity_tsne(self, embedding_fn: Callable, top_n: int = 80, 
                        perplexity: int = 30, colormap: Optional[str] = None,
                        figsize: Tuple[int, int] = (10, 8)) -> Optional[plt.Figure]:
        """t-SNE embedding visualization."""
        if not SKLEARN_AVAILABLE:
            return None

        top, embs, domains = self._get_entity_embeddings(embedding_fn, top_n)
        if len(top) < 5:
            return None
    ...
    def plot_entity_umap(self, embedding_fn: Callable, top_n: int = 80,
    ...
    def plot_entity_pca(self, embedding_fn: Callable, top_n: int = 80,
                       colormap: Optional[str] = None,
                       figsize: Tuple[int, int] = (10, 8)) -> Optional[plt.Figure]:
        """PCA embedding visualization with variance explanation."""
        if not SKLEARN_AVAILABLE:
            return None
    ...
    # TREEMAP
    # =====================================================================
    def plot_entity_treemap(self, colormap: Optional[str] = None) -> go.Figure:
        rows = []
        for norm, ents in self.graph.entities.items():
            if not ents:
                continue
    ...
    def render_pyvis_salience(self, nx_graph: nx.Graph, concept_abstract_map: Dict,
                              physics_enabled: bool = True, top_n_nodes: int = 0,
        ...
        for node in nx_graph.nodes():
            ...
            if self.is_core_pillar(node):
                color = "#dc2626"  # Red for core pillars
...
def initialize_session_state():
    defaults = {
        ...
        # NEW: Visualization customization settings
...
def process_documents(uploaded_files):
    ...
    with st.spinner(f"Processing {len(new_files)} PDF(s) with semantic chunking and salience extraction..."):
        try:
            ...
            # Full-text concept extraction with salience
...
def main():
    st.set_page_config(
        page_title="🔬 DECLARMIMA: Full-Text Concept Graph + Salience + Publication Viz",
        page_icon="🔬",
        layout="wide",
    ...
    st.markdown('<h1 class="main-header">🔬 DECLARMIMA: Enhanced Salience + Publication Viz</h1>', unsafe_allow_html=True)
    st.markdown("""
    ...
    Our system extracts concepts, computes <strong>salience</strong> (importance) based on frequency, cross‑document presence, 
    section, quantitative signal, proposal alignment, and <strong>semantic similarity to core pillars</strong>.<br>
            ...
            if st.session_state.knowledge_graph:
                ...
                if summary['high_salience_concepts']:
                    ...
                    for ent, meta in summary['high_salience_concepts'][:5]:
                        ...
                        st.markdown(f'<span class="{badge_class}">{ent} (salience {meta["salience"]:.2f})</span>', unsafe_allow_html=True)
        elif uploaded_files:
    ...
    with col2:
        if st.session_state.processing_complete and st.session_state.vectorstore:
            ...
            st.markdown("""
            <div class="info-card">
            <h3>👋 Welcome to Enhanced Salience + Publication-Quality Visualization</h3>
            ...
            <li>Explore the visualisation tabs with full customization</li>
            </ol>
            </div>
            """, unsafe_allow_html=True)
    ...
    if st.session_state.knowledge_graph and st.session_state.processing_complete:
        ...
        # Initialize visualization engine with user preferences
        ...
        # Colormap selector for this session
        ...
        with tab1:
            ...
            st.plotly_chart(viz.plot_entity_treemap(colormap=active_cmap), use_container_width=True)

        with tab2:
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("**Static Bipartite Network (Salience‑Aware)**")
                fig_net = viz.plot_static_knowledge_network(top_n=30, layout=layout, colormap=active_cmap)
                st.pyplot(fig_net)
            ...
            with c2:
                ...
                st.plotly_chart(viz.plot_chord_cooccurrence(top_n=16, colormap=active_cmap), use_container_width=True)

            # Bokeh chord diagram
            if BOKEH_AVAILABLE:
                st.markdown("**Interactive Chord-Style Network (Bokeh)**")
                bokeh_fig = viz.plot_bokeh_chord(top_n=20, colormap=active_cmap)
                if bokeh_fig:
                    from bokeh.embed import file_html
                    from bokeh.resources import CDN
        ...
        with tab3:
            ...
            with c1:
                st.plotly_chart(viz.plot_methods_sunburst(top_n_per_category=20, colormap=active_cmap), use_container_width=True)
            with c2:
                st.plotly_chart(viz.plot_materials_sunburst(top_n_per_category=20, colormap=active_cmap), use_container_width=True)
            with c3:
        ...
        with tab4:
            c1, c2 = st.columns(2)
            with c1:
                st.plotly_chart(viz.plot_document_radar(colormap=active_cmap), use_container_width=True)
            with c2:
        ...
        with tab6:
            ...
            with dr_col1:
                ...
                if SKLEARN_AVAILABLE:
                    fig_tsne = viz.plot_entity_tsne(embedding_fn=emb_wrapper, top_n=60, colormap=active_cmap)
                    if fig_tsne:
                        st.pyplot(fig_tsne)
                        buf = BytesIO()
            ...
            with dr_col2:
                st.markdown("**UMAP**")
                if UMAP_AVAILABLE:
                    fig_umap = viz.plot_entity_umap(embedding_fn=emb_wrapper, top_n=60, colormap=active_cmap)
                    if fig_umap:
                        st.pyplot(fig_umap)
                        buf = BytesIO()
            ...
            with dr_col3:
                st.markdown("**PCA**")
                if SKLEARN_AVAILABLE:
                    fig_pca = viz.plot_entity_pca(embedding_fn=emb_wrapper, top_n=60, colormap=active_cmap)
                    if fig_pca:
                        st.pyplot(fig_pca)
                        buf = BytesIO()
