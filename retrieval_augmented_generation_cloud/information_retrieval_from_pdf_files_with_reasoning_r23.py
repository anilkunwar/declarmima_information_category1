...
# =====================================================================
# FAST PARALLEL PDF PROCESSOR
# =====================================================================
class FastPDFProcessor:
    """
    High-performance PDF processing using PyMuPDF with multiprocessing.
    Falls back to PyPDFLoader if PyMuPDF is unavailable.
    """

    @staticmethod
    def extract_text_pymupdf(pdf_path: str) -> List[Document]:
        """Extract text from PDF using PyMuPDF (3-160x faster than PyPDF2)."""
        docs = []
        try:
    ...
    @staticmethod
    def process_single_pdf(args: Tuple) -> Tuple[str, List[Document], str]:
        """
        Process a single PDF file. Designed for multiprocessing.
        Returns: (filename, pages, temp_path)
        """
        file_obj, file_name, use_pymupdf = args
        ...
        try:
            if use_pymupdf and PYMUPDF_AVAILABLE:
                pages = FastPDFProcessor.extract_text_pymupdf(tmp_path)
                # Tag pages with filename
                for p in pages:
        ...
        return file_name, pages, tmp_path

    @classmethod
    def process_multiple_pdfs(cls, uploaded_files, max_workers: int = None) -> Tuple[List[Document], List[str]]:
        """
        Process multiple PDFs in parallel using ProcessPoolExecutor.
        Returns: (all_pages, temp_file_paths)
        """
        if max_workers is None:
            max_workers = min(cpu_count(), len(uploaded_files), 4)

        use_pymupdf = PYMUPDF_AVAILABLE
        if use_pymupdf:
            st.info(f"⚡ Using PyMuPDF for fast extraction ({max_workers} parallel workers)")
        else:
            st.info(f"Using PyPDFLoader ({max_workers} parallel workers)")

        all_pages = []
        temp_paths = []

        # For small numbers of files, use ThreadPoolExecutor (avoids pickle issues)
        # For larger numbers, ProcessPoolExecutor is better
        if len(uploaded_files) <= 4:
            executor_class = ThreadPoolExecutor
        else:
            executor_class = ThreadPoolExecutor  # Safer for Streamlit

        args = [(f, f.name, use_pymupdf) for f in uploaded_files]

        with st.spinner(f"Processing {len(uploaded_files)} PDFs with {max_workers} workers..."):
            with executor_class(max_workers=max_workers) as executor:
                futures = {executor.submit(cls.process_single_pdf, arg): arg[1] for arg in args}
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        name, pages, tmp_path = future.result()
...
class FullTextConceptExtractor:
    ...
    def _extract_candidates_fast(self, chunks: List[Document]) -> List[str]:
        ...
        for chunk in chunks:
            ...
            # Fast keyword matching using compiled patterns
            ...
            # Fast quantity context extraction
    ...
    def extract_concepts_fast(self, chunks: List[Document], min_salience: float = 0.42) -> Tuple[List[str], Dict[str, Dict]]:
        """Optimized concept extraction with caching and batch processing."""
    ...
    def _compute_salience_fast(self, candidates: List[str], chunks: List[Document]) -> Dict[str, float]:
        ...
        for idx, concept in enumerate(candidates):
            ...
            # Section importance (vectorized mean)
...
class EnhancedCrossDocumentKnowledgeGraph:
    ...
    def _extract_entities_from_chunk_fast(self, chunk: Document, chunk_id: int) -> List[EnhancedScientificEntity]:
        ...
        # Pre-compiled quantity patterns (already compiled in QUANTITY_PATTERNS)
        ...
        # Fast material/method matching using pre-built regex
...
def semantic_chunk_document(pages: List[Document], filename: str) -> List[Document]:
    ...
    # Pre-configure splitters for each section type to avoid re-creation
    splitters = {
    ...
    for section_name, section_text in sections:
        ...
        # Batch update metadata (fixed: was extending inside inner loop causing duplication)
        base_idx = len(chunks)
    ...
    # Single pass for final indexing
    total = len(chunks)
    for i, chunk in enumerate(chunks):
...
def load_pdf_chunks(uploaded_files, use_parallel: bool = True):
    """
    Load PDF chunks with optional parallel processing.
    ...
    if use_parallel and len(uploaded_files) > 1:
        try:
            ...
            all_chunks = []
            # Group pages by source file for semantic chunking
            pages_by_file = {}
...
def process_documents(uploaded_files):
    ...
    try:
        # Step 1: PDF Loading (with parallel processing)
        status.text("📄 Step 1/5: Loading and extracting text from PDFs...")
        all_chunks = load_pdf_chunks(new_files, use_parallel=True)
        ...
        status.text("🧠 Step 2/5: Loading embedding model...")
        ...
        # Step 3: Concept extraction (fast)
        status.text("🔍 Step 3/5: Extracting high-salience concepts...")
        ...
        # Use fast extraction
        valid_concepts, concept_metadata = extractor.extract_concepts_fast(all_chunks, min_salience=0.42)
        status.text(f"🔍 Extracted {len(valid_concepts)} high-salience concepts.")
        progress.progress(0.55)

        # Step 4: LLM ranking (optional)
        llm_ranked_data = []
        if st.session_state.llm_extraction_enabled and st.session_state.llm_tokenizer:
            status.text("🤖 Step 4/5: LLM-enhanced concept ranking...")
            llm_extractor = LLMEnhancedConceptExtractor(
                llm_generate_fn=lambda p: generate_local_response(
                    tokenizer=st.session_state.llm_tokenizer,
                    model_or_tag=st.session_state.llm_model,
            ...
            status.text("⏭️ Step 4/5: Skipping LLM ranking (disabled)...")
        progress.progress(0.75)

        # Step 5: Build knowledge graph and vector store
        status.text("🕸️ Step 5/5: Building knowledge graph and vector store...")
        ...
        # Use fast document addition
        for src, chunks in doc_chunks.items():
            graph.add_document_fast(src, chunks, dummy_bib, concept_metadata=concept_metadata, llm_ranked=llm_ranked_data)
...
def create_local_vector_store(chunks: List[Document], embedding_model_key: str):
    try:
        ...
        if hasattr(embeddings, 'embed_documents'):
            ...
            # Create FAISS index from embeddings directly (faster)
            import faiss
            from langchain_community.vectorstores import FAISS
            from langchain_community.docstore.in_memory import InMemoryDocstore

            embedding_array = np.array(all_embeddings, dtype=np.float32)
            index = faiss.IndexFlatIP(embedding_array.shape[1])  # Inner product for cosine similarity
            faiss.normalize_L2(embedding_array)  # Normalize for cosine
...
<li>Enable LLM extraction in sidebar if desired</li>
<li>Wait for processing – salience extraction may take a moment</li>
<li>Ask comparative or synthesizing questions</li>
<li>Use the Dynamic Concept Selector panel to filter visualizations</li>
</ol>
</div>
""", unsafe_allow_html=True)
