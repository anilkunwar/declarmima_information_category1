...
========================================================================================
UPGRADED VERSION (CODE 16+ -> 20.1): Query-Biased Quantitative Extraction (QB-QESA)
Mathematical Theory Integration, Semantic Expansion, Unit Normalization & Advanced Reasoning

KEY ADDITIONS:
1. MATHEMATICAL THEORY: Query-Biased Quantitative Extraction & Semantic Association (QB-QESA)
2. UNIT NORMALIZATION ENGINE: SI conversion, tolerance bounds, and dimensionality validation
3. CONTEXT WINDOW SCORING: Proximity weighting, semantic alignment, and positional decay
4. GRAPH DIFFUSION RETRIEVAL: Multi-hop relevance propagation across document-entity bipartite graphs
5. LLM-ASSISTED DISAMBIGUATION: Contextual numeric validation, implicit value inference, and confidence calibration
6. EXPANDED VISUALIZATION DASHBOARD: Unit-normalized histograms, semantic association matrices, extraction traces
7. COMPREHENSIVE LOGGING, PROGRESS TRACKING, & MEMORY MANAGEMENT
8. ALL ORIGINAL FUNCTIONALITY PRESERVED & EXTENDED

MATHEMATICAL FRAMEWORK: QUERY-BIASED QUANTITATIVE EXTRACTION & SEMANTIC ASSOCIATION (QB-QESA)
===============================================================================================
Let D be a corpus of documents, Q a user query, and T a target quantity concept.
We define a multi-stage extraction pipeline:

1. CANDIDATE GENERATION:
   C = {c_i | c_i = (v_i, u_i, W_i, E_i, doc_i, pos_i)}
   where v_i ∈ ℝ is the numeric value, u_i is the extracted unit,
   W_i ⊂ Σ* is the context window (±L tokens around v_i),
   E_i = {e_{i,1}, ..., e_{i,m}} is the set of co-occurring entities (materials, methods, parameters),
   doc_i ∈ D is the source document, and pos_i ∈ [0,1] is the normalized document position.

2. QUERY-ALIGNED RELEVANCE SCORING:
   R(c_i) = α·cos(Emb(W_i), Emb(Q)) + β·Σ_{e∈E_i} Sim(Emb(e), Emb(Q)) + 
            γ·UnitNorm(v_i, u_i) + δ·Proximity(pos_i, T) + ε·CrossDocSupport(c_i) + ζ·LLMConf(c_i)
   
   - α, β, γ, δ, ε, ζ ∈ [0,1] with Σ weights = 1 (configurable)
   - Emb(·) ∈ ℝ^d denotes sentence embedding projection
   - cos(a,b) = (a·b)/(||a||·||b||) is cosine similarity
   - UnitNorm(v,u) = exp(-|log10(v/v_ref)|) penalizes orders-of-magnitude outliers
   - Proximity(p,T) = exp(-κ·|p - p_T|) applies exponential decay from keyword locations
   - CrossDocSupport(c) = |{d ∈ D | ∃c' ∈ d : c' ≈ c}| / |D| measures consensus
   - LLMConf(c) ∈ [0,1] is model-calibrated contextual validity

3. SEMANTIC GRAPH DIFFUSION FOR EXPANSION:
   Let G = (V, E) be a bipartite graph with V = D ∪ E (documents ∪ entities).
   Initialize relevance vector q₀ ∈ ℝ^{|V|} where q₀(v) = R(c) if v corresponds to candidate c, else 0.
   Iterate: s^{(t+1)} = λ·M·s^{(t)} + (1-λ)·q₀
   where M is row-stochastic adjacency matrix, λ ∈ [0,1] controls diffusion depth.
   Converges to s* = (I - λM)^{-1}(1-λ)q₀. Threshold s*_i > τ to expand candidate set.

4. UNIT NORMALIZATION & CONVERSION:
   v'_i = v_i · K(u_i → u_base)
   where K is conversion matrix, u_base is SI base unit for T (e.g., W for power).
   Tolerance: |v'_i - μ_T| ≤ σ_T · η ensures physical plausibility.

5. CONFIDENCE CALIBRATION & RANKING:
   P(c_i | Q, D) = softmax(R(c_i) / τ)
   τ is temperature parameter. Final output F = {c_i | P(c_i) ≥ θ_min}
   Sorted by R(c_i) descending, with uncertainty bounds computed via bootstrap resampling.

This framework ensures comprehensive extraction across documents, handling implicit mentions,
unit variations, semantic proximity, and cross-document consensus without relying on rigid regex matches alone.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("declarmima_qbesa_app.log")
    ]
)
logger = logging.getLogger("DECLARMIMA_QBESA")
...
class AppConfig:
    ...
    DEFAULT_CONFIG = {
        ...
        "qbesa_confidence_threshold": 0.35,
    ...
    def __init__(self):
        ...
        self._overrides = {}
        logger.info("AppConfig initialized with QB-QESA defaults")
...
QUANTITY_PATTERNS = {
    ...
    "laser_power": re.compile(r'(\d+(?:\.\d+)?)\s*(?:W|mW|kW)\s*(?:laser\s*power|nominal\s*power|output\s*power|applied\s*power|beam\s*power)', re.I),
...
class UnitConversionEngine:
    """
    Handles SI unit normalization, conversion matrices, and physical plausibility validation.
    ...
    CONVERSION_TABLES = {
        ...
        "scan_speed": {"base_unit": "mm/s", "factors": {"mm/s": 1.0, "mm/min": 1/60, "m/s": 1000.0, "m/min": 1000/60}},
        "spot_size": {"base_unit": "µm", "factors": {"µm": 1.0, "um": 1.0, "nm": 1e-3, "mm": 1000.0}},
        "pulse_duration": {"base_unit": "fs", "factors": {"fs": 1.0, "ps": 1e3, "ns": 1e6, "ms": 1e9}},
        "repetition_rate": {"base_unit": "kHz", "factors": {"Hz": 1e-3, "kHz": 1.0, "MHz": 1e3, "GHz": 1e6}},
    ...
    @classmethod
    def normalize_unit(cls, value: float, unit: str, quantity_label: str) -> Tuple[float, str]:
        ...
        if unit_clean in factors:
            ...
            return normalized, table["base_unit"]
        return value, unit
    ...
    def validate_plausibility(cls, value: float, quantity_label: str, tolerance: float = 0.20) -> bool:
        ...
        plausible_ranges = {
            ...
            "wavelength": (100, 11000),
            "fluence": (1e-4, 1000),
            "scan_speed": (0.01, 5000),
            "spot_size": (0.1, 5000),
            "pulse_duration": (10e-15, 10e-9),
            "repetition_rate": (0.1, 10000),
...
class FullTextConceptExtractor:
    ...
    def __init__(self, embed_model, proposal_text: str = None):
        ...
        self.core_pillars = {
            ...
            "multicomponent alloy": 1.00, "multicomponent": 0.98, "alloy": 0.95,
            "laser microstructure interaction": 1.00, "laser-matter interaction": 1.00,
            "laser alloy interaction": 0.98, "laser multicomponent interaction": 1.00,
        }
        self.domain_seeds = {
...
# =====================================================================
# QUERY-BIASED QUANTITATIVE EXTRACTOR (QB-QESA IMPLEMENTATION)
# =====================================================================
class QueryBiasedQuantitativeExtractor:
    """
    Implements the QB-QESA mathematical framework for query-biased, context-aware, 
    multi-hop quantitative extraction across documents.
    """
    def __init__(self, graph: EnhancedCrossDocumentKnowledgeGraph, embedding_fn: Callable):
        self.graph = graph
        self.embedding_fn = embedding_fn
        self.alpha = app_config.get("qbesa_alpha", 0.30)
        self.beta = app_config.get("qbesa_beta", 0.25)
        self.gamma = app_config.get("qbesa_gamma", 0.15)
        self.delta = app_config.get("qbesa_delta", 0.15)
        self.epsilon = app_config.get("qbesa_epsilon", 0.10)
        ...
        self.confidence_threshold = app_config.get("qbesa_confidence_threshold", 0.35)
    ...
    def extract_with_query_bias(self, quantity_label: str, query: str, 
        ...
        """Main extraction pipeline implementing QB-QESA theory."""
        if query_embedding is None:
            query_embedding = self.embedding_fn(query)
        
        # 1. Candidate Generation
        candidates = self._generate_candidates(quantity_label)
        if not candidates:
            return pd.DataFrame()
            
        # 2. Compute Initial Relevance Scores
        scored_candidates = []
        for c in candidates:
            score = self._compute_relevance_score(c, query, query_embedding)
            c.query_relevance_score = score
            scored_candidates.append(c)
            
        # 3. Graph Diffusion Expansion
        expanded_candidates = self._graph_diffusion_expansion(scored_candidates, query_embedding)
        ...
        for c in expanded_candidates:
            ...
            if not self.unit_converter.validate_plausibility(norm_val, quantity_label, app_config.get("qbesa_unit_tolerance", 0.20)):
                continue
            c.value = norm_val
            c.unit = norm_unit
            normalized_records.append(c)
            
        # 5. Confidence Calibration & Thresholding
        final_records = []
        ...
        # 6. Inference of Associated Entities
        records_dicts = []
        for c in final_records:
            associated_material = self._infer_associated_entity(c, "MATERIAL")
            associated_method = self._infer_associated_entity(c, "METHOD")
            doc_stem = Path(c.doc_source).stem
            records_dicts.append({
                ...
                "doc_stem": doc_stem,
                "material": associated_material or "Unknown",
                "method": associated_method or "Unknown",
    ...
    def _generate_candidates(self, quantity_label: str) -> List[EnhancedScientificEntity]:
        """Gather all raw candidates matching the quantity label across the graph."""
        candidates = []
        target_entities = []
    ...
    def _compute_relevance_score(self, candidate: EnhancedScientificEntity, query: str, query_emb: np.ndarray) -> float:
        """R(c) = α·cos(Emb(W), Emb(Q)) + β·ΣSim(Emb(e), Emb(Q)) + γ·UnitNorm + δ·Proximity + ε·CrossDoc + ζ·LLMConf"""
        # Semantic Alignment
        ...
        # Entity Match
        entity_sim = 0.0
        if hasattr(candidate, 'associated_entities') and candidate.associated_entities:
            sims = []
            for ent in candidate.associated_entities:
                ent_emb = self.embedding_fn(ent)
    ...
    def _graph_diffusion_expansion(self, candidates: List[EnhancedScientificEntity], query_emb: np.ndarray) -> List[EnhancedScientificEntity]:
        """Applies s^(t+1) = λ·M·s^(t) + (1-λ)·q₀ to expand candidate set via semantic neighbors."""
        if len(candidates) < 5:
            return candidates
            
        # Build local adjacency from existing graph
        node_to_idx = {c.text: i for i, c in enumerate(candidates)}
        n = len(candidates)
        adj = np.zeros((n, n))
        
        # Fill adjacency based on co-occurrence in chunks
        for i, c1 in enumerate(candidates):
            for j, c2 in enumerate(candidates):
                if i == j: continue
                # Check if they share chunk/document
                if c1.doc_source == c2.doc_source and c1.chunk_id == c2.chunk_id:
                    adj[i][j] = 1.0
                    adj[j][i] = 1.0
        ...
        # Initial query bias vector
        q0 = np.array([c.query_relevance_score for c in candidates])
        s = q0.copy()
        
        # Iterate diffusion
        for _ in range(5):
            s = self.lambda_diffusion * M.dot(s) + (1 - self.lambda_diffusion) * q0
            
        # Update candidates with diffused scores
        expanded = []
        for i, c in enumerate(candidates):
            c.query_relevance_score = float(np.clip(s[i], 0, 1))
            expanded.append(c)
            
        return expanded

    def _infer_associated_entity(self, target: EnhancedScientificEntity, target_domain: str) -> Optional[str]:
    ...
    def summarize(self, quantity_label: str, query: str, group_by: str = "material") -> Dict[str, Any]:
        df = self.extract_with_query_bias(quantity_label, query, group_by=group_by)
        if df.empty:
...
class QuantitativeDataExtractor:
    """Wrapper for backward compatibility, delegates to QB-QESA when query is provided."""
    def __init__(self, graph: EnhancedCrossDocumentKnowledgeGraph, embedding_fn: Optional[Callable] = None):
        self.graph = graph
        self.embedding_fn = embedding_fn

    def extract(self, quantity_label: str, group_by: str = "material", query: str = "", 
                query_embedding: Optional[np.ndarray] = None) -> pd.DataFrame:
        if self.embedding_fn and (query or query_embedding is not None):
            q_extractor = QueryBiasedQuantitativeExtractor(self.graph, self.embedding_fn)
            return q_extractor.extract_with_query_bias(quantity_label, query, query_embedding, group_by)
        ...
        for norm, ent_list in self.graph.entities.items():
            for ent in ent_list:
                if ent.label == quantity_label and ent.value is not None:
                    records.append({
                        "value": float(ent.value), "unit": ent.unit or "a.u.", "raw_text": ent.text,
                        "doc_source": ent.doc_source, "doc_stem": Path(ent.doc_source).stem,
                        "material": "Unknown", "method": "Unknown", "context": ent.context[:250],
                        "chunk_id": ent.chunk_id, "confidence": ent.confidence
                    })
        df = pd.DataFrame(records)
        return df if not df.empty else pd.DataFrame()

    def summarize(self, quantity_label: str, query: str = "") -> Dict[str, Any]:
        df = self.extract(quantity_label, query=query)
        if df.empty:
            return {"found": False, "quantity": quantity_label}
        return {
            "found": True, "quantity": quantity_label, "count": len(df),
            "unit": df["unit"].mode()[0] if not df["unit"].empty else "N/A",
            "value_range": (float(df["value"].min()), float(df["value"].max())),
            "mean": float(df["value"].mean()), "std": float(df["value"].std()),
            "by_material": df.groupby("material")["value"].agg(["count", "mean", "std", "min", "max"]).to_dict(),
            "by_document": df.groupby("doc_stem")["value"].agg(["count", "mean", "std"]).to_dict(),
        }
...
def retrieve_and_answer_quantitative(
    ...
    extractor = QueryBiasedQuantitativeExtractor(graph, emb_fn)
    df = extractor.extract_with_query_bias(quantity_label, query, group_by=grouping_dim)
    summary = extractor.summarize(quantity_label, query, group_by=grouping_dim)
    def llm_generate(prompt: str) -> str:
        return generate_local_response(
            tokenizer=tokenizer, model_or_tag=model, device_or_host=device_or_host,
            prompt=prompt, backend=backend, backend_type=backend_type
