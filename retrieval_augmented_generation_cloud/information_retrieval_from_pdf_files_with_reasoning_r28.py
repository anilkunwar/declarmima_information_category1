...
========================================================================================
UPGRADED VERSION (CODE 20.2): Query-Biased Quantitative Extraction (QB-QESA)
Mathematical Theory Integration, Semantic Expansion, Unit Normalization & Advanced Reasoning
ENHANCED with multi-factor pattern confidence, value clustering, and consensus boost.

KEY ADDITIONS (beyond original QB-QESA):
1. ENHANCED PATTERN DEFINITIONS: unit, keyword, range, and regex multi-factor confidence
2. VALUE CLUSTERING: DBSCAN to identify distinct operating regimes
3. CONSENSUS BOOST: cross-document agreement factor (η·log(1+|D_v|)·(1-σ/μ))
4. FIXED SYNTAX ERROR in section detection (line 867)
5. UPGRADED QueryBiasedQuantitativeExtractor merging both theories
6. All original functionality preserved & extended
"""
...
# =====================================================================
# ENHANCED QUANTITY PATTERNS (multi-factor confidence)
# =====================================================================
...
# =====================================================================
# QUERY-BIASED QUANTITATIVE EXTRACTOR (QB-QESA IMPLEMENTATION - UPGRADED)
# =====================================================================
class QueryBiasedQuantitativeExtractor:
    ...
    def _compute_relevance_score(self, candidate: EnhancedScientificEntity, query: str, query_emb: np.ndarray, pattern_conf: float) -> float:
        ...
        # Apply pattern confidence as a modulator (ensures at least 60% of score if pattern_conf=0)
    ...
    def _compute_consensus_boost(self, value: float, quantity_label: str) -> float:
        ...
        """
        # Collect all values of this quantity from the graph
        all_vals = []
        ...
        if mean > 0:
            cv = std / mean
            boost = 1 + eta * np.log(1 + unique_docs) * (1 - cv)
        else:
    ...
    def extract_with_query_bias(self, quantity_label: str, query: str,
        ...
        for c in expanded:
            ...
            if not self.unit_converter.validate_plausibility(norm_val, quantity_label,
                                                             app_config.get("qbesa_unit_tolerance", 0.20)):
                continue
            c.value = norm_val
            c.unit = norm_unit
...
class CrossDocumentThinker:
    ...
    def _extract_query_entities(self, query: str) -> List[str]:
        ...
        for param_name in ENHANCED_QUANTITY_PATTERNS.keys():
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.processing_complete and st.session_state.knowledge_graph:
        ...
        tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
            "Static Network", "Chord (Plotly)", "Chord (Bokeh)", "Chord (HoloViews)",
            "Material Sunburst", "Method Sunburst", "Topics Sunburst", "Entity Embeddings", "Contradictions/Consensus"
        ])
        with tab1:
            fig = viz.plot_static_knowledge_network(filtered_concepts=filtered, top_n=st.session_state.viz_top_n,
                                                    layout=layout, colormap=active_cmap)
            st.pyplot(fig)
            buf = BytesIO()
            fig.savefig(buf, format="png", dpi=300, bbox_inches="tight")
            st.download_button("📥 Download Network (PNG)", buf.getvalue(), file_name="knowledge_network.png",
                               mime="image/png", key="net_download")
        with tab2:
            fig = viz.plot_chord_cooccurrence(filtered_concepts=filtered, top_n=st.session_state.viz_top_n,
                                              colormap=active_cmap)
            st.plotly_chart(fig, use_container_width=True)
        with tab3:
            if BOKEH_AVAILABLE:
                p = viz.plot_bokeh_chord(filtered_concepts=filtered, top_n=min(20, st.session_state.viz_top_n),
                                         colormap=active_cmap)
                if p:
                    st.bokeh_chart(p, use_container_width=True)
                else:
                    st.info("Bokeh chord requires at least 3 entities with co‑occurrence.")
            else:
                st.info("Bokeh not installed. Install with `pip install bokeh`")
        with tab4:
            if HOLOVIEWS_AVAILABLE:
                chord = viz.plot_holoviews_chord(filtered_concepts=filtered, top_n=min(20, st.session_state.viz_top_n))
                if chord:
                    st.bokeh_chart(hv.render(chord, backend='bokeh'))
                else:
                    st.info("HoloViews chord requires at least 3 entities.")
            else:
                st.info("HoloViews not installed. Install with `pip install holoviews`")
        with tab5:
            fig = viz.plot_materials_sunburst(filtered_concepts=filtered, top_n_per_category=10,
                                              colormap=active_cmap)
            st.plotly_chart(fig, use_container_width=True)
        with tab6:
            fig = viz.plot_methods_sunburst(filtered_concepts=filtered, top_n_per_category=10,
                                            colormap=active_cmap)
            st.plotly_chart(fig, use_container_width=True)
        with tab7:
            fig = viz.plot_topics_sunburst(colormap=active_cmap)
            st.plotly_chart(fig, use_container_width=True)
        with tab8:
            ...
            with col_emb:
                ...
                if emb_fn:
                    if SKLEARN_AVAILABLE:
                        ...
                        if fig1:
                            st.pyplot(fig1)
                        fig2 = viz.plot_entity_tsne(emb_fn, filtered_concepts=filtered, top_n=80, colormap=active_cmap)
                        if fig2:
                            st.pyplot(fig2)
                    if UMAP_AVAILABLE:
                        fig3 = viz.plot_entity_umap(emb_fn, filtered_concepts=filtered, top_n=80, colormap=active_cmap)
                        if fig3:
                            st.pyplot(fig3)
                else:
                    st.info("Embedding function not available for manifold plots.")
        with tab9:
            col_cons, col_contr = st.columns(2)
            with col_cons:
                fig = viz.plot_consensus_waterfall(top_n=10, colormap=active_cmap)
                st.plotly_chart(fig, use_container_width=True)
            with col_contr:
                fig = viz.plot_contradiction_matrix(colormap=active_cmap)
                st.plotly_chart(fig, use_container_width=True)
            fig_radar = viz.plot_document_radar(filtered_concepts=filtered, colormap=active_cmap)
            st.plotly_chart(fig_radar, use_container_width=True)
            fig_timeline = viz.plot_temporal_timeline(colormap=active_cmap)
        ...
        if PYVIS_AVAILABLE and st.session_state.knowledge_graph:
            ...
            for doc_id in st.session_state.knowledge_graph.documents:
                nx_graph.add_node(Path(doc_id).stem, node_type="doc")
