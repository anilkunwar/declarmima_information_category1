...
# =====================================================================
# ENHANCED QUANTITY PATTERNS (Query-Biased Quantitative Extraction)
# =====================================================================
# Replaced rigid regex with soft pattern matching framework
# Each quantity has: multiple bidirectional patterns, context keywords,
# typical range for plausibility scoring, and synonym ontology
# =====================================================================

ENHANCED_QUANTITY_PATTERNS = {
    "laser_power": {
        "patterns": [
            # Forward: number → keyword
...
# =====================================================================
# =====================================================================
# QUERY-BIASED QUANTITATIVE EXTRACTOR (QBQE)
# =====================================================================
# Mathematical framework for enhanced quantitative extraction with
# query-biased salience scoring, soft pattern matching, and cross-document
# consensus boosting. Replaces rigid regex with contextual disambiguation.
# =====================================================================
...
class QueryBiasedQuantitativeExtractor:
    ...
    def extract_all_values(self, chunks: List[Document], query: str = "", 
        ...
        for chunk in chunks:
            ...
            for candidate in candidates:
                ...
                context_window = self._get_context_window(text, position, window=250)
    ...
    def _find_numeric_candidates(self, text: str) -> List[Dict]:
        ...
        number_unit_pattern = re.compile(
            ...
            r'nm|µm|um|microns?|mm|cm|Å|'  # Length units
            r'fs|ps|ns|μs|microseconds?|ms|'  # Time units
            r'kHz|MHz|Hz|GHz|'  # Frequency units
            r'µJ|uJ|mJ|nJ|J|kJ|'  # Energy units
            r'\%|percent|vol\.\%|vol%|at\.\%|wt\.\%|at%|wt%|'  # Percentage/composition
            r'K|°C|°F|'  # Temperature
            r'GPa|MPa|Pa|N/m²|'  # Pressure
            r'g/cm³|kg/m³|'  # Density
            r'J/m²|J/m2|mJ/m²|mJ/m2|erg/cm²|N/m|dyn/cm|'  # Surface energy
            r'W/(?:m·?K|mK|m\s*K)|W\s*m[-¹1]\s*K[-¹1])',  # Thermal conductivity
            re.IGNORECASE
        )
    ...
    def _compute_salience(self, value: float, unit: str, qty_type: str,
        ...
        """
        # Component 1: Query embedding similarity (α = 0.40)
        query_sim = 0.5
        if query_emb is not None and self.embed_model is not None:
            try:
                ...
                query_sim = (query_sim + 1) / 2  # Normalize to [0,1]
            except:
                pass

        # Component 2: Keyword overlap (β = 0.25)
        keyword_score = self._keyword_overlap_score(query, context)

        # Component 3: Entity proximity (γ = 0.20)
        entity_score = self._entity_proximity_score(query_entities, context)

        # Component 4: Section relevance (δ = 0.15)
        section_weights = {
    ...
    def aggregate_by_document(self, df: pd.DataFrame, qty_type: str) -> pd.DataFrame:
        """Aggregate values by document with statistics."""
        if df.empty:
            return df
        ...
        df_filtered['doc_stem'] = df_filtered['doc_source'].apply(lambda x: Path(x).stem)
    ...
    def summarize(self, df: pd.DataFrame, qty_type: str) -> Dict[str, Any]:
        """Comprehensive summary with query-biased insights."""
        df_filtered = df[df['quantity_type'] == qty_type]
        if df_filtered.empty:
            return {"found": False, "quantity": qty_type}

        # Compute consensus boosts
        consensus = self.compute_consensus_boost(df, qty_type)

        # Get top values by salience
        top_values = df_filtered.nlargest(10, 'salience')[['value', 'unit', 'salience', 
                                                           'confidence', 'material', 'doc_source']]
...
def retrieve_and_answer_quantitative(
    ...
    """
    Enhanced quantitative retrieval using Query-Biased Quantitative Extraction (QBQE).

    Mathematical pipeline:
    1. Detect quantitative intent from query
    2. Extract ALL candidate values with soft pattern matching
    3. Score each with query-biased salience
    4. Aggregate by material/document with consensus boosting
    5. Generate comprehensive answer with uncertainty quantification
    """
    ...
    df = extractor.extract_all_values(all_chunks, query=query, min_confidence=0.25)

    # Step 2: Summarize with enhanced statistics
    summary = extractor.summarize(df, quantity_label)

    # Step 3: Find value clusters (operating regimes)
    clusters = extractor.find_value_clusters(df, quantity_label, eps=0.15)

    # Step 4: Aggregate by grouping dimension
...
"""
    ...
    if not df.empty:
        # Core quantitative visualizations
        ...
        # Dimensionality reduction plots (if enough data points)
        if len(df) >= 5:
        ...
        # Cluster visualization if clusters found
        if 'cluster' in clusters.columns and len(clusters['cluster'].unique()) > 1:
            # Add cluster scatter plot
            cluster_fig = go.Figure()
            for cid in sorted(clusters['cluster'].unique()):
                if cid == -1:
                    continue
                cluster_data = clusters[clusters['cluster'] == cid]
                cluster_fig.add_trace(go.Scatter(
                    x=cluster_data['material'],
                    y=cluster_data['value'],
                    mode='markers',
                    name=f'Regime {cid}',
                    marker=dict(size=12 + cluster_data['salience'] * 20),
                    text=cluster_data['doc_source'],
                    hovertemplate='<b>%{x}</b><br>Value: %{y:.2f}<br>Doc: %{text}<extra></extra>'
                ))
            cluster_fig.update_layout(
                title=f"{quantity_label.replace('_', ' ').title()} Operating Regimes (Clustered)",
                xaxis_title="Material",
                yaxis_title=f"{quantity_label.replace('_', ' ').title()} ({df['unit'].iloc[0]})",
                height=500
            )
            ply_figs.append(cluster_fig)
    ...
    chain.add_step("quantitative_intent", f"Detected quantitative query: {quantity_label}", 
                   {"group_by": grouping_dim, "method": "QBQE"})
    chain.add_step("extraction", f"Extracted {len(df)} values via soft pattern matching", 
    ...
    if st.session_state.processing_complete and st.session_state.knowledge_graph:
        ...
        with tab7:
            ...
            if st.session_state.knowledge_graph:
                # Use QBQE with optional query context from last user query
                last_query = ""
