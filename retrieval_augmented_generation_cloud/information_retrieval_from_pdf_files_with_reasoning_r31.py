...
Key Upgrades:
1. FIXED: NameError: name 'matplotlib' not defined (added import matplotlib + mcolors)
...
24. NEW: QUANTITATIVE EXTRACTION WITH DYNAMIC UNIT HANDLING (QBQE 2.0)
25. NEW: QUERY UNIT ANALYZER & DYNAMIC UNIT EXTRACTOR
26. NEW: LLM QUANTITATIVE REFINER (optional)
27. NEW: ADAPTIVE VISUALIZATION ENGINE WITH QUERY-DRIVEN PLOTS
"""
...
# =====================================================================
# ENHANCED QUANTITY PATTERNS (Extended with Mechanical/Thermodynamic)
# =====================================================================

ENHANCED_QUANTITY_PATTERNS = {
    # ---------- EXISTING ENTRIES (keep as originally defined) ----------
    "laser_power": {
        "patterns": [
            ...
            r'(\d+(?:\.\d+)?)\s*(?:W|watts?)\s*(?:laser|beam|fiber|diode|CO2|Nd:YAG|fiber|Yb|YAG|disk|CO\\u2082)',
            r'(?:power|output)\s*(?:of\s*)?(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'(?:laser|beam)\s*(?:power|output)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'(?:operating|working|applied|set|used|employed|utilized)\s*(?:at|with|under|to)\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'(?:using|with|at)\s+a\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)\s*(?:laser|source|beam)',
            r'P\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'P_(?:laser|L)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'(?:Laser\s+power|Power|Laser\s*output)\s*[:\|\t]\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)',
            r'(?:Parameter|Settings)\s*[:\|]\s*(?:Laser\s*)?(?:Power|Output)\s*[:\|]\s*(\d+(?:\.\d+)?)\s*(?:W|watts?)',
        ],
        "context_keywords": ["laser", "beam", "processing", "ablation", "melting", 
                            "sintering", "welding", "cutting", "drilling", "surface",
                            "irradiation", "treatment", "machining", "annealing"],
        "unit": "W",
        "typical_range": (1, 10000),
        "synonyms": ["power", "output power", "laser output", "beam power", 
                    "incident power", "applied power", "nominal power", "average power",
                    "peak power", "cw power", "pulsed power", "laser output power"],
        "domain": "process"
    },
    "scan_speed": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)\s*(?:scan\s*speed|scanning\s*speed|travel\s*speed|feed\s*rate|writing\s*speed)',
            r'(?:scan\s*speed|scanning\s*speed|travel\s*speed)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)',
            r'v\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)',
            r'v_s\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)',
            r'(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)\s*(?:scan|scanning|hatch|raster|meander|contour)',
            r'(?:scanning|hatch)\s*(?:rate|velocity)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:mm/s|mm/min|m/s)',
        ],
        "context_keywords": ["scan", "scanning", "hatch", "raster", "speed", "velocity", 
                            "feed", "traversal", "beam path", "writing", "meander", "contour"],
        "unit": "mm/s",
        "typical_range": (0.1, 5000),
        "synonyms": ["scan speed", "scanning speed", "travel speed", "feed rate", 
                    "scanning velocity", "hatch speed", "writing speed", "traverse speed"],
        "domain": "process"
    },
    "fluence": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2|J\s*cm[-²2]|mJ/cm²|µJ/cm²)\s*(?:fluence|energy\s*density|threshold)',
            r'(?:fluence|energy\s*density|threshold\s*fluence)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2|mJ/cm²)',
            r'F\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2)',
            r'F_(?:th|thres)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2)',
            r'(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2)\s*(?:pulse|laser|beam|threshold|damage)',
            r'(?:threshold|damage)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2|mJ/cm²)',
        ],
        "context_keywords": ["fluence", "threshold", "ablation", "energy density", 
                            "laser", "pulse", "damage", "optical breakdown", "dielectric"],
        "unit": "J/cm²",
        "typical_range": (0.01, 100),
        "synonyms": ["fluence", "energy density", "laser fluence", "pulse fluence",
                    "threshold fluence", "ablation threshold", "damage threshold"],
        "domain": "process"
    },
    "wavelength": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)\s*(?:wavelength|λ|lambda|emission|excitation|operating)',
            r'(?:wavelength|λ|lambda|emission\s*wavelength)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)',
            r'λ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)',
            r'λ_(?:exc|em|laser)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)',
            r'(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)\s*(?:laser|beam|source|fiber|diode|Nd:YAG|Ti:Sapphire|CO2|excimer|UV|IR|visible)',
            r'(?:operating|emission|excitation)\s*(?:wavelength|λ)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:nm|nanometers?)',
        ],
        "context_keywords": ["wavelength", "laser", "emission", "spectral", "IR", 
                            "UV", "visible", "infrared", "ultraviolet", "monochromatic",
                            "frequency", "photon", "absorption"],
        "unit": "nm",
        "typical_range": (100, 11000),
        "synonyms": ["wavelength", "emission wavelength", "laser wavelength", "operating wavelength",
                    "excitation wavelength", "central wavelength", "peak wavelength"],
        "domain": "process"
    },
    "pulse_duration": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:fs|femtoseconds?|ps|picoseconds?|ns|nanoseconds?|μs|microseconds?|ms|milliseconds?)\s*(?:pulse|duration|width|length|fwhm|FWHM)',
            r'(?:pulse\s*duration|pulse\s*width|fwhm|FWHM|temporal\s*width)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:fs|ps|ns|μs|ms)',
            r'τ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:fs|ps|ns|μs|ms)',
            r'τ_p\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:fs|ps|ns|μs|ms)',
            r'Δt\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:fs|ps|ns|μs|ms)',
            r'(\d+(?:\.\d+)?)\s*(?:fs|ps|ns|μs|ms)\s*(?:pulse|laser|temporal|duration)',
        ],
        "context_keywords": ["pulse", "duration", "width", "fwhm", "temporal", 
                            "femtosecond", "picosecond", "nanosecond", "ultrashort",
                            "chirped", "stretched", "compressed"],
        "unit": "fs",
        "typical_range": (1, 1e9),
        "synonyms": ["pulse duration", "pulse width", "pulse length", "fwhm", "temporal width",
                    "temporal duration", "pulse FWHM", "pulse length"],
        "domain": "process"
    },
    "repetition_rate": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)\s*(?:repetition|rep\s*rate|frequency|rate|pulse\s*rate|PRR)',
            r'(?:repetition\s*rate|rep\s*rate|frequency|PRR)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)',
            r'f\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)',
            r'f_(?:rep|R)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)',
            r'(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)\s*(?:laser|pulse|beam|oscillator|amplifier)',
            r'(?:pulse|shot)\s*(?:rate|frequency)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:kHz|MHz|Hz|GHz)',
        ],
        "context_keywords": ["repetition", "frequency", "rate", "pulse rate", "kHz", "MHz", 
                            "oscillator", "amplifier", "burst", "train", "pulses"],
        "unit": "kHz",
        "typical_range": (1, 1e6),
        "synonyms": ["repetition rate", "pulse repetition rate", "rep rate", "frequency", 
                    "pulse frequency", "PRR", "pulse rate", "repetition frequency"],
        "domain": "process"
    },
    "spot_size": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|microns?|mm|nm|cm)\s*(?:spot|beam\s*radius|waist|diameter|focus|focal\s*spot|beam\s*size)',
            r'(?:spot\s*size|beam\s*radius|waist|diameter|beam\s*diameter)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)',
            r'w₀\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)',
            r'w_0\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)',
            r'd\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)\s*(?:spot|beam|focus)',
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)\s*(?:focal\s*spot|beam\s*waist|focused\s*beam|spot)',
        ],
        "context_keywords": ["spot", "beam", "waist", "focus", "focal", "diameter", "radius",
                            "beam width", "spot diameter", "focused", "defocused", "collimated"],
        "unit": "µm",
        "typical_range": (0.1, 1000),
        "synonyms": ["spot size", "beam radius", "beam waist", "focal spot", "spot diameter",
                    "beam diameter", "focused spot size", "beam width"],
        "domain": "process"
    },
    "hatch_distance": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm)\s*(?:hatch\s*distance|hatch\s*spacing|line\s*spacing|track\s*spacing)',
            r'(?:hatch\s*distance|hatch\s*spacing|line\s*spacing)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm)',
            r'h\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm)',
            r'h_d\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm)',
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm)\s*(?:hatch|track|line)',
        ],
        "context_keywords": ["hatch", "spacing", "distance", "overlap", "scan strategy",
                            "track", "line", "raster", "pattern", "fill"],
        "unit": "µm",
        "typical_range": (10, 500),
        "synonyms": ["hatch distance", "hatch spacing", "line spacing", "scan spacing",
                    "track spacing", "hatch offset", "line offset"],
        "domain": "process"
    },
    "layer_thickness": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)\s*(?:layer\s*thickness|layer\s*height|slice\s*thickness|build\s*thickness|deposition\s*thickness)',
            r'(?:layer\s*thickness|layer\s*height|slice\s*thickness)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)',
            r't\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)\s*(?:layer|slice|build)',
            r't_l\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)',
            r'(\d+(?:\.\d+)?)\s*(?:µm|um|mm|nm|cm)\s*(?:per\s*layer|layer|slice)',
        ],
        "context_keywords": ["layer", "thickness", "height", "slice", "build", "deposition",
                            "stratified", "laminated", "powder bed", "build chamber"],
        "unit": "µm",
        "typical_range": (10, 200),
        "synonyms": ["layer thickness", "layer height", "slice thickness", "build thickness",
                    "deposition thickness", "stratified thickness", "lamina thickness"],
        "domain": "process"
    },
    "pulse_energy": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:µJ|uJ|mJ|nJ|J|kJ)\s*(?:pulse\s*energy|energy\s*per\s*pulse|single\s*pulse|per\s*pulse)',
            r'(?:pulse\s*energy|energy\s*per\s*pulse)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:µJ|mJ|nJ|J|kJ)',
            r'E_p\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µJ|mJ|nJ|J|kJ)',
            r'E_(?:pulse|p)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:µJ|mJ|nJ|J|kJ)',
            r'(\d+(?:\.\d+)?)\s*(?:µJ|mJ|nJ|J|kJ)\s*(?:per\s*pulse|pulse|shot)',
            r'(?:single|per)\s*pulse\s*(?:energy|fluence)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:µJ|mJ|nJ|J|kJ)',
        ],
        "context_keywords": ["pulse", "energy", "per pulse", "single pulse", "pulse energy",
                            "shot", "fluence", "impulse", "packet"],
        "unit": "mJ",
        "typical_range": (0.001, 100),
        "synonyms": ["pulse energy", "energy per pulse", "single pulse energy", "pulse fluence",
                    "per pulse energy", "shot energy", "pulse packet energy"],
        "domain": "process"
    },
    "roughness": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)\s*(?:roughness|Ra|RMS|Rq|Rz|R_a|RMS|surface\s*finish|surface\s*quality)',
            r'(?:roughness|Ra|RMS|Rq|Rz)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)',
            r'Ra\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)',
            r'R_a\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)',
            r'RMS\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)',
            r'Rq\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)',
            r'(\d+(?:\.\d+)?)\s*(?:nm|µm|um|mm|Å)\s*(?:Sa|Sq|Sz|Rsk|Rku)',
        ],
        "context_keywords": ["roughness", "surface", "finish", "Ra", "RMS", "quality", 
                            "smoothness", "texture", "topography", "profile", "asperity"],
        "unit": "nm",
        "typical_range": (0.1, 100),
        "synonyms": ["roughness", "surface roughness", "Ra", "RMS roughness", "surface finish",
                    "arithmetic roughness", "root mean square roughness", "surface texture"],
        "domain": "geometric"
    },
    "porosity": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:\%|percent|vol\.\%|vol%)\s*(?:porosity|pore\s*fraction|void\s*fraction|porous\s*fraction)',
            r'(?:porosity|pore\s*fraction|void\s*fraction)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:\%|percent|vol\.\%|vol%)',
            r'P\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:\%|percent)',
            r'(\d+(?:\.\d+)?)\s*(?:\%|percent)\s*(?:porous|voids|pores|porosity)',
            r'(?:relative|total)\s*density\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:\%|percent)',
            r'(\d+(?:\.\d+)?)\s*(?:\%|percent)\s*(?:dense|density|theoretical\s*density)',
        ],
        "context_keywords": ["porosity", "pore", "void", "defect", "density", "fraction",
                            "permeability", "infiltration", "gas", "bubble", "inclusion"],
        "unit": "%",
        "typical_range": (0, 50),
        "synonyms": ["porosity", "pore fraction", "void fraction", "porous fraction",
                    "void content", "pore content", "porous percentage", "relative density"],
        "domain": "defect"
    },
    "component_fraction": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:at\.\%|wt\.\%|at\%|wt\%|atomic\s*percent|weight\s*percent|mol\%|volume\s*\%)\s*(?:of\s*)?([A-Za-z]{1,3})',
            r'([A-Za-z]{1,3})\s*(?:content|fraction|concentration|composition)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:at\.\%|wt\.\%|at\%|wt\%|atomic\s*percent|weight\s*percent)',
            r'(?:composition|alloy|formula)\s*[:=]\s*([A-Za-z\d\.\s]+)',
            r'([A-Za-z]{1,3})_(\d+(?:\.\d+)?)',
        ],
        "context_keywords": ["composition", "alloy", "at%", "wt%", "concentration", 
                            "fraction", "stoichiometry", "doping", "impurity", "solute"],
        "unit": "at%",
        "typical_range": (0, 100),
        "synonyms": ["composition", "atomic percent", "weight percent", "concentration",
                    "mole fraction", "content", "stoichiometric ratio"],
        "domain": "material"
    },
    "interfacial_energy": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:J/m²|J/m2|mJ/m²|mJ/m2|erg/cm²|N/m|dyn/cm)\s*(?:interfacial\s*energy|surface\s*tension|surface\s*energy|interfacial\s*tension)',
            r'(?:interfacial\s*energy|surface\s*tension|surface\s*energy)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:J/m²|J/m2|mJ/m²|mJ/m2|N/m)',
            r'γ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/m²|J/m2|mJ/m²|mJ/m2|N/m)',
            r'σ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/m²|J/m2|mJ/m²|mJ/m2|N/m)',
            r'(\d+(?:\.\d+)?)\s*(?:J/m²|J/m2|mJ/m²|mJ/m2|N/m)\s*(?:interface|boundary|surface|grain\s*boundary)',
        ],
        "context_keywords": ["interfacial", "surface", "tension", "energy", "boundary", 
                            "grain boundary", "interface", "wetting", "contact angle", "capillary"],
        "unit": "J/m²",
        "typical_range": (0.001, 10),
        "synonyms": ["interfacial energy", "surface tension", "surface energy", "interfacial tension",
                    "boundary energy", "grain boundary energy", "surface free energy"],
        "domain": "thermodynamic"
    },
    "thermal_conductivity": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK|m\s*K)|W\s*m[-¹1]\s*K[-¹1])\s*(?:thermal\s*conductivity|heat\s*conductivity|thermal\s*diffusivity)',
            r'(?:thermal\s*conductivity|heat\s*conductivity)\s*(?:of\s*|[:=]\s*)(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK)|W\s*m[-¹1]\s*K[-¹1])',
            r'k\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK)|W\s*m[-¹1]\s*K[-¹1])',
            r'κ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK)|W\s*m[-¹1]\s*K[-¹1])',
            r'λ\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK)|W\s*m[-¹1]\s*K[-¹1])',
            r'(\d+(?:\.\d+)?)\s*(?:W/(?:m·?K|mK)|W\s*m[-¹1]\s*K[-¹1])\s*(?:thermal|heat|conductivity|diffusivity)',
        ],
        "context_keywords": ["thermal", "conductivity", "heat", "diffusivity", "transport", 
                            "phonon", "electron", "lattice", "cooling", "dissipation"],
        "unit": "W/(m·K)",
        "typical_range": (0.01, 500),
        "synonyms": ["thermal conductivity", "heat conductivity", "thermal diffusivity",
                    "heat transfer coefficient", "thermal transport", "heat conduction"],
        "domain": "thermodynamic"
    },
    # ========== NEW QUANTITIES (ENHANCEMENT) ==========
    "yield_strength": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:MPa|GPa|Pa)\s*(?:yield\s*strength|yield\s*stress|σ_y|YS|Yield\s+Point)',
            r'(?:yield\s*strength|yield\s*stress|σ_y|YS)\s*(?:of|[:=])\s*(\d+(?:\.\d+)?)\s*(?:MPa|GPa)',
            r'σ_?y\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:MPa|GPa)',
        ],
        "context_keywords": ["yield", "strength", "stress", "mechanical", "tensile"],
        "unit": "MPa",
        "typical_range": (10, 3000),
        "synonyms": ["yield strength", "yield stress", "σ_y", "YS", "yield point"],
        "domain": "mechanical"
    },
    "ultimate_tensile_strength": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:MPa|GPa)\s*(?:ultimate\s*tensile\s*strength|UTS|tensile\s*strength)',
            r'(?:UTS|tensile\s*strength)\s*(?:of|[:=])\s*(\d+(?:\.\d+)?)\s*(?:MPa|GPa)',
        ],
        "context_keywords": ["ultimate", "tensile", "strength", "UTS"],
        "unit": "MPa",
        "typical_range": (20, 2000),
        "synonyms": ["ultimate tensile strength", "UTS", "tensile strength"],
        "domain": "mechanical"
    },
    "von_mises_stress": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:MPa|GPa)\s*(?:von\s*Mises|von-mises|equivalent\s*stress)',
            r'σ_?vm\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:MPa|GPa)',
        ],
        "context_keywords": ["von mises", "equivalent stress", "vm stress"],
        "unit": "MPa",
        "typical_range": (1, 2000),
        "synonyms": ["von mises stress", "equivalent stress", "σ_vm"],
        "domain": "mechanical"
    },
    "youngs_modulus": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:GPa|MPa)\s*(?:Young.?s\s*modulus|elastic\s*modulus|E\s*=\s*\d+)',
            r'E\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:GPa|MPa)',
        ],
        "context_keywords": ["young", "modulus", "elastic", "stiffness"],
        "unit": "GPa",
        "typical_range": (10, 500),
        "synonyms": ["young's modulus", "elastic modulus", "E"],
        "domain": "mechanical"
    },
    "viscosity": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:Pa·?s|mPa·?s|Pa\s*s|cP)\s*(?:viscosity|dynamic\s*viscosity|η)',
            r'η\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:Pa·?s|mPa·?s)',
        ],
        "context_keywords": ["viscosity", "dynamic viscosity", "melt flow"],
        "unit": "Pa·s",
        "typical_range": (0.001, 100),
        "synonyms": ["viscosity", "dynamic viscosity", "η"],
        "domain": "thermodynamic"
    },
    "enthalpy": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:J/mol|kJ/mol|J/g|kJ/kg)\s*(?:enthalpy|ΔH|latent\s*heat)',
            r'ΔH\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/mol|kJ/mol)',
        ],
        "context_keywords": ["enthalpy", "latent heat", "ΔH"],
        "unit": "J/mol",
        "typical_range": (1000, 500000),
        "synonyms": ["enthalpy", "latent heat", "ΔH"],
        "domain": "thermodynamic"
    },
    "gibbs_free_energy": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:J/mol|kJ/mol)\s*(?:Gibbs\s*free\s*energy|ΔG|free\s*energy)',
            r'ΔG\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/mol|kJ/mol)',
        ],
        "context_keywords": ["gibbs", "free energy", "ΔG"],
        "unit": "J/mol",
        "typical_range": (-100000, 100000),
        "synonyms": ["gibbs free energy", "ΔG", "free energy"],
        "domain": "thermodynamic"
    },
    "energy_density": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:J/mm³|J/cm³|MJ/m³)\s*(?:energy\s*density|volumetric\s*energy\s*density|E_v)',
            r'E_?v\s*[=:]\s*(\d+(?:\.\d+)?)\s*(?:J/mm³|J/cm³)',
        ],
        "context_keywords": ["energy density", "volumetric energy", "laser energy density"],
        "unit": "J/mm³",
        "typical_range": (0.01, 500),
        "synonyms": ["energy density", "volumetric energy density", "E_v"],
        "domain": "process"
    },
    "hardness": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:HV|HRC|HB|Vickers|Rockwell)\s*(?:hardness|microhardness)',
            r'(?:hardness|microhardness)\s*(?:of|[:=])\s*(\d+(?:\.\d+)?)\s*(?:HV|HRC)',
        ],
        "context_keywords": ["hardness", "microhardness", "vickers", "rockwell"],
        "unit": "HV",
        "typical_range": (50, 1500),
        "synonyms": ["hardness", "microhardness", "Vickers hardness"],
        "domain": "mechanical"
    },
    "cooling_rate": {
        "patterns": [
            r'(\d+(?:\.\d+)?)\s*(?:K/s|°C/s|K/min|°C/min)\s*(?:cooling\s*rate|solidification\s*rate|cooling\s*speed)',
            r'(?:cooling\s*rate|solidification\s*rate)\s*(?:of|[:=])\s*(\d+(?:\.\d+)?)\s*(?:K/s|°C/s)',
        ],
        "context_keywords": ["cooling", "solidification", "rate", "thermal history"],
        "unit": "K/s",
        "typical_range": (1, 1e6),
        "synonyms": ["cooling rate", "solidification rate", "cooling speed"],
        "domain": "thermal"
    },
}

# For backward compatibility, keep QUANTITY_PATTERNS as compiled regex
QUANTITY_PATTERNS = {k: re.compile('|'.join(f'(?:{p})' for p in v['patterns']), re.I) 
                     for k, v in ENHANCED_QUANTITY_PATTERNS.items()}

# =====================================================================
# QUERY UNIT ANALYZER (NEW)
# =====================================================================
class QueryUnitAnalyzer:
    """Extracts unit preferences from user query to bias extraction."""
    UNIT_PREFERENCES = {
        "GPa": ["GPa", "gigapascal", "gpa"],
        "MPa": ["MPa", "megapascal", "mpa"],
        "Pa": ["Pa", "pascal"],
        "J/mm³": ["J/mm³", "J/mm3", "j/mm³", "energy density"],
        "J/cm³": ["J/cm³", "J/cm3", "j/cm³"],
        "kW": ["kW", "kilowatt"],
        "W": ["W", "watt"],
        "wt.%": ["wt.%", "wt%", "weight percent"],
        "at.%": ["at.%", "at%", "atomic percent"],
        "HV": ["HV", "Vickers", "hardness vickers"],
        "HRC": ["HRC", "Rockwell C"],
        "K/s": ["K/s", "K s⁻¹", "kelvin per second", "cooling rate"],
        "°C/s": ["°C/s", "C/s", "celsius per second"],
    }

    @classmethod
    def extract_unit_preferences(cls, query: str) -> Dict[str, float]:
        """Returns a dictionary of preferred units with weights."""
        q = query.lower()
        prefs = {}
        for canonical, variants in cls.UNIT_PREFERENCES.items():
            score = 0.0
            for v in variants:
                if v.lower() in q:
                    score += 2.0
            if score > 0:
                prefs[canonical] = score
        return prefs

    @classmethod
    def get_preferred_unit(cls, query: str, quantity_domain: str = None) -> Optional[str]:
        """Returns the single most preferred unit if any."""
        prefs = cls.extract_unit_preferences(query)
        if not prefs:
            return None
        # If domain hints (e.g., mechanical -> prefer GPa/MPa), we could refine.
        return max(prefs, key=prefs.get)

# =====================================================================
# DYNAMIC UNIT EXTRACTOR (NEW)
# =====================================================================
class DynamicUnitExtractor:
    """
    Extracts numbers with arbitrary units not covered by predefined patterns.
    Uses context scoring and query preference to rank candidates.
    """
    NUMBER_UNIT_PATTERN = re.compile(
        r'(\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*'
        r'([a-zA-Zµμ°%‰]+(?:/[a-zA-Zµμ°%‰]+)?(?:·?[a-zA-Zµμ°%‰]+)?(?:\s*[-–]?\s*\d+)?)',
        re.IGNORECASE
    )
    
    COMMON_SCIENTIFIC_UNITS = {
        'MPa', 'GPa', 'kPa', 'Pa', 'N/m²', 'N/mm²',
        'J/cm³', 'J/mm³', 'MJ/m³', 'kJ/mol', 'J/mol', 'J/g',
        'W/m·K', 'W/mK', 'mW/m·K',
        'Pa·s', 'mPa·s', 'cP',
        'K/s', '°C/s', 'K/min',
        'HV', 'HRC', 'HB', 'Vickers',
    ...
    @staticmethod
    def extract_dynamic_quantities(text: str, query: str = "", query_prefs: Dict[str, float] = None) -> List[Dict]:
        candidates = []
        query_lower = query.lower()
        if query_prefs is None:
            query_prefs = {}
        
        for match in DynamicUnitExtractor.NUMBER_UNIT_PATTERN.finditer(text):
            ...
            try:
                ...
                continue
            unit = unit.strip()
            # Skip units that are too long or suspicious
            if len(unit) > 15 or unit.isdigit():
                continue
            context = text[max(0, match.start()-120):match.end()+120].strip()
            
            # Score
            score = 0.0
            # Known scientific unit
            if unit in DynamicUnitExtractor.COMMON_SCIENTIFIC_UNITS:
                score += 0.6
            elif any(known in unit for known in ['MPa','GPa','J/','W/','Pa·s','HV','K/s']):
                score += 0.4
            # Context keywords
            ctx_lower = context.lower()
            for kw in ['stress', 'strength', 'modulus', 'viscosity', 'enthalpy', 
                       'cooling rate', 'hardness', 'energy density', 'yield', 'tensile']:
                if kw in ctx_lower:
                    score += 0.5
                    break
            # Query preference boost
            for pref_unit, pref_weight in query_prefs.items():
                if pref_unit.lower() in unit.lower():
                    score += pref_weight * 0.8
            # Mention in query
            if unit.lower() in query_lower:
                score += 1.0
            
            if score > 0.4:   # tunable threshold
                guessed_qty = DynamicUnitExtractor.guess_quantity(unit, context)
                candidates.append({
                    "value": value,
                    "unit": unit,
                    "raw": match.group(0),
                    ...
                    "quantity_guess": guessed_qty,
                    "is_dynamic": True
                })
        # Remove duplicates (same value, unit, similar context)
        unique = {}
        for c in candidates:
            key = (c['value'], c['unit'], c['quantity_guess'])
            if key not in unique or c['dynamic_score'] > unique[key]['dynamic_score']:
                unique[key] = c
        return list(unique.values())
    
    @staticmethod
    def guess_quantity(unit: str, context: str) -> str:
        ctx = context.lower()
        if 'stress' in ctx or 'strength' in ctx:
            if 'yield' in ctx:
                return "yield_strength"
            elif 'ultimate' in ctx or 'tensile' in ctx:
                return "ultimate_tensile_strength"
            elif 'von mises' in ctx or 'equivalent' in ctx:
                return "von_mises_stress"
            else:
                return "mechanical_stress"
        if 'modulus' in ctx or 'young' in ctx:
            return "youngs_modulus"
        if 'viscos' in ctx:
            return "viscosity"
        if 'enthalp' in ctx or 'Δh' in ctx or 'latent' in ctx:
            return "enthalpy"
        if 'gibbs' in ctx or 'Δg' in ctx:
            return "gibbs_free_energy"
        if 'energy density' in ctx or 'volumetric' in ctx:
            return "energy_density"
        if 'hardness' in ctx or 'vickers' in ctx or 'rockwell' in ctx:
            return "hardness"
        if 'cooling' in ctx or 'solidification' in ctx:
            return "cooling_rate"
...
class QuantitativeQueryEngine:
    ...
    QUANTITY_SYNONYMS: Dict[str, List[str]] = {
        ...
        "yield_strength": ["yield strength", "yield stress", "ys", "σ_y"],
        "ultimate_tensile_strength": ["ultimate tensile strength", "uts", "tensile strength"],
        "youngs_modulus": ["young's modulus", "elastic modulus", "e modulus"],
        "viscosity": ["viscosity", "dynamic viscosity", "melt viscosity"],
        "enthalpy": ["enthalpy", "latent heat", "Δh"],
        "gibbs_free_energy": ["gibbs free energy", "free energy", "δg"],
        "energy_density": ["energy density", "volumetric energy density", "ev"],
        "hardness": ["hardness", "microhardness", "vickers", "hv"],
        "cooling_rate": ["cooling rate", "solidification rate", "cooling speed"],
    }
...
# =====================================================================
# QUANTITATIVE DATA EXTRACTOR (Legacy wrapper)
# =====================================================================
class QuantitativeDataExtractor:
    """
    Legacy wrapper - now delegates to QueryBiasedQuantitativeExtractor.
    Maintains backward compatibility while using enhanced QBQE internally.
    """
    ...
    def _get_qbqe(self, embed_model=None):
        if self._qbqe is None:
            self._qbqe = QueryBiasedQuantitativeExtractor(embed_model)
        return self._qbqe

    def extract(self, quantity_label: str, group_by: str = "material", 
                query: str = "", embed_model=None) -> pd.DataFrame:
        """Extract with optional query biasing."""
        # Collect all chunks from graph
        all_chunks = []
        for doc_id in self.graph.chunk_index:
            all_chunks.extend(self.graph.chunk_index[doc_id])

        qbqe = self._get_qbqe(embed_model)
        df = qbqe.extract_all_values(all_chunks, query=query, min_confidence=0.25)

        # Filter by requested quantity type
        df = df[df['quantity_type'] == quantity_label]
    ...
    def summarize(self, quantity_label: str, query: str = "", embed_model=None) -> Dict[str, Any]:
        """Enhanced summary with query biasing."""
        qbqe = self._get_qbqe(embed_model)
        df = self.extract(quantity_label, query=query, embed_model=embed_model)
        return qbqe.summarize(df, quantity_label)

# =====================================================================
# QUERY-BIASED QUANTITATIVE EXTRACTOR (QBQE) - ENHANCED
# =====================================================================
class QueryBiasedQuantitativeExtractor:
    """
    Enhanced version with dynamic unit detection and query preference integration.
    """
    ...
    def extract_all_values(self, chunks: List[Document], query: str = "", 
                          min_confidence: float = 0.25) -> pd.DataFrame:
        records = []
        query_prefs = QueryUnitAnalyzer.extract_unit_preferences(query)
        
        # 1. Known quantity patterns
        for chunk in chunks:
            text = chunk.page_content
            ...
            for qty_name, config in self.patterns.items():
                for pattern in config['patterns']:
                    for match in re.finditer(pattern, text, re.IGNORECASE):
                        ...
                        confidence = self._compute_pattern_confidence(value, unit, context_window, config, query_prefs)
                        if confidence < min_confidence:
                            continue
                        
                        # Compute query-biased salience
                        salience = self._compute_salience(
                            value, unit, qty_name, context_window, 
                            query, None, [], confidence, section
                        )
                        # Boost by unit preference
                        if unit in query_prefs:
                            salience *= (1 + 0.3 * query_prefs[unit])
        ...
        # 2. Dynamic extraction for unknown units
        for chunk in chunks:
            dynamic_cands = DynamicUnitExtractor.extract_dynamic_quantities(
                chunk.page_content, query, query_prefs
            )
            for d in dynamic_cands:
                records.append({
                    'value': d['value'],
                    'unit': d['unit'],
                    'quantity_type': d['quantity_guess'],
                    'confidence': d['dynamic_score'],
                    'salience': d['dynamic_score'],
                    'doc_source': chunk.metadata.get("source", "unknown"),
                    'doc_stem': Path(chunk.metadata.get("source", "unknown")).stem,
                    'section': chunk.metadata.get("section", "BODY"),
        ...
        # Post-process: re-extract material/method from context for dynamic entries
        for rec in records:
            if rec['material'] == 'Unknown' and rec['context']:
                assoc = self._extract_associated_entities(rec['context'])
                rec['material'] = assoc.get('material', 'Unknown')
                rec['method'] = assoc.get('method', 'Unknown')
    ...
    def _find_numeric_candidates(self, text: str) -> List[Dict]:
        ...
        number_unit_pattern = re.compile(
            ...
            r'(W|watts?|mW|kW|MW|'
            r'mm/s|mm/min|m/s|'
            r'J/cm²|J/cm2|J\s*cm[-²2]|mJ/cm²|µJ/cm²|'
            r'nm|µm|um|microns?|mm|cm|Å|'
            r'fs|ps|ns|μs|microseconds?|ms|'
            r'kHz|MHz|Hz|GHz|'
            r'µJ|uJ|mJ|nJ|J|kJ|'
            r'\%|percent|vol\.\%|vol%|at\.\%|wt\.\%|at%|wt%|'
            r'K|°C|°F|'
            r'GPa|MPa|Pa|N/m²|'
            r'g/cm³|kg/m³|'
            r'J/m²|J/m2|mJ/m²|mJ/m2|erg/cm²|N/m|dyn/cm|'
            r'W/(?:m·?K|mK|m\s*K)|W\s*m[-¹1]\s*K[-¹1])',
            re.IGNORECASE
        )
        for match in number_unit_pattern.finditer(text):
            ...
            try:
                if '×' in value_str or 'x' in value_str.lower() or '10^' in value_str:
                    ...
                    value_str = re.sub(r'10e\(?([-+]?\d+)\)?', r'1e', value_str)
                value = float(value_str)
            except:
                continue
            candidates.append({
    ...
    def _classify_quantity(self, value: float, unit: str, context: str, full_text: str, position: int) -> Tuple[str, float]:
        ...
        for qty_name, config in self.patterns.items():
            ...
            for pattern in config['patterns']:
                if re.search(pattern, broader_context, re.IGNORECASE):
                    ...
                    break
            keyword_conf = self._keyword_cooccurrence_confidence(broader_context, config['context_keywords'])
    ...
    def _unit_match_confidence(self, found_unit: str, expected_unit: str) -> float:
        ...
        aliases = {
            ...
            'khz': ['khz', 'kilohertz'],
            'mhz': ['mhz', 'megahertz'],
            'j/cm²': ['j/cm²', 'j/cm2', 'j/cm^2', 'mj/cm²', 'µj/cm²'],
            '%': ['%', 'percent', 'percentage', 'vol.%', 'vol%', 'at.%', 'wt.%', 'at%', 'wt%'],
            'j/m²': ['j/m²', 'j/m2', 'mj/m²', 'mj/m2', 'erg/cm²', 'n/m', 'dyn/cm'],
            'w/(m·k)': ['w/(m·k)', 'w/mk', 'w m-1 k-1', 'w/m·k'],
        }
        ...
        if expected in power_units and found in power_units:
            return 0.7
        length_units = ['nm', 'um', 'µm', 'mm', 'cm', 'm', 'å']
        if expected in length_units and found in length_units:
            return 0.7
        time_units = ['fs', 'ps', 'ns', 'μs', 'ms']
        if expected in time_units and found in time_units:
            return 0.7
        energy_units = ['µj', 'uj', 'mj', 'nj', 'j', 'kj']
        if expected in energy_units and found in energy_units:
            return 0.7
        return 0.0
    ...
    def _compute_salience(self, value: float, unit: str, qty_type: str,
        ...
        salience = (0.40 * query_sim + 0.25 * keyword_score + 0.20 * entity_score + 0.15 * section_score)
        salience *= base_conf
        return salience
    ...
    def _get_context_window(self, text: str, position: int, window: int = 250) -> str:
        start = max(0, position - window)
        end = min(len(text), position + window)
        return text[start:end]

    def _compute_pattern_confidence(self, value: float, unit: str, context: str, 
                                   config: Dict, query_prefs: Dict) -> float:
        """Compute confidence for a known pattern match."""
        ...
        range_conf = self._range_plausibility(value, config.get("typical_range", (0, 1e6)))
        conf = 0.4 * unit_conf + 0.4 * keyword_conf + 0.2 * range_conf
        # Boost if unit matches query preference
        if unit in query_prefs:
    ...
    def aggregate_by_document(self, df: pd.DataFrame, qty_type: str) -> pd.DataFrame:
        if df.empty:
            return df
        df_filtered = df[df['quantity_type'] == qty_type].copy()
        if df_filtered.empty:
            return df_filtered
        df_filtered['doc_stem'] = df_filtered['doc_source'].apply(lambda x: Path(x).stem if pd.notna(x) else 'unknown')
        grouped = df_filtered.groupby('doc_stem').agg({
            'value': ['count', 'mean', 'std', 'min', 'max'],
            'salience': 'mean',
            'confidence': 'mean',
            'material': lambda x: list(set(x))
        }).reset_index()
        grouped.columns = ['doc_stem', 'count', 'mean', 'std', 'min', 'max',
                          'avg_salience', 'avg_confidence', 'materials']
        grouped['std'] = grouped['std'].fillna(0)
        return grouped

    def find_value_clusters(self, df: pd.DataFrame, qty_type: str, eps: float = 0.15) -> pd.DataFrame:
        try:
    ...
    def summarize(self, df: pd.DataFrame, qty_type: str) -> Dict[str, Any]:
        df_filtered = df[df['quantity_type'] == qty_type]
        if df_filtered.empty:
            return {"found": False, "quantity": qty_type}
        consensus = self.compute_consensus_boost(df, qty_type)
        top_values = df_filtered.nlargest(10, 'salience')[['value', 'unit', 'salience', 'confidence', 'material', 'doc_source']]
        return {
            "found": True,
            "quantity": qty_type,
            "count": len(df_filtered),
            "unit": df_filtered["unit"].mode()[0] if not df_filtered["unit"].empty else "N/A",
            "value_range": (float(df_filtered["value"].min()), float(df_filtered["value"].max())),
            "mean": float(df_filtered["value"].mean()),
            "std": float(df_filtered["value"].std()),
            "median": float(df_filtered["value"].median()),
            "by_material": self.aggregate_by_material(df, qty_type).to_dict('records'),
            "by_document": self.aggregate_by_document(df, qty_type).to_dict('records'),
            "top_values": top_values.to_dict('records'),
            "consensus_boosts": consensus,
            "cluster_count": len(df_filtered['value'].unique()),
            "salience_range": (float(df_filtered["salience"].min()), float(df_filtered["salience"].max())),
        }

# =====================================================================
# LLM QUANTITATIVE REFINER (NEW OPTIONAL)
# =====================================================================
class LLMQuantitativeRefiner:
    """
    Uses the loaded LLM to refine ambiguous extractions, resolve unit conflicts,
    and suggest visualizations.
    """
    PROMPT_REFINE = """
You are an expert in laser materials processing and mechanical properties.
Below is a candidate quantitative extraction that may be ambiguous.
Please refine it and return JSON.

Context: {context}
Extracted value: {value} {unit}
Quantity guess: {quantity_guess}
User query: {query}

Tasks:
1. Confirm or correct the quantity type (choose from: yield_strength, ultimate_tensile_strength, youngs_modulus, viscosity, enthalpy, gibbs_free_energy, energy_density, hardness, cooling_rate, laser_power, fluence, etc.)
2. If the unit is ambiguous (e.g., 'W' could be power or weight percent), disambiguate.
3. Suggest a preferred unit for display (if query indicates preference).
4. Return JSON: {{ "quantity": "...", "value": ..., "unit": "...", "confidence": 0.0-1.0, "display_unit": "..." }}
"""
    def __init__(self, llm_generate_fn: Callable):
        self.llm_generate_fn = llm_generate_fn

    def refine_candidate(self, candidate: Dict, query: str) -> Dict:
        prompt = self.PROMPT_REFINE.format(
            context=candidate.get('context', '')[:500],
            value=candidate['value'],
            unit=candidate.get('unit', 'unknown'),
            quantity_guess=candidate.get('quantity_type', candidate.get('quantity_guess', 'unknown')),
            query=query
        )
        try:
            response = self.llm_generate_fn(prompt)
            ...
            if json_match:
                ...
                candidate['llm_refined'] = True
            else:
                candidate['llm_refined'] = False
        except Exception as e:
...
class PublicationQualityVisualizationEngine:
    ...
    def plot_quantitative_histogram(self, df: pd.DataFrame, quantity_name: str,
        ...
        title_text = custom_title or f"{quantity_name.replace('_', ' ').title()} Values by {group_by.title()}"
        fig.update_layout(
            barmode='group',
            title=title_text,
    ...
    def generate_query_driven_plots(self, df: pd.DataFrame, quantity_label: str,
                                     query: str = "", preferred_unit: str = None) -> Tuple[List[plt.Figure], List[go.Figure]]:
        """
        Generate a set of plots adapted to the query and preferred unit.
        Returns (matplotlib_figs, plotly_figs).
        """
        if df.empty:
            return [], []
        
        # Determine quantity domain from label or from query keywords
        domain = "general"
        if any(x in quantity_label for x in ['strength', 'modulus', 'hardness', 'stress']):
            domain = "mechanical"
        elif any(x in quantity_label for x in ['viscosity', 'enthalpy', 'gibbs', 'free_energy']):
            domain = "thermodynamic"
        elif any(x in quantity_label for x in ['energy_density', 'fluence', 'power']):
            domain = "process"
        elif 'cooling' in quantity_label:
            domain = "thermal"
        
        # Choose title based on query
        title = self._generate_adaptive_title(quantity_label, query, preferred_unit)
        
        mpl_figs = []
        ply_figs = []
        
        # Always include histogram and sunburst
        ply_figs.append(self.plot_quantitative_histogram(df, quantity_label, group_by="material", colormap=self.default_colormap, custom_title=title))
        ply_figs.append(self.plot_quantitative_sunburst(df, quantity_label, group_by="material", colormap=self.default_colormap))
        
        # Domain-specific plots (basic versions)
        if domain == "mechanical":
            ply_figs.append(self.plot_mechanical_radar(df, quantity_label))
            mpl_figs.append(self.plot_stress_strain_like(df, quantity_label))
        elif domain == "thermodynamic":
            ply_figs.append(self.plot_thermodynamic_comparison(df, quantity_label))
        elif domain == "process":
            ply_figs.append(self.plot_process_window(df, quantity_label))
        
        # Always add knowledge graph and dimensionality reduction if enough data
        mpl_figs.append(self.plot_quantitative_knowledge_graph(df, quantity_label, group_by="material", colormap=self.default_colormap))
        if len(df) >= 5:
            emb_fn = getattr(st.session_state, 'embeddings', None)
            if emb_fn:
                fig_tsne = self.plot_quantitative_tsne(df, emb_fn, quantity_label, group_by="material")
                if fig_tsne: mpl_figs.append(fig_tsne)
        
        return mpl_figs, ply_figs

    def _generate_adaptive_title(self, quantity_label: str, query: str, preferred_unit: str) -> str:
        base = quantity_label.replace('_', ' ').title()
        if preferred_unit:
            return f"{base} (in {preferred_unit})"
        words = query.split()
        if len(words) > 5:
            short_query = ' '.join(words[:5]) + '...'
            return f"{base} - Query: '{short_query}'"
        return base

    def plot_mechanical_radar(self, df: pd.DataFrame, quantity_label: str) -> go.Figure:
        """Radar chart comparing mechanical properties across materials."""
        if 'material' not in df.columns:
            return go.Figure()
        grouped = df.groupby('material')['value'].agg(['mean', 'std']).reset_index()
        fig = go.Figure()
        ...
        fig.update_layout(title=f"Mechanical Property Radar: {quantity_label}")
        return fig

    def plot_stress_strain_like(self, df: pd.DataFrame, quantity_label: str) -> plt.Figure:
        """Mock stress-strain like plot (scatter with trend)."""
        fig, ax = plt.subplots()
        materials = df['material'].unique()
        for mat in materials:
            subset = df[df['material'] == mat]
            ax.scatter(subset['value'], subset['salience'], label=mat, s=50)
        ax.set_xlabel(f"{quantity_label} ({df['unit'].iloc[0]})")
        ax.set_ylabel("Salience")
        ax.set_title(f"{quantity_label} vs Salience")
        ax.legend()
        return fig

    def plot_thermodynamic_comparison(self, df: pd.DataFrame, quantity_label: str) -> go.Figure:
        """Bar chart with error bars for thermodynamic quantities by material."""
        if 'material' not in df.columns:
            return go.Figure()
        grouped = df.groupby('material')['value'].agg(['mean', 'std', 'count']).reset_index()
        fig = go.Figure()
        fig.add_trace(go.Bar(
            x=grouped['material'],
            y=grouped['mean'],
        ...
        fig.update_layout(title=f"Thermodynamic Property: {quantity_label}",
                          yaxis_title=f"{quantity_label} ({df['unit'].iloc[0]})")
        return fig

    def plot_process_window(self, df: pd.DataFrame, quantity_label: str) -> go.Figure:
        """Scatter plot showing process window by material."""
        fig = px.scatter(df, x='value', y='material', color='doc_stem',
                         title=f"Process Window for {quantity_label}",
                         labels={'value': f"{quantity_label} ({df['unit'].iloc[0]})"})
        return fig
...
def initialize_session_state():
    defaults = {
        ...
        "viz_layout": "spring",
        "llm_extraction_enabled": False,
        "llm_quantitative_refinement": False,  # NEW
        "viz_top_n": 25,
...
def retrieve_and_answer_quantitative(
    ...
    """
    Enhanced quantitative retrieval using Query-Biased Quantitative Extraction (QBQE) + dynamic units + LLM refinement.
    """
    ...
    if not quantity_label:
        # Fall back to standard retrieval if no quantitative intent detected
    ...
    # Initialize QBQE with embedding model for query biasing
    emb_source = getattr(vectorstore, 'embedding_function', 
                        getattr(vectorstore, 'embeddings', vectorstore))
    emb_fn = EmbeddingWrapper(emb_source)

    # Extract ALL chunks from all documents for comprehensive extraction
    all_chunks = []
    for doc_id in graph.chunk_index:
        all_chunks.extend(graph.chunk_index[doc_id])

    # Step 1: Query-biased extraction
    extractor = QueryBiasedQuantitativeExtractor(emb_fn)
    df = extractor.extract_all_values(all_chunks, query=query, min_confidence=st.session_state.get("qbqe_min_confidence", 0.25))

    # Step 2: Optional LLM refinement
    if st.session_state.get('llm_quantitative_refinement', False) and tokenizer is not None and model is not None:
        def llm_generate(prompt):
            return generate_local_response(
                tokenizer, model, device_or_host, prompt, backend, backend_type
            )
        refiner = LLMQuantitativeRefiner(llm_generate)
        # Refine top 10 candidates
        top_candidates = df.head(10).to_dict('records')
        refined = []
        for cand in top_candidates:
            refined.append(refiner.refine_candidate(cand, query))
        # Update DataFrame with refined info
        for i, r in enumerate(refined):
            if i < len(df):
                df.loc[df.index[i], 'confidence'] = r.get('confidence', df.loc[df.index[i], 'confidence'])
                if 'display_unit' in r:
                    df.loc[df.index[i], 'display_unit'] = r['display_unit']
        st.info("LLM quantitative refinement applied to top extractions.")

    # Step 3: Summarize with enhanced statistics
    summary = extractor.summarize(df, quantity_label)

    # Step 4: Find value clusters (operating regimes)
    if st.session_state.get("qbqe_enable_clustering", True):
        clusters = extractor.find_value_clusters(df, quantity_label, eps=st.session_state.get("qbqe_cluster_eps", 0.15))
    else:
        clusters = df

    # Step 5: Aggregate by grouping dimension
    if grouping_dim == "material":
        agg = extractor.aggregate_by_material(df, quantity_label)
    elif grouping_dim == "document":
        agg = extractor.aggregate_by_document(df, quantity_label)
    else:
        agg = extractor.aggregate_by_material(df, quantity_label)
    ...
    preferred_unit = QueryUnitAnalyzer.get_preferred_unit(query, quantity_label)
    quant_prompt = f"""You are an expert scientific assistant analyzing quantitative data from multiple papers using Query-Biased Quantitative Extraction (QBQE).

QUANTITY: {quantity_label.replace('_', ' ').title()}
USER QUERY: {query}
PREFERRED UNIT: {preferred_unit if preferred_unit else 'any'}

EXTRACTION SUMMARY:
- Total values extracted: {summary.get('count', 0)}
- Unit: {summary.get('unit', 'N/A')}
- Value range: {summary.get('value_range', ('N/A', 'N/A'))}
- Mean ± Std: {summary.get('mean', 0):.2f} ± {summary.get('std', 0):.2f}
- Median: {summary.get('median', 0):.2f}
- Salience range: {summary.get('salience_range', ('N/A', 'N/A'))}

BY MATERIAL:
{json.dumps(summary.get('by_material', []), indent=2, default=str)[:1200]}

BY DOCUMENT:
{json.dumps(summary.get('by_document', []), indent=2, default=str)[:800]}

TOP VALUES (by query salience):
{json.dumps(summary.get('top_values', []), indent=2, default=str)[:600]}

CLUSTER ANALYSIS:
{json.dumps(clusters.groupby('cluster')['value'].agg(['count', 'mean', 'std']).to_dict() if 'cluster' in clusters.columns and not clusters.empty else {}, indent=2, default=str)[:400]}

INSTRUCTIONS:
1. Synthesize findings RIGOROUSLY across all extracted values
2. Discuss trends, outliers, agreements/discrepancies across materials/documents
3. Identify distinct operating regimes or process windows from clusters
4. Report uncertainty explicitly (ranges, confidence intervals)
5. Note when data is sparse or inconsistent
6. Cite specific documents and values

OUTPUT STRUCTURE:
1. **Direct Answer** - Concise summary of key findings
2. **Evidence Synthesis** - Detailed analysis with specific values and sources
3. **Operating Regimes** - Distinct process windows or clusters identified
4. **Consensus & Variability** - Agreement across documents, statistical spread
5. **Contradictions & Limitations** - Discrepancies, sparse data, caveats
...
"""
    ...
    mpl_figs, ply_figs = viz.generate_query_driven_plots(df, quantity_label, query, preferred_unit)
    ...
    meta = {
        ...
        "extraction_method": "QBQE+Dynamic",
        "cluster_count": len(clusters['cluster'].unique()) if 'cluster' in clusters.columns else 0,
        "preferred_unit": preferred_unit,
        "llm_refined": st.session_state.get('llm_quantitative_refinement', False),
        "salience_stats": {
            "min": float(df['salience'].min()) if not df.empty else 0,
            "max": float(df['salience'].max()) if not df.empty else 0,
            "mean": float(df['salience'].mean()) if not df.empty else 0,
        }
    }

    chain = ReasoningChain(query)
    chain.add_step("quantitative_intent", f"Detected quantitative query: {quantity_label}", 
                   {"group_by": grouping_dim, "method": "QBQE+Dynamic"})
    chain.add_step("extraction", f"Extracted {len(df)} values via soft pattern matching and dynamic detection", 
                   {"unit": summary.get("unit"), "salience_range": summary.get("salience_range")})
    chain.add_step("clustering", f"Found {meta['cluster_count']} operating regimes", {})
    chain.add_step("aggregation", f"Aggregated by {grouping_dim}", 
                   {"materials": len(summary.get('by_material', [])), 
                    "documents": len(summary.get('by_document', []))})
...
def render_sidebar():
    with st.sidebar:
        ...
        st.session_state.llm_quantitative_refinement = st.checkbox(
            "🔍 LLM Quantitative Refinement", value=False,
            help="Use LLM to disambiguate units and refine extracted values (slower)."
        )
        
        st.markdown("#### 🔢 QBQE Quantitative Extraction")
        st.session_state.qbqe_min_confidence = st.slider(
            "Min Pattern Confidence", 0.0, 1.0, st.session_state.get("qbqe_min_confidence", 0.25), step=0.05,
            help="Minimum confidence for soft pattern matching (lower = more values found)"
        )
        st.session_state.qbqe_salience_threshold = st.slider(
            "Min Query Salience", 0.0, 1.0, st.session_state.get("qbqe_salience_threshold", 0.3), step=0.05,
            help="Minimum query-biased salience score for inclusion"
        )
        st.session_state.qbqe_enable_clustering = st.checkbox(
            "Enable Value Clustering", value=st.session_state.get("qbqe_enable_clustering", True),
            help="Cluster values to identify distinct operating regimes"
        )
        if st.session_state.qbqe_enable_clustering:
            st.session_state.qbqe_cluster_eps = st.slider(
                "Cluster Tightness (eps)", 0.05, 0.5, st.session_state.get("qbqe_cluster_eps", 0.15), step=0.05,
                help="Lower = tighter clusters, higher = more inclusive"
            )
        st.info("ℹ️ QBQE uses soft pattern matching + query biasing + dynamic unit detection to find ALL relevant values")
...
<li><strong>QBQE 2.0:</strong> Query-biased quantitative extraction with dynamic unit detection and LLM refinement</li>
</ul>
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.processing_complete and st.session_state.knowledge_graph:
        ...
        with tab7:
            ...
            qty_options = list(ENHANCED_QUANTITY_PATTERNS.keys())
            ...
            if st.session_state.knowledge_graph:
                ...
                df_qty = extractor.extract(selected_qty, group_by=group_opt, 
                                          query=last_query, embed_model=emb_fn)
                summary_qty = extractor.summarize(selected_qty, query=last_query, embed_model=emb_fn)
                if not df_qty.empty:
