...
def main():
    st.set_page_config(
        page_title="🔬 DECLARMIMA: Pure LLM‑Native Extraction (No Regex, No Viz)",
        page_icon="🔬",
        layout="wide",
...
Our system now uses <strong>pure LLM-native structured extraction</strong> – no regex patterns anywhere.
All quantitative measurements and claims are extracted by the local LLM using Pydantic schemas, chain-of-thought, and self-critique.
...
""", unsafe_allow_html=True)
    ...
    with col2:
        if st.session_state.query_processor and st.session_state.query_processor.raw_files:
            ...
            st.markdown("""
<div class="info-card">
<h3>👋 Welcome to Query-First Processing with LLM‑Native Extraction</h3>
<p>This system features:</p>
<ul>
<li><strong>No regex patterns</strong> – all extraction is performed by the local LLM using structured schemas.</li>
<li><strong>LLM-native quantitative extraction (QBQE v3)</strong> – the LLM reads the text and outputs <code>QuantitativeMeasurement</code> objects with confidence and reasoning trace.</li>
<li><strong>Cross-document knowledge graph</strong> built from LLM-extracted entities and claims.</li>
<li><strong>Hybrid retrieval</strong> (vector similarity + graph diffusion).</li>
<li><strong>Explicit reasoning chains</strong> for full transparency.</li>
<li><strong>Consensus & contradiction detection</strong> across multiple papers.</li>
<li><strong>Zero visualizations</strong> – optimised for accurate retrieval and synthesis.</li>
</ul>
