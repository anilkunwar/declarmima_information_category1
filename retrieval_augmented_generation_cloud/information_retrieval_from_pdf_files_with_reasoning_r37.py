...
class LLMStructuredExtractor:
    ...
    def _pre_filter_chunks(self, chunks: List[Document], query: Optional[str]) -> List[Document]:
        """
        Pre-filter chunks to only those likely containing quantitative data.
        Skip references, acknowledgments, and chunks with no numbers.
        """
        if not query:
        ...
        for chunk in chunks:
            ...
            # Skip chunks with no numbers (unlikely to have measurements)
            ...
            score += sum(0.5 for kw in param_keywords if kw in text)
        ...
        max_chunks = app_config.get("max_chunks_for_llm_extraction", 15)
...
class QueryDrivenProcessor:
    ...
    def process_for_query(self, query: str, progress_bar: Any = None,
        ...
        if progress_bar: progress_bar.progress(0.85, text=f"🤖 LLM extracting from {len(retrieved_chunks)} retrieved chunks only...")
...
def main():
    st.set_page_config(
        page_title="🔬 DECLARMIMA: Inverted Pipeline (Fast LLM Extraction)",
        page_icon="🔬",
        layout="wide",
...
""", unsafe_allow_html=True)
    st.markdown('<h1 class="main-header">🔬 DECLARMIMA: Inverted Pipeline v5.1</h1>', unsafe_allow_html=True)
    st.markdown("""
<div style="text-align:center;color:#64748b;margin-bottom:1.5rem">
<span style="background:#dcfce7;border:1px solid #16a34a;color:#166534;padding:0.5rem 1rem;border-radius:0.5rem;font-weight:600;display:inline-block;margin:0.5rem 0;">⚡ FAST: LLM extraction now only on retrieved chunks (not all!)</span><br><br>
...
""", unsafe_allow_html=True)
    ...
    with col2:
        if st.session_state.query_processor and st.session_state.query_processor.raw_files:
            ...
            st.markdown("""
<div class="info-card">
<h3>👋 Welcome to Inverted Pipeline v5.1 (Performance Fixed)</h3>
<p><strong>What changed in v5.1:</strong></p>
<ul>
<li><strong>Inverted pipeline</strong>: Vector retrieval FIRST, then LLM extraction only on top-K chunks</li>
<li><strong>Chunk pre-filtering</strong>: Skip references, acknowledgments, and chunks with no numbers</li>
<li><strong>Per-chunk timeout</strong>: 10-second limit per LLM extraction call</li>
<li><strong>Max 15 chunks</strong>: LLM only processes the most relevant 15 chunks (configurable)</li>
<li><strong>Query-aware extraction</strong>: LLM prompt includes the query for targeted extraction</li>
</ul>
