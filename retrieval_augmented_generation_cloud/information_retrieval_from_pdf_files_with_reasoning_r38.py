...
class LLMStructuredExtractor:
    ...
    def _extract_single_chunk(self, chunk: Document, query: Optional[str] = None) -> Tuple[List[QuantitativeMeasurement], List[ScientificClaim]]:
        ...
        system = """You are an expert scientific information extractor. 
CRITICAL INSTRUCTION: Extract ALL quantitative measurements from the text, not just the first one.
If multiple values exist for the same parameter (e.g., laser power for different materials, conditions, or experiments), extract EVERY instance.
...
- method (if mentioned)
- conditions (dict: include what makes this measurement different from others)
...
IMPORTANT: If you see "laser power: 250W" and later "laser power: 400W", extract BOTH as separate measurements.
"""
...
class QueryDrivenProcessor:
    ...
    def process_for_query(self, query: str, progress_bar: Any = None,
        ...
        """
        INVERTED PIPELINE - THE FIX:
        1. Build vector index (fast)
        2. Retrieve top-K chunks for query (fast)
        3. Run LLM extraction ONLY on those K chunks (fast)
        4. Generate answer (fast)

        OLD broken flow:
        1. Build vector index
        2. Extract from ALL chunks ← 1+ hour here
        3. Retrieve top-K
        4. Generate answer
        """
        chain = ReasoningChain(query)
        ...
        retriever = vectorstore.as_retriever(
            search_type="similarity_score_threshold",
            search_kwargs={"k": app_config.get("max_chunks_for_llm_extraction", 15), "score_threshold": 0.2}
        )
        retrieved_chunks = retriever.invoke(query)

        if progress_bar: progress_bar.progress(0.85, text=f"🤖 LLM extracting from {len(retrieved_chunks)} expanded chunks (ensuring doc diversity)...")
...
def main():
    st.set_page_config(
        page_title="🔬 DECLARMIMA: Smart Hybrid v5.2 (Complete + Fast)",
        page_icon="🔬",
        layout="wide",
...
""", unsafe_allow_html=True)
    st.markdown('<h1 class="main-header">🔬 DECLARMIMA: Smart Hybrid v5.2</h1>', unsafe_allow_html=True)
    st.markdown("""
<div style="text-align:center;color:#64748b;margin-bottom:1.5rem">
<span style="background:#dcfce7;border:1px solid #16a34a;color:#166534;padding:0.5rem 1rem;border-radius:0.5rem;font-weight:600;display:inline-block;margin:0.5rem 0;">⚡ SMART: Comprehensive extraction from top chunks across ALL documents</span><br><br>
...
""", unsafe_allow_html=True)
    ...
    with col2:
        if st.session_state.query_processor and st.session_state.query_processor.raw_files:
            ...
            st.markdown("""
<div class="info-card">
<h3>👋 Welcome to Smart Hybrid v5.2 (Complete + Fast)</h3>
<p><strong>What changed in v5.2:</strong></p>
<ul>
<li><strong>Smart expansion</strong>: Starts with vector retrieval, then expands to ensure ALL documents are represented</li>
<li><strong>Numerical detection</strong>: Prioritizes chunks containing values + units (e.g., "250 W", "400 W")</li>
<li><strong>Document diversity</strong>: Ensures at least 2 chunks per document for cross-document comparison</li>
<li><strong>Extract ALL values</strong>: LLM instructed to find every instance, not just the first</li>
<li><strong>Up to 25 chunks</strong>: Increased from 15 to capture more values while staying fast</li>
</ul>
