...
class LLMStructuredExtractor:
    ...
    def extract_from_chunks(self, chunks: List[Document], query: Optional[str] = None,
        ...
        # ANTI-HALLUCINATION: Validate against source text
        validated_measurements = []
        for m in all_measurements:
            # Check if value actually appears in claimed source
            src_match = re.search(r'\[(.*?)\]', m.context)
            claimed_src = src_match.group(1) if src_match else ""

            # Verify value string appears in some chunk from that source
            value_str = str(m.value)
            found_in_text = False
            for chunk in chunks:
                chunk_src = chunk.metadata.get("source", "")
                if claimed_src in chunk_src and value_str in chunk.page_content:
                    found_in_text = True
                    break

            if not found_in_text:
                logger.warning(f"HALLUCINATION FILTERED: {m.value} {m.unit} not found in {claimed_src}")
                continue

            # Filter low-confidence extractions
            if m.confidence < 0.6:
                logger.warning(f"LOW CONFIDENCE FILTERED: {m.value} {m.unit} (conf={m.confidence})")
                continue

            validated_measurements.append(m)

        # Deduplicate
        unique_measurements = {}
        for m in validated_measurements:
        ...
        logger.info(f"Anti-hallucination: {len(all_measurements)}/{len(validated_measurements)} measurements validated")
...
{"parameter_name": "laser power", "value": 250, "unit": "W", "context": "exact sentence from text", "doc_source": "EXACT_FILENAME.pdf"}
...
8. Return ONLY JSON: {"measurements": [...], "claims": []}
9. No extra text before or after JSON
"""
        user = f"""SOURCE DOCUMENT: {doc_source}\n\nTEXT TO EXTRACT FROM:\n{text[:4000]}\n\nEXTRACTION TASK: Find ALL laser power values (numbers with units like W, kW) mentioned in the text above.\n\nREQUIREMENTS:\n- Only extract values that appear in the text above\n- Include exact sentence as context\n- Use filename '{doc_source}' as doc_source\n- Return valid JSON only\n\nQUERY CONTEXT: {query if query else 'Extract all quantitative values'}"""
        prompt = f"{system}\n\n{user}"
        try:
            response = self.llm_generate_fn(prompt)
            ...
            if json_str:
                ...
                measurements = self._validate_extractions(measurements, text, doc_source)
    ...
    def _extract_json(self, text: str) -> Optional[str]:
        ...
        patterns = [
            r'\{.*"measurements".*\}',  # Standard JSON with measurements key
            r'```json\s*(\{.*?\})\s*```',
            r'(\{.*\})',
        ]
    ...
    def _validate_extractions(self, measurements: List[QuantitativeMeasurement], 
                              chunk_text: str, doc_source: str) -> List[QuantitativeMeasurement]:
        """Validate that extracted values actually exist in source text."""
        validated = []
        for m in measurements:
            # Check if value appears in original text
            value_str = str(int(m.value)) if m.value == int(m.value) else str(m.value)
            if value_str not in chunk_text:
                logger.warning(f"HALLUCINATION FILTERED: {m.value} {m.unit} not found in {doc_source}")
                continue
...
def main():
    st.set_page_config(
        page_title="🔬 DECLARMIMA: Anti-Hallucination v5.4",
        page_icon="🔬",
        layout="wide",
...
""", unsafe_allow_html=True)
    st.markdown('<h1 class="main-header">🔬 DECLARMIMA: Anti-Hallucination v5.4</h1>', unsafe_allow_html=True)
    st.markdown("""
<div style="text-align:center;color:#64748b;margin-bottom:1.5rem">
<span style="background:#fee2e2;border:1px solid #dc2626;color:#991b1b;padding:0.5rem 1rem;border-radius:0.5rem;font-weight:600;display:inline-block;margin:0.5rem 0;">🛡️ ANTI-HALLUCINATION: Source-grounded extraction with validation</span><br><br>
...
""", unsafe_allow_html=True)
    ...
    with col2:
        if st.session_state.query_processor and st.session_state.query_processor.raw_files:
            ...
            st.markdown("""
<div class="info-card">
<h3>👋 Welcome to Anti-Hallucination v5.4</h3>
<p><strong>Why v5.3 hallucinated and how v5.4 prevents it:</strong></p>
<ul>
<li><strong>Anti-hallucination prompt</strong>: Explicitly forbids inventing documents, authors, or values</li>
<li><strong>Source validation</strong>: Every extracted value is verified against original chunk text</li>
<li><strong>Exact filename requirement</strong>: LLM must cite exact source filename, not "Smith et al."</li>
<li><strong>Confidence filtering</strong>: Low-confidence extractions (<0.6) are discarded</li>
</ul>
<p><strong>v5.3 vs v5.4 output comparison:</strong></p>
<table style="width:100%;border-collapse:collapse;margin:1rem 0;">
<tr style="background:#f1f5f9;"><th style="padding:0.5rem;border:1px solid #cbd5e1;">Aspect</th><th style="padding:0.5rem;border:1px solid #cbd5e1;">v5.3 (Hallucinated)</th><th style="padding:0.5rem;border:1px solid #cbd5e1;">v5.4 (Grounded)</th></tr>
<tr><td style="padding:0.5rem;border:1px solid #cbd5e1;">Document names</td><td style="padding:0.5rem;border:1px solid #cbd5e1;">"Smith et al., 2023" (invented)</td><td style="padding:0.5rem;border:1px solid #cbd5e1;"><strong>"Bialas2023-JESTCH.pdf" (exact)</strong></td></tr>
<tr><td style="padding:0.5rem;border:1px solid #cbd5e1;">Values</td><td style="padding:0.5rem;border:1px solid #cbd5e1;">"10 W to 200 W" (invented range)</td><td style="padding:0.5rem;border:1px solid #cbd5e1;"><strong>"250 W" (from text)</strong></td></tr>
<tr><td style="padding:0.5rem;border:1px solid #cbd5e1;">Context</td><td style="padding:0.5rem;border:1px solid #cbd5e1;">Paraphrased summary</td><td style="padding:0.5rem;border:1px solid #cbd5e1;"><strong>Exact sentence from paper</strong></td></tr>
<tr><td style="padding:0.5rem;border:1px solid #cbd5e1;">Missing docs</td><td style="padding:0.5rem;border:1px solid #cbd5e1;">Not mentioned</td><td style="padding:0.5rem;border:1px solid #cbd5e1;"><strong>"No values in X.pdf, Y.pdf"</strong></td></tr>
</table>
<p><strong>⚠️ Important:</strong> Small models (0.5B-3B) may still hallucinate despite these measures. For critical applications, use Qwen2.5-7B or larger.</p>
