...
def extract_metadata_from_pdf_file(pdf_path: str, filename: str) -> BibliographicMetadata:
    ...
    try:
        ...
        for field in ['doi', 'arxiv_id', 'title', 'journal', 'year', 'volume', 'issue']:
            text_val = getattr(text_meta, field, None)
            current_val = getattr(meta, field, None)
...
class CrossDocumentKnowledgeGraph:
    ...
    def add_document(self, doc_id: str, chunks: List[Document], bib_meta: BibliographicMetadata):
        ...
        for i, chunk in enumerate(chunks):
            ...
            self.documents[doc_id]["topics"].add(ent.label for ent in entities)
    ...
    def _extract_claims_from_chunk(self, chunk: Document, chunk_id: int) -> List[ScientificClaim]:
        ...
        claim_patterns = [
            ...
            (r'([a-z\-]+?)\s+(?:exhibits|shows|displays|forms|produces)\s+([a-z\-]+?(?:ripples|LIPSS|structures|morphology))', 'exhibits_morphology'),
            (r'(?:periodicity|period|spacing)\s*(?:of|for)\s+([a-z\-]+?)\s+(?:is|was|≈|~)\s+(\d+\.?\d*\s*(?:nm|µm|um))', 'has_periodicity'),
...
def extract_laser_metadata(text: str, filename: str) -> Dict[str, any]:
    ...
    param_patterns = {
        ...
        "fluence_Jcm2": r'(\d+(?:\.\d+)?)\s*(?:J/cm²|J/cm2|J\s*cm[-²2]|fluence)',
...
def render_sidebar():
    with st.sidebar:
        ...
        st.session_state.citation_style = st.selectbox(
            ...
            format_func=lambda x: {"apa": "APA: FirstAuthor et al., Journal, Year", "doi": "DOI: 10.10.xxxx/xxxx", "full": "Full: Author (Year). Title. Journal, Vol(Issue), Pages", "short": "Short: [FirstAuthor Year] or [DOI]"}[x]
        )
...
def render_chat_interface():
    ...
    has_model = (
        ...
        or (st.session_state.llm_backend_type == "transformers" and st.session_state.llm_tokenizer is not None)
    )
    if not has_model:
    ...
    if prompt := st.chat_input("Ask about laser parameters, multicomponent alloys, digital twins, etc."):
