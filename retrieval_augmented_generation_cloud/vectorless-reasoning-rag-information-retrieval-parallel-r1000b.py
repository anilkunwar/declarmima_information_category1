...
v18.0 Major upgrade (4 core) + v18.1 Fixes (5 gaps) + v18.2 Equation retrieval hardening (3 fixes)
   - QueryIntentRouter + DynamicIntentAnalyzer classify queries into:
     * value_extraction (numbers/parameters)
     * mechanism_explanation (why/how/causal)
     * comparison (vs/across all)
     * boolean_verification (is it/does)
     * equation_hunting (models/formulas/PDEs)
   - Automatically shifts retrieval weights:
     * value_extraction: metadata=0.8, keyword=0.6
     * mechanism_explanation: tree_search=0.95 (LLM reads full text)
     * equation_hunting: tree_search=0.95 + LaTeX extraction

v18.0 CORE UPGRADES:
1. 100% VECTORLESS RETRIEVAL
   - Removed sentence-transformers semantic blending dependency
   - Replaced 200+ hardcoded if/else keyword blocks with TF-IDF-style
     token overlap + metadata richness scoring
   - Dynamic weight adjustment based on query intent

2. DYNAMIC INTENT ANALYZER
   - QueryIntentRouter + DynamicIntentAnalyzer classify queries into:
     * value_extraction (numbers/parameters)
     * mechanism_explanation (why/how/causal)
     * comparison (vs/across all)
...
3. EQUATION-AWARE EXTRACTION
   - Expanded Pydantic schema: equation_latex, model_name, variables_defined
   - New item_type="equation" for constitutive models & PDEs
   - LLM prompt injects CRITICAL RULES FOR EQUATIONS & MECHANISMS
   - Supports Hollomon, Ramberg-Osgood, Cahn-Hilliard, Butler-Volmer, etc.
   - Streamlit MathJax rendering via $$...$$ blocks

4. PAGEINDEX-STYLE CITATION ENFORCEMENT
   - CitationValidator cross-checks extracted values against raw page text
   - PageIndexAnswerGenerator enforces strict output schema:
     * Direct Answer | Evidence & Citations | Cross-Document Consensus
     * Limitations/Uncertainty | Confidence Assessment
   - Standardized @@CITE:doc=filename;page=N@@ format
   - Prevents Geng2025-AMS style hallucinations

v18.1 TARGETED FIXES (closes 5 critical gaps):
P0 #1: pymupdf4llm integration for LaTeX-aware PDF math extraction
P0 #2: Equation validation now checks model_name/variables (not raw LaTeX tokens)
P1 #5: Dynamic item limits (30 for comparison/equation_hunting vs 15) + doc-grouping
P1 #4: Equation normalization (normalized_model) for cross-document comparison
P2 #3: LLM entity pre-pass for complex queries (>10 words) with shallow extraction
BUG: Fixed undefined vars_text in LLMReasoningSynthesizer.synthesize()

v18.2 EQUATION RETRIEVAL HARDENING (closes 3 architectural bottlenecks):
B1: Math-aware tokenization (_math_aware_tokenize) — preserves LaTeX commands like \sigma
B2: Equation-aware chunk expansion (MATH_PATTERN) — doubles context + sibling text for math nodes
B3: Pre-indexed math blocks (MATH_BLOCK_RE) — caches equations per-page, boosts equation_hunting docs

Original v17.1+ features preserved:
- Hierarchical tree indexing with hybrid summarization (Tier 0/1/2)
- 200+ physical quantity classifier + concept normalizer
- 35+ visualization types (Sankey, PyVis, sunburst, radar, etc.)
- Electrochemical, microstructural, AI/ML, multi-physics tracking
- Short-label document registry [A], [B]... with Unicode markers

Author: Upgraded from DECLARMIMA v17.1+ Extended codebase
Date: 2026-05-30
"""
...
class UniversalExtractionItem(BaseModel):
    ...
    context: str
    # === NEW: Equation & Free-Flow Fields ===
    equation_latex: Optional[str] = None  # e.g., r"\sigma = K \varepsilon^n"
    model_name: Optional[str] = None      # e.g., "Hollomon Power-Law", "Cahn-Hilliard"
    variables_defined: Optional[Dict[str, str]] = {}  # e.g., {"sigma": "true stress"}
    doc_source: str
    page: int
...
class DocumentMetadata(BaseModel):
    ...
    flow_speed_values: List[float] = []
    feed_rate_values: List[float] = []
    ...
    spot_size_values: List[float] = []
    exposure_time_values: List[float] = []
    hatch_distance_values: List[float] = []
    rotation_angle_values: List[float] = []
    # === Mechanical Properties ===
    yield_strength_values: List[float] = []
    tensile_strength_values: List[float] = []
    ultimate_tensile_strength_values: List[float] = []
    ...
    # === Plasticity / Work Hardening (NEW) ===
    work_hardening_rate_values: List[float] = []
    hollomon_strength_values: List[float] = []
    hollomon_exponent_values: List[float] = []
    ramberg_osgood_k_values: List[float] = []
    ramberg_osgood_n_values: List[float] = []
    # === Thermal Properties ===
    ...
    specific_heat_capacity_values: List[float] = []
    latent_heat_fusion_values: List[float] = []
    thermal_diffusivity_values: List[float] = []
    mass_diffusivity_values: List[float] = []
    surface_tension_values: List[float] = []
    # === Lewis / Jackson ===
    ...
    meltpool_morphology_values: List[str] = []
    marangoni_velocity_values: List[float] = []
    boussinesq_density_variation_values: List[float] = []
    lead_lag_time_values: List[float] = []
    positional_time_lag_values: List[float] = []
    # === Spray Dynamics ===
    ...
    # === Microstructural Advanced (NEW) ===
    martensite_start_temperature_values: List[float] = []
    spinodal_temperature_values: List[float] = []
    nucleation_rate_values: List[float] = []
    growth_rate_values: List[float] = []
    grain_boundary_energy_values: List[float] = []
    diffuse_interface_width_values: List[float] = []
    common_tangent_composition_values: List[float] = []
    phase_stability_driving_force_values: List[float] = []
    solute_cluster_size_values: List[float] = []
    eigenstrain_values: List[float] = []
    # === Electrochemical ===
    ...
    apparent_polarization_resistance_values: List[float] = []
    current_density_values: List[float] = []
    exchange_current_density_values: List[float] = []
    tafel_slope_values: List[float] = []
    charge_transfer_coefficient_values: List[float] = []
    diffusion_coefficient_values: List[float] = []
    pren_values: List[float] = []
    # === Simulation / Computational ===
    phase_field_iterations: Optional[int] = None
    md_steps: Optional[int] = None
    fem_elements_values: List[int] = []
    pinn_epochs_values: List[int] = []
    unet_dice_values: List[float] = []
    convlstm_rmse_values: List[float] = []
...
class PaginationAwareReader:
    ...
    def extract_pages(self, doc_path: str, page_numbers: List[int]) -> Dict[int, str]:
        ...
        v18.1 FIX #1: pymupdf4llm integration for LaTeX-aware PDF math extraction.
        Uses page_chunks=True for per-page extraction with metadata.
        Falls back gracefully to standard PyMuPDF text extraction.
        """
        result = {}

        # --- v18.1 FIX #1: Try pymupdf4llm first for equation-aware PDF reading ---
        ...
        if pymupdf4llm_available:
            try:
                import pymupdf4llm
                # Use page_chunks=True to get per-page markdown with metadata
                # This preserves LaTeX equations, tables, and structural formatting
                chunks = pymupdf4llm.to_markdown(doc_path, page_chunks=True)

                # Build a lookup map: page_number (1-based) -> markdown text
                chunk_map = {}
                for chunk in chunks:
                    meta = chunk.get("metadata", {})
                    # page_number in metadata is 1-based
                ...
                for pnum in page_numbers:
                    if pnum in chunk_map:
                        ...
                        result[pnum] = text
                        logger.debug(f"Page {pnum}: Using pymupdf4llm LaTeX-aware extraction")
                    else:
                        logger.debug(f"Page {pnum}: Not found in pymupdf4llm chunks, will fallback")

                # If all requested pages were found, return early
                if len(result) == len(page_numbers):
                    return result

            except Exception as e:
                logger.warning(f"pymupdf4llm extraction failed, falling back to standard: {e}")
                result = {}  # Reset and fallback

        # Fallback: Standard PyMuPDF text extraction for missing pages
...
class StructuredMetadataExtractor:
    ...
    POWER_PATTERN = r'(?:laser\s+power|power|beam\s+power|P_{?I?}?)\D{0,20}?(\d{2,4}(?:\.\d+)?)\s*(?:\\mathrm\{W\}|W|kW|mW|\s*W)'
    SCAN_SPEED_PATTERN = r'(?:scan\s+speed|scanning\s+speed|v_scan|Vs|travel\s+speed)\D{0,20}?(\d+(?:\.\d+)?)\s*(mm/s|cm/s|m/s|mm/min|m/min)'
    YIELD_PATTERN = r'(?:yield\s+strength|YS|0\.2%\s+proof|proof\s+stress)\D{0,20}?(\d+(?:\.\d+)?)\s*(MPa|GPa|psi|ksi)'
    TENSILE_PATTERN = r'(?:tensile\s+strength|UTS|ultimate\s+strength|Rm)\D{0,20}?(\d+(?:\.\d+)?)\s*(MPa|GPa|psi|ksi)'
    HARDNESS_PATTERN = r'(?:hardness|HV|Vickers|microhardness)\D{0,20}?(\d+(?:\.\d+)?)\s*(HV|MPa|GPa|HRC)'
    TEMP_PATTERN = r'(?:temperature|temp|T|melting)\D{0,20}?(\d+(?:\.\d+)?)\s*(°C|K|°F|deg\s*C|degC)'
    VED_PATTERN = r'(?:volumetric\s+energy\s+density|VED|energy\s+density)\D{0,20}?(\d+(?:\.\d+)?)\s*(J/mm³|J/mm3|J/cm³|J/cm3|kJ/mm³)'
    ...
    def __init__(self):
        self.compiled_patterns = {
            ...
            "ramberg_osgood_n": (re.compile(r"(?:ramberg osgood n|ramberg-osgood n|n_RG|q_RG)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)", re.IGNORECASE), float),
            # === NEW: Meltpool / Fluid ===
            "meltpool_width": (re.compile(r"(?:meltpool width|pool width|melt width|w_pool)\s*[=:]\s*(\d+(?:\.\d+)?)\s*(um|nm|mm|µm)", re.IGNORECASE), float),
            "marangoni_velocity": (re.compile(r"(?:marangoni|thermo-capillary)\s*(?:velocity|speed|v)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(mm/s|m/s|cm/s|um/s)", re.IGNORECASE), float),
            # === NEW: Microstructural ===
            "martensite_start_temperature": (re.compile(r"(?:martensite start|ms temperature|M_s|Ms|martensite start temp)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(C|K|°C|°F|degC|deg F)", re.IGNORECASE), float),
            "spinodal_temperature": (re.compile(r"(?:spinodal temperature|T_spinodal|spinodal temp)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(C|K|°C|degC)", re.IGNORECASE), float),
            "nucleation_rate": (re.compile(r"(?:nucleation rate|J_nuc|nucleation freq)\s*[=:]\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*(m-3s-1|1/m3s|s-1|1/s)", re.IGNORECASE), float),
            "growth_rate": (re.compile(r"(?:growth rate|v_growth|interface velocity|growth velocity)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(mm/s|m/s|um/s|nm/s|µm/s)", re.IGNORECASE), float),
            "grain_boundary_energy": (re.compile(r"(?:grain boundary energy|interface energy|gb energy|gamma_gb)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(J/m2|mJ/m2|J/m²)", re.IGNORECASE), float),
            "diffuse_interface_width": (re.compile(r"(?:diffuse interface width|interface thickness|delta_interface)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(nm|um|µm|mm)", re.IGNORECASE), float),
            "solute_cluster_size": (re.compile(r"(?:solute cluster|cluster radius|cluster size|sro size|mro size)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(nm|um|µm|mm|angstrom|Å)", re.IGNORECASE), float),
            # === NEW: Thermal Properties ===
            "surface_tension": (re.compile(r"(?:surface tension|liquid-vapor tension|sigma_st)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(N/m|mN/m|J/m2)", re.IGNORECASE), float),
            "mass_diffusivity": (re.compile(r"(?:mass diffusivity|diffusion coefficient|D\s*[=:])\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*(m2/s|cm2/s|m²/s|mm2/s)", re.IGNORECASE), float),
            "thermal_diffusivity": (re.compile(r"(?:thermal diffusivity|alpha_th|thermal diff)\s*[=:]\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*(m2/s|cm2/s|m²/s|mm2/s)", re.IGNORECASE), float),
            ...
            "unet_dice": (re.compile(r"(?:unet dice|dice coefficient|dice score|segmentation dice)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)", re.IGNORECASE), float),
            "convlstm_rmse": (re.compile(r"(?:convlstm rmse|rmse|root mean square error|prediction rmse)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)", re.IGNORECASE), float),
            # === NEW: Multi-physics ===
            "eigenstrain": (re.compile(r"(?:eigenstrain|transformation strain|misfit strain|stress-free strain|epsilon_star)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(unitless|strain|mm/mm|microstrain|%)", re.IGNORECASE), float),
            "boussinesq_density_variation": (re.compile(r"(?:boussinesq|density variation|delta_rho|density diff)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(kg/m3|g/cm3|kg/m³|g/cm³)", re.IGNORECASE), float),
            "lead_lag_time": (re.compile(r"(?:lead-lag time|thermal diffusion lag|chemical diffusion lag|diffusion lag)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(ms|s|us|ns|min)", re.IGNORECASE), float),
            "positional_time_lag": (re.compile(r"(?:positional time lag|delta_t_pos|phase front lag|time lag)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(ms|s|us|ns|min)", re.IGNORECASE), float),
            # === NEW: Electrochemical ===
            "exchange_current_density": (re.compile(r"(?:exchange current density|j0|i0|J0|I0)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(uA/cm2|mA/cm2|A/cm2|uA|mA|µA/cm²)", re.IGNORECASE), float),
            "tafel_slope": (re.compile(r"(?:tafel slope|tafel|b_a|b_c|beta_tafel)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)\s*(mV/dec|V/dec|mV/decade)", re.IGNORECASE), float),
            "charge_transfer_coefficient": (re.compile(r"(?:charge transfer coefficient|transfer coefficient|alpha|alpha_ct)\s*[=:]\s*([+-]?\d+(?:\.\d+)?)", re.IGNORECASE), float),
            "diffusion_coefficient": (re.compile(r"(?:diffusion coefficient|diffusivity|D_electrochemical)\s*[=:]\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*(cm2/s|m2/s|cm²/s|m²/s)", re.IGNORECASE), float),
        }
    ...
    def extract_metadata(self, doc_name: str, full_text: str) -> DocumentMetadata:
        ...
        for field, (pattern, cast_func) in self.compiled_patterns.items():
            ...
            for m in matches:
                try:
                    # Handle both tuple (group capture) and string returns
            ...
            if field_lower in ["phase_field_iterations", "md_steps"]:
                # These are Optional[int], not list fields
                attr_name = field_lower
                if values and hasattr(meta, attr_name):
                    setattr(meta, attr_name, values[0])
            else:
                attr_name = f"{field_lower}_values"
                if hasattr(meta, attr_name):
                    setattr(meta, attr_name, values)
                else:
                    # Fallback: store in other_parameters if attribute does not exist
                    if values:
                        meta.other_parameters.setdefault(attr_name, []).extend(values)
...
class IterativeTreeNavigator:
    ...
    async def navigate_and_retrieve(self, query: str, trees: List[Dict]) -> List[Dict]:
        ...
        for step in range(self.max_steps):
            ...
            # v20.0 FIX #2: Cap nodes to prevent local LLM context overflow
            ...
            # 🛑 CRITICAL FIX: Desperation Fallback if LLM fails JSON on first step
            if not actions and step == 0:
                logger.warning("⚠️ MCTS JSON failed on Step 1. Falling back to top 3 summary scores.")
                # Force extraction from the top 3 nodes currently in view
                ...
                break  # Exit the loop, we have our text

            if not actions:
                break  # Normal termination if no actions on later steps
  ...
  "actions": [
    ...
    def _parse_actions(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            # 3. FINAL FALLBACK: Try to parse ANY JSON-like structure with regex
            broad_match = re.search(r'(?:\{|\[).*?(?:\}|\])', response, re.DOTALL)
            if broad_match:
                try:
                ...
                if 'extract' in line or 'drill' in line or 'text' in line:
                    ...
                    if node_id_match:
                        text_actions.append({
                            ...
                            'action': 'extract_text' if 'extract' in line or 'text' in line else 'drill_down'
                        })
            if text_actions:
                logger.warning(f"Ultra-fallback parsed {len(text_actions)} actions from plain text")
                return text_actions
            return []
        except Exception as e:
...
class ScientificIntentRouter:
    ...
    Value extraction is GATED - only activates on explicit numerical triggers.
    Default is open semantic/conceptual search.
    """

    # EXPLICIT triggers required to activate value_extraction mode
    VALUE_TRIGGERS = [
        ...
        r"\b\d+\s*(?:W|kW|mW|mm/s|MPa|GPa|K|°C|J/mm|µm|nm|mJ/m²)",  # Query contains units
        ...
        # === v20.0 FIX #1: Spelled-out units and "describe [parameter]" patterns ===
        r"\bwatts\b", r"\bkilowatts\b", r"\bkW\b", r"\bmm/s\b", r"\bmpa\b",  # Spelled out units
        ...
        # === NEW: Constitutive model parameter triggers (v19.0 FIX #1) ===
        r"\bhollomon\b", r"\bramberg[- ]osgood\b", r"\bcahn[- ]hilliard\b",
        r"\bbutler[- ]volmer\b", r"\bnavier[- ]stokes\b",
        r"\bparameters?\b.*\b(of|for)\b",  # "parameters of AlMgSiZr"
        r"\bcoefficient\b", r"\bexponent\b", r"\bconstant\b",
    ...
    INTENT_PATTERNS = {
        "equation": [
            r"\bequation\b", r"\bformula\b", r"\bgoverning\b", r"\bconstitutive\b",
            r"\bnavier[- ]stokes\b", r"\bboussinesq\b", r"\bmarangoni\b",
            r"\bmomentum\s+equation\b", r"\bcontinuity\s+equation\b",
            r"\benergy\s+equation\b", r"\bcahn[- ]hilliard\b",
            r"\bbutler[- ]volmer\b", r"\bhollomon\b",
            r"\bramberg[- ]osgood\b", r"\bderive\b", r"\bmathematical model\b",
            r"\bpde\b", r"\bpartial\s+differential\b", r"\bgoverning\s+equation\b",
            r"\bconstitutive\s+model\b", r"\bfluid\s+flow\s+equation\b",
            r"\bheat\s+equation\b", r"\bdiffusion\s+equation\b",
            r"\bschrodinger\b", r"\bmaxwell\b", r"\bfick\b",
            r"\bfourier\b", r"\barrhenius\b", r"\bjohnson[- ]cook\b",
            r"\bvoce\b", r"\bludwik\b", r"\bswift\b",
        ],
        # === NEW: Constitutive intent bridges equations and parameters (v19.0 FIX #2) ===
        "constitutive": [
            r"\bhollomon\b", r"\bramberg[- ]osgood\b", r"\bludwik\b",
            r"\bswift\b", r"\bvoce\b", r"\bjohnson[- ]cook\b",
            r"\bconstitutive\s+(model|law|relation|equation|parameters?)\b",
            r"\bflow\s+stress\b", r"\bhardening\s+(law|model|rule)\b",
            r"\byield\s+(criterion|surface|function)\b",
            r"\bparameters?\b.*\b(hollomon|ramberg|osgood|ludwik|swift|voce|johnson)\b",
            r"\b(strength\s+coefficient|hardening\s+exponent|strain\s+hardening)\b",
        ],
        "mechanism": [
            r"\bwhy\b", r"\bhow does\b", r"\bmechanism\b", r"\bexplain\b",
            r"\bcause\b", r"\bdriving force\b", r"\bphysical process\b",
            r"\bfluid flow\b", r"\bmelt[- ]?pool\s+(dynamics|behavior|regime|formation)\b",
            r"\bporosity\s+formation\b", r"\bgrain\s+growth\b",
            r"\bsolidification\b", r"\bphase\s+transformation\b",
            r"\brecrystallization\b", r"\bnucleation\b", r"\bgrowth\b",
            r"\bsegregation\b", r"\bdiffusion\s+mechanism\b",
            r"\bcorrosion\s+mechanism\b", r"\bfatigue\s+mechanism\b",
            r"\bfracture\s+mechanism\b", r"\bwear\s+mechanism\b",
            r"\bthermal\s+cycle\b", r"\bresidual\s+stress\b",
            r"\bwhat\s+happens\b", r"\bwhat\s+is\s+the\s+reason\b",
            r"\bdescribe\s+the\s+process\b",
        ],
        "comparison": [
            r"\bcompare\b", r"\bvs\.?\b", r"\bversus\b", r"\bdifference\b",
            r"\bacross all\b", r"\bwhich is better\b", r"\bcontrast\b",
            r"\bwhich\b.*\b(higher|lower|stronger|weaker|better|worse)\b",
            r"\brelative\b.*\b(performance|strength|behavior)\b",
            r"\bsimilarities\b", r"\bdifferences\b",
        ],
        "sketch_diagram": [
            r"\bsketch\b", r"\bdiagram\b", r"\bdraw\b", r"\bvisualize\b",
            r"\bschematic\b", r"\bflowchart\b", r"\billustrate\b",
            r"\bfigure\b.*\b(show|depict|display|present)\b",
            r"\bshow\b.*\b(process|setup|structure|microstructure)\b",
            r"\bprocess\s+flow\b", r"\bsetup\s+diagram\b",
            r"\bmicrostructure\s+image\b", r"\bschematic\s+diagram\b",
            r"\bblock\s+diagram\b", r"\bphase\s+diagram\b",
        ],
        "boolean": [
            r"\bis it\b", r"\bdoes\b", r"\bdo\b", r"\bare there\b", r"\bcan\b",
            r"\bwas\b", r"\bwere\b", r"\bhas\b", r"\bhave\b",
            r"\bis there\b", r"\bare they\b", r"\bmentioned\b",
        ],
        "definition": [
            r"\bwhat is\b", r"\bwhat are\b", r"\bdefine\b",
            r"\bmeaning of\b", r"\bdefinition of\b",
            r"\bdescribe\b", r"\btell me about\b",
            r"\bexplain\b.*\bwhat\b",
        ],
    }
    ...
    def route(self, query: str) -> Dict[str, Any]:
        ...
        # 1. Check for EXPLICIT value triggers FIRST (gating mechanism)
        has_value_trigger = any(re.search(p, q_lower) for p in self.VALUE_TRIGGERS)

        # 2. Classify intent - default to open_query
        intent = "open_query"
        output_format = "prose"
        ...
        # Override: If explicit value trigger found, switch to value_extraction
        if has_value_trigger:
            intent = "value_extraction"
            output_format = "table"
        # === NEW: Constitutive intent routing (v19.0 FIX #2b) ===
        elif intent == "constitutive":
            output_format = "constitutive_hybrid"  # NEW hybrid format
        ...
        # 3. Dynamic weights based on intent
        weights = {
    ...
    def _build_focus(self, intent, query, has_value_trigger):
        ...
        # === NEW: Constitutive focus instruction (v19.0 FIX #2d) ===
        elif intent == "constitutive":
...
class QueryAnalyzer:
    """Analyzes user intent and dynamically adjusts retrieval/extraction weights."""

    # Weight matrix: How much to trust metadata vs semantic search based on intent
    WEIGHT_MATRIX = {
        "value_extraction": {"metadata": 0.8, "semantic": 0.2, "process": 0.3},
        "mechanism_explanation": {"metadata": 0.2, "semantic": 0.7, "process": 0.5},
        "comparison": {"metadata": 0.6, "semantic": 0.4, "process": 0.4},
        "boolean_verification": {"metadata": 0.5, "semantic": 0.5, "process": 0.2},
        "open_query": {"metadata": 0.4, "semantic": 0.6, "process": 0.3},
        "equation_hunting": {"metadata": 0.3, "semantic": 0.7, "process": 0.2}
    }

    def __init__(self, phys_classifier: PhysicalQuantityClassifier, concept_normalizer: ConceptNormalizer, llm: Optional["HybridLLM"] = None):
        self.phys_classifier = phys_classifier
        self.concept_normalizer = concept_normalizer
        self.llm = llm  # --- v18.1: LLM for complex query entity pre-pass ---
    ...
    def analyze(self, query: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "target_parameters": [],
            "target_materials": [],
            "weights": self.WEIGHT_MATRIX["value_extraction"],
            "focus_instruction": "",
            "output_format": "table"
        }

        # 1. Intent Detection
        mechanism_kw = ["why", "how", "mechanism", "effect", "influence", "role", "explain", "cause"]
        comparison_kw = ["compare", "vs", "versus", "difference", "across all", "which is better", "contrast"]
        boolean_kw = ["is it", "does", "do", "are there", "can", "mentioned"]
        equation_kw = ["equation", "model", "formula", "derive", "governing", "constitutive", "kinetics"]

        if any(kw in q_lower for kw in equation_kw):
            analysis["intent"] = "equation_hunting"
            analysis["output_format"] = "latex"
        elif any(kw in q_lower for kw in mechanism_kw):
            analysis["intent"] = "mechanism_explanation"
            analysis["output_format"] = "causal_chain"
        elif any(kw in q_lower for kw in comparison_kw):
            analysis["intent"] = "comparison"
            analysis["output_format"] = "contrastive_table"
        elif any(kw in q_lower for kw in boolean_kw):
            analysis["intent"] = "boolean_verification"
            analysis["output_format"] = "yes_no_evidence"

        analysis["weights"] = self.WEIGHT_MATRIX.get(analysis["intent"], self.WEIGHT_MATRIX["value_extraction"])

        # 2. Entity Extraction (Leveraging existing classifiers)
        for canonical, keywords in self.phys_classifier.CANONICAL.items():
            if any(kw in q_lower for kw in keywords):
                analysis["target_parameters"].append(canonical)

        for canonical, aliases in self.concept_normalizer.ALIAS_DICTIONARIES.items():
            if any(alias in q_lower for alias in aliases):
                if canonical not in ["unknown", "multicomponent", "heas_mpeas"]:
                    analysis["target_materials"].append(canonical)

        # 3. Generate Dynamic Focus Instruction for the LLM Extractor
        if analysis["target_parameters"]:
            params = [self.phys_classifier.get_human_readable(p) for p in analysis["target_parameters"][:3]]
            analysis["focus_instruction"] = f"CRITICAL FOCUS: The user is specifically asking about {', '.join(params)}. Prioritize extracting these exact values, equations, and context over generic numbers."

        if analysis["target_materials"]:
            mats = [m.replace('_', ' ').title() for m in analysis["target_materials"][:3]]
            analysis["focus_instruction"] += f" Ensure all extracted data is explicitly linked to: {', '.join(mats)}."

        if analysis["intent"] == "mechanism_explanation":
            analysis["focus_instruction"] += " Extract causal relationships, physical models, and qualitative explanations, not just numbers."
        elif analysis["intent"] == "equation_hunting":
            analysis["focus_instruction"] += " Extract ALL governing equations, constitutive models, and PDEs in LaTeX format."

        # --- v18.1 FIX #3: LLM pre-pass for complex queries with shallow keyword extraction ---
        if len(query.split()) > 10 and not analysis["target_parameters"]:
            try:
                entity_prompt = f"""Extract target parameters and materials from this scientific query. Return ONLY a JSON object with this exact structure: {{"parameters": ["param1", "param2"], "materials": ["material1"]}}

Query: "{query}"

JSON:"""
                resp = self.llm.generate(entity_prompt, max_new_tokens=100, fast_json=True)
                # Try to extract JSON from response
                json_match = re.search(r'\{.*\}', resp, re.DOTALL)
                if json_match:
                    entities = json.loads(json_match.group())
                    analysis["target_parameters"] = entities.get("parameters", [])
                    analysis["target_materials"] = entities.get("materials", [])
            except Exception:
...
# =============================================================================
# CITATION VALIDATOR (Prevents Hallucinations) - v18.0 UPGRADE
# =============================================================================
class CitationValidator:
    """Cross-checks extracted values against raw page text to prevent hallucinations."""

    def __init__(self, reader: PaginationAwareReader):
        self.reader = reader

    def verify_items(self, items: List[UniversalExtractionItem], max_check_chars: int = 3000) -> List[UniversalExtractionItem]:
        """Cross-check extracted items against raw page text to prevent hallucinations.

        v18.1 FIX #2: Equation validation now checks model_name/variables instead of raw LaTeX tokens.
        Raw LaTeX commands (e.g., \sigma, \varepsilon) are rarely preserved in PDF text extraction,
        so we verify equation items by checking for the model name and variable descriptions in context.
        """
        verified = []
        for item in items:
            try:
                page_text = self.reader.extract_pages(item.doc_source, [item.page]).get(item.page, "")
                if not page_text:
                    item.confidence *= 0.5  # Penalize missing page
                    verified.append(item)
                    continue

                # Check if value/parameter actually appears in context
                val_str = str(item.value) if item.value is not None else ""
                param = (item.parameter_name or "").lower()
                context_check = page_text[:max_check_chars].lower()

                if val_str and val_str in page_text:
                    item.confidence = min(1.0, item.confidence + 0.1)
                elif param and param in context_check:
                    item.confidence = min(1.0, item.confidence + 0.05)
                elif item.item_type == "equation":
                    # --- v18.1 FIX #2: Equation validation checks model_name/variables, not raw LaTeX ---
                    # Raw LaTeX tokens (\sigma, \varepsilon) are NOT preserved in standard PDF text.
                    # We verify by checking if the model name or variable descriptions appear in page text.
                    check_terms = []
                    if item.model_name:
                        check_terms.append(item.model_name.lower())
                        # Also check for common aliases of the model name
                        model_lower = item.model_name.lower()
                        if "hollomon" in model_lower:
                            check_terms.extend(["power law", "power-law", "strain hardening"])
                        elif "ramberg" in model_lower or "osgood" in model_lower:
                            check_terms.extend(["ramberg", "osgood", "elastoplastic"])
                        elif "cahn" in model_lower or "hilliard" in model_lower:
                            check_terms.extend(["cahn", "hilliard", "phase field"])
                        elif "butler" in model_lower or "volmer" in model_lower:
                            check_terms.extend(["butler", "volmer", "charge transfer"])
                    if item.variables_defined:
                        # Check variable descriptions (e.g., "true stress", "strain hardening exponent")
                        check_terms.extend(v.lower() for v in item.variables_defined.values() if v)

                    # Check if any verification term appears in page text
                    page_lower = page_text.lower()
                    matched_terms = [term for term in check_terms if term and term in page_lower]

                    if matched_terms:
                        item.confidence = min(1.0, item.confidence + 0.15)
                        item.reasoning_trace += f" [OK] Equation context confirmed: {', '.join(matched_terms[:3])}."
                    else:
                        # Fallback: check if key LaTeX symbols appear (only works with pymupdf4llm)
                        if item.equation_latex:
                            eq_symbols = re.findall(r'\\[a-zA-Z]+', item.equation_latex)
                            if any(sym in page_text for sym in eq_symbols[:3]):
                                item.confidence = min(1.0, item.confidence + 0.1)
                                item.reasoning_trace += " [OK] LaTeX symbols confirmed in page text."
                            else:
                                # Reduced penalty - equation extraction is inherently uncertain
                ...
                item.confidence *= 0.4
                item.reasoning_trace += f" [ERROR] Verification failed: {e}"
                verified.append(item)
        return sorted(verified, key=lambda x: x.confidence, reverse=True)


# =============================================================================
# PAGEINDEX ANSWER GENERATOR (Structured Citation-Enforced Output) - v18.0 UPGRADE
# =============================================================================

# =============================================================================
# ADAPTIVE RESPONSE GENERATOR (v19.0) - Replaces PageIndexAnswerGenerator + LLMReasoningSynthesizer
# =============================================================================
class AdaptiveResponseGenerator:
    """Generates responses in the appropriate format based on intent."""

    FORMATTERS = {
        "table": "_format_as_value_comparison",  # v20.0 FIX: Route table intent to value_comparison
        "latex": "_format_as_equations",
        "constitutive_hybrid": "_format_as_constitutive_hybrid",  # NEW (v19.0 FIX #3)
        "causal_chain": "_format_as_mechanism",
        "contrastive_table": "_format_as_value_comparison",  # v20.0 FIX: Also use value_comparison for contrastive
    ...
    def _format_as_equations(self, query, items, qa):
        ...
        if not eq_items:
            # Fallback: search for any items with equation_latex even if not typed as equation
    ...
    def _format_as_constitutive_hybrid(self, query, items, qa):
        """STRICT template-enforced hybrid formatter for constitutive models.

        v19.0 FIX #3: Bridges equation-only and value-only formatters.
        Uses rigid template structure because small LLMs ignore soft formatting requests.
        """
        # Collect BOTH equation items AND relevant quantitative parameters
        eq_items = [i for i in items if i.item_type == "equation" and i.equation_latex]
        param_items = [i for i in items if i.value is not None and 
                       i.parameter_name and any(kw in (i.parameter_name or "").lower() 
                       for kw in ["hollomon", "ramberg", "osgood", "strength_coefficient", 
                                  "hardening_exponent", "strain_hardening", "ludwik", 
                                  "swift", "voce", "johnson", "cook", "flow_stress"])]

        # Also include items with constitutive model names in content
        content_items = [i for i in items if i.value is not None and 
                        any(kw in i.content.lower() for kw in ["hollomon", "ramberg", "osgood",
                               "strength coefficient", "hardening exponent", "strain hardening",
                               "ludwik", "swift", "voce", "johnson-cook", "flow stress"])]

        # Merge param_items and content_items, deduplicate by (content, doc_source, page)
        seen_keys = set()
        merged_params = []
        ...
        if not eq_items and not merged_params:
            # Fallback: try to find any items that mention constitutive terms
        ...
        # Build strict evidence blocks for LLM template enforcement
        eq_block = []
        if eq_items:
            for item in eq_items:
        ...
        # STRICT TEMPLATE - small LLMs need rigid structure, not suggestions
...
- If parameters are temperature-dependent, note the temperature range"""
    ...
    def _format_as_visual(self, query, items, qa):
        ...
        if len(lines) == 1:
            # Try to generate a visual from any available content
...
Include citations in @@CITE:doc=X;page=Y@@ format."""
    ...
    def _format_as_prose(self, query, items, qa):
        """STRICT template-enforced open-query format. Small LLMs require rigid structure, not suggestions."""
        if not items:
            return f"No relevant information found for '{query}'. Try rephrasing your question or uploading additional documents."

        # Build evidence summary with strict citation format
        evidence_summary = []
        for i in items[:25]:
        ...
        # STRICT TEMPLATE - not a suggestion. Small LLMs need rigid structure.
        prompt = f"""You are a scientific analyst. You MUST follow this EXACT output structure. Do NOT deviate.
...
- If information is missing, explicitly state the limitation"""
    ...
    def _format_as_table(self, query, items, qa):
        """STRICT template-enforced table format. Only called when value_extraction is EXPLICITLY triggered."""
        ...
        # Build evidence blocks for strict template
        param_block = []
        ...
        # STRICT TEMPLATE - small LLMs need rigid structure
        prompt = f"""You are a scientific analyst. You MUST follow this EXACT output structure. Do NOT deviate.
...
- Include ALL extracted parameters, not just a subset"""
    ...
    def _format_as_value_comparison(self, query, items, qa):
        """v20.0 FIX: Formats extracted values into a narrative by material, followed by a table and key takeaway.

        This formatter is explicitly engineered to force Qwen 14B to output:
        1. Narrative by Material/Alloy (laser powers tested, outcomes, key findings)
        2. Summary Table (Material | Process | Laser Power (W) | Optimal Power)
        3. Key Takeaway (synthesizing macro-trends across all materials)
        """
        # Filter for quantitative data and tabular data
        value_items = [i for i in items if i.value is not None or i.item_type == "tabular_data"]
        if not value_items:
            return self._format_as_prose(query, items, qa)

        # Build structured evidence blocks for the LLM
        evidence_blocks = []
        for item in sorted(value_items, key=lambda x: x.confidence, reverse=True):
            ...
            # Provide enough context for the LLM to write the "Outcomes & Context" bullet
            evidence_blocks.append(f"- [{mat}] {pq}: {val} {unit} | Context: {item.content[:300]} {cite}")

        evidence_text = "\n".join(evidence_blocks)

        # STRICT TEMPLATE matching the exact desired output style
...
- Keep the tone professional, precise, and scientific."""
        # Qwen 14B handles this 2000-token prompt with rigid structure effortlessly
        return self.llm.generate(prompt, max_new_tokens=2048, temperature=0.1)

    def _format_as_mechanism(self, query, items, qa):
        """STRICT template-enforced causal chain explanation."""
        if not items:
...
- Number steps sequentially"""
    ...
    def _format_as_comparison(self, query, items, qa):
        """STRICT template-enforced contrastive analysis."""
        if not items:
...
- Use markdown tables for quantitative comparisons"""
    ...
    def _format_as_boolean(self, query, items, qa):
        """STRICT template-enforced yes/no verification."""
        if not items:
...
class TwoStageRetriever:
    ...
    @staticmethod
    def _math_aware_tokenize(text: str) -> Set[str]:
        """v18.2: Preserve LaTeX command names while stripping structural chars."""
        if not text:
            return set()
        text = re.sub(r'\\[a-zA-Z]+', lambda m: m.group().replace('\\', ''), text)
        text = re.sub(r'[{}_^=]', ' ', text)
        return set(re.findall(r'\b\w+\b', text.lower()))

    def index_document(self, doc_name: str, metadata: DocumentMetadata, summary: str, page_math_cache: Optional[Dict[int, List[str]]] = None):
        self.doc_metadata[doc_name] = metadata
        self.doc_summaries[doc_name] = summary
        if page_math_cache:
            self.doc_math_cache[doc_name] = page_math_cache

    def retrieve_relevant_docs(self, query: str, query_analysis: Optional[Dict[str, Any]] = None, top_k: int = 10) -> List[Tuple[str, float]]:
        """
        100% Vectorless Dynamic Retrieval.
        Replaces hardcoded keyword matching with TF-IDF-style token overlap
        + metadata richness scoring, dynamically weighted by query intent.
        """
        scores = []
        weights = query_analysis.get("weights", {"metadata": 0.15, "keyword": 0.25, "semantic": 0.7, "tree_search": 0.95}) if query_analysis else {"metadata": 0.15, "keyword": 0.25, "semantic": 0.7, "tree_search": 0.95}
        q_lower = query.lower()
        q_tokens = self._math_aware_tokenize(q_lower)  # v18.2: math-aware

        for name, meta in self.doc_metadata.items():
            score = 0.0

            # 1. Dynamic Metadata Scoring (Vectorless)
            meta_text = " ".join(meta.alloys + meta.process_types).lower()
            meta_tokens = self._math_aware_tokenize(meta_text)  # v18.2
            meta_overlap = len(q_tokens.intersection(meta_tokens))
            score += meta_overlap * 0.5 * weights.get("metadata", 0.5)

            # Reward documents with rich populated metadata fields (richness heuristic)
            populated_fields = 0
            for field_name in meta.__dataclass_fields__ if hasattr(meta, '__dataclass_fields__') else []:
                val = getattr(meta, field_name, None)
                if val is not None and str(val) not in ["[]", "None", "", "0", "0.0"]:
                    populated_fields += 1
            score += populated_fields * 0.02 * weights.get("metadata", 0.5)

            # 2. Vectorless Keyword Scoring (TF-IDF style using summary)
            summary_text = self.doc_summaries.get(name, "").lower()
            summary_tokens = self._math_aware_tokenize(summary_text)  # v18.2
            summary_overlap = len(q_tokens.intersection(summary_tokens))
            score += summary_overlap * 0.3 * weights.get("keyword", 0.5)

            # v18.2: Equation-aware retrieval boost
            if query_analysis and query_analysis.get("intent") == "equation_hunting":
                doc_math = self.doc_math_cache.get(name, {})
                total_math_blocks = sum(len(v) for v in doc_math.values())
            ...
            if query_analysis:
                ...
                for param in target_params:
                    attr_name = f"{param}_values"
                    if hasattr(meta, attr_name):
                        val = getattr(meta, attr_name, None)
                        if val and str(val) not in ["[]", "None", "", "0", "0.0"]:
                            score += 1.0 * weights.get("metadata", 0.5)

                # Target Material Matching
                target_mats = set(query_analysis.get("target_materials", []))
                doc_mats = set(self.concept_normalizer.normalize_list(meta.alloys)) if hasattr(self, 'concept_normalizer') else set(m.lower() for m in meta.alloys)
                mat_overlap = len(target_mats.intersection(doc_mats))
                score += mat_overlap * 0.8 * weights.get("metadata", 0.5)

            scores.append((name, min(score, 1.0)))

        scores.sort(key=lambda x: x[1], reverse=True)

        # v20.0 FIX #5: Removed "definition" from the bypass list.
        # Definition queries still need document filtering!
        if query_analysis and query_analysis.get("intent") in ["mechanism", "equation", "open_query", "sketch_diagram"]:
            return scores[:max(top_k, len(scores))]
...
# =============================================================================
# HYBRID SUMMARIZER v18.0+ - Fully Optimized
# =============================================================================
class HybridSummarizer:
    """
    Three-tier summarization with fast_llm separation:
    - Tier 0: Extractive (~1ms) — small text < 600 chars
    - Tier 1: Fast LLM (~50-200ms with 0.5b/1.5b model) — medium nodes
    - Tier 2: Main LLM roll-up (~1-2s with 7b/14b model) — important sections

    KEY OPTIMIZATION: Separate fast_llm for Tier 1 gives 5-10x speedup
    on ~60-70% of nodes while maintaining v18.0 quality on critical sections.
    """
    def __init__(self, main_llm, fast_llm=None, max_summary_length: int = 220,
                 use_rollup_fallback: bool = True):
        self.main_llm = main_llm
        self.fast_llm = fast_llm or main_llm
        self.max_summary_length = max_summary_length
        self.use_rollup_fallback = use_rollup_fallback

        # Tier 2: High-priority quantitative/mechanical keywords
        self.tier2_keywords = [
            # Core process parameters
            "laser power", "scan speed", "energy density", "ved", "aed", "led",
            "layer thickness", "hatch distance", "spot size", "exposure time",
            # Mechanical properties
            "yield strength", "tensile strength", "uts", "hardness", "elongation",
            "modulus", "young's modulus", "poisson's ratio", "coefficient of thermal expansion",
            # Microstructural
            "stacking fault energy", "sfe", "usfe", "ideal shear strength",
            "grain size", "cell size", "porosity", "relative density",
            "bimodal", "martensite", "austenite", "ferrite", "phase fraction",
            # Electrochemical
            "corrosion potential", "ecorr", "pitting potential", "epit",
            "repassivation potential", "erp", "breakdown potential", "ebr",
            "corrosion current density", "jcorr", "polarization resistance", "rp",
            "pren", "eis", "cpp", "tafel",
            # Multi-physics simulation
            "meltpool", "meltpool depth", "meltpool width", "marangoni",
            "lewis number", "jackson parameter", "eigenstrain", "boussinesq",
            "phase field", "molecular dynamics", "calphad", "digital twin",
            "pinn", "unet", "convlstm", "xai", "uncertainty quantification",
            # Materials
            "ti3au", "sdss 2507", "alsimgzr", "tib2", "co-cr-ni", "nt-cu",
            "metallic glass", "hea", "mpea",
            # Spray/thermal
            "sauter mean diameter", "smd", "spray penetration", "plume height",
            "film thickness", "absorption coefficient", "enthalpy", "viscosity",
            "thermal conductivity", "density", "surface tension"
        ]

        # Tier 1: Scientific/method keywords (less critical)
        self.tier1_keywords = [
            "method", "experiment", "simulation", "result", "analysis",
            "microstructure", "mechanical property", "thermal", "electrochemical",
            "alloy", "composite", "coating", "deposition", "fusion", "casting",
            "microhardness", "nanoindentation", "indentation",
            "xrd", "sem", "tem", "ebsd", "eds", "dta", "dsc",
        ...
        # Tier boosters: These ALONE trigger Tier 2 even if small
        self.tier2_boosters = [
    ...
    def _detect_tier(self, node: PageNode) -> int:
        """
        Smart tier detection combining keyword scoring, text size, and node level.
        Returns: 0 (extractive), 1 (fast LLM), 2 (main LLM roll-up)
        """
        text = (node.full_text or node.summary or "").lower()
        text_len = len(text)

        # Tier 0: Very small leaves
        if text_len < 500:
            return 0

        # Tier 2: Root and major sections always get quality
        if node.level <= 1:
            return 2
        if node.level == 2 and text_len > 1000:
            return 2

        # Check for Tier 2 boosters (critical keywords)
        for booster in self.tier2_boosters:
            if booster in text:
                return 2

        # Tier 2 scoring
        tier2_score = sum(1 for kw in self.tier2_keywords if kw in text)
        if tier2_score >= 3:
            return 2
        if tier2_score >= 2 and text_len < 4000:
            return 2
        if tier2_score >= 1 and text_len < 2000:
            return 2

        # Tier 1 scoring
        tier1_score = sum(1 for kw in self.tier1_keywords if kw in text)
        if tier1_score >= 3:
            return 1
        if tier1_score >= 1 and text_len < 3000:
            return 1
    ...
    def _tier0_extractive(self, node: PageNode, text: str):
        """Tier 0: Fast extractive summarization (~1ms)."""
        ...
        for s in sentences:
            s = s.strip()
            if 15 < len(s) < 250:
                score = sum(1 for kw in self.tier2_keywords if kw in s.lower())
                scored.append((score, s))

        # Sort by score, take top 3
        scored.sort(reverse=True)
        meaningful = [s for _, s in scored[:3]]
        if not meaningful:
            meaningful = [s.strip() for s in sentences if 15 < len(s.strip()) < 250][:3]

        node.summary = ' '.join(meaningful)[:self.max_summary_length]

    async def _tier1_fast(self, node: PageNode, text: str):
        """Tier 1: Fast LLM summary using small model (~50-200ms with 0.5b/1.5b)."""
        prompt = f"""Summarize this scientific section concisely (max 180 chars).
Focus on: materials, methods, key numerical values, and main findings.
...
Summary:"""
        try:
            summary = await asyncio.to_thread(
                self.fast_llm.generate, prompt, max_new_tokens=200, temperature=0.1
            )
            cleaned = summary.strip()
            for prefix in ["Summary:", "This section", "The text", "This text"]:
                if cleaned.lower().startswith(prefix.lower()):
                    cleaned = cleaned[len(prefix):].strip()
            node.summary = cleaned[:self.max_summary_length]
        except Exception as e:
            logger.warning(f"Tier 1 fast LLM failed for {node.node_id}: {e}")
            self._tier0_extractive(node, text)

    async def _tier2_rollup(self, node: PageNode, text: str):
        """Tier 2: High-quality roll-up with child context (~1-2s with main model)."""
        # Collect child summaries for hierarchical context
        child_summaries = []
        for child in node.children:
            if child.summary and len(child.summary) > 10:
                child_summaries.append(f"- {child.title}: {child.summary}")

        child_context = "\n".join(child_summaries[:6]) if child_summaries else "No subsections."

        prompt = f"""You are an expert scientific document analyst. Create a rich,
specific summary of this important section (max {self.max_summary_length} chars).

SECTION: "{node.title}" (Level {node.level})
...
INSTRUCTIONS:
1. Include specific quantitative values with units
2. Mention materials/alloys by name
3. Note methods and techniques
...
SUMMARY:"""
        try:
            summary = await asyncio.to_thread(
                self.main_llm.generate, prompt, max_new_tokens=280, temperature=0.12
            )
            cleaned = summary.strip()
            for prefix in ["SUMMARY:", "Summary:", "This section"]:
                if cleaned.lower().startswith(prefix.lower()):
                    cleaned = cleaned[len(prefix):].strip()
            node.summary = cleaned[:self.max_summary_length]
        except Exception as e:
            logger.warning(f"Tier 2 roll-up failed for {node.node_id}: {e}")
            # Fallback to Tier 1 with main LLM
            await self._tier1_fast(node, text)

    async def summarize_node(self, node: PageNode):
        """Main entry — dispatches to appropriate tier."""
        if node.summary and len(node.summary) > 40:
            return

        text = node.get_text(max_chars=7000)
        if not text.strip():
            node.summary = node.title or "Empty section"
            return

        tier = self._detect_tier(node)

        if tier == 0:
            self._tier0_extractive(node, text)
            logger.debug(f"Tier 0 (extractive) for {node.node_id}")
        elif tier == 1:
            await self._tier1_fast(node, text)
            logger.debug(f"Tier 1 (fast LLM) for {node.node_id}")
        else:
            await self._tier2_rollup(node, text)
            logger.debug(f"Tier 2 (roll-up) for {node.node_id}")

    async def summarize_tree(self, root: PageNode):
        """Summarize entire tree with bottom-up processing."""
        nodes = []
        def collect(n):
            nodes.append(n)
            for c in n.children:
        ...
        # Sort by level (deepest first) for better hierarchical context
        nodes.sort(key=lambda n: n.level, reverse=True)

        tier_counts = {0: 0, 1: 0, 2: 0}
        for node in nodes:
            await self.summarize_node(node)
            tier_counts[self._detect_tier(node)] += 1

        logger.info(f"Hybrid summary: T0={tier_counts[0]}, T1={tier_counts[1]}, "
                   f"T2={tier_counts[2]} (total={len(nodes)})")

        # Log speed optimization info
        if self.fast_llm is not self.main_llm:
            fast_pct = (tier_counts[0] + tier_counts[1]) / len(nodes) * 100
            logger.info(f"Speed optimization: {fast_pct:.0f}% of nodes used fast path")
...
class FastHierarchicalIndex(HierarchicalIndex):
    ...
    async def build_from_pdfs_fast(self, files: List, max_workers: int = 4) -> Dict[str, PageNode]:
        ...
        # v18.2: Build per-document math block cache from raw pages
        for doc_name, pages in raw_docs:
            self.doc_math_cache[doc_name] = {
                p['page_num']: p.get('math_blocks', [])
                for p in pages if p.get('has_equation')
            }
        return trees
    ...
    def _extract_pages_raw(self, file_obj) -> Tuple[str, List[Dict]]:
        ...
        for p in range(len(doc)):
            page = doc[p]
            text = page.get_text("text")
            # v18.2: Pre-extract math blocks for equation-aware retrieval
            math_blocks = [m.group(0).strip() for m in self.MATH_BLOCK_RE.finditer(text)]
            pages.append({
                'page_num': p+1,
                'text': text,
                ...
                'has_equation': len(math_blocks) > 0,
            })
        doc.close()
...
Return ONLY valid JSON."""
    ...
    async def _generate_summaries_async(self, trees: Dict[str, PageNode]):
        """Hybrid tiered summarization — v18.0 quality on key sections, v17.1 speed overall."""
        if not self.llm:
            return

        # Use existing hybrid_summarizer if initialized (with fast_llm), else create default
        if not hasattr(self, 'hybrid_summarizer') or self.hybrid_summarizer is None:
            self.hybrid_summarizer = HybridSummarizer(main_llm=self.llm)

        for doc_name, root in trees.items():
            logger.info(f"Hybrid summarization for {doc_name}")
            await self.hybrid_summarizer.summarize_tree(root)
            logger.info(f"Completed hybrid summarization for {doc_name}")
...
class QuantitativeKnowledgeGraph:
    ...
    # v18.1 FIX #4: Equation normalization alias map for cross-document comparison
    EQUATION_MODEL_ALIASES = {
        # Hollomon Power-Law and variants
        "hollomon power law": "hollomon",
        "hollomon powerlaw": "hollomon",
        "power law": "hollomon",
        "powerlaw": "hollomon",
        "hollomon equation": "hollomon",
        "ludwik equation": "hollomon",
        "swift equation": "hollomon",
        # Ramberg-Osgood and variants
        "ramberg osgood": "ramberg_osgood",
        "ramberg osgoood": "ramberg_osgood",
        "osgood": "ramberg_osgood",
        "ramberg": "ramberg_osgood",
        "elastoplastic": "ramberg_osgood",
        # Cahn-Hilliard and variants
        "cahn hilliard": "cahn_hilliard",
        "cahnhilliard": "cahn_hilliard",
        "phase field": "cahn_hilliard",
        "phasefield": "cahn_hilliard",
        "allen cahn": "cahn_hilliard",
        "allencahn": "cahn_hilliard",
        # Butler-Volmer and variants
        "butler volmer": "butler_volmer",
        "butlervolmer": "butler_volmer",
        "charge transfer": "butler_volmer",
        "tafel equation": "butler_volmer",
        # Navier-Stokes and variants
        "navier stokes": "navier_stokes",
        "navierstokes": "navier_stokes",
        "ns equations": "navier_stokes",
        # Fick's laws
        "fick": "fick_diffusion",
        "ficks law": "fick_diffusion",
        "fickian": "fick_diffusion",
        # Fourier's law
        "fourier": "fourier_heat",
        "fouriers law": "fourier_heat",
        # Arrhenius
        "arrhenius": "arrhenius",
        # Johnson-Cook
        "johnson cook": "johnson_cook",
    ...
    def _normalize_equation_model(self, model_name: str) -> str:
        """Normalize equation model name for cross-document comparison.

        v18.1 FIX #4: Maps variant names to canonical forms (e.g., 'Hollomon Power-Law' -> 'hollomon')
        to enable cross-document equation comparison and deduplication.
        """
        if not model_name:
            return "unknown"
        ...
        normalized = re.sub(r'[-_\s]+', ' ', normalized)  # Collapse separators to single space
        normalized = normalized.strip()

        # Check direct alias match
        if normalized in self.EQUATION_MODEL_ALIASES:
            return self.EQUATION_MODEL_ALIASES[normalized]

        # Check partial matches (e.g., "modified hollomon" -> "hollomon")
        for alias, canonical in self.EQUATION_MODEL_ALIASES.items():
    ...
    def add_extractions(self, doc_id: str, items: List[UniversalExtractionItem]):
        """Add extracted items to the knowledge graph with equation normalization.

        v18.1 FIX #4: Equation normalization (normalized_model) for cross-document comparison.
        All equation items get a normalized_model field that maps variant names 
        (e.g., 'Hollomon Power-Law', 'Power Law', 'Ludwik') to canonical forms.
        """
        graph = {
            "doc_id": doc_id,
            ...
            "by_equation_model": defaultdict(list),  # v18.1 FIX #4: Cross-doc equation comparison
            "all_items": []
        }
        ...
        for item in items:
            ...
            # v18.1 FIX #4: Normalize equation model names for cross-document comparison
            if item.item_type == "equation" and item.model_name:
                normalized_model = self._normalize_equation_model(item.model_name)
                item_dict["normalized_model"] = normalized_model
                item_dict["original_model_name"] = item.model_name  # Preserve original
                graph["by_equation_model"][normalized_model].append(item_dict)
                logger.debug(f"Equation normalized: '{item.model_name}' -> '{normalized_model}'")
...
{{
"item_type": "quantitative|qualitative|definition|comparison|relationship|process|material|method|equation|phase_field|molecular_dynamics|plasticity|thermal|mechanical|microstructural|electrochemical|multiphysics|ai_ml|digital_twin|informatics|sketch_description|figure_caption",
...
CRITICAL RULES FOR EQUATIONS & MECHANISMS:
...
CRITICAL RULES FOR VISUAL/SKETCH QUERIES:
14. If the section contains figures, diagrams, schematics, or visual elements:
    - Extract ALL figure captions and descriptive text about diagrams
    - Set item_type="sketch_description" or "figure_caption"
    - Generate a Mermaid diagram in "mermaid_diagram" field representing the process/structure
    - Generate an ASCII art sketch in "ascii_sketch" field if Mermaid is insufficient
    - Include the page number where the original figure appears in "figure_page"
15. For OPEN QUERIES (no explicit value request):
    - Prioritize conceptual explanations, mechanisms, and governing equations
    - Only include numerical values if they are CENTRAL to answering the query
    - Do NOT build tables unless explicitly requested
    - Extract a balanced mix of qualitative and quantitative information
16. For temperature-dependent constitutive parameters (Hollomon, Ramberg-Osgood, etc.):
    - If the parameter is given as a polynomial/function of T, extract the FULL EXPRESSION
      into "equation_latex" field AND set item_type="equation"
...
Return [] if no relevant information found."""
        ...
        for chunk in chunks:
            ...
            try:
                ...
                if json_str:
                    ...
                    # Normalize data to a list of items (handles lists, dicts with "items" key, or single objects)
                    if isinstance(data, list):
                        ...
                        if "items" in data and isinstance(data["items"], list):
                            ...
                            items_data = [data]  # It's a single item object
                        else:
                            items_data = []
                    else:
                        items_data = []
                    ...
                    for item_data in items_data:
                        # --- BULLETPROOF: Ensure all required Pydantic fields exist and are correct types ---
                        ...
                        if "page" not in item_data:
                            ...
                            # Coerce page to int (handles "page 4", "4", 4, etc.)
                            try:
                                item_data["page"] = int(str(item_data["page"]).replace("page", "").replace("p", "").strip())
                            except:
                                item_data["page"] = page

                        # Coerce confidence to float (handles "high", "0.8", 0.8, etc.)
                        ...
                        try:
                            ...
                            logger.debug(f"Item parse error even after bulletproofing: {e} | Data: {item_data}")
            except Exception as e:
    ...
    def _extract_json(self, text: str) -> Optional[str]:
        """Bulletproof JSON extraction for local LLMs. Handles arrays, objects, and markdown wrappers."""
        ...
        # 2. Try to find any JSON object or array in the raw text
        for pattern in [r'\{.*\}', r'\[.*\]']:
            match = re.search(pattern, text, re.DOTALL)
            if match:
                json_str = match.group(0)
                # Fix common local LLM JSON errors (trailing commas)
...
class HierarchicalTreeRetriever:
    # v18.2: Detect math/equation content in retrieved chunks
    ...
    async def retrieve_quantitative(self, query: str, annotated_trees: List[Dict]) -> List[Dict]:
        ...
        for sel in sorted(all_selections, key=lambda x: x.get('confidence', 0), reverse=True):
            ...
            if node:
                ...
                # v18.2: Expand context window for equation-containing chunks
...
Include up to {self.max_results} selections."""
    ...
    def _find_parent_node(self, trees: List[Dict], doc_id: str, child_node_id: str) -> Optional[Dict]:
        """v18.2: Find parent of a node for sibling context expansion."""
        for tree in trees:
            if tree.get("doc_id") == doc_id or tree.get("doc_name") == doc_id:
    ...
    def build_cross_document_meta_tree(self, query: str, trees: List[Dict]) -> Dict:
        ...
        # 2. Build Meta-Root
        # v20.0 FIX #4: Use "nodes" instead of "children" to match IterativeTreeNavigator expectations
        meta_root = {
            "node_id": "meta_root",
            "title": f"Cross-Document Meta-Tree: {target_section}",
            "doc_id": "META",
            "nodes": []  # <--- CHANGED FROM "children"
        }
        ...
        for tree in trees:
            ...
            if matching_node:
                meta_root["nodes"].append({  # <--- CHANGED FROM "children"
    ...
    def _find_section_by_keyword(self, tree: Dict, keyword: str) -> Optional[Dict]:
        """Recursively finds a node whose title loosely matches the keyword.

        Uses fuzzy matching with synonym expansion to handle cases where the LLM
        returns "Experimental Setup" but actual PDF headings are "2. Experimental"
        or "Materials and Methods".
        """
        # Extract core root words from the LLM's suggestion
        core_keywords = [k for k in keyword.lower().split() if len(k) > 3]
        if not core_keywords:
            core_keywords = [keyword.lower()]

        # Synonym map for scientific paper sections
        synonym_map = {
        ...
        # Expand core keywords with synonyms
        ...
        def search(node):
            ...
            # Match if ANY core keyword or synonym is in the title
...
def run_streamlit():
    ...
    if st.session_state.query_processor.get("files") and not st.session_state.annotated_trees:
        with st.spinner("Building hierarchical index with metadata extraction..."):
            ...
            # === SPEED OPTIMIZATION: Auto-detect and use fast LLM for Tier 1 ===
            model_choice = st.session_state.llm_model_choice.lower()
            fast_llm = None
            if "7b" in model_choice or "14b" in model_choice or "8b" in model_choice or "9b" in model_choice or "10b" in model_choice:
                try:
                    # Try to load a smaller model for Tier 1 summarization
                    fast_model_key = "ollama:qwen2.5:1.5b"  # Reliable fast model
                    if "0.5b" in model_choice:
                        fast_model_key = "ollama:qwen2.5:0.5b"
                    fast_llm = HybridLLM(model_key=fast_model_key, use_4bit=True)
                    logger.info(f"✅ Speed boost: Using {fast_model_key} for Tier 1 summarization")
                except Exception as e:
                    logger.warning(f"Could not load fast LLM: {e}. Using main model for all tiers.")
                    fast_llm = None

            # Initialize HybridSummarizer with fast_llm if available
            ...
            for doc_name, tree in trees.items():
                ...
                # FIXED: Proper metadata handling for ALL documents
                # v18.2: Pass pre-indexed math blocks to retriever for equation-aware scoring
                page_math_cache = idx.doc_math_cache.get(doc_name) if hasattr(idx, 'doc_math_cache') else None
                if tree.metadata:
                    kg.add_document_metadata(doc_name, tree.metadata)
                    two_stage.index_document(doc_name, tree.metadata, tree.summary, page_math_cache=page_math_cache)
                else:
                    # Fallback: create metadata from extracted items for this document
                    ...
                    two_stage.index_document(doc_name, meta, tree.summary, page_math_cache=page_math_cache)
    ...
    if st.session_state.annotated_trees:
        ...
        if run_query:
            with st.chat_message("assistant"):
                ...
                # === v18.0 UPGRADE: Dynamic Vectorless Intent Analysis ===
                router = ScientificIntentRouter()
                query_analysis = router.route(active_prompt)

                # 🔍 DEBUG: Verify routing is working (v19.0 diagnostic)
                ...
                st.caption(
                    ...
                    f"**Format:** `{query_analysis['output_format']}` | "
                    f"**Values Gated:** {'✅ Explicit' if query_analysis['value_extraction_gated'] else '❌ Implicit'}"
                )
                progress.progress(0.1)

                # === v18.0 UPGRADE: Vectorless Dynamic Retrieval ===
                if st.session_state.get("two_stage", True) and st.session_state.two_stage_retriever is not None:
                    progress.text("Stage 1: Document filtering (vectorless dynamic)...")
                    relevant_docs = st.session_state.two_stage_retriever.retrieve_relevant_docs(
                        active_prompt, query_analysis=query_analysis, top_k=8
                    )
                ...
                # v20.0 FIX #4b: Check "nodes" instead of "children" to match meta_tree structure
                all_trees = [meta_tree] + filtered_trees if meta_tree.get("nodes") else filtered_trees
                ...
                # === v18.0 UPGRADE: Citation Validation (Hallucination Prevention) ===
                # Build a temporary reader for verification
                reader = PaginationAwareReader()
                validator = CitationValidator(reader)
                items = validator.verify_items(items)
                progress.progress(0.8)

                # === v18.0 UPGRADE: PageIndex-Style Synthesis ===
                # v19.0: Using AdaptiveResponseGenerator for intent-aware formatting
                extracted_values = []
                equation_items = [i for i in items if i.item_type == "equation"]
                for item in items:
                ...
                # v19.0: Adaptive Response Generation based on intent
                ...
                # Cache result with query analysis for visualization persistence
                st.session_state.cached_query_result = {
                    ...
                    "multiphysics_flags": list(PhysicalQuantityClassifier().CANONICAL.keys()) if any(item.item_type in ["phase_field", "molecular_dynamics", "plasticity", "thermal", "mechanical", "microstructural", "electrochemical", "multiphysics", "ai_ml", "digital_twin", "informatics"] for item in items) else [],
                    "electrochemical_flags": ["eis", "cpp", "tafel"] if any(item.item_type == "electrochemical" for item in items) else [],
                    "ai_ml_flags": ["uq", "xai", "digital_twin"] if any(item.item_type in ["ai_ml", "digital_twin"] for item in items) else [],
        ...
        if display_mode == "Table" and extracted_values:
            ...
            generator = AdaptiveResponseGenerator(get_cached_llm(st.session_state.llm_model_choice, st.session_state.get("use_4bit", True)))
            ...
            conclusion = generator.generate_human_conclusion(active_prompt, report)
        ...
        if st.session_state.knowledge_graph and st.session_state.annotated_trees:
            ...
            if not df_all.empty:
                ...
                with st.expander("Publication Marker Reference", expanded=False):
                    ...
                    _global_reg = DocumentMarkerRegistry()
                    _global_reg.register_documents(all_doc_ids)
                    render_streamlit_marker_legend(all_doc_ids, aliases, _global_reg, "All Publications")
