...
class AdaptiveResponseGenerator:
    ...
    FORMATTERS = {
        ...
        "contrastive_table": "_format_as_comparison",
    ...
    def _format_as_constitutive_hybrid(self, query, items, qa):
        """Hybrid formatter for constitutive models: finds BOTH equations AND parameter values.

        v19.0 FIX #3: Bridges the gap between equation-only and value-only formatters.
        Handles temperature-dependent polynomial expressions (e.g., σ₀(T) = aT² + bT + c)
        by extracting them as equations while also creating parameter tables.
        """
...
")
        ...
        if merged_params:
            ...
            lines.append("| Parameter | Value/Expression | Unit | Material | Document | Page | Confidence |")
            lines.append("|-----------|-----------------|------|----------|----------|------|------------|")
            for item in sorted(merged_params, key=lambda x: x.confidence, reverse=True)[:30]:
                pq = self.phys_classifier.get_human_readable(item.physical_quantity or "unknown")
                # Handle both numeric values and text expressions
                if item.value is not None:
                    val = f"{item.value:.4g}"
                else:
                    val = item.content[:80] if item.content else "N/A"
                unit = item.unit or ""
                mat = item.material or ""
                doc = item.doc_source
                page = item.page
                conf = f"{item.confidence:.2f}"
                lines.append(f"| {pq} | {val} | {unit} | {mat} | {doc} | {page} | {conf} |")
...
Include citations in @@CITE:doc=X;page=Y@@ format."""
    ...
    def _format_as_prose(self, query, items, qa):
        """Default open-query format: balanced mix of concepts, equations, and selective data."""
        if not items:
        ...
        prompt = f"""You are an expert scientific analyst. Answer this query comprehensively.
...
RULES:
- Write in natural prose paragraphs with clear scientific reasoning
- Include equations inline using $$ ... $$ where relevant
- Only mention specific numerical values if they directly support the explanation
- Use @@CITE:doc=X;page=Y@@ for EVERY claim
- Do NOT create tables unless the evidence naturally warrants one
- If a sketch/diagram was extracted, describe it verbally
- If the query asks about mechanisms, explain causal chains step by step
- If the query asks about definitions, provide clear conceptual explanations
- Be thorough but concise - aim for 3-5 well-structured paragraphs
- If information is missing or uncertain, explicitly state the limitation"""

        return self.llm.generate(prompt, max_new_tokens=2048, temperature=0.15)

    def _format_as_table(self, query, items, qa):
        """Only called when value_extraction is EXPLICITLY triggered."""
        ...
        lines = [f"## Extracted Values: {query}\n"]
        lines.append("| Parameter | Value | Unit | Material | Document | Page | Confidence |")
        lines.append("|-----------|-------|------|----------|----------|------|------------|")
    ...
    def _format_as_mechanism(self, query, items, qa):
        ...
        prompt = f"""Explain the physical mechanism for: {query}

EVIDENCE FROM DOCUMENTS:
{chr(10).join(evidence)}

FORMAT REQUIREMENTS:
1. Provide a causal chain explanation with numbered steps
2. Each step should explain 'why' something happens
3. Include governing equations where relevant (in $$...$$ format)
4. Connect steps with clear logical transitions
5. Use @@CITE:doc=X;page=Y@@ for EACH step
6. Conclude with a summary of the overall mechanism
7. Note any uncertainties or alternative explanations"""

        return self.llm.generate(prompt, max_new_tokens=2048, temperature=0.1)

    def _format_as_comparison(self, query, items, qa):
        """Contrastive table across documents/materials."""
        if not items:
            return f"No comparison data found for '{query}'."

        # Group by material and physical quantity
        by_mat_pq = {}
        for item in items:
            mat = item.material or "Unspecified"
            pq = item.physical_quantity or "general"
            key = (mat, pq)
            if key not in by_mat_pq:
                by_mat_pq[key] = []
            by_mat_pq[key].append(item)

        lines = [f"## Comparative Analysis: {query}\n"]

        # Build comparison table
        materials = sorted(set(k[0] for k in by_mat_pq.keys()))
        quantities = sorted(set(k[1] for k in by_mat_pq.keys()))

        if len(materials) > 1:
            lines.append("### Cross-Material Comparison")
            header = "| Quantity | " + " | ".join(materials) + " |"
            lines.append(header)
            lines.append("|" + "|".join(["-"*10 for _ in range(len(materials)+1)]) + "|")

            for pq in quantities:
                pq_readable = self.phys_classifier.get_human_readable(pq)
                row = [pq_readable]
                for mat in materials:
                    vals = by_mat_pq.get((mat, pq), [])
                    if vals:
                        mean_val = sum(v.value for v in vals if v.value is not None) / max(1, len([v for v in vals if v.value is not None]))
                        unit = vals[0].unit or ""
                        row.append(f"{mean_val:.3g} {unit} (n={len(vals)})")
                    else:
                        row.append("—")
        ...
        for doc, doc_items in by_doc.items():
            lines.append(f"\n**{doc}**:")
            for item in sorted(doc_items, key=lambda x: x.confidence, reverse=True)[:5]:
                cite = f"@@CITE:doc={item.doc_source};page={item.page}@@"
                if item.value is not None:
                    pq = self.phys_classifier.get_human_readable(item.physical_quantity or "unknown")
                    lines.append(f"- {pq}: {item.value} {item.unit or ''} {cite}")
                else:
                    lines.append(f"- {item.content[:150]} {cite}")
    ...
    def _format_as_boolean(self, query, items, qa):
        ...
        prompt = f"""Answer this yes/no verification question based on the evidence.
...
FORMAT:
**Answer:** [Yes / No / Partially / Uncertain]
**Confidence:** [High / Medium / Low]
**Explanation:** [2-3 sentences explaining the reasoning]
**Key Evidence:**
- [Most relevant piece of evidence with citation]
- [Second piece if applicable]

Use @@CITE:doc=X;page=Y@@ for all claims."""

        return self.llm.generate(prompt, max_new_tokens=1024, temperature=0.1)
...
class TwoStageRetriever:
    ...
    def retrieve_relevant_docs(self, query: str, query_analysis: Optional[Dict[str, Any]] = None, top_k: int = 10) -> List[Tuple[str, float]]:
        ...
        # If intent is mechanism/equation, bypass strict top_k and pass more docs to LLM Tree Retriever
        if query_analysis and query_analysis.get("intent") in ["mechanism", "equation", "open_query", "definition", "sketch_diagram"]:
