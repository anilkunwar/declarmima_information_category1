...
def _inject_doc_id_recursive(node, doc_id: str):
    """Recursively ensure every node in the tree carries the parent doc_id. 
    FIXED to handle malformed LLM JSON (strings/lists instead of dicts)."""
    if not isinstance(node, dict):
        return  # Skip if the LLM returned a string or list instead of a dict

    if "doc_id" not in node:
        node["doc_id"] = doc_id

    children = node.get("nodes", [])
    if isinstance(children, list):  # Ensure 'nodes' is actually a list
        for child in children:
...
def agentic_retrieve_and_answer(query: str, selected_docs: Dict, llm: HybridLLM, 
    ...
    FIXED: Added Two-Stage Filtering to prevent Context Window Explosion with 20+ PDFs.
    """
    # ==================================================================
    # STAGE 1: DOCUMENT FILTERING (Prevents Context Window Explosion)
    # ==================================================================
    doc_summaries = []
    for doc_name, data in selected_docs.items():
        root_summary = data.get('tree', {}).get('summary', '')
        if not root_summary and data['pages']:
            root_summary = data['pages'][0]['text'][:200]
        doc_summaries.append(f"- {doc_name}: {root_summary}")

    doc_list_text = "\n".join(doc_summaries)

    filter_prompt = f"""You are a research assistant. Given a query and a list of documents, identify which documents are most likely to contain the answer.
    Query: {query}
    Documents:
    {doc_list_text}

    Return a JSON list of the relevant document names: ["doc_name1", "doc_name2"].
    If none are relevant, return []."""

    filter_resp = llm.generate(filter_prompt, max_new_tokens=256, system_prompt="Return ONLY a valid JSON list of strings.")
    relevant_doc_names = extract_json(filter_resp)

    if not relevant_doc_names or not isinstance(relevant_doc_names, list):
        relevant_doc_names = list(selected_docs.keys()) # Fallback to all if LLM fails

    filtered_docs = {k: v for k, v in selected_docs.items() if k in relevant_doc_names}
    if not filtered_docs:
        filtered_docs = selected_docs # Fallback if filtering removed everything
    # ==================================================================

    # 2. Build the "Forest" Description ONLY for filtered docs
    forest_desc = []
    for doc_name, data in filtered_docs.items():
    ...
    # Safeguard: truncate if still too long
    if len(tree_text) > 25000:
        tree_text = tree_text[:25000] + "\n...[TRUNCATED]"

    # 3. Agentic Navigation - LLM decides which sections are relevant
    ...
    if search_result and isinstance(search_result, dict):
        ...
        rel_ids = search_result.get('relevant_ids', [])
        if isinstance(rel_ids, list): # Prevent crash if LLM returns a string
            relevant_ids = rel_ids

    # 4. Fetch Content (Lossless Retrieval)
    context = ""
    retrieved_info = []
    ...
    for ns_id in relevant_ids:
        ...
        # Use filtered_docs for navigation, but fallback to selected_docs if needed
        data = filtered_docs.get(doc_name) or selected_docs.get(doc_name)
        if data:
    ...
    # 5. Generate Answer using the same LLM
...
def build_cross_document_meta_tree(query: str, doc_trees: List[Dict], llm: HybridLLM) -> Dict:
    ...
    for tree in doc_trees:
        ...
        if matching_node:
            ...
            # ======================================================================
            # CRITICAL FIX: If keyword matching fails, DO NOT leave it empty!
            # Just inject the document's top-level children so the navigator
            # still has nodes to search through.
            # ======================================================================
            logger.warning(f"Keyword '{target_section}' not found in {doc_id}. Injecting top-level nodes.")
            for child in tree.get('nodes', []):
                meta_root["nodes"].append(child)
...
class JSONMCTSNavigator:
    ...
    def navigate(self, query: str, meta_tree: Dict) -> List[Dict]:
        ...
        for step in range(self.max_steps):
            ...
            # ======================================================================
            # CRITICAL FIX: On the final step, FORCE extraction.
            # This prevents the LLM from just drilling down forever and returning 0 chunks.
            # ======================================================================
            if step == self.max_steps - 1:
                self.trace.append("Final step reached. Forcing text extraction on all current nodes.")
                for node in current_nodes:
                    chunk = self._get_full_text_from_tree(meta_tree, node['node_id'])
                    if chunk: 
            ...
            # CRITICAL FIX: Limit to top 50 nodes to prevent context window explosion
            nodes_to_show = current_nodes[:50] 
