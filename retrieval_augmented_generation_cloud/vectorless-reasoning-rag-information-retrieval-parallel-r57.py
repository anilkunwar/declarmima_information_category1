...
# -*- coding: utf-8 -*-
"""
DECLARMIMA v12.1-VECTORLESS - COMPLETE INTEGRATED STREAMLIT APPLICATION
=======================================================================
...
- UniversalLLMExtractor: batch LLM extraction with anti-hallucination validation + REGEX REPAIR FALLBACK
...
- [PATCH 12.1] Enhanced few-shot extraction prompts, parameter aliasing, dynamic confidence thresholding, regex validation fallback

AUTHOR: DECLARMIMA Team
LICENSE: MIT
VERSION: 12.1-VECTORLESS-PATCHED
DATE: 2026-05-07
"""
...
class QuantitativeKnowledgeGraph:
    def __init__(self):
        ...
        # [FIX 3] Parameter aliasing for robust cross-document matching
        self.PARAM_ALIASES = {
            "laser power": ["laser_power", "P", "beam power", "laser output", "laser_power_value"],
            "irradiance": ["power density", "energy density", "fluence", "laser_intensity"],
            "scan speed": ["scanning speed", "v", "travel_speed", "scan_velocity"],
            "energy density": ["volumetric energy density", "VED", "specific energy", "heat input"]
        }
    ...
    def add_extractions(self, doc_id: str, items: List[UniversalExtractionItem]):
        ...
        for item in items:
            ...
            # [FIX 3] Normalize parameter name before indexing
            if item.parameter_name:
                normalized = self._normalize_param_name(item.parameter_name)
                graph["parameters"][normalized].append(item_dict)
            if item.material:
    ...
    def get_parameter_across_docs(self, param_name: str) -> List[Dict]:
        results = []
        param_key = self._normalize_param_name(param_name)
...
class UniversalLLMExtractor:
    # [FIX 1] Enhanced prompt with few-shot examples, explicit unit/value rules, and anti-truncation directives
...
Text: "a laser power of 1000 W was used with scan speed 200 mm/s"
→ {{"item_type": "quantitative", "parameter_name": "laser power", "value": 1000, "unit": "W", "confidence": 0.95, "context": "a laser power of 1000 W was used with scan speed 200 mm/s"}}

Text: "irradiance values of 134.6 kW/cm² and 150 kW/cm² were tested"
→ [{{"parameter_name": "irradiance", "value": 134.6, "unit": "kW/cm²", "confidence": 0.95, "context": "irradiance values of 134.6 kW/cm² and 150 kW/cm² were tested"}}, {{"value": 150, "confidence": 0.95, "context": "irradiance values of 134.6 kW/cm² and 150 kW/cm² were tested"}}]

Text: "laser power P = 250 W (spot size 70 µm)"
→ {{"parameter_name": "laser power", "value": 250, "unit": "W", "context": "laser power P = 250 W (spot size 70 µm)"}}
...
7. NEVER truncate numbers: output full value "1000" not "1"
8. ALWAYS capture unit even if separated: "1000" + "W" → value=1000, unit="W"
9. If multiple values exist in one sentence, return a JSON array with separate objects"""

    def __init__(self, llm: HybridLLM):
        self.llm = llm

    # [FIX 2] Regex validation fallback for LLM parsing errors
    def _repair_extraction(self, item_data: Dict) -> Dict:
        if item_data.get("value") in [1, "1", ""] and item_data.get("unit") in [None, ""]:
            match = re.search(r'(\d+\.?\d*)\s*(W|kW|mW|kW/cm²|J/mm²|A/cm²|mm/s|m/s)', item_data.get("context", ""), re.I)
            if match:
                item_data["value"] = float(match.group(1))
                item_data["unit"] = match.group(2)
                item_data["confidence"] = max(item_data.get("confidence", 0.5), 0.85)
                logger.debug(f"Regex repaired item: value={item_data['value']}, unit={item_data['unit']}")
        
        # Normalize common unit typos
        unit_map = {"kw": "kW", "mw": "mW", "w": "W", "kw/cm²": "kW/cm²", "mm/s": "mm/s"}
        if item_data.get("unit"):
            item_data["unit"] = unit_map.get(item_data["unit"].lower().strip(), item_data["unit"])
    ...
    def extract_from_chunks(self, chunks: List[Dict], query: str, query_analysis: Optional[Dict] = None) -> List[UniversalExtractionItem]:
        ...
        for chunk in chunks:
            ...
            try:
                ...
                logger.debug(f"Raw LLM response for {doc}:{page}: {response[:300]}...")
                ...
                if json_str:
                    ...
                    for item_data in raw_list:
                        if not isinstance(item_data, dict):
                            continue
                        item_data = self._repair_extraction(item_data) # Apply fallback
                        try:
        ...
        # [FIX 4] Dynamic threshold will be applied later in UI, but keep base filter here
        min_conf = 0.55
...
def run_streamlit():
    ...
    st.markdown("# 🔬 DECLARMIMA v12.1-VECTORLESS (PATCHED)")
    st.caption("Hierarchical tree navigation with LLM-powered quantitative reasoning & robust extraction")
    ...
    if st.session_state.annotated_trees:
        ...
        if prompt:
            ...
            with st.chat_message("assistant"):
                ...
                extractor = UniversalLLMExtractor(llm)
                items = []
                # [FIX 4] Dynamic confidence threshold based on retrieval confidence
                min_conf_base = st.session_state.get("min_confidence", 0.55)
                for r in retrieved:
                    # If tree retrieval was highly confident (>0.9), relax extraction threshold
                    retrieval_conf = r.get("confidence", 0.0)
                    dynamic_threshold = min_conf_base if retrieval_conf < 0.85 else 0.45
                    
                    r_chunk = {"full_text": r["full_text"], "page_start": r["page_start"], "doc_id": r["doc_id"]}
                    chunk_items = extractor.extract_from_chunks([r_chunk], prompt)
                    
                    # Apply dynamic filter
                    filtered = [i for i in chunk_items if i.confidence >= dynamic_threshold]
                    if len(filtered) < len(chunk_items):
                        logger.debug(f"Filtered {len(chunk_items)-len(filtered)} items below dynamic threshold {dynamic_threshold} for {r['doc_id']}")
                            ...
                            if stats.get("count", 0) > 0:
                                with st.expander(f"📊 {param.title()} Statistics", expanded=False):
                                    ...
                                    if stats.get("values"):
                                        ...
                                        st.info("Values present but missing numeric fields for aggregation.")
