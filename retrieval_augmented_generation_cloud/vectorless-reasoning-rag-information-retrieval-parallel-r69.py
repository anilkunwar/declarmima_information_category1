...
# -*- coding: utf-8 -*-
"""
DECLARMIMA v13.2 - ENHANCED KNOWLEDGE GRAPH INTERACTION
=======================================================================
...
- Publication-quality visualization engine (50+ colormaps, UMAP, t-SNE, PCA, chord, sunbursts)
- NEW: Interactive knowledge graph explorer (entity consensus, contradictions, PyVis network)
- NEW: Dynamic concept selector with domain/top‑N/salience filtering
- NEW: Quantitative data explorer (histograms, radar, knowledge graph) without a query
"""
...
# ============================================================================
# PHYSICAL QUANTITY CLASSIFIER (unchanged)
# ============================================================================
...
# ============================================================================
# STRUCTURED METADATA EXTRACTOR (unchanged)
# ============================================================================
...
# ============================================================================
# HIERARCHICAL PDF INDEX (unchanged, but abbreviated for length - full version in original)
# ============================================================================
@dataclass
class PageNode:
    id: str
...
# ============================================================================
# QUANTITATIVE KNOWLEDGE GRAPH (enhanced for interactive exploration)
# ============================================================================
class QuantitativeKnowledgeGraph:
    ...
    def build_extracted_values(self, query: str) -> List[ExtractedValue]:
        ...
        return all_values
    # New methods for interactive knowledge graph exploration
    def get_all_entity_names(self) -> List[str]:
        """Return all unique entity names (materials + physical quantities + methods)."""
        entities = set()
        for doc_id, graph in self.doc_graphs.items():
            for item in graph["all_items"]:
                if item.get("material"):
    ...
    def get_entity_consensus(self, entity_name: str) -> Dict[str, Any]:
        """Find consensus (mean, std, range) across documents for a given entity."""
        values = []
        units = set()
        docs = set()
        for doc_id, graph in self.doc_graphs.items():
            for item in graph["all_items"]:
                if (item.get("material") == entity_name or 
                    item.get("physical_quantity") == entity_name or
                    item.get("method") == entity_name or
                    item.get("parameter_name") == entity_name):
    ...
    def get_entity_contradictions(self, entity_name: str, threshold_factor: float = 2.0) -> List[Dict[str, Any]]:
        """Find contradictory values between different documents."""
        by_doc = defaultdict(list)
        for doc_id, graph in self.doc_graphs.items():
            for item in graph["all_items"]:
                if (item.get("material") == entity_name or 
                    item.get("physical_quantity") == entity_name):
                    if item.get("value") is not None:
                    ...
                    if mean_i > 0 and mean_j > 0:
                        ...
                        if ratio > threshold_factor:
                            contradictions.append({
                                "entity": entity_name,
                                "doc_a": docs[i], "value_a": mean_i,
                                "doc_b": docs[j], "value_b": mean_j,
                                "ratio": ratio,
...
# ============================================================================
# UNIVERSAL LLM EXTRACTOR (unchanged)
# ============================================================================
...
# ============================================================================
# LLM REASONING SYNTHESIZER (unchanged)
# ============================================================================
...
# ============================================================================
# HIERARCHICAL TREE RETRIEVER (abbreviated for length)
# ============================================================================
...
# ============================================================================
# PUBLICATION-QUALITY VISUALIZATION ENGINE (full with Plotly colorscale fix)
# ============================================================================
class PublicationVisualizationEngine:
    ...
    def _get_plotly_colorscale(self, name: Optional[str] = None) -> str:
        ...
        plotly_builtins = ['aggrnyl', 'agsunset', 'algae', 'amp', 'armyrose', 'balance',
    ...
    def plot_quantitative_histogram(self, df: pd.DataFrame, quantity_name: str, group_by: str = "material", colormap: Optional[str] = None) -> go.Figure:
        ...
        for i, grp in enumerate(groups):
            ...
            fig.add_trace(go.Bar(name=str(grp), x=[grp], y=[data.mean()], error_y=dict(type='data', array=[data.std()] if len(data)>1 else [0], visible=True), marker_color=color, text=[f"n={len(data)}<br>μ={data.mean():.2f}<br>σ={data.std():.2f}"], textposition="outside"))
    ...
    def plot_material_sunburst(self, colormap: Optional[str] = None) -> go.Figure:
        ...
        fig = px.sunburst(agg, path=["material", "physical_quantity"], values="count", title="Material & Quantity Hierarchy", color="count", color_continuous_scale=self._get_plotly_colorscale(colormap))
        fig.update_layout(font=dict(family=self.font_family, size=self.font_size))
        return fig
    ...
    def plot_retrieval_sankey(self, query: str, relevant_docs: List[Tuple[str, float]],
        ...
        fig = go.Figure(data=[go.Sankey(
            node=dict(
                ...
                label=labels,
                color=["#1f77b4"] + ["#ff7f0e"] * len(doc_nodes) + ["#2ca02c"] * len(node_labels) + ["#d62728"] * len(pq_nodes) + ["#9467bd"]
            ),
            link=dict(source=sources, target=targets, value=values)
        )])
        fig.update_layout(
            title_text=f"Retrieval Provenance Flow: '{query[:40]}...'",
    ...
    def plot_node_confidence_distribution(self, retrieved_nodes: List[Dict]) -> go.Figure:
        ...
        fig.add_trace(go.Histogram(
            x=confidences, nbinsx=20, marker_color="#3b82f6", opacity=0.75, name="All Nodes"
        ))
    ...
    def plot_doc_filter_scores(self, relevant_docs: List[Tuple[str, float]],
        ...
        fig.update_layout(
            ...
            xaxis_title="Document", yaxis_title="Relevance Score",
    ...
    def plot_semantic_vs_vectorless(self, query: str,
        ...
        fig.add_trace(go.Scatter(
            x=keyword_scores, y=semantic_scores, mode='markers+text',
            text=doc_labels, textposition="top center",
            marker=dict(size=14, color="#3b82f6"), name="Documents"
        ))
        min_val = min(min(keyword_scores), min(semantic_scores))
        max_val = max(max(keyword_scores), max(semantic_scores))
        fig.add_trace(go.Scatter(
            x=[min_val, max_val], y=[min_val, max_val],
            mode='lines', line=dict(dash='dash', color='#ef4444'), name="Agreement Line"
        ))
        fig.update_layout(
            title="Semantic (Embedding) vs Vectorless (Keyword/Heuristic) Retrieval Scores",
            xaxis_title="Vectorless Score (Keyword/Heuristic)",
            yaxis_title="Semantic Score (Cosine Similarity)",
...
def run_streamlit():
    st.set_page_config(page_title="DECLARMIMA v13.2 - Knowledge Graph Explorer", layout="wide")
    st.markdown("# 🔬 DECLARMIMA v13.2 - Material Extraction + Interactive Knowledge Graph")
    st.caption("Extract alloys, mechanical properties, laser parameters; explore consensus/contradictions; visualise networks.")
    ...
    if st.session_state.annotated_trees:
        ...
        if st.session_state.knowledge_graph and st.session_state.annotated_trees:
            ...
            if not df_all.empty:
                ...
                with tabs[7]:  # 🧠 Interactive Knowledge Graph Explorer
                    st.markdown("### 🔍 Interactive Knowledge Graph Explorer")
                    st.markdown("Select an entity (material, parameter, method) to see cross‑document consensus, contradictions, and all extracted values.")
                    ...
                    if entities:
                        ...
                        if selected_entity:
                            ...
                            for doc_id, graph in st.session_state.knowledge_graph.doc_graphs.items():
                                for item in graph["all_items"]:
                                    if (item.get("material") == selected_entity or 
                                        item.get("physical_quantity") == selected_entity or
                                        item.get("method") == selected_entity or
                                        item.get("parameter_name") == selected_entity):
                                        items_for_entity.append(item)
                            if items_for_entity:
                                df_entity = pd.DataFrame([{
                                    "Doc": i["doc_source"], "Page": i.get("page",0), "Type": i.get("item_type",""),
                                    "Content": i.get("content","")[:150], "Value": i.get("value",""), "Unit": i.get("unit",""),
                    ...
                    if PYVIS_AVAILABLE and st.checkbox("Show interactive PyVis network (experimental)", key="show_pyvis"):
                        ...
                        G = nx.Graph()
                        for doc_id, graph in st.session_state.knowledge_graph.doc_graphs.items():
                            G.add_node(Path(doc_id).stem, node_type="doc", group="doc")
                            for item in graph["all_items"]:
                                if item.get("material"):
                                    G.add_node(item["material"], node_type="material", group="material")
                                    G.add_edge(Path(doc_id).stem, item["material"])
                                if item.get("physical_quantity"):
                                    G.add_node(item["physical_quantity"], node_type="param", group="param")
                                    G.add_edge(Path(doc_id).stem, item["physical_quantity"])
                        if len(G.nodes()) > 0:
                            net = Network(height="700px", width="100%", bgcolor="#ffffff", font_color="#000000")
                            net.barnes_hut(gravity=-1800, spring_length=140)
                            for node, data in G.nodes(data=True):
                                group = data.get("group", "unknown")
                                color = {"doc": "#1e40af", "material": "#10b981", "param": "#f59e0b"}.get(group, "#6b7280")
                                net.add_node(node, label=node[:25], color=color, title=f"Type: {group}")
                            for u, v in G.edges():
                                net.add_edge(u, v)
                ...
                with tabs[8]:  # 🔍 Retrieval Diagnostics (moved inside the if block)
                    ...
                    if tree_doc_options:
                        ...
                        if fig_tree:
                            st.pyplot(fig_tree)
                            buf = BytesIO()
                            fig_tree.savefig(buf, format="png", dpi=300)
                    ...
                    if st.session_state.embedding_model is not None:
                        st.markdown("#### ⚖️ Semantic vs Vectorless Score Comparison")
                        emb_fn = lambda x: np.array(st.session_state.embedding_model.encode(x))
                    ...
                    if retrieved_nodes:
                        df_ret = pd.DataFrame([{
                            "Document": r.get("doc_id", ""),
                            "Node ID": r.get("node_id", ""),
                            "Section": r.get("section_title", ""),
                            "Page": r.get("page_start", 0),
                            "Confidence": r.get("confidence", 0),
                            "Reasoning": r.get("selection_reasoning", "")[:100]
                        } for r in retrieved_nodes])
