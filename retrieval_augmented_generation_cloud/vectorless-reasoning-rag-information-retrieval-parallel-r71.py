...
# -*- coding: utf-8 -*-
"""
DECLARMIMA v17.0 - UNIFIED ROBUST VECTORLESS RAG WITH FULL VISUALIZATION
========================================================================
...
class PublicationVisualizationEngine:
    ...
    def plot_static_knowledge_network(self, filtered_concepts: Optional[List[str]] = None, top_n: int = 30, figsize: Tuple[int,int] = (14, 12), layout: str = "spring", colormap: Optional[str] = None, node_size_factor: float = 1.0, edge_alpha: float = 0.25, show_labels: bool = True) -> plt.Figure:
        G = nx.Graph()
        ...
        ax.set_title("Salience-Aware Cross-Document Knowledge Network\n(Node size = importance)", fontsize=self.title_font_size, fontweight='bold', fontfamily=self.font_family)
        ax.axis("off")
        plt.tight_layout()
        return fig

    def render_pyvis_salience(self, filtered_concepts: Optional[List[str]] = None, top_n_nodes: int = 30, physics_enabled: bool = True, colormap: Optional[str] = None) -> str:
        ...
        for i, node in enumerate(G.nodes()):
            ...
            net.add_node(node, label=node[:25], size=size, color=color, borderWidth=2, title=f"{node}\nSalience: {salience}")
...
def run_streamlit():
    st.set_page_config(page_title="DECLARMIMA v17.0 - Unified Robust RAG", layout="wide")
    st.markdown("# DECLARMIMA v17.0 - Unified Robust Vectorless RAG + Full Visualizations")
        ...
        if st.session_state.knowledge_graph and st.session_state.annotated_trees:
            ...
            if not df_all.empty:
                ...
                with tabs[5]:
                    ...
                    fig_net = viz.plot_knowledge_network(df_all, colormap)
                    st.pyplot(fig_net)
                    fig_static = viz.plot_static_knowledge_network(None, st.session_state.get("viz_top_n", 25), colormap=colormap)
                    st.pyplot(fig_static)
                    if PYVIS_AVAILABLE:
                        html_network = viz.render_pyvis_salience(None, st.session_state.get("viz_top_n", 25), True, colormap)
                        st.components.v1.html(html_network, height=750, scrolling=True)
                        st.download_button("Download PyVis HTML", html_network.encode('utf-8'), "kg_interactive.html", mime="text/html")
