...
class PhysicalQuantityClassifier:
    ...
    def classify(self, parameter_name: Optional[str], unit: Optional[str], context: str) -> str:
        """
        Multi-stage classification with optional LLM fallback for ambiguous terms.
        Stage 1: Exact keyword match on parameter name
        Stage 2: Context keyword matching
        Stage 3: Unit hint matching
        Stage 4: LLM reasoning fallback (if LLM available and stages 1-3 return "unknown")
        """
        # Stage 1: Parameter name matching
        if parameter_name:
        ...
        # Stage 4: LLM reasoning fallback for ambiguous/domain-specific terms
        if self.llm is not None and parameter_name:
            try:
                return self._llm_classify(parameter_name, unit, context)
            except Exception as e:
                logger.debug(f"LLM classification fallback failed: {e}")

        return "unknown"

    def _llm_classify(self, parameter_name: str, unit: Optional[str], context: str) -> str:
        """
        Ask LLM to classify an ambiguous parameter into a canonical physical quantity.
        This bridges vocabulary gaps like "0.2% proof" → yield_strength.
        """
        prompt = f"""You are a materials science expert. Classify this parameter into exactly one canonical category.

Parameter name: "{parameter_name}"
Unit: {unit or "unknown"}
Context: {context[:300]}

Available categories (respond with EXACTLY one):
- laser_power, scan_speed, yield_strength, tensile_strength, hardness, elongation
- modulus, youngs_modulus, poisson_ratio, coefficient_thermal_expansion
- corrosion_potential, pitting_potential, repassivation_potential, breakdown_potential
- corrosion_current_density, polarization_resistance, PREN
- energy_density, areal_energy_density, linear_energy_density
- grain_size, cell_size, porosity, relative_density, surface_roughness
- stacking_fault_energy, ideal_shear_strength
- sauter_mean_diameter, spray_penetration, plume_height
- thermal_conductivity, viscosity, density, enthalpy, temperature
- hollomon_strength_coeff, hollomon_exponent, vec, lewis_number
- jackson_parameter, omega_parameter, delta_h_mix, delta_s_mix
- atomic_size_difference, lambda_parameter
- indentation_force, indentation_depth, dislocation_density
- melt_pool_depth, melt_pool_width, melt_pool_length
- hatch_distance, build_platform_temperature
- unknown (if truly unclassifiable)

Respond with ONLY the category name, no explanation."""

        response = self.llm.generate(prompt, max_new_tokens=50, temperature=0.0)
        cleaned = response.strip().lower().replace("'", "").replace('"', "")

        # Validate against known categories
        if cleaned in self.CANONICAL or cleaned == "unknown":
            logger.info(f"LLM classified '{parameter_name}' as '{cleaned}'")
            return cleaned

        # Check if it's a close match
        for canonical in self.CANONICAL:
            if canonical in cleaned or cleaned in canonical:
                logger.info(f"LLM classified '{parameter_name}' as '{canonical}' (fuzzy match)")
                return canonical

        logger.warning(f"LLM returned unrecognizable category '{cleaned}' for '{parameter_name}', falling back to unknown")
        return "unknown"
...
Return ONLY valid JSON."""
    ...
    async def _generate_summaries_async(self, trees: Dict[str, PageNode]):
        ...
        """
        # Phase 1: Leaf summaries from raw text
        all_nodes = []
        ...
        # Process leaves first (bottom-up)
        ...
        for i in range(0, len(leaves), batch_size):
            ...
            for node in batch:
                if len(node.full_text) > 200:
                    tasks.append(self._summarize_node(node, "leaf"))
                else:
                    node.summary = node.full_text[:200]
            if tasks:
                await asyncio.gather(*tasks)

        # Phase 2: Roll-up summaries for internal nodes
        # Sort by depth (deepest first) so children are summarized before parents
        internal_nodes.sort(key=lambda n: n.level, reverse=True)
        for i in range(0, len(internal_nodes), batch_size):
            batch = internal_nodes[i:i+batch_size]
            tasks = []
            for node in batch:
                tasks.append(self._rollup_summarize_node(node))
            if tasks:
                await asyncio.gather(*tasks)

        # Phase 3: Root executive summaries
        for tree in trees.values():
            await self._executive_summary(tree)

    async def _summarize_node(self, node: PageNode, node_type: str = "leaf"):
        """Generate summary for a leaf node from its raw text."""
...
Summary:"""
    ...
    async def _rollup_summarize_node(self, node: PageNode):
        ...
        if not node.children:
            return  # Leaf node, already summarized
        ...
        prompt = f"""You are synthesizing a document hierarchy. Given summaries of child sections, 
write a concise parent summary (max 200 chars) that captures the overall theme and key findings.

Child sections:
{combined}

Parent section title: {node.title}
Parent summary:"""
        try:
            summary = await asyncio.to_thread(self.llm.generate, prompt, max_new_tokens=150, temperature=0.1)
            node.summary = summary.strip()[:200]
        except Exception as e:
            logger.warning(f"Roll-up summary failed: {e}")
            # Fallback: concatenate child summaries
            node.summary = f"{node.title}: covers {len(node.children)} subsections."

    async def _executive_summary(self, root: PageNode):
        """Generate document-level executive summary from root children."""
        if not root.children:
            return
        child_summaries = []
        for child in root.children[:10]:  # Top 10 sections
            child_summaries.append(f"- {child.title}: {child.summary}")
        combined = "\n".join(child_summaries)
        prompt = f"""Write a one-sentence executive summary (max 250 chars) of this entire document,
capturing its main contribution, key findings, and most important quantitative results.
...
Executive summary:"""
        try:
            summary = await asyncio.to_thread(self.llm.generate, prompt, max_new_tokens=200, temperature=0.1)
            root.summary = summary.strip()[:250]
            root.prefix_summary = root.summary  # Also set as prefix for navigation
        except Exception as e:
            logger.warning(f"Executive summary failed: {e}")
            root.summary = f"Document covering {len(root.children)} sections."

    def _save_tree_fast(self, doc_name: str, tree: PageNode):
        """
        Persist full annotated tree with content-addressable hashing.
        Enables instant reload without re-parsing PDF or re-summarizing.
        """
        safe = re.sub(r'[^\w\-_.]', '_', doc_name)
        # Use tree content hash, not just filename, for cache invalidation
        tree_dict = tree.to_dict()
        content_hash = hashlib.sha256(
            fast_json_dumps(tree_dict, indent=False)
        ).hexdigest()[:16]
        path = self.cache_dir / f"{safe}.{content_hash}.annotated.tree.json"
        meta_path = self.cache_dir / f"{safe}.{content_hash}.tree.meta.json"
        try:
            # Save full annotated tree
            with open(path, "wb") as f:
                f.write(fast_json_dumps(tree_dict, indent=True))
            # Save metadata for quick cache validation
            meta = {
                "doc_name": doc_name,
                "content_hash": content_hash,
                "created_at": datetime.now().isoformat(),
                "node_count": self._count_nodes(tree),
                "has_summaries": bool(tree.summary),
                "has_metadata": tree.metadata is not None
            }
            with open(meta_path, "wb") as f:
                f.write(fast_json_dumps(meta, indent=True))
            logger.info(f"Cached annotated tree to {path} ({meta['node_count']} nodes)")
        except Exception as e:
            logger.warning(f"Fast save failed: {e}")

    def _count_nodes(self, tree: PageNode) -> int:
        count = 1
        for child in tree.children:
            count += self._count_nodes(child)
        return count

    def load_cached_tree(self, doc_name: str, content_hash: Optional[str] = None) -> Optional[PageNode]:
        """
        Load a fully annotated tree from cache if available.
        If content_hash is provided, validates against it for cache coherence.
        """
        safe = re.sub(r'[^\w\-_.]', '_', doc_name)

        # Find matching cache files
        pattern = f"{safe}.*.annotated.tree.json"
        cache_files = list(self.cache_dir.glob(pattern))

        if not cache_files:
            return None

        # Use most recent cache if no hash specified
            ...
            try:
                ...
                if content_hash:
                    actual_hash = hashlib.sha256(
                        fast_json_dumps(data, indent=False)
                    ).hexdigest()[:16]
                    if actual_hash != content_hash:
                        logger.debug(f"Cache hash mismatch for {doc_name}, skipping")
                        continue

                tree = PageNode.from_dict(data)
                logger.info(f"Loaded cached tree for {doc_name} ({self._count_nodes(tree)} nodes)")
                return tree
            except Exception as e:
...
class HierarchicalTreeRetriever:
    ...
    def __init__(self, llm: HybridLLM, max_results=30, max_text_chars=20000):
        ...
        self._navigation_cache: Dict[str, Any] = {}  # Cache navigation results
    ...
    async def retrieve_quantitative(self, query: str, annotated_trees: List[Dict]) -> List[Dict]:
        """
        Phase 1 (Navigation): LLM reasons over condensed tree → selects node_ids
        Phase 2 (Retrieval): Dictionary lookup of node_id → raw text (no LLM)
        """
        # Phase 1: NAVIGATION (LLM Call #1 - cached per query)
        cache_key = hashlib.sha256(query.encode()).hexdigest()[:16]
        if cache_key in self._navigation_cache:
            all_selections = self._navigation_cache[cache_key]
            logger.info(f"Navigation cache hit for query: {query[:50]}...")
        else:
            trees_json = []
            ...
            self._navigation_cache[cache_key] = all_selections
            logger.info(f"Navigation complete: {len(all_selections)} nodes selected for query: {query[:50]}...")

        # Phase 2: RETRIEVAL (Dictionary lookup - zero LLM calls)
        # This is the "vectorless" part - pure structured lookup
        results = []
...
def run_streamlit():
    ...
    if "knowledge_graph" not in st.session_state:
        st.session_state.knowledge_graph = QuantitativeKnowledgeGraph(llm=None)
