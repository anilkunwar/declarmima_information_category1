...
Return ONLY valid JSON."""
    ...
    async def _generate_summaries_async(self, trees: Dict[str, PageNode]):
        ...
        # ── Roll-up summarization: bottom-up propagation ──
        for tree in trees.values():
            await self._rollup_summaries(tree)

    async def _rollup_summaries(self, node: PageNode):
        """Bottom-up roll-up: parent summary = synthesis of child summaries."""
        if not node.children:
            return
        child_tasks = []
        for child in node.children:
            if child.children:
                child_tasks.append(self._rollup_summaries(child))
        if child_tasks:
            await asyncio.gather(*child_tasks)
        child_summaries = []
        for child in node.children:
            s = child.summary or child.full_text[:200]
            child_summaries.append(f"- {child.title}: {s}")
        if not child_summaries:
            return
        rollup_text = "\n".join(child_summaries)
        prompt = f"""You are synthesizing a document hierarchy. Given summaries of child sections, write a concise parent summary (max 200 chars) that captures the overarching theme, key parameters, and methods.
Child sections:
{rollup_text}
Parent summary:"""
        try:
            ...
            logger.warning(f"Roll-up summary failed for {node.title}: {e}")
            node.summary = rollup_text[:200]
