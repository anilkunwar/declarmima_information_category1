...
class PhysicalQuantityClassifier:
    ...
    def __init__(self, llm: Optional[Any] = None):
        ...
        self.llm = llm
        self._llm_cache: Dict[Tuple[str, str, str], Tuple[str, float]] = {}
    ...
    def classify(self, parameter_name: Optional[str], unit: Optional[str], context: str) -> str:
        result = self._classify_fast(parameter_name, unit, context)
        if result != "unknown":
            return result
        # LLM fallback for ambiguous terms
        if self.llm is not None:
            return self._classify_with_llm(parameter_name, unit, context)
        return "unknown"

    def _classify_fast(self, parameter_name: Optional[str], unit: Optional[str], context: str) -> str:
        if parameter_name:
    ...
    def _classify_with_llm(self, parameter_name: Optional[str], unit: Optional[str], context: str) -> str:
        cache_key = (str(parameter_name or ""), str(unit or ""), context[:200])
        if cache_key in self._llm_cache:
            return self._llm_cache[cache_key][0]
        canonical_list = ", ".join(sorted(self.CANONICAL.keys())[:60]) + ", ..."
        prompt = f"""You are a scientific document analyst. Given a parameter mention, classify it into the most appropriate canonical physical quantity.
Parameter name: {parameter_name or "N/A"}
Unit: {unit or "N/A"}
Context snippet: {context[:500]}
Canonical options include: {canonical_list}
Respond with ONLY the canonical name (e.g., yield_strength) or "unknown". No explanation."""
        try:
            resp = self.llm.generate(prompt, max_new_tokens=64, temperature=0.0)
            resp_clean = resp.strip().lower().split()[0].strip(".,;:")
            if resp_clean in self.CANONICAL:
                self._llm_cache[cache_key] = (resp_clean, 0.85)
                return resp_clean
        except Exception as e:
            logger.debug(f"LLM classification fallback failed: {e}")
        self._llm_cache[cache_key] = ("unknown", 0.0)
        return "unknown"
...
# =============================================================================
# ROLL-UP HIERARCHICAL SUMMARIZER (ADAPTIVE BATCH SIZING)
# =============================================================================
class RollupSummarizer:
    ...
    - Start with batch_size=12, max_node_chars=1500
    - On context-length error: halve batch_size, halve max_node_chars
    - Minimum floor: batch_size=2, max_node_chars=800 (degrades to single-node)
    - Recovery: after 3 successful calls, slowly increase back toward defaults
    """

    CONTEXT_LENGTH_ERRORS = [
        "context length", "token", "too long", "exceeds", "maximum context",
        "input length", "prompt is too long", "max_tokens", "context window",
        "reduce the length", "payload too large", "413", "400",
        "context_length_exceeded", "max_context_length",
    ]
    ...
    async def _adaptive_batch_rollup(self, root: PageNode) -> None:
        ...
        for depth in sorted(levels.keys(), reverse=True):
            ...
            uncached = []
            for node in nodes:
                cache_key = f"{node.doc_id}:{node.node_id}"
            ...
            for node in uncached:
                ...
                if not node.children and len(text.strip()) < 80:
                    ...
                    self._summary_cache[f"{node.doc_id}:{node.node_id}"] = node.summary
                else:
    ...
    def _get_node_source_text(self, node: PageNode) -> str:
        ...
        if not node.children:
            ...
            own = (node.summary or "")[:min(400, budget // 4)]
    ...
    async def _summarize_batch_with_retry(self, batch: List[Tuple[PageNode, str]]) -> None:
        max_retries = 4
        for attempt in range(max_retries):
            try:
                ...
                if (
                    ...
                    self._current_batch_size = min(self._current_batch_size + 1, self.batch_size)
                    self._current_max_node_chars = min(int(self._current_max_node_chars * 1.2), self.max_node_chars)
                    logger.info(f"Adaptive recovery: batch_size={self._current_batch_size}, max_chars={self._current_max_node_chars}")
                return
            except Exception as e:
                ...
                if is_context_error and attempt < max_retries - 1:
                    ...
                    self._current_max_node_chars = max(int(self._current_max_node_chars * 0.6), self.min_node_chars)
                    self._consecutive_successes = 0
                    ...
                    logger.warning(f"Context-length error. Shrinking: batch {old_batch}->{self._current_batch_size}, chars {old_chars}->{self._current_max_node_chars}. Retry {attempt+1}/{max_retries}")
                    if self._current_batch_size == 1 and len(batch) > 1:
                        logger.info("Degrading to single-node fallback.")
                    ...
                    logger.error(f"Batch summarization failed after {attempt + 1} attempts: {e}")
                    for node, text in batch:
                        await self._fallback_single_node(node, text)
                    return

    async def _execute_batch_llm_call(self, batch: List[Tuple[PageNode, str]]) -> None:
        if not batch:
            return
        sections = []
        for idx, (node, text) in enumerate(batch):
            sections.append(f"<SECTION id={idx}>\n{text[:self._current_max_node_chars]}\n</SECTION>")
...
JSON Output:"""
        ...
        parsed_map = {item["id"]: item["summary"] for item in parsed if isinstance(item, dict) and "id" in item and "summary" in item}
        for idx, (node, text) in enumerate(batch):
    ...
    async def _fallback_single_node(self, node: PageNode, text: str, return_summary: bool = False) -> Optional[str]:
        ...
        try:
            response = await asyncio.to_thread(self.llm.generate, prompt, max_new_tokens=200, temperature=0.05)
            cleaned = response.strip().replace("Summary:", "").strip()
            result = cleaned[:self.max_summary_length]
            if not return_summary:
                node.summary = result
                self._summary_cache[f"{node.doc_id}:{node.node_id}"] = result
            return result
        except Exception as e:
    ...
    def _parse_batch_json(self, text: str, expected: int) -> List[Dict]:
        ...
        if matches:
            return [{"id": int(idx), "summary": summ.replace('\\"', '"')} for idx, summ in matches]
        return []
...
class HierarchicalIndex:
    ...
    def _annotated_cache_path(self, doc_name: str, doc_hash: str) -> Path:
        safe = re.sub(r'[^\w\-_.]', "_", doc_name)
        return self.cache_dir / f"{safe}.{doc_hash}.annotated.json"

    def save_annotated_tree(self, doc_name: str, annotated: Dict, doc_hash: str):
        path = self._annotated_cache_path(doc_name, doc_hash)
        try:
            with open(path, "wb") as f:
                f.write(fast_json_dumps(annotated, indent=True))
            logger.info(f"Saved annotated tree cache: {path}")
        except Exception as e:
            logger.warning(f"Annotated tree cache save failed: {e}")

    def load_annotated_tree(self, doc_name: str, doc_hash: str) -> Optional[Dict]:
        path = self._annotated_cache_path(doc_name, doc_hash)
        if not path.exists():
            return None
        try:
            ...
            logger.info(f"Loaded annotated tree from cache: {path}")
            return data
        except Exception as e:
            logger.warning(f"Annotated tree cache load failed: {e}")
            return None
...
Return ONLY valid JSON."""
    ...
    async def _generate_summaries_async(self, trees: Dict[str, PageNode]):
        """Adaptive batched roll-up summarization for all trees."""
        ...
        for doc_name, tree in trees.items():
            ...
            await summarizer.batch_summarize_tree(tree)
            stats = summarizer.get_adaptive_stats()
            logger.info(
                f"Finished {doc_name}: batch_size={stats['current_batch_size']}, "
                f"max_chars={stats['current_max_node_chars']}, errors={stats['error_count']}"
            )
...
# =============================================================================
# DUAL-LLM MANAGER: LIGHTWEIGHT INDEXING + POWERFUL QUERY
# =============================================================================
class DualLLMManager:
    """
    Manages two separate LLM instances: one for indexing (fast), one for queries (accurate).
    Lazily initializes models to save memory. Thread-safe singleton pattern per session.
    """
    def __init__(self, indexing_model_key: str, query_model_key: str, use_4bit: bool = True):
        self.indexing_model_key = LOCAL_LLM_OPTIONS.get(indexing_model_key, indexing_model_key)
        self.query_model_key = LOCAL_LLM_OPTIONS.get(query_model_key, query_model_key)
        self.use_4bit = use_4bit
        self._indexing_llm: Optional[HybridLLM] = None
        self._query_llm: Optional[HybridLLM] = None
        self._init_lock = threading.Lock()

    def get_indexing_llm(self) -> HybridLLM:
        """Get or create the lightweight indexing LLM."""
        if self._indexing_llm is None:
            with self._init_lock:
                if self._indexing_llm is None:
                    logger.info(f"Initializing indexing LLM: {self.indexing_model_key}")
                    self._indexing_llm = HybridLLM(
                        model_key=self.indexing_model_key,
                        use_4bit=self.use_4bit
                    )
        return self._indexing_llm

    def get_query_llm(self) -> HybridLLM:
        """Get or create the powerful query LLM."""
        if self._query_llm is None:
            with self._init_lock:
                if self._query_llm is None:
                    logger.info(f"Initializing query LLM: {self.query_model_key}")
                    self._query_llm = HybridLLM(
                        model_key=self.query_model_key,
                        use_4bit=self.use_4bit
                    )
        return self._query_llm

    def get_llm_for_task(self, task: Literal["indexing", "query"]) -> HybridLLM:
        """Get appropriate LLM for the given task."""
        return self.get_indexing_llm() if task == "indexing" else self.get_query_llm()

    def unload_indexing_llm(self):
        """Unload indexing model to free VRAM for query model."""
        if self._indexing_llm is not None:
            logger.info("Unloading indexing LLM to free memory...")
            self._indexing_llm = None
            import gc
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            logger.info("Indexing LLM unloaded.")
    ...
    def clear_cache(self):
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        logger.info("DualLLMManager cache cleared")
...
class HierarchicalTreeRetriever:
    ...
    async def retrieve_quantitative(self, query: str, annotated_trees: List[Dict]) -> List[Dict]:
        """Call 1: Tree navigation → node_id selections (reasoning-based, no raw text)."""
        trees_json = []
        ...
        for sel in sorted(all_selections, key=lambda x: x.get('confidence', 0), reverse=True):
            ...
            node = self.find_node_by_id(annotated_trees, doc_id, node_id)
            if node:
    ...
    async def call1_tree_navigation(self, query: str, annotated_trees: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        """Strict 2-call architecture: Call 1 returns (selections, condensed_trees)."""
        trees_json = []
        for tree in annotated_trees:
            doc_id = tree.get("doc_id", "unknown")
            if doc_id not in self._condensed_cache:
                self._condensed_cache[doc_id] = self._condense_tree(tree, use_rollup=True)
            trees_json.append(self._condensed_cache[doc_id])
        batches = self._batch_trees(trees_json, max_tokens=6000)
        all_selections = []
        for batch in batches:
        ...
        for sel in sorted(all_selections, key=lambda x: x.get('confidence', 0), reverse=True):
            ...
            if key not in seen:
                seen.add(key)
                deduped.append(sel)
        return deduped[:self.max_results], trees_json

    def _condense_tree(self, tree: Dict, max_depth: int = 3, use_rollup: bool = False) -> Dict[str, Any]:
        def condense(node: Dict, depth: int = 0) -> Dict[str, Any]:
            if depth > max_depth:
                return {"node_id": node.get("node_id", ""), "title": node.get("title", ""), "leaf": True}
            # Use rollup summary if available (parent summary synthesized from children)
            raw_summary = node.get("summary", "") or ""
            prefix = node.get("prefix_summary", "") or ""
            summary = (prefix + " " + raw_summary).strip()[:200] if use_rollup else raw_summary[:150]
            result = {"node_id": node.get("node_id", ""), "title": node.get("title", ""), "summary": summary}
...
Include up to {self.max_results} selections."""
    ...
    def find_node_by_id(self, trees: List[Dict], doc_id: str, node_id: str) -> Optional[Dict]:
        """Public API: find node by doc_id + node_id across annotated trees."""
        for tree in trees:
    ...
    def _find_node_by_id(self, trees: List[Dict], doc_id: str, node_id: str) -> Optional[Dict]:
        ...
        return self.find_node_by_id(trees, doc_id, node_id)
...
class TwoCallEngine:
    ...
    Strict 2-call query architecture:
      Call 1: HierarchicalTreeRetriever.call1_tree_navigation() → node_ids
      Call 2: TwoCallEngine.call2_extract_and_synthesize() → answer + items
    """
    def __init__(self, llm: HybridLLM):
        self.llm = llm
    ...
    CALL2_PROMPT = """You are an expert scientific analyst performing structured extraction and synthesis.
...
TASK: Perform TWO operations in a single response:

1. EXTRACTION: Extract ALL quantitative information relevant to the query.
   For each extraction, provide:
   - item_type: "quantitative|qualitative|definition|comparison|relationship|process|material|method|phase_field|molecular_dynamics|plasticity|thermal|mechanical|microstructural|electrochemical|multiphysics|ai_ml|digital_twin|informatics"
   - content: exact phrase with full numerical value
   - parameter_name: the parameter name
   - value: numeric value
   - unit: unit (W, MPa, GPa, HV, mV, µA/cm², J/mm³, etc.)
   - physical_quantity: canonical name (laser_power, yield_strength, hollomon_strength, hollomon_exponent, ramberg_osgood_k, ramberg_osgood_n, stacking_fault_energy, etc.)
   - material: alloy/material name
   - confidence: 0.0-1.0
   - context: exact sentence from text
   - doc_source: document name
   - page: page number
   - method: experimental or simulation method
   - conditions: temperature, strain rate, etc.

2. SYNTHESIS: Provide a comprehensive, structured answer to the query using ONLY the extracted values.
   Format:
   **Direct Answer** — concise answer citing sources
   **Evidence by Physical Quantity** — group findings by quantity
   **Evidence by Material** — group by alloy/material
   **Consensus & Variability** — range/mean when multiple values exist
   **Contradictions & Limitations** — highlight discrepancies
   **Confidence Assessment** — High/Medium/Low

CRITICAL RULES:
- NEVER truncate numbers
- If an alloy appears, create a material item
- Map Ecorr/Erp/Epit to corrosion_potential/pitting_potential/etc.
- Capture Hollomon K, n and Ramberg-Osgood parameters explicitly
- Return ONLY valid JSON with keys: "extractions" (array) and "synthesis" (string)
- If no relevant info, return {{"extractions": [], "synthesis": "No relevant quantitative data found."}}

Return JSON:"""

    def call2_extract_and_synthesize(self, query: str, retrieved_chunks: List[Dict]) -> Tuple[List[UniversalExtractionItem], str]:
        ...
        for i, chunk in enumerate(retrieved_chunks[:10]):
            ...
            text = chunk.get("full_text", "")[:4000]
            sections_text += f"\n--- SECTION {i+1} [{doc} p.{page}] ---\n{text}\n"
        prompt = self.CALL2_PROMPT.format(query=query, sections_text=sections_text)
        try:
            response = self.llm.generate(prompt, max_new_tokens=4096, temperature=0.1, fast_json=True, system_prompt=self.template.get("system"))
            data = self._extract_json_safe(response)
            if not data or not isinstance(data, dict):
                return [], self._fallback_synthesis(query, retrieved_chunks)
            extractions_raw = data.get("extractions", [])
            synthesis = data.get("synthesis", "")
            items = []
            for item_data in extractions_raw:
                if "physical_quantity" not in item_data or not item_data["physical_quantity"]:
                    item_data["physical_quantity"] = self.phys_classifier.classify(
                        item_data.get("parameter_name"), item_data.get("unit"), item_data.get("context", "")
                    )
                item_data.setdefault("material", None)
                item_data.setdefault("multiphysics_context", None)
                item_data.setdefault("simulation_type", None)
                ...
                try:
                    ...
                    logger.debug(f"Call2 item parse error: {e}")
            min_conf = UNIVERSAL_CONFIG.get("min_confidence_threshold", 0.55)
            items = [i for i in items if i.confidence >= min_conf]
            if not synthesis:
                synthesis = self._fallback_synthesis(query, retrieved_chunks)
            return items, synthesis
        except Exception as e:
            logger.error(f"Call2 failed: {e}")
            return [], self._fallback_synthesis(query, retrieved_chunks)

    def _fallback_synthesis(self, query: str, chunks: List[Dict]) -> str:
        lines = [f"Query: {query}\nFound {len(chunks)} relevant sections:\n"]
        for c in chunks[:5]:
            lines.append(f"- {c.get('doc_id')} p.{c.get('page_start')}: {c.get('section_title', '')}")
        return "\n".join(lines)

    def _extract_json_safe(self, text: str) -> Optional[Dict]:
        patterns = [r'\{.*\}', r'```json\s*(\{.*?\})\s*```', r'```json\s*(\[.*?\])\s*```']
        for pattern in patterns:
            match = re.search(pattern, text, re.DOTALL)
            if match:
                json_str = match.group(1) if match.groups() else match.group(0)
...
def render_sidebar():
    with st.sidebar:
        ...
        # --- Dual-LLM Mode ---
        st.markdown("---")
        st.markdown("### 🔄 Dual-LLM Mode")
        enable_dual = st.checkbox(
            "Use separate LLMs for indexing vs. queries",
            value=st.session_state.get("dual_llm_enabled", True),
            key="dual_llm_toggle",
            help="Use a lightweight model for fast indexing, and a powerful model for accurate query answers."
        )
        st.session_state.dual_llm_enabled = enable_dual

        if enable_dual:
            col_idx, col_qry = st.columns(2)
            with col_idx:
                st.caption("🔧 Indexing LLM (fast, low VRAM)")
                idx_options = [
                    "[Ollama] qwen2.5:0.5b (Fastest, CPU OK)",
                    "[Ollama] qwen2.5:1.5b (Balanced)",
                    "[Ollama] mistral:7b (High JSON Reliability)",
                ]
                current_idx = st.session_state.get("indexing_llm_choice", idx_options[0])
                idx_sel_index = idx_options.index(current_idx) if current_idx in idx_options else 0
                st.session_state.indexing_llm_choice = st.selectbox(
                    "Indexing Model",
                    options=idx_options,
                    index=idx_sel_index,
                    key="idx_llm_select"
            ...
            with col_qry:
                st.caption("🧠 Query LLM (accurate, reasoning)")
                qry_options = [
                    "[Ollama] qwen2.5:7b (Recommended for RAG)",
                    "[Ollama] qwen2.5:14b (Max Reasoning)",
                    "[Ollama] llama3.1:8b (Meta Standard)",
                    "[Ollama] gemma2:9b (Scientific Nuance)",
                ]
                current_qry = st.session_state.get("query_llm_choice", qry_options[0])
                qry_sel_index = qry_options.index(current_qry) if current_qry in qry_options else 0
                st.session_state.query_llm_choice = st.selectbox(
                    "Query Model",
                    options=qry_options,
                    index=qry_sel_index,
                    key="qry_llm_select"
            ...
            st.session_state.dual_llm_enabled = False
...
def run_streamlit():
    ...
    if "indexing_llm_choice" not in st.session_state:
        st.session_state.indexing_llm_choice = "[Ollama] qwen2.5:0.5b (Fastest, CPU OK)"
    if "query_llm_choice" not in st.session_state:
        st.session_state.query_llm_choice = "[Ollama] qwen2.5:7b (Recommended for RAG)"
    if "dual_llm_enabled" not in st.session_state:
        st.session_state.dual_llm_enabled = True
    render_sidebar()
    ...
    if st.session_state.query_processor.get("files") and not st.session_state.annotated_trees:
        with st.spinner("Building hierarchical index with metadata extraction..."):
            ...
            # Initialize DUAL LLM manager for indexing
            if st.session_state.get("dual_llm_enabled", False):
                llm_manager = DualLLMManager(
                    indexing_model_key=st.session_state.indexing_llm_choice,
                    query_model_key=st.session_state.query_llm_choice,
                    use_4bit=st.session_state.get("use_4bit", True)
                )
                llm = llm_manager.get_indexing_llm()
                st.info(f"🔧 Using **{st.session_state.indexing_llm_choice}** for indexing (Dual-LLM mode)")
            else:
                llm = get_cached_llm(st.session_state.llm_model_choice, st.session_state.get("use_4bit", True))
                st.info(f"🔧 Using **{st.session_state.llm_model_choice}** for indexing (Single-LLM mode)")
            ...
            annotated = []
            for doc_name, tree in trees.items():
                # Check annotated tree cache first
                doc_hash = hashlib.sha256(doc_name.encode()).hexdigest()[:16]
                cached_ann = idx.load_annotated_tree(doc_name, doc_hash)
                if cached_ann:
                    annotated.append(cached_ann)
                    continue
                ...
                idx.save_annotated_tree(doc_name, ann, doc_hash)
    ...
    if st.session_state.annotated_trees:
        ...
        if run_query:
            with st.chat_message("assistant"):
                ...
                # Initialize Processor with Dual-LLM support
                if st.session_state.get("dual_llm_enabled", False):
                    if "llm_manager" not in st.session_state:
                        st.session_state.llm_manager = DualLLMManager(
                            indexing_model_key=st.session_state.indexing_llm_choice,
                            query_model_key=st.session_state.query_llm_choice,
                            use_4bit=st.session_state.get("use_4bit", True)
                        )
                    query_llm = st.session_state.llm_manager.get_query_llm()
                    if hasattr(st.session_state, 'llm_manager'):
                        st.session_state.llm_manager.unload_indexing_llm()
                else:
                    query_llm = get_cached_llm(st.session_state.llm_model_choice, st.session_state.get("use_4bit", True))
                llm = query_llm
                progress.progress(0.1)

                # ── CALL 1: Tree Navigation (reasoning-based node selection) ──
                progress.text("Call 1/2: Tree navigation (LLM-as-navigator)...")
                ...
                for sel in selections:
                    doc_id = sel.get('doc_id')
                    node_id = sel.get('node_id')
                    node = retriever.find_node_by_id(filtered_trees, doc_id, node_id)
                    if node:
                        full_text = node.get('text', '')
                        if len(full_text) > max_retrieval_chars:
                            full_text = full_text[:max_retrieval_chars] + "..."
                        retrieved.append({"full_text": full_text, "page_start": node.get('start_index'), "doc_id": doc_id, "section_title": node.get('title'), "quantitative_items": node.get('quantitative_items', []), "citation": f'<cite doc="{doc_id}" page="{node.get("start_index")}"/>', "selection_reasoning": sel.get('reasoning', ''), "confidence": sel.get('confidence', 0)})
                progress.progress(0.5)

                # ── CALL 2: Combined Extraction + Synthesis ──
                progress.text("Call 2/2: Extract & synthesize from retrieved sections...")
                engine = TwoCallEngine(llm)
                items, answer = engine.call2_extract_and_synthesize(active_prompt, retrieved)

                progress.progress(0.9)

                # Build extracted values for visualization
                extracted_values = []
                for item in items:
                    if item.item_type == "quantitative" and item.value is not None:
                        phys_q = item.physical_quantity or engine.phys_classifier.classify(item.parameter_name, item.unit, item.context)
                ...
                # Cache for persistence across reruns
                st.session_state.cached_query_result = {
                    ...
                    "answer": answer,
                    "multiphysics_flags": list(engine.phys_classifier.CANONICAL.keys()) if any(item.item_type in ["phase_field", "molecular_dynamics", "plasticity", "thermal", "mechanical", "microstructural", "electrochemical", "multiphysics", "ai_ml", "digital_twin", "informatics"] for item in items) else [],
