...
# =============================================================================
# HYBRID SUMMARIZER - Tiered Strategy (v18.0 Quality + v17.1 Speed)
# =============================================================================
class HybridSummarizer:
    """
    Three-tier summarization strategy:
    - Tier 0: Extractive (fast, almost free) — for small leaves < 600 chars
    - Tier 1: Fast LLM (cheap) — for medium nodes with important keywords
    - Tier 2: High-quality roll-up (expensive, ~15-25% of nodes) — for 
              high-level nodes, sections with quantitative data, 
              or nodes matching critical scientific keywords
    """
    def __init__(self, main_llm, fast_llm=None, max_summary_length: int = 220):
        ...
        # Keywords that trigger Tier 2 (high-quality) summarization
        self.tier2_keywords = [
            "laser power", "scan speed", "energy density", "yield strength",
            "tensile", "hardness", "stacking fault", "lewis number", "jackson",
            "meltpool", "phase field", "molecular dynamics", "calphad",
            "digital twin", "eigenstrain", "marangoni", "sfe", "usfe",
            "corrosion potential", "pitting potential", "polarization",
            "grain size", "porosity", "relative density", "young's modulus",
            "thermal conductivity", "viscosity", "enthalpy", "density",
            "ved", "aed", "led", "pren", "eis", "cpp", "tafel"
        ]

        # Keywords for Tier 1 (fast LLM) — scientific but less critical
        self.tier1_keywords = [
            "method", "experiment", "simulation", "result", "analysis",
            "microstructure", "mechanical property", "thermal", "electrochemical",
            "alloy", "composite", "coating", "deposition", "fusion",
            "microhardness", "nanoindentation", "xrd", "sem", "tem",
            "ebsd", "eds", "dta", "ttt", "cct", "ms temperature"
        ]

    def _detect_tier(self, node: PageNode) -> int:
        """Determine summarization tier for a node. Returns 0, 1, or 2."""
        text = (node.full_text or node.summary or "").lower()
        text_len = len(text)

        # Tier 0: Very small text — extractive is sufficient
        if text_len < 600:
            return 0

        # Tier 2 triggers: High-level nodes OR important scientific content
        if node.level <= 2:  # Root and major sections always get quality
            return 2

        # Check for Tier 2 keywords (quantitative/mechanical properties)
        tier2_score = sum(1 for kw in self.tier2_keywords if kw in text)
        if tier2_score >= 2:  # Multiple important keywords
            return 2
        if tier2_score >= 1 and text_len < 3000:  # Single keyword + manageable size
            return 2
        ...
        # Default: Tier 1 for medium text, Tier 0 for small
        if text_len < 2500:
            return 1
        return 2  # Large text gets quality treatment

    def _tier0_extractive(self, node: PageNode, text: str) -> str:
        """Tier 0: Fast extractive summarization."""
        sentences = re.split(r'(?<=[.!?])\s+', text)
        # Filter out very short sentences and take first 3 meaningful ones
        meaningful = [s.strip() for s in sentences 
                      if len(s.strip()) > 15 and len(s.strip()) < 300]
        summary = ' '.join(meaningful[:3])
        return summary[:self.max_summary_length]

    async def _tier1_fast_llm(self, node: PageNode, text: str) -> str:
        """Tier 1: Fast LLM summary using smaller/faster model."""
...
Summary:"""
        try:
            ...
            logger.warning(f"Tier 1 summarization failed: {e}")
            return self._tier0_extractive(node, text)

    async def _tier2_high_quality(self, node: PageNode, text: str) -> str:
        """Tier 2: High-quality hierarchical roll-up summary."""
        # Collect child summaries for context
        child_summaries = []
        for child in node.children:
            if child.summary and len(child.summary) > 10:
                child_summaries.append(f"- {child.title}: {child.summary}")

        child_context = "\n".join(child_summaries[:8]) if child_summaries else "No subsections."

        prompt = f"""You are an expert scientific document analyst. Create a rich, 
hierarchical summary of this document section that captures:
1. Main topic and scope
2. Key quantitative parameters (with units and values)
3. Materials/alloys mentioned
4. Methods and techniques
5. Relationships to parent/child sections

PARENT CONTEXT: This is a level-{node.level} section titled "{node.title}".
...
- Mention exact values with units
- Include material names
- Max {self.max_summary_length} characters
- Write in complete sentences

HIERARCHICAL SUMMARY:"""
        try:
            summary = await asyncio.to_thread(
                self.main_llm.generate, prompt,
                max_new_tokens=300, temperature=0.15
            )
            cleaned = summary.strip()
            for prefix in ["HIERARCHICAL SUMMARY:", "Summary:", "This section"]:
                if cleaned.lower().startswith(prefix.lower()):
                    cleaned = cleaned[len(prefix):].strip()
            return cleaned[:self.max_summary_length]
        except Exception as e:
            logger.warning(f"Tier 2 summarization failed: {e}")
            return await self._tier1_fast_llm(node, text)

    async def summarize_node(self, node: PageNode):
        """Main entry point — dispatches to appropriate tier."""
        if node.summary and len(node.summary) > 30:
            return  # Already summarized

        text = node.get_text(max_chars=8000)
        if not text.strip():
        ...
        if tier == 0:
            node.summary = self._tier0_extractive(node, text)
            logger.debug(f"Tier 0 (extractive) summary for {node.node_id}: {node.summary[:60]}...")
        elif tier == 1:
            node.summary = await self._tier1_fast_llm(node, text)
            logger.debug(f"Tier 1 (fast LLM) summary for {node.node_id}: {node.summary[:60]}...")
        else:
            node.summary = await self._tier2_high_quality(node, text)
            logger.debug(f"Tier 2 (high quality) summary for {node.node_id}: {node.summary[:60]}...")

    async def summarize_tree(self, root: PageNode, progress_callback=None):
        """Summarize entire tree with bottom-up approach for Tier 2 nodes."""
        all_nodes = []
        def collect(node):
        ...
        # Sort by level (deepest first) for bottom-up Tier 2 processing
        all_nodes.sort(key=lambda n: n.level, reverse=True)
        ...
        logger.info(f"Hybrid summarization complete: Tier0={tier_counts[0]}, "
                   f"Tier1={tier_counts[1]}, Tier2={tier_counts[2]} (total={total})")
...
Return ONLY valid JSON."""
    ...
    async def _generate_summaries_async(self, trees: Dict[str, PageNode]):
        """Hybrid tiered summarization: quality where it matters, speed elsewhere."""
        if not self.llm:
            return

        self.hybrid_summarizer = HybridSummarizer(self.llm)
...
def run_streamlit():
    ...
    if st.session_state.query_processor.get("files") and not st.session_state.annotated_trees:
        ...
        # Optional: Use a smaller/faster model for Tier 1 summarization
        # Uncomment and configure if you have a lightweight model available:
        # fast_llm = HybridLLM(model_key="ollama:qwen2.5:0.5b", use_4bit=True)
        # idx.hybrid_summarizer.fast_llm = fast_llm
