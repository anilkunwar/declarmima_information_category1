...
# =============================================================================
# HYBRID SUMMARIZER - v18.0 Quality + v17.1 Speed (Compact Edition)
# =============================================================================
class HybridSummarizer:
    """
    Three-tier summarization balancing quality and speed:
    - Tier 0: Extractive (fast, ~1ms) — small text < 600 chars
    - Tier 1: Fast LLM (~100-500ms) — medium nodes, scientific content
    - Tier 2: High-quality LLM (~1-2s) — important sections with quantitative data
    """
    def __init__(self, main_llm, fast_llm=None, max_summary_length: int = 220):
        ...
        self.tier2_keywords = [
            "laser power", "scan speed", "energy density", "yield strength", "tensile",
            "hardness", "stacking fault", "lewis number", "jackson", "meltpool",
            "phase field", "molecular dynamics", "calphad", "digital twin",
            "eigenstrain", "marangoni", "sfe", "usfe", "corrosion potential",
            "pitting potential", "repassivation", "polarization resistance",
            ...
            "martensite", "bimodal", "eigenstrain", "spinodal", "nucleation"
        ]

        # Keywords for Tier 1 (fast LLM)
        self.tier1_keywords = [
    ...
    def _detect_tier(self, node: PageNode) -> int:
        """Determine summarization tier. Returns 0, 1, or 2."""
        text = (node.full_text or node.summary or "").lower()
        text_len = len(text)

        # Tier 0: Very small text
        if text_len < 600:
            return 0

        # Tier 2: High-level nodes or important scientific content
        if node.level <= 2:
            return 2
        ...
        if tier2_score >= 2:
            return 2
        if tier2_score >= 1 and text_len < 3000:
            return 2

        # Tier 1: Scientific but less critical
    ...
    def _tier0_extractive(self, node: PageNode, text: str):
        ...
        meaningful = [s.strip() for s in sentences if 15 < len(s.strip()) < 250]
        node.summary = ' '.join(meaningful[:3])[:self.max_summary_length]

    async def _tier1_fast(self, node: PageNode, text: str):
        """Tier 1: Fast LLM summary."""
...
Summary:"""
        try:
            ...
            logger.warning(f"Tier 1 failed for {node.node_id}: {e}")
            self._tier0_extractive(node, text)

    async def _tier2_high_quality(self, node: PageNode, text: str):
        """Tier 2: High-quality summary with child context."""
...
SUMMARY:"""
        try:
            ...
            logger.warning(f"Tier 2 failed for {node.node_id}: {e}")
    ...
    async def summarize_node(self, node: PageNode):
        ...
        if tier == 0:
            ...
            await self._tier2_high_quality(node, text)
            logger.debug(f"Tier 2 (high quality) for {node.node_id}")

    async def summarize_tree(self, root: PageNode):
        """Summarize entire tree."""
        nodes = []
        def collect(n):
            nodes.append(n)
            for c in n.children:
        ...
        logger.info(f"Hybrid summary complete: T0={tier_counts[0]}, "
                   f"T1={tier_counts[1]}, T2={tier_counts[2]} (total={len(nodes)})")
