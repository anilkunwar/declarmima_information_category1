...
class TwoStageRetriever:
    ...
    def retrieve_relevant_docs(self, query: str, top_k: int = 10) -> List[Tuple[str, float]]:
        ...
        for name, meta in self.doc_metadata.items():
            ...
            if "flow speed" in query_lower and _has(meta, "flow_speed_values"): score += 0.5
            if "feed rate" in query_lower and _has(meta, "feed_rate_values"): score += 0.5
            if any(t in query_lower for t in ["irradiance", "intensity", "power density", "heat flux"]) and _has(meta, "irradiance_values"): score += 0.5
            if any(t in query_lower for t in ["layer thickness", "powder layer"]) and _has(meta, "layer_thickness_values"): score += 0.4
            if any(t in query_lower for t in ["spot size", "beam diameter"]) and _has(meta, "spot_size_values"): score += 0.4
            ...
            if "ultimate tensile" in query_lower and _has(meta, "ultimate_tensile_strength_values"): score += 0.4
            if "hardness" in query_lower and _has(meta, "hardness_values"): score += 0.4
            if "elongation" in query_lower and _has(meta, "elongation_values"): score += 0.4
            ...
            if any(t in query_lower for t in ["plasticity", "ramberg", "hollomon", "work hardening", "strain hardening", "flow stress"]):
                if any([
                    ...
                    _has(meta, "hollomon_strength_values"),
                    _has(meta, "hollomon_exponent_values"),
            ...
            if "enthalpy" in query_lower and _has(meta, "enthalpy_values"): score += 0.4
            if "viscosity" in query_lower and _has(meta, "viscosity_values"): score += 0.4
            if "thermal conductivity" in query_lower and _has(meta, "thermal_conductivity_values"): score += 0.4
            if "density" in query_lower and _has(meta, "density_values"): score += 0.3
            if "specific heat" in query_lower and _has(meta, "specific_heat_capacity_values"): score += 0.4
            if "latent heat" in query_lower and _has(meta, "latent_heat_fusion_values"): score += 0.4
            if "thermal diffusivity" in query_lower and _has(meta, "thermal_diffusivity_values"): score += 0.4
            if "mass diffusivity" in query_lower and _has(meta, "mass_diffusivity_values"): score += 0.4
            ...
            if any(t in query_lower for t in ["corrosion", "pitting", "repassivation", "polarization", "eis", "cpp"]):
                if any([
                    _has(meta, "corrosion_potential_values"),
                    _has(meta, "polarization_resistance_values"),
                    _has(meta, "apparent_polarization_resistance_values")
                ]): score += 0.6
            if "current density" in query_lower and _has(meta, "corrosion_current_density_values"): score += 0.5
            ...
            if any(t in query_lower for t in ["austenite", "ferrite", "phase fraction", "martensite"]):
                if any([
                    _has(meta, "phase_fraction_values"),
                    _has(meta, "austenite_fraction_values"),
                    _has(meta, "ferrite_fraction_values")
                ]): score += 0.5
            if "grain size" in query_lower and _has(meta, "grain_size_values"): score += 0.4
            if "cell size" in query_lower and _has(meta, "cell_size_values"): score += 0.4
            if "porosity" in query_lower and _has(meta, "porosity_values"): score += 0.4
            ...
            if any(t in query_lower for t in ["nucleation rate", "growth rate", "interface velocity"]):
                if _has(meta, "nucleation_rate_values") or _has(meta, "growth_rate_values"): score += 0.5
            if "grain boundary" in query_lower and _has(meta, "grain_boundary_energy_values"): score += 0.5
            if "interface width" in query_lower and _has(meta, "diffuse_interface_width_values"): score += 0.5
            ...
            if "spray penetration" in query_lower and _has(meta, "spray_penetration_values"): score += 0.5
            ...
            if "meltpool" in query_lower:
                if any([
                    _has(meta, "meltpool_depth_values"),
                    _has(meta, "meltpool_width_values")
                ]): score += 0.6
            if "marangoni" in query_lower and _has(meta, "marangoni_velocity_values"): score += 0.5
            ...
            if any(t in query_lower for t in ["molecular dynamics", "lammps", "md simulation"]):
            ...
            if any(t in query_lower for t in ["pinn", "physics-informed"]):
                if _has(meta, "pinn_epochs_values"): score += 0.5
            if "unet" in query_lower and _has(meta, "unet_dice_values"): score += 0.4
            if "convlstm" in query_lower and _has(meta, "convlstm_rmse_values"): score += 0.4
            if "xai" in query_lower or "shap" in query_lower or "lime" in query_lower:
