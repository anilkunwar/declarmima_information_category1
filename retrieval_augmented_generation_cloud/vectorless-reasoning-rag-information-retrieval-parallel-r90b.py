...
class PaginationAwareReader:
    ...
    def extract_pages(self, doc_path: str, page_numbers: List[int]) -> Dict[int, str]:
        ...
        for pnum in page_numbers:
            ...
            page = doc[pnum-1]
            text = None
            # Try pymupdf4llm first (preserves LaTeX equations from PDF)
            if pymupdf4llm_available:
                try:
                    import pymupdf4llm
                    # to_markdown returns full doc markdown; extract just our page
                    md_text = pymupdf4llm.to_markdown(doc_path, pages=[pnum-1])
                    if md_text and len(md_text.strip()) > 10:
                        text = md_text
                        logger.debug(f"Page {pnum}: Using pymupdf4llm LaTeX-aware extraction")
                except Exception as e:
                    logger.debug(f"pymupdf4llm failed for page {pnum}, falling back: {e}")
            # Fallback to standard PyMuPDF text extraction
            if text is None:
        ...
        for item in items:
            try:
                ...
                if val_str and val_str in page_text:
                    ...
                    # --- v18.1 FIX #2: Check model name and variables instead of raw LaTeX tokens ---
                    check_terms = []
                    if item.model_name:
                        check_terms.append(item.model_name.lower())
                    if item.variables_defined:
                        check_terms.extend(v.lower() for v in item.variables_defined.values())
                    if check_terms and any(term in page_text.lower() for term in check_terms if term):
                        item.confidence = min(1.0, item.confidence + 0.1)
                    elif item.equation_latex:
                        # Fallback: check if key LaTeX symbols appear
                        eq_terms = re.findall(r'\\[a-zA-Z]+|[_^\{\}]', item.equation_latex)
                        if any(term in page_text for term in eq_terms[:3]):
                            item.confidence = min(1.0, item.confidence + 0.1)
                        else:
                            item.confidence = max(0.0, item.confidence - 0.1)  # Reduced penalty
    ...
    def add_extractions(self, doc_id: str, items: List[UniversalExtractionItem]):
        ...
        for item in items:
            ...
            if item.item_type == "equation" and item.model_name:
                normalized_model = item.model_name.lower().replace("-", " ").replace("_", " ")
