...
class DynamicIntentAnalyzer:
    """Vectorless intent router that dynamically adjusts retrieval weights."""
    INTENT_PATTERNS = {
        "equation_hunting": [r"\bequation\b", r"\bmodel\b", r"\bformula\b", r"\bderive\b", r"\bgoverning\b", r"\bconstitutive\b", r"\bkinetics\b"],
        "mechanism_explanation": [r"\bwhy\b", r"\bhow\b", r"\bmechanism\b", r"\beffect\b", r"\binfluence\b", r"\bcause\b", r"\bexplain\b", r"\bdriving force\b"],
        "comparison": [r"\bcompare\b", r"\bvs\b", r"\bversus\b", r"\bdifference\b", r"\bacross all\b", r"\bwhich is better\b"],
        "boolean_verification": [r"\bis it\b", r"\bdoes\b", r"\bdo\b", r"\bare there\b", r"\bcan\b", r"\bmentioned\b"],
        "value_extraction": [r"\bvalue\b", r"\bparameter\b", r"\bpower\b", r"\bspeed\b", r"\bstrength\b", r"\btemperature\b", r"\bfind out\b"]
    }

    def analyze(self, query: str) -> Dict[str, Any]:
        q_lower = query.lower()
        intent = "value_extraction"  # Default fallback
        ...
        # Dynamic Weights: Dictates how much to trust metadata vs LLM Tree Search
        weights = {
            "metadata": 0.8 if intent == "value_extraction" else 0.2,
            "keyword": 0.6 if intent in ["value_extraction", "boolean_verification"] else 0.3,
            "tree_search": 0.4 if intent == "value_extraction" else 0.95  # High weight for LLM tree nav on mechanisms/equations
        }

        return {
            "intent": intent,
            "weights": weights,
            "focus_instruction": self._generate_focus(intent, query),
            "output_format": "table" if intent == "comparison" else "latex" if intent == "equation_hunting" else "standard",
            "target_parameters": [],
            "target_materials": []
        }

    def _generate_focus(self, intent: str, query: str) -> str:
        if intent == "equation_hunting":
            return "CRITICAL: Extract governing equations, constitutive models, and PDEs. Format them strictly in LaTeX."
        elif intent == "mechanism_explanation":
            return "CRITICAL: Extract causal relationships, physical explanations, and qualitative mechanisms. Do not just look for numbers."
        return f"Focus on extracting exact values and parameters related to: {query}"


# =============================================================================
# QUERY ANALYZER (Intent-Aware Dynamic Weighting) - v18.0 UPGRADE
# =============================================================================
class QueryAnalyzer:
...
# =============================================================================
class PageIndexAnswerGenerator:
    """Generates PageIndex-style answers with strict citation enforcement."""

    PROMPT_SCHEMA = """You are a precision scientific analyst. Answer the query using ONLY verified extracted values.
QUERY: {query}
VERIFIED ITEMS: {items_json}
...
- Return ONLY the structured text."""
    ...
    def generate(self, query: str, verified_items: List[UniversalExtractionItem], query_analysis: Optional[Dict[str, Any]] = None) -> str:
        # --- v18.1 FIX #5: Dynamic item limit based on intent; group by doc for comparisons ---
        qa = query_analysis or {}
        intent = qa.get("intent", "")
        max_items = 30 if intent in ["comparison", "equation_hunting"] else 15

        if intent == "comparison" and verified_items:
            # Group by document to ensure cross-document coverage
            by_doc = defaultdict(list)
            for item in verified_items:
                by_doc[item.doc_source].append(item)
            # Take top items per document to maximize coverage
            selected = []
            for doc_items in sorted(by_doc.values(), key=lambda items: max(i.confidence for i in items), reverse=True):
                selected.extend(doc_items[:3])
            selected = sorted(selected, key=lambda x: x.confidence, reverse=True)[:max_items]
            items_json = json.dumps([i.to_dict() for i in selected], ensure_ascii=False, default=str)
        else:
            items_json = json.dumps([i.to_dict() for i in verified_items[:max_items]], ensure_ascii=False, default=str)

        prompt = self.PROMPT_SCHEMA.format(query=query, items_json=items_json)
        raw = self.llm.generate(prompt, max_new_tokens=1500, temperature=0.05)
        return self._format_to_pageindex(raw, query)
...
class TwoStageRetriever:
    ...
    def retrieve_relevant_docs(self, query: str, query_analysis: Optional[Dict[str, Any]] = None, top_k: int = 10) -> List[Tuple[str, float]]:
        ...
        """
        scores = []
        weights = query_analysis.get("weights", {"metadata": 0.5, "keyword": 0.5}) if query_analysis else {"metadata": 0.5, "keyword": 0.5}
        q_lower = query.lower()
        ...
        if query_analysis and query_analysis.get("intent") in ["mechanism_explanation", "equation_hunting"]:
...
class UniversalLLMExtractor:
    EXTRACTION_PROMPT = """Extract ALL quantitative information, mathematical models, and qualitative mechanisms relevant to the query from these document sections.
QUERY: {query}
...
{{
"item_type": "quantitative|qualitative|definition|comparison|relationship|process|material|method|equation|phase_field|molecular_dynamics|plasticity|thermal|mechanical|microstructural|electrochemical|multiphysics|ai_ml|digital_twin|informatics",
...
"model_name": "Name of constitutive model or PDE (e.g., Hollomon, Cahn-Hilliard, Butler-Volmer)",
...
class LLMReasoningSynthesizer:
    ...
    def synthesize(self, query: str, items: List[UniversalExtractionItem], query_analysis: Optional[Dict] = None) -> str:
        if not items:
            return f"No relevant information found for query: '{query}'. Try rephrasing or check the documents."

        qa = query_analysis or {}
        intent = qa.get("intent", "value_extraction")
        out_fmt = qa.get("output_format", "standard")
        extracted_lines = []

        for item in items:
            cite_tag = f"@@CITE:doc={item.doc_source};page={item.page}@@"
...
(Create a Markdown table comparing values across documents/materials)
**Key Differences & Consensus**
"""
        elif out_fmt == "causal_chain":
            format_instruction = """
**Physical Mechanism / Explanation**
(Explain the 'why' and 'how' based on the text, citing equations or models)
**Supporting Evidence & Parameters**
"""
        elif out_fmt == "yes_no_evidence":
            format_instruction = """
**Direct Answer (Yes/No/Presence)**
**Evidence & Citations**
"""
        elif out_fmt == "latex":
            format_instruction = """
**Governing Equations & Models**
(Preserve ALL LaTeX equations exactly as they appear, wrapped in $$)
...
TASK: Synthesize the extracted information using EXACTLY this structure:
{format_instruction}
**Confidence Assessment**
(High/Medium/Low)

RULES:
1. Preserve ALL LaTeX equations exactly as they appear (wrapped in $$).
2. Use @@CITE:doc=filename;page=X@@ for EVERY claim.
3. Do NOT invent connections between papers.
4. If a parameter is NOT in a paper, explicitly state: "Not reported in [doc]."
Return ONLY the structured text."""
        try:
            answer = self.llm.generate(prompt, max_new_tokens=2048, temperature=0.1)
...
def run_streamlit():
    st.set_page_config(page_title="DECLARMIMA v18.2 - Unified Multi-Physics RAG", layout="wide")
    st.markdown("# DECLARMIMA v18.2 - Unified Robust Vectorless RAG + Query-Driven Visualizations")
    ...
    if st.session_state.annotated_trees:
        ...
        if run_query:
            with st.chat_message("assistant"):
                ...
                query_analysis = analyzer.analyze(active_prompt)
                st.caption(f"🧠 **Intent:** `{query_analysis['intent']}` | **Weights:** {query_analysis['weights']} | **Format:** {query_analysis['output_format']}")
                progress.progress(0.1)
                ...
                # Use PageIndex generator for structured output, fallback to synthesizer
                if extracted_values or equation_items:
                    page_gen = PageIndexAnswerGenerator(llm)
                    answer = page_gen.generate(active_prompt, items, query_analysis=query_analysis)
                else:
                    answer = synthesizer.synthesize(active_prompt, items, query_analysis=query_analysis)
                ...
                st.session_state.cached_query_result = {
                    ...
                    "multiphysics_flags": list(synthesizer.phys_classifier.CANONICAL.keys()) if any(item.item_type in ["phase_field", "molecular_dynamics", "plasticity", "thermal", "mechanical", "microstructural", "electrochemical", "multiphysics", "ai_ml", "digital_twin", "informatics"] for item in items) else [],
