...
class HybridLLM:
    ...
    def _ollama_generate(self, prompt: str, max_tokens: int, temp: float, 
        ...
        try:
            ...
            if fast_json:
                options.update({"temperature": 0.0, "stop": ["```", "</code>", "</think>"]})
...
INSTRUCTIONS:
1. Select ONLY section IDs likely to contain information relevant to the query.
...
6. Set confidence based on clarity: 0.9+ for explicit statements, 0.6-0.8 for inferred, <0.6 for uncertain"""
    ...
    def _extract_single_chunk(self, chunk: Dict, query: str, 
        ...
        system = f"""Extract ONLY information that EXIST in the provided text below.
...
STRICT RULES:
1. ONLY extract from the text provided - NEVER invent
2. For each item, include the EXACT source filename and EXACT sentence
...
REQUIREMENTS:
- Only extract information that appears in the text above
...
class EnhancedCrossDocumentKnowledgeGraph:
    ...
    def add_extraction(self, doc_id: str, items: List[UniversalExtractionItem]):
        """Add extracted items to the graph."""
...
def format_universal_answer(items: List[UniversalExtractionItem], 
    ...
    if not items:
        lines.append("❌ No relevant information found in the uploaded documents.")
