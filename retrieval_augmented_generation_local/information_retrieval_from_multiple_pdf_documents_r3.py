...
# --- Configuration ---
DEFAULT_MODELS = ["llama3.1:8b", "gemma3:latest", "mistral:7b", "qwen2.5:7b"]
...
def main():
    ...
    with st.sidebar:
        ...
        with col2:
            refresh_models = st.button("🔄", help="Refresh available models")
        
        # Get available models
        available_models = get_available_ollama_models(st.session_state.ollama_base_url)
        
        if available_models:
            # Create model options combining default models and detected models
            all_model_options = list(set(DEFAULT_MODELS + available_models))
            all_model_options.sort()
            
            # Add "Arena Model" as a special option
            if "arena" not in [m.lower() for m in all_model_options]:
                all_model_options.append("Arena Model")
            
            # Default model selection
            default_model = "gemma3:latest"
            if default_model not in all_model_options and all_model_options:
                default_model = all_model_options[0]
            
            st.session_state.llm_model = st.selectbox(
                "LLM Model for Generation",
                options=all_model_options,
                index=all_model_options.index(default_model) if default_model in all_model_options else 0,
                help="Select the OLLAMA model to use for generating answers."
            )
            ...
            st.session_state.llm_model = st.selectbox(
                "LLM Model for Generation",
                options=DEFAULT_MODELS + ["Arena Model"],
                index=0,
                help="Using default models. Check Ollama connection if you don't see your models."
            )
