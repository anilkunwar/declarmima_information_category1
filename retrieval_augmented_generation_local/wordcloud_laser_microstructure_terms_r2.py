...
def generate_wordcloud_with_strategy(strategy, llm, chunks):
    ...
    # =============================================
    # TF-IDF + Fallback to Frequency (ROBUST VERSION)
    # =============================================
    ...
    # Build corpus: one doc per source
    ...
    # === CASE 1: Multiple sources → Use TF-IDF ===
    if n_sources >= 2:
        try:
            ...
            st.warning(f"TF-IDF failed ({e}), falling back to frequency scoring.")
            tfidf_scores = {}

    # === CASE 2: Single source → Use frequency only ===
    else:
        st.info("Only 1 source document detected. Using frequency-based scoring (TF-IDF skipped).")
        tfidf_scores = {}
    ...
    for term in all_terms:
        ...
        # Boost with TF-IDF if available
