...
+ FIXED: Material labels always use dark text (#1a1a2e) on white background
+ FIXED: rotation_mode='default' prevents bounding-box collapse toward anchor
"""
...
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
    ...
    def place_label_at_node_angle(ax, angle_rad, text, label_r, font_size,
        ...
        if label_mode == 'radial':
            # rotation_mode='default' rotates the whole bounding box around the
            # alignment point. ha='left' + rotation=angle makes text extend
            # outward along the radius for every quadrant.
            text_rot = rot_norm
            ha = 'left'
            va = 'center'

            # Top center: upright vertical, centered above the anchor
            if 80 <= rot_norm <= 100:
                text_rot = 90
                ha = 'center'
                va = 'bottom'
            # Bottom center: upright vertical, centered below the anchor
            ...
            text_rot = tangent
            # Keep readable from the outside (flip upside-down quadrants)
            if 90 < text_rot < 270:
                text_rot = (text_rot + 180) % 360
            ha = 'left'
            va = 'center'
        ...
        # CRITICAL FIX: rotation_mode='default' prevents the bounding-box
        # collapse toward the anchor that 'anchor' mode causes when ha='center'.
        ax.text(x, y, text, ha=ha, va=va, fontsize=font_size,
                fontweight=font_weight, color=text_color, zorder=6,
                clip_on=False, rotation=text_rot, rotation_mode='default')
    ...
    for mat, ang in m_angles.items():
        ...
        # outside the colored arcs. get_contrast_text_color(fill) would produce white
        # text for dark-colored nodes, making them invisible on white.
        txt_color = '#1a1a2e'
    ...
    for i, mat in enumerate(mats_sorted):
        ...
        if show_labels:
            ...
            if label_rotation_mode == 'auto':
                # Radiate outward. rotation_mode='default' keeps the whole
                # bounding box rigid so ha='center' does not collapse.
                text_rot = rot_norm
                ha = 'left'
                va = 'center'

                # Top center: force upright vertical
                if 85 <= rot_norm <= 95:
                    text_rot = 90
                    ha = 'center'
                    va = 'bottom'
                # Bottom center: force upright vertical
            ...
            # CRITICAL FIX: rotation_mode='default' prevents bounding-box
            # collapse toward the anchor when ha='center'.
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    ...
    ✅ 4 Label Modes (radial / parallel / vertical / inclined) with <code>rotation_mode='default'</code> |
