...
MODEL_META = {
    'modelA': {'name': 'Model A (Falcon 10B)', 'color': '#F39C12', 'short': 'Falcon 10B',
               'context_limit': 10000, 'context_note': '10k chars', 'is_limited': True},
    'modelB': {'name': 'Model B (Mistral 7B)', 'color': '#C0392B', 'short': 'Mistral 7B',
               'context_limit': 50000, 'context_note': '50k chars', 'is_limited': False},
...
def load_data_from_disk_or_default():
    ...
    paths = {
        'modelA': os.path.join(ALLOYS_DIR, "alloys_materials_in_documents_searched_via_LLM_modelA.csv"),
    ...
    for key, path in paths.items():
        if os.path.exists(path):
            ...
            default = {'modelA': DEFAULT_DATA_A, 'modelB': DEFAULT_DATA_B, 'modelC': DEFAULT_DATA_C, 'modelD': DEFAULT_DATA_D}[key]
...
def get_network_html_cached(display_data_json):
    ...
    <span style="display:inline-block; width:14px; height:14px; background:#F39C12; margin-right:6px; border:1px solid black;"></span> Falcon 10B (10k chars)<br>
    ...
    <span style="display:inline-block; width:14px; height:14px; background:#8E44AD; margin-right:6px; border:1px solid black;"></span> Consensus (≥2 models)
    </div>
    """
...
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
                       ...
                       label_mode='radial',
                       show_models=None,
                       show_legend=True, legend_position='lower right'):
    """
    ...
    legend_elements = []
    if show_models.get('modelA', True):
        legend_elements.append(mpatches.Patch(facecolor=MODEL_META['modelA']['color'], edgecolor='black', label=MODEL_META['modelA']['name']))
    ...
    if show_models.get('consensus', True):
        legend_elements.append(mpatches.Patch(facecolor='#8E44AD', edgecolor='black', label='Consensus (≥2 models)'))
    if show_models.get('doi_nodes', True):
    ...
    if show_legend and legend_elements:
        bbox_map = {
            'lower right': (1.02, -0.02),
            'upper right': (1.02, 1.02),
            'upper left': (-0.02, 1.02),
            'lower left': (-0.02, -0.02),
            'center right': (1.02, 0.5),
            'center left': (-0.02, 0.5),
            'best': None,
        }
        bbox = bbox_map.get(legend_position, None)
        legend_kw = {
            'loc': legend_position, 'fontsize': 10,
            'framealpha': 0.95, 'title': 'Legend', 'title_fontsize': 11,
            'edgecolor': 'black', 'fancybox': True
        }
        if bbox is not None:
            legend_kw['bbox_to_anchor'] = bbox
        ax.legend(handles=legend_elements, **legend_kw)
    return fig
...
def draw_circos_clean(all_models_data, figsize=(14, 14), inner_r=1.0,
    ...
    - REMOVED: inner bar count numbers
    """
    figsize = (min(figsize[0], 24), min(figsize[1], 24))
    ...
    for i, mat in enumerate(mats_sorted):
        ...
        for ti, (mk, meta) in enumerate(MODEL_META.items()):
            ...
            # REMOVED: count labels inside bars
    ...
    if show_legend:
        ...
        bbox_map = {
            'upper right': (1.22, 1.08),
            'lower right': (1.22, -0.08),
            'upper left': (-0.22, 1.08),
            'lower left': (-0.22, -0.08),
            'center right': (1.22, 0.5),
            'center left': (-0.22, 0.5),
            'best': None,
        }
        bbox = bbox_map.get(legend_position, None)
        legend_kw = {
            'loc': legend_position, 'fontsize': fs,
            'framealpha': 0.95, 'title': 'Model Track', 'title_fontsize': fs + 1,
            'edgecolor': 'black', 'fancybox': True
        }
        if bbox is not None:
            legend_kw['bbox_to_anchor'] = bbox
...
with st.sidebar:
    ...
    if upload_mode == "Manual upload":
        ...
        fB = st.file_uploader("Model B CSV (Mistral 7B)", type="csv", key="fb")
        fC = st.file_uploader("Model C CSV (Qwen 14B)", type="csv", key="fc")
        fD = st.file_uploader("Model D CSV (Qwen 7B)", type="csv", key="fd")
        if fA:
            raw_data['modelA'] = parse_alloys_csv_cached(fA.getvalue().decode('utf-8'), 'modelA')
            st.cache_data.clear()
        if fB:
    ...
    with st.expander("Controls", expanded=False):
        ...
        show_chord_modelA = st.checkbox("Show Model A (Falcon 10B)", value=True, key="chord_show_a")
        show_chord_modelB = st.checkbox("Show Model B (Mistral 7B)", value=True, key="chord_show_b")
        show_chord_modelC = st.checkbox("Show Model C (Qwen 14B)", value=True, key="chord_show_c")
        show_chord_modelD = st.checkbox("Show Model D (Qwen 7B)", value=True, key="chord_show_d")
        ...
        st.markdown("<small><b>Chord Legend Display</b></small>", unsafe_allow_html=True)
        show_chord_legend = st.checkbox("Show Legend", value=True, key="chord_show_leg")
        chord_legend_pos = st.selectbox(
            "Legend position",
            ['lower right', 'upper right', 'upper left', 'lower left', 'center right', 'center left', 'best'],
            index=0,
            key="chord_leg_pos",
            help="Placement of the legend box relative to the diagram."
        )
    ...
    with st.expander("Controls", expanded=False):
        ...
        # Legend ON/OFF & Position
        st.markdown("<small><b>Circos Legend Display</b></small>", unsafe_allow_html=True)
        show_circos_legend = st.checkbox("Show Legend", value=True, key="circos_show_leg")
        circos_legend_pos = st.selectbox(
            "Legend position",
            ['upper right', 'lower right', 'upper left', 'lower left', 'center right', 'center left', 'best'],
            index=0,
            key="circos_leg_pos",
            help="Placement of the legend box relative to the diagram."
        )
...
# Context-limit banner for Model A
st.markdown("""
<div style="background: linear-gradient(135deg, #fef9e7 0%, #fdebd0 100%); border: 2px solid #f39c12; border-left: 5px solid #f39c12; padding: 1.2rem; border-radius: 8px; margin-bottom: 1.5rem; box-shadow: 0 2px 8px rgba(243,156,18,0.15);">
    <strong>⚠️ Context Limit Distinction — Model A (Falcon 10B)</strong><br>
    Model A was evaluated with a <strong>10,000-character</strong> input limit, while Models B, C, and D used <strong>50,000 characters</strong>. 
    This five-fold difference in available context affects extraction completeness — shorter contexts favor precision over recall. 
    Model A's lower counts should be interpreted in light of this constraint.
</div>
""", unsafe_allow_html=True)
...
for i, (key, meta) in enumerate(MODEL_META.items()):
    ...
    context_badge = f'<span style="display:inline-block; background-color:#f39c12; color:white; font-size:0.65rem; font-weight:800; padding:1px 6px; border-radius:3px; margin-left:6px; vertical-align:middle;">{meta["context_note"]}</span>' if meta.get("is_limited") else ""
    highlight_class = ["gold", "red", "green", "blue"][i]
    with cols[i]:
        st.markdown(f'<div class="metric-card highlight-{highlight_class}">'
            f'<strong>{meta["name"]}</strong>{context_badge}<br>'
...
if show_chord:
    with tabs[t_idx]:
        ...
        fig = draw_unified_chord(
            ...
            show_legend=show_chord_legend, legend_position=chord_legend_pos
        )
...
# ------------------------------------------------------------------
# TAB 5: CIRCOS (v2.4 — Enhanced Padding, Numbers Removed)
# ------------------------------------------------------------------
if show_circos:
    with tabs[t_idx]:
        st.markdown("### Circos-Style Radial Histogram")
        st.markdown("<div class='caption'>v2.4: enhanced label padding + buffer prevents text-image overlap. Smart rotation keeps labels readable. Inner bar count numbers removed.</div>", unsafe_allow_html=True)
        fig_circ = draw_circos_clean(
            ...
            show_legend=show_circos_legend, legend_position=circos_legend_pos
        )
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    ...
    ✅ Model A (Falcon 10B) with 10k-character context limit | 
    ...
    ✅ Chord & Circos Legend Position Control (7 positions) + ON/OFF Toggle | 
    ✅ Enhanced Circos Padding (label_padding + label_buffer prevents overlap) | 
    ✅ Circos Inner Numbers Removed | 
    ✅ 50+ Colormaps (turbo, jet, inferno, rainbow, plasma, magma, cividis, etc.)<br><br>
    <strong>Context Limit Note:</strong> Model A (Falcon 10B) results were obtained under a 10,000-character context limit; 
    Models B, C, and D used 50,000 characters. Direct extraction count comparisons should account for this five-fold difference.<br><br>
