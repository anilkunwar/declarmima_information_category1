"""
AlloyExtraction Nexus — Memory-Hardened, Publication-Grade Cross-Model Visualization v2.2
...
+ >50 Color Colormaps (Turbo/HSV) · Interactive Category Toggling (Morphology Control)
"""
...
# ------------------------------------------------------------------
st.set_page_config(
    page_title="AlloyExtraction Nexus — Memory-Hardened v2.2",
    page_icon="🔬",
    layout="wide",
...
# ------------------------------------------------------------------
# UNIFIED CHORD (w/ Colormap & Morphology Control)
# ------------------------------------------------------------------
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
    ...
    n_mats = len(all_mats)
    # Auto-upgrade to continuous colormap if tab20 is selected but N > 20
...
with st.sidebar:
    ...
    with st.expander("Controls", expanded=False):
        ...
        # NEW: Category Filter (Morphology Control)
        ...
        # NEW: Colormap Selector
        chord_cmap = st.selectbox(
            "🎨 Material Colormap", 
            ["turbo", "hsv", "gist_rainbow", "tab20", "viridis"], 
            index=0, 
            key="chord_cmap",
            help="'turbo', 'hsv', and 'gist_rainbow' support >50 distinct colors."
        )
...
# MAIN HEADER
# ------------------------------------------------------------------
st.markdown('<div class="main-header">AlloyExtraction Nexus v2.2</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Memory-Hardened • >50 Color Maps • Morphology Control</div>', unsafe_allow_html=True)
...
# ------------------------------------------------------------------
# TAB 1: UNIFIED CHORD (w/ Category Filtering)
# ------------------------------------------------------------------
if show_chord:
    with tabs[t_idx]:
        st.markdown("### Unified Multi-Model Chord Diagram")
        st.markdown("<div class='caption'>Use the <b>Toggle Categories</b> multiselect in the sidebar to erase specific nodes/edges and dynamically alter the diagram's morphology.</div>", unsafe_allow_html=True)
...
if show_circos:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>Clean radial layout. Outer rim uses the selected colormap (supports >50 colors). Concentric rings = model extraction counts.</div>", unsafe_allow_html=True)
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    <strong>v2.2 Upgrades:</strong> 
    ✅ Added >50 color continuous maps (Turbo, HSV, Rainbow) | 
    ✅ Interactive Category Toggles erase nodes/links to alter morphology | 
    ✅ Auto-fallback to continuous maps if tab20 is exceeded<br><br>
