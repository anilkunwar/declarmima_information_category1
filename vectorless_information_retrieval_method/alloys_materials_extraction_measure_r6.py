...
+ v2.4: 4 Label Alignment Modes (Horizontal/Radial/Tangential/Smart) + Legend Visibility Toggles
"""
...
# ------------------------------------------------------------------
# LABEL ALIGNMENT HELPER (v2.4)
# ------------------------------------------------------------------
def compute_label_alignment(angle_rad, mode='horizontal'):
    """
    Returns (rotation_deg, ha, va, rotation_mode) for a label at given angle.
    angle_rad: angle in radians (0 = right, pi/2 = top, pi = left, -pi/2 = bottom)
    mode: 'horizontal' | 'radial' | 'tangential' | 'smart'
    """
    theta = angle_rad
    deg = np.degrees(theta) % 360

    if mode == 'horizontal':
        # Always upright, quadrant-aware ha/va
        if 80 <= deg <= 100:
            ha, va = 'center', 'bottom'
        elif 260 <= deg <= 280:
            ha, va = 'center', 'top'
        ...
        # Text follows radius line, flipped on left side for readability
        needs_flip = 90 < deg < 270
        rot = (deg + 180) % 360 if needs_flip else deg
        ha = 'right' if needs_flip else 'left'
        va = 'center'
        return rot, ha, va, 'anchor'

    elif mode == 'tangential':
        # Text follows arc tangent (perpendicular to radius, CCW direction)
        rot_tan = (deg + 90) % 360
        needs_flip = 90 < rot_tan < 270
        if needs_flip:
            rot_tan = (rot_tan + 180) % 360
        ha = 'right' if needs_flip else 'left'
        va = 'center'
        return rot_tan, ha, va, 'anchor'

    elif mode == 'smart':
        # Hybrid: radial for sparse, horizontal for dense
        return compute_label_alignment(theta, 'radial')

    else:
        return compute_label_alignment(theta, 'horizontal')

# ------------------------------------------------------------------
# IMPROVED UNIFIED CHORD (v2.4 — 4 Label Modes + Legend Toggles)
# ------------------------------------------------------------------
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
                       ...
                       show_legend_modelB=True, show_legend_modelC=True,
                       show_legend_modelD=True, show_legend_consensus=True,
                       show_legend_doi=True):
    """
    v2.4 Chord diagram with:
    - 4 Label alignment modes: horizontal, radial, tangential, smart
    - Per-element legend visibility toggles
    ...
    for doi, ang in p_angles.items():
        ...
        rot, ha, va, rot_mode = compute_label_alignment(ang['mid'], label_mode)

        kw = {'rotation': rot, 'rotation_mode': rot_mode} if rot_mode else {'rotation': rot}
    ...
    # --- AXIS & LEGEND (v2.4: visibility toggles) ---
...
# ------------------------------------------------------------------
# CIRCOS (v2.3 — Label & Padding Fixes)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# SIDEBAR (v2.4 with Label Modes + Legend Toggles)
# ------------------------------------------------------------------
with st.sidebar:
    ...
    with st.expander("Controls", expanded=False):
        ...
        chord_label_mode = st.selectbox(
            "📐 Label Alignment",
            ["horizontal", "radial", "tangential", "smart"],
            index=0,
            key="chord_lbl_mode",
            help="horizontal=always upright | radial=follows radius line | tangential=along arc tangent | smart=auto"
        )

        # v2.4: Legend visibility toggles
        st.markdown("<small><b>Legend Visibility</b></small>", unsafe_allow_html=True)
        leg_modelB = st.checkbox("Show Model B", value=True, key="leg_B")
        leg_modelC = st.checkbox("Show Model C", value=True, key="leg_C")
        leg_modelD = st.checkbox("Show Model D", value=True, key="leg_D")
        leg_consensus = st.checkbox("Show Consensus", value=True, key="leg_cons")
        leg_doi = st.checkbox("Show DOI", value=True, key="leg_doi")
...
st.markdown('<div class="sub-header">Memory-Hardened • 50+ Colormaps • 4 Label Modes • Legend Toggles</div>', unsafe_allow_html=True)
...
for i, (key, meta) in enumerate(MODEL_META.items()):
    ...
    with cols[i]:
        st.markdown(f'<div class="metric-card highlight-{["red","green","blue"][i]}"><strong>{meta["name"]}</strong><br><span style="font-size:1.3rem">{n_doi}</span> docs | <span style="font-size:1.3rem">{n_mat}</span> unique concepts | <span style="font-size:1.3rem">{n_conn}</span> extractions</div>', unsafe_allow_html=True)
...
# ------------------------------------------------------------------
# TAB 1: UNIFIED CHORD (v2.4 — 4 Label Modes + Legend Toggles)
# ------------------------------------------------------------------
if show_chord:
    with tabs[t_idx]:
        st.markdown("### Unified Multi-Model Chord Diagram")
        st.markdown("<div class='caption'>v2.4: Choose label alignment (horizontal/radial/tangential/smart) and toggle legend items individually.</div>", unsafe_allow_html=True)
        ...
        fig = draw_unified_chord(
            ...
            show_legend_modelB=leg_modelB,
            show_legend_modelC=leg_modelC,
            show_legend_modelD=leg_modelD,
            show_legend_consensus=leg_consensus,
            show_legend_doi=leg_doi
        )
...
if show_circos:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>v2.3: smart label rotation (never upside-down), configurable padding, 50+ colormaps, better gap distribution.</div>", unsafe_allow_html=True)
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    ...
    ✅ 4 Label Alignment Modes (horizontal/radial/tangential/smart) | 
    ✅ Per-Element Legend Toggles (Models, Consensus, DOI) | 
    ✅ Fixed Chord spatial layout (gap padding, label offset, curve tension) | 
