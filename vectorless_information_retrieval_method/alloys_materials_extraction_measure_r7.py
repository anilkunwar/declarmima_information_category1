...
Enhanced Label Alignment for Chord & Circos Diagrams
- Separate DOI / Material label offsets
- Inclined label mode with custom angle
- Improved radial / tangential anchor handling
- All legend toggles default to True
+ Expanded prettification for HEAs/IMCs
"""
...
def prettify_material(name: str) -> str:
    """
    v2.6: Expanded with ground-truth mappings from PDF chord diagrams.
...
# ------------------------------------------------------------------
# ENHANCED LABEL ORIENTATION ENGINE (v2.6)
# ------------------------------------------------------------------
def get_label_transform(angle_rad, mode, incline_deg=0):
    """
    Returns (rotation_deg, ha, va) for a given angle and orientation mode.
    mode: 'radial', 'tangential', 'horizontal', 'vertical', 'inclined'
    incline_deg: additional rotation (only for 'inclined' mode)
    """
    ...
    if mode == 'horizontal':
        ...
        return 90, 'center', 'center'
    elif mode == 'tangential':
        rot = deg + 90
        if 90 < deg < 270:
            rot += 180
        return rot % 360, 'center', 'center'
    elif mode == 'inclined':
        # base radial rotation + incline offset
        rot = deg
        if 90 < deg < 270:
            rot += 180
        rot += incline_deg
        # choose anchor based on original radial direction
        if 90 < deg < 270:
            ha = 'right'
        else:
            ha = 'left'
        return rot % 360, ha, 'center'
    else:  # radial (default)
        rot = deg
        if 90 < deg < 270:
            rot += 180
...
# ------------------------------------------------------------------
# UNIFIED CHORD v2.6 — Enhanced Label Placement
# ------------------------------------------------------------------
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
                       ...
                       label_offset_doi=0.14, label_offset_mat=0.14,
                       curve_tension=0.35, label_mode='radial', incline_deg=0,
    ...
    if visible_doi:
        for doi, ang in p_angles.items():
            ...
            ax.add_patch(wdg)
            lr = radius + node_width + label_offset_doi
            x = lr * np.cos(ang['mid'])
            y = lr * np.sin(ang['mid'])
            alias = get_alias(doi)
            rot, ha, va = get_label_transform(ang['mid'], label_mode, incline_deg)
    ...
    for mat, ang in m_angles.items():
        ...
        wdg = Wedge((0, 0), radius, np.degrees(ang['start']), np.degrees(ang['end']),
                    ...
                    linewidth=0.8, alpha=0.9, zorder=5)
        ax.add_patch(wdg)
        lr = radius + node_width + label_offset_mat
        x = lr * np.cos(ang['mid'])
        y = lr * np.sin(ang['mid'])
    ...
    # --- AXIS & LEGEND ---
    max_offset = max(label_offset_doi, label_offset_mat) + node_width + radius + 0.25
...
# ------------------------------------------------------------------
# CIRCOS v2.6 — Enhanced Label Placement
# ------------------------------------------------------------------
def draw_circos_clean(all_models_data, figsize=(14, 14), inner_r=1.0,
                      ...
                      label_padding=0.25, label_mode='radial', incline_deg=0):
    figsize = (min(figsize[0], 24), min(figsize[1], 24))
    ...
    for i, mat in enumerate(mats_sorted):
        ...
        if show_labels:
            ...
            rot, ha, va = get_label_transform(ma, label_mode, incline_deg)
...
# SIDEBAR
# ------------------------------------------------------------------
LABEL_MODE_OPTIONS = ['radial', 'tangential', 'horizontal', 'vertical', 'inclined']
...
with st.sidebar:
    ...
    with st.expander("Controls", expanded=False):
        ...
        chord_lbl_off_doi = st.slider("DOI Label Offset", 0.05, 0.40, 0.14, 0.01, key="chord_lo_doi")
        chord_lbl_off_mat = st.slider("Material Label Offset", 0.05, 0.40, 0.14, 0.01, key="chord_lo_mat")
        ...
        chord_incline = st.slider("Inclination angle (deg)", -90, 90, 0, 5, key="chord_incline", disabled=(chord_label_mode != 'inclined'))

        st.markdown("**👁️ Legend Visibility (all default True)**")
    ...
    with st.expander("Controls", expanded=False):
        ...
        circos_incline = st.slider("Inclination angle (deg)", -90, 90, 0, 5, key="circos_incline", disabled=(circos_label_mode != 'inclined'))
...
st.markdown('<div class="sub-header">Enhanced Label Alignment • Inclined Mode • Separate Offsets • Expanded Prettification</div>', unsafe_allow_html=True)
...
if show_chord:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>v2.6: Separate DOI/Material label offsets, Inclined label mode. Use Legend Visibility to toggle models/consensus/DOI (all default True).</div>", unsafe_allow_html=True)
        ...
        fig = draw_unified_chord(
            ...
            cmap_name=chord_cmap, gap_padding=chord_gap,
            label_offset_doi=chord_lbl_off_doi, label_offset_mat=chord_lbl_off_mat,
            curve_tension=chord_tension, label_mode=chord_label_mode, incline_deg=chord_incline,
...
if show_circos:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>v2.6: Inclined label mode, separate label padding and orientation.</div>", unsafe_allow_html=True)
        fig_circ = draw_circos_clean(
            ...
            label_padding=circos_lbl_pad, label_mode=circos_label_mode, incline_deg=circos_incline
        )
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    ...
    ✅ Separate DOI / Material label offsets | 
    ✅ Inclined label mode with adjustable angle | 
    ✅ Enhanced radial/tangential anchor logic | 
    ✅ Legend visibility toggles default True | 
    ✅ Expanded prettification for HEAs, IMCs, and nitrides<br><br>
