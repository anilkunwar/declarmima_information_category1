"""
AlloyExtraction Nexus — Memory-Hardened, Publication-Grade Cross-Model Visualization v2.6
...
+ 50+ Color Colormaps · Interactive Category Toggling (Morphology Control)
+ v2.6: Precision Alignment Engine (Parallel/Vertical Inclined/Radial/Horizontal)
+ v2.6: Interactive Legend Filtering w/ Dynamic Consensus Recalculation
+ v2.6: Adaptive Circos Padding (prevents text-bar overlap)
+ v2.5: Expanded Prettification for Complex HEAs/IMCs from PDF Ground Truth
"""
...
# ------------------------------------------------------------------
st.set_page_config(
    page_title="AlloyExtraction Nexus — Memory-Hardened v2.6",
    page_icon="🔬",
    layout="wide",
...
TAXONOMY = {
    ...
    'Alloy / Composition': [
        ...
        'ti2cu_ti6al4v', 'cr0.5w0.3(v-nb-ta)0.2', 'cr0.4w0.5(zr-hf-nb)0.1',
        'mo0.1ti0.8(v-nb-ta)0.1', 'ti0.7(zr-hf-nb)0.3'
    ],
...
SYNONYM_MAP = {
    ...
    'bo.3ero.5al0.2n': 'b0.3er0.5al0.2n',
    ...
    'cr0.5w0.3(v-nb-ta)0.2': 'cr0.5w0.3(vnbta)0.2',
    'cr0.4w0.5(zr-hf-nb)0.1': 'cr0.4w0.5(zrhfnb)0.1',
    'mo0.1ti0.8(v-nb-ta)0.1': 'mo0.1ti0.8(vnbta)0.1',
    'ti0.7(zr-hf-nb)0.3': 'ti0.7(zrhfnb)0.3',
}
...
ALL_COLORMAPS = sorted(list(set([c for group in COLORMAP_CATALOG.values() for c in group])))
...
def prettify_material(name: str) -> str:
    """
    v2.5+: Expanded with ground-truth mappings from PDF chord diagrams.
    Handles complex HEAs, IMCs, and subscript formatting.
    """
    overrides = {
        ...
        'cr0.5w0.3(vnbta)0.2': 'Cr₀.₅W₀.₃(V-Nb-Ta)₀.₂',
        'cr0.4w0.5(zrhfnb)0.1': 'Cr₀.₄W₀.₅(Zr-Hf-Nb)₀.₁',
        'mo0.1ti0.8(vnbta)0.1': 'Mo₀.₁Ti₀.₈(V-Nb-Ta)₀.₁',
        'ti0.7(zrhfnb)0.3': 'Ti₀.₇(Zr-Hf-Nb)₀.₃',
        'ti-au': 'Ti-Au',
        'ti-cr': 'Ti-Cr',
        'cu-mn': 'Cu-Mn',
        'cu6sn5': 'Cu₆Sn₅',
        ...
        'al2cu': 'Al₂Cu',
        'ti6al4v': 'Ti-6Al-4V',
        'alsimgzr': 'Al-Si-Mg-Zr',
        'aln': 'AlN',
    }
    if name in overrides:
        return overrides[name]

    # Fallback: intelligent element parsing
    s = name.lower()
    parts = []
    i = 0
    while i < len(s):
...
def compute_umap_embedding_cached(features_tuple, n_neighbors, min_dist, metric):
    ...
    reducer = umap.UMAP(n_components=3, n_neighbors=nn, min_dist=min_dist, metric=metric, random_state=42)
...
def get_network_html_cached(display_data_json):
    ...
    net.barnes_hut(gravity=-9000, central_gravity=0.35, spring_length=140, spring_strength=0.04, damping=0.09)
    ...
    for doi in sorted(all_papers):
        ...
        net.add_node(doi, label=alias, title=f"DOI: {doi}", shape='box',
                     color={'background': '#E8F4FD', 'border': '#2980B9'}, borderWidth=2,
                     font={'size': 14, 'face': 'arial', 'color': '#1a1a2e'}, size=18)

    for mat in sorted(all_materials):
        cat = get_category(mat)
        fill = CATEGORY_COLORS.get(cat, '#FFFFFF')
        net.add_node(mat, label=get_mat_label(mat), shape='dot',
                     color={'background': fill, 'border': '#555555'}, borderWidth=2,
                     font={'size': 12, 'color': '#2c3e50'}, size=14, title=f"Category: {cat}")
...
def render_matplotlib_figure(fig, filename, dpi):
    ...
    try:
        ...
        buf.seek(0)
        st.download_button(f"⬇️ Download {filename}", buf, file_name=filename, mime="image/png", key=f"dl_{filename.replace('.', '_')}")
...
# ------------------------------------------------------------------
# PRECISION ALIGNMENT ENGINE (v2.6)
# ------------------------------------------------------------------
def get_label_transform(angle_rad, mode):
    """
    v2.6 Precision Alignment Engine.
    Returns (rotation_deg, ha, va) with mathematically guaranteed node correspondence.
    
    Modes:
      - radial:           Text points outward. Flipped on left hemisphere.
      - parallel:         Text tangent to arc. Always reads left-to-right.
      - vertical_inclined: Text perpendicular to radial (90° spoke). Balanced appearance.
    ...
    if mode == 'horizontal':
    ...
    elif mode == 'vertical_inclined':
        # Perpendicular to radial axis — creates balanced "spoke" labels
        rot = deg + 90.0
        # Flip so text never reads bottom-to-top
        if 90 < deg < 270:
            rot += 180.0
        return rot, 'center', 'center'

    elif mode == 'parallel':
        # Tangent to the arc at this angle
        rot = deg + 90.0
        # Ensure left-to-right readability on both hemispheres
        if 90 < deg < 270:
            rot += 180.0
        return rot, 'center', 'center'
        ...
        if 90 < deg < 270:
            ...
            ha = 'left'
        return rot, ha, 'center'


# ------------------------------------------------------------------
# UNIFIED CHORD v2.6 — Precision Alignment + Interactive Legend
# ------------------------------------------------------------------
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
                       ...
                       label_mode='radial',
                       visible_models=None, visible_consensus=True, visible_doi=True):
    """
    v2.6 Chord Diagram with:
    - Precision alignment engine (radial/parallel/vertical_inclined/horizontal)
    - Interactive legend filtering with dynamic consensus recalculation
    - Trigonometric anchor positioning for perfect label-node correspondence
    - All memory hardening preserved
    """
    figsize = (min(figsize[0], 24), min(figsize[1], 24))

    if visible_models is None:
        visible_models = set(MODEL_META.keys())

    # Filter data based on visible models
    filtered_data = {}
    for mk, data in all_models_data.items():
        if mk in visible_models:
    ...
    if visible_doi:
        for doi, ang in p_angles.items():
            wdg = Wedge((0, 0), radius, np.degrees(ang['end']), np.degrees(ang['start']),
                        width=node_width, facecolor=doi_color, edgecolor='black',
                        linewidth=0.8, alpha=0.9, zorder=5)
            ax.add_patch(wdg)

            # v2.6: Precision anchor positioning
            lr = radius + node_width + label_offset
            x = lr * np.cos(ang['mid'])
            y = lr * np.sin(ang['mid'])
            alias = get_alias(doi)
            rot, ha, va = get_label_transform(ang['mid'], label_mode)
    ...
    for mat, ang in m_angles.items():
        fill = mat_colors[mat]
        wdg = Wedge((0, 0), radius, np.degrees(ang['start']), np.degrees(ang['end']),
    ...
    for doi, mat, count, consensus, models in unique_connections:
        ...
        # Dynamic consensus: only count visible models toward consensus threshold
        is_consensus = consensus >= 2
        if is_consensus and not visible_consensus:
            continue
...
def draw_sankey(paper_materials, model_name, model_color):
    ...
    fig = go.Figure(data=[go.Sankey(
        node=dict(label=node_labels, color=node_colors, pad=18, thickness=22, line=dict(color='black', width=0.6)),
        link=dict(source=sources, target=targets, value=values, color=[hex_to_rgba(c, 0.25) for c in link_colors])
    )])
    fig.update_layout(title_text=f"{model_name} — Paper → Material Flow", font_size=12,
        ...
        for j in range(len(all_materials)):
            if matrix[i, j] > 0:
                ax.text(j, i, int(matrix[i, j]), ha="center", va="center",
                        color="white" if matrix[i, j] > 1.5 else "black", fontweight='bold', fontsize=9)
...
# ------------------------------------------------------------------
# CIRCOS v2.6 — Precision Alignment + Adaptive Padding
# ------------------------------------------------------------------
def draw_circos_clean(all_models_data, figsize=(14, 14), inner_r=1.0,
                      ...
                      label_padding=0.25, label_rotation_mode='radial'):
    """
    v2.6 Circos with:
    - Precision alignment engine (radial/parallel/vertical_inclined/horizontal)
    - Adaptive padding: max(user_value, track_extent * 0.15) prevents text-bar overlap
    - rotation_mode='anchor' on all polar text for exact node correspondence
    - All memory hardening preserved
    """
    figsize = (min(figsize[0], 24), min(figsize[1], 24))
    ...
    # v2.6: Adaptive padding — guarantee clearance regardless of bar scale
    track_extent = inner_r + n_tracks * track_h
    effective_padding = max(label_padding, track_extent * 0.15)
    outer_extent = track_extent + effective_padding + 0.1
    ...
    for i, mat in enumerate(mats_sorted):
        ...
        for ti, (mk, meta) in enumerate(MODEL_META.items()):
            ...
            ax.bar(ma, h, width=step * 0.85, bottom=bottom,
                   color=meta['color'], alpha=0.92, edgecolor='black', linewidth=0.4, zorder=3)
            if cnt >= 2 and h > track_h * 0.35:
                ax.text(ma, bottom + h / 2, str(cnt), ha='center', va='center',
                        fontsize=max(fs - 2, 7), color='white', fontweight='bold', zorder=4)

        # Outer rim indicator
        rim_bottom = track_extent + 0.04
        ...
        if show_labels:
            ...
            # v2.6: Use effective_padding for guaranteed clearance
            label_r = rim_bottom + 0.08 + effective_padding
            rot, ha, va = get_label_transform(ma, label_rotation_mode)
    ...
    handles = [mpatches.Patch(facecolor=meta['color'], edgecolor='black', label=meta['short']) for meta in MODEL_META.values()]
...
def build_umap_features_cached(all_models_data_json):
    ...
    for mat in all_mats:
        ...
        n_dois = len({doi for mk, data in all_models_data.items() for doi, mats in data.items() if mat in mats})
    ...
    return {'features': features, 'labels': labels, 'categories': categories,
            'presence': presence_list, 'totals': totals, 'doi_counts': doi_counts}
...
def draw_umap_3d(all_models_data, n_neighbors=5, min_dist=0.3, metric='euclidean'):
    ...
    df = pd.DataFrame({
        ...
        'Material': [get_mat_label(m) for m in labels], 'RawMaterial': labels,
        'Category': categories, 'TotalCount': totals, 'DOIs': doi_counts,
        'InB': [p[0] for p in presence], 'InC': [p[1] for p in presence], 'InD': [p[2] for p in presence],
    })
    color_map = {cat: CATEGORY_COLORS.get(cat, '#cccccc') for cat in set(categories)}
    fig = px.scatter_3d(df, x='UMAP1', y='UMAP2', z='UMAP3', color='Category', size='TotalCount',
                        hover_data=['RawMaterial', 'TotalCount', 'DOIs', 'InB', 'InC', 'InD'],
                        color_discrete_map=color_map, title='3D UMAP Material Embedding', opacity=0.9)
    ...
    fig.update_layout(scene=dict(xaxis_title='UMAP 1', yaxis_title='UMAP 2', zaxis_title='UMAP 3',
                                 camera=dict(eye=dict(x=1.6, y=1.6, z=1.3)), aspectmode='cube'),
                      paper_bgcolor='white', plot_bgcolor='white', margin=dict(l=0, r=0, t=50, b=0),
...
def build_animation_data(all_models_data):
    ...
    for mk, meta in MODEL_META.items():
        ...
        for mat, cnt in mat_counts.items():
            rows.append({'Frame': meta['short'], 'Model': meta['name'], 'Material': get_mat_label(mat),
                         'Count': cnt, 'Category': mat_cats[mat], 'LogCount': np.log1p(cnt)})
...
def build_consensus_animation_data(all_models_data):
    ...
    for level in [1, 2, 3]:
        for mat, ag in mat_agreement.items():
            if ag >= level:
                rows.append({'Frame': f'≥{level} Model{"s" if level > 1 else ""}', 'Material': get_mat_label(mat),
                             'Category': get_category(mat), 'MaxCount': mat_maxcnt[mat], 'Agreement': ag})
    return pd.DataFrame(rows)
...
# SIDEBAR
# ------------------------------------------------------------------
LABEL_MODE_OPTIONS = ['radial', 'parallel', 'vertical_inclined', 'horizontal']
...
with st.sidebar:
    ...
    with st.expander("✏️ DOI Aliases", expanded=False):
        ...
        if filtered_dois:
            doi_df = pd.DataFrame({"DOI": filtered_dois,
    ...
    with st.expander("✏️ Material Labels", expanded=False):
        ...
        if filtered_mats:
            mat_df = pd.DataFrame({"Raw Material": filtered_mats,
    ...
    with st.expander("Controls", expanded=False):
        ...
        # v2.6: Precision Alignment Mode Selector
        chord_label_mode = st.selectbox("🔄 Label Orientation", LABEL_MODE_OPTIONS, index=0, key="chord_lm",
            help="Radial=outward from center. Parallel=tangent to arc (reads L→R). Vertical Inclined=90° spoke. Horizontal=always upright.")

        # v2.6: Interactive Legend Filtering (all default checked)
        st.markdown("**👁️ Legend Visibility**")
        vis_modelB = st.checkbox("Model B (Mistral 7B)", value=True, key="vis_mb")
        vis_modelC = st.checkbox("Model C (Qwen 14B)", value=True, key="vis_mc")
        vis_modelD = st.checkbox("Model D (Qwen 7B)", value=True, key="vis_md")
        vis_consensus = st.checkbox("Consensus (≥2 models)", value=True, key="vis_con")
        vis_doi = st.checkbox("Document (DOI)", value=True, key="vis_doi")

        all_cats = sorted(list(TAXONOMY.keys()) + ['Other'])
        selected_cats = st.multiselect("📂 Toggle Categories (Erase Nodes/Edges)", options=all_cats, default=all_cats, key="chord_cat_filter")
        chord_cmap = st.selectbox("🎨 Material Colormap", ALL_COLORMAPS,
            index=ALL_COLORMAPS.index('turbo') if 'turbo' in ALL_COLORMAPS else 0, key="chord_cmap")
    ...
    with st.expander("Controls", expanded=False):
        ...
        circos_lbl_pad = st.slider("Label padding", 0.1, 0.8, 0.25, 0.05, key="circos_lp",
            help="Minimum gap between outer rim and labels. Auto-increased if bars extend close to labels.")

        # v2.6: Precision Alignment Mode Selector for Circos
        circos_label_mode = st.selectbox("🔄 Label Orientation", LABEL_MODE_OPTIONS, index=0, key="circos_lm",
            help="Radial=outward. Parallel=tangent to rim. Vertical Inclined=90° spoke. Horizontal=always upright.")

        circos_cmap = st.selectbox("🎨 Material Colormap", ALL_COLORMAPS,
            index=ALL_COLORMAPS.index('viridis') if 'viridis' in ALL_COLORMAPS else 0, key="circos_cmap")
    ...
    with st.expander("🩺 System Diagnostics", expanded=False):
        ...
        st.markdown(f"""<div style="font-family:monospace; font-size:0.85rem;">
            ...
            <span class="diag-metric">💾 Cache: Auto-managed (TTL + max_entries)</span></div>""", unsafe_allow_html=True)
        if mem_warn:
...
display_data = filter_data_by_validation_cached(
    tuple((k, tuple(sorted(v.items()))) for k, v in raw_data_plain.items()), materials_only, enable_validator)

# ------------------------------------------------------------------
# MAIN HEADER
# ------------------------------------------------------------------
st.markdown('<div class="main-header">AlloyExtraction Nexus v2.6</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Precision Alignment • Parallel/Vertical Labels • Interactive Legend • Adaptive Padding</div>', unsafe_allow_html=True)
...
for i, (key, meta) in enumerate(MODEL_META.items()):
    ...
    with cols[i]:
        st.markdown(f'''<div class="metric-card highlight-{['red','green','blue'][i]}">
            ...
            <span style="font-size:1.3rem">{n_conn}</span> extractions</div>''', unsafe_allow_html=True)
st.markdown("---")
...
# ------------------------------------------------------------------
# TAB 1: UNIFIED CHORD (v2.6)
# ------------------------------------------------------------------
if show_chord:
    with tabs[t_idx]:
        st.markdown("### Unified Multi-Model Chord Diagram")
        st.markdown("<div class='caption'>v2.6: <b>Precision Alignment Engine</b> guarantees label-node correspondence. Use <b>Legend Visibility</b> to toggle models/consensus/DOI. Consensus recalculates dynamically.</div>", unsafe_allow_html=True)

        # Build visible models set from checkbox state
        visible_models = set()
        if vis_modelB: visible_models.add('modelB')
        if vis_modelC: visible_models.add('modelC')
        if vis_modelD: visible_models.add('modelD')
        ...
        fig = draw_unified_chord(
            ...
            cmap_name=chord_cmap, gap_padding=chord_gap, label_offset=chord_lbl_off,
            curve_tension=chord_tension, label_mode=chord_label_mode,
            visible_models=visible_models, visible_consensus=vis_consensus, visible_doi=vis_doi
        )
...
if show_sankey:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>Per-model paper-to-material flow.</div>", unsafe_allow_html=True)
        if not HAS_PLOTLY:
    ...
    with tabs[t_idx]:
        ...
        if not HAS_PYVIS:
            ...
            if html:
                ...
                st.download_button("⬇️ Download Network HTML", html.encode('utf-8'), "alloy_network.html", "text/html")
                del html
                gc.collect()
    t_idx += 1
...
if show_circos:
    with tabs[t_idx]:
        ...
        st.markdown("<div class='caption'>v2.6: <b>Adaptive padding</b> prevents text-bar overlap. <b>Precision alignment</b> with parallel/vertical/radial/horizontal modes.</div>", unsafe_allow_html=True)
        fig_circ = draw_circos_clean(
            ...
            label_padding=circos_lbl_pad, label_rotation_mode=circos_label_mode
        )
...
if show_umap:
    with tabs[t_idx]:
        ...
        if not HAS_UMAP or not HAS_SKLEARN:
            st.warning("UMAP or scikit-learn missing.")
        else:
            fig_umap, err = draw_umap_3d(display_data, n_neighbors=umap_neighbors, min_dist=umap_dist, metric=umap_metric)
            if err:
                st.error(err)
            elif fig_umap:
                st.plotly_chart(fig_umap, use_container_width=True)
                html_str = fig_umap.to_html(include_plotlyjs='cdn')
                st.download_button("⬇️ Download UMAP HTML", html_str.encode('utf-8'), "umap_3d.html", "text/html")
            ...
            if anim_type == "Model Sweep":
                ...
                if df_anim.empty:
                    ...
                    fig_anim = px.scatter(df_anim, x='Material', y='Count', animation_frame='Frame',
                                          color='Category', size='Count', color_discrete_map=cat_color_map,
                                          range_y=[0, ymax], title='Model Extraction Landscape', height=650)
                ...
                if df_cons.empty:
                    ...
                    fig_cons = px.scatter(df_cons, x='Material', y='MaxCount', animation_frame='Frame',
                                          color='Category', size='MaxCount', color_discrete_map=cat_color_map,
                                          range_y=[0, ymax], title='Consensus Buildup', height=650)
...
if show_validation:
    with tabs[t_idx]:
        ...
        for mat in sorted(all_materials):
            ...
            val_rows.append({'Term': mat, 'Display Label': get_mat_label(mat),
                'Is Material': '✅ Yes' if res['is_material'] else '❌ No', 'Method': res['method'], 'Reason': res['reason']})
...
if show_legend_panel:
    with tabs[t_idx]:
        ...
        legend_df = pd.DataFrame([{"Alias": get_alias(doi), "DOI": doi, "Models Found In": ", ".join([
            MODEL_META[k]['short'] for k in display_data if doi in display_data[k]])} for doi in all_dois]).drop_duplicates(subset=['Alias'])
        st.dataframe(legend_df, use_container_width=True, height=500)
        alias_json = json.dumps(st.session_state.doi_aliases, indent=2)
        st.download_button("⬇️ Export Aliases JSON", alias_json.encode('utf-8'), "doi_aliases.json", "application/json")
        ...
        if uploaded_aliases is not None:
            ...
            st.success("Aliases imported! Click Rerun to refresh.")
...
<div style="text-align:center; color:#666; font-size:0.88rem; padding:1rem 0; font-family:serif;">
    <strong>v2.6 Upgrades:</strong> 
    ✅ Precision Alignment Engine (trigonometric anchor positioning) | 
    ✅ New label modes: Parallel (tangent) &amp; Vertical Inclined (90° spoke) | 
    ✅ Interactive legend filtering with dynamic consensus recalculation | 
    ✅ Adaptive Circos padding prevents text-bar overlap at any scale<br><br>
    <strong>Memory Hardening:</strong> All expensive operations wrapped in <code>@st.cache_data</code> / 
    <code>@st.cache_resource(max_entries=3)</code>. Figures force-closed via <code>plt.close()</code> + <code>gc.collect()</code>.
