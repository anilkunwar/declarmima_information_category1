...
def draw_unified_chord(all_models_data, figsize=(16, 16), radius=1.0,
    ...
    for (doi, mat), models in pair_models.items():
        ...
        # Respect consensus toggle
        if consensus >= 2 and not show_models.get('consensus', True):
            # If consensus hidden, only show if a single selected model would still show it
            pass  # Keep it but color as single model
    ...
    def compute_label_transform(angle_rad, mode, is_paper=True):
        ...
        if mode == 'radial':
            # Text points outward from center; flip left side for readability
            if 90 < rot_deg < 270:
                text_rot = rot_deg - 180
                ha = 'right' if rot_deg < 180 else 'left'
                va = 'center'
            else:
                text_rot = rot_deg
                ha = 'left' if rot_deg < 90 else 'right'
                va = 'center'
            # Fine-tune for top/bottom center
            if 80 <= rot_deg <= 100:
                ha, va = 'center', 'bottom'
            elif 260 <= rot_deg <= 280:
                ha, va = 'center', 'top'
            return text_rot, ha, va, 0, 0

        elif mode == 'parallel':
            # Text follows arc tangent (perpendicular to radius)
            ...
            if tangent > 90 or tangent < -90:
                tangent += 180
                if tangent > 180:
                    tangent -= 360
            # Alignment: for top arc, text sits below the arc; for bottom, above
            if is_paper:
                if 45 <= rot_deg <= 135:
                    ha, va = 'center', 'bottom'
                elif rot_deg < 45:
                ...
                if 225 <= rot_deg <= 315 or (-135 <= rot_deg <= -45):
                    ha, va = 'center', 'top'
                elif rot_deg > 315 or rot_deg < -135:
                    ha, va = 'left', 'center'
                else:
                    ha, va = 'right', 'center'
            return tangent, ha, va, 0, 0

        elif mode == 'vertical':
            # Always upright horizontal
            if is_paper:
                if 80 <= rot_deg <= 100:
                    ha, va = 'center', 'bottom'
                elif 260 <= rot_deg <= 280:
            ...
            # Fixed 30° slant, direction depends on side
            if is_paper:
                if rot_deg < 90:
                    text_rot = 30
                    ha, va = 'left', 'bottom'
    ...
    for doi, mat, count, consensus, models in unique_connections:
        if doi not in p_angles or mat not in m_angles:
            continue
        # Skip if DOI nodes hidden
        if not show_models.get('doi_nodes', True):
            continue
        # Determine if any visible model contributes
        ...
        # Color logic respecting toggles
        if consensus >= 2 and show_models.get('consensus', True):
            color = '#F39C12'
            alpha = 0.9
        else:
            # Use first visible model's color
...
def draw_circos_clean(all_models_data, figsize=(14, 14), inner_r=1.0,
    ...
    # v2.4: outer_extent now includes label_buffer for extra clearance
    ...
    for i, mat in enumerate(mats_sorted):
        ...
        if show_labels:
            ...
            # v2.4: label radius starts after rim + padding + buffer
