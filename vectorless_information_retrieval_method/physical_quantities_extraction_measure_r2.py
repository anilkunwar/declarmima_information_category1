"""
Enhanced Streamlit App: Cross-Model Physical Quantity Extraction Comparison
Publication-quality figures with full editability controls
"""
...
<style>
    ...
    .control-section { background: #f0f2f6; padding: 0.8rem; border-radius: 8px; margin-bottom: 0.8rem; }
</style>
...
with st.sidebar:
    with st.expander("🎨 Figure B Controls", expanded=False):
        ...
        figB_width = st.slider("Width (inches)", 8, 24, 14, key="fb_w")
        figB_height = st.slider("Height (inches)", 4, 16, 7, key="fb_h")
        
        # -- Bar Appearance --
        st.markdown("**📊 Bar Appearance**")
        bar_width = st.slider("Bar width", 0.05, 0.5, 0.22, 0.01, key="bar_w")
        bar_alpha = st.slider("Bar opacity", 0.3, 1.0, 0.9, 0.05, key="bar_a")
        bar_edge = st.slider("Edge linewidth", 0.0, 2.0, 0.7, 0.1, key="bar_e")
        show_bar_labels = st.checkbox("Show value labels on bars", value=True, key="bar_lbl")
        bar_label_size = st.slider("Label font size", 5, 14, 7, key="bar_lbl_sz")
        
        # -- Font Controls --
        st.markdown("**🔤 Font Controls**")
        xlabel_font = st.slider("X-axis label size", 8, 20, 13, key="xlf")
        ylabel_font = st.slider("Y-axis label size", 8, 20, 13, key="ylf")
        xtick_font = st.slider("X-tick label size", 6, 16, 9, key="xtf")
        ytick_font = st.slider("Y-tick label size", 6, 16, 10, key="ytf")
        title_font = st.slider("Title size", 10, 24, 14, key="tf")
        legend_font = st.slider("Legend size", 6, 16, 10, key="legf")
        cat_label_font = st.slider("Category label size", 8, 18, 10, key="catf")
        ...
        grid_alpha = st.slider("Grid opacity", 0.0, 1.0, 0.3, 0.05, key="ga")
        grid_style = st.selectbox("Grid style", ["--", "-", ":", "-."], index=0, key="gs")
        
        # -- Legend Controls --
        st.markdown("**📋 Legend**")
        legend_pos = st.selectbox("Legend position", 
            ["upper right", "upper left", "lower right", "lower left", "best", "center"], index=0, key="lp")
        ...
        st.markdown("**🎨 Category Shading**")
        show_cat_bg = st.checkbox("Show category backgrounds", value=True, key="scb")
        cat_bg_alpha = st.slider("Background alpha", 0.0, 1.0, 0.5, 0.05, key="cba")
        
        # -- Spines & Box --
        st.markdown("**📦 Figure Box**")
...
with st.sidebar:
    with st.expander("🎨 Figure C Controls", expanded=False):
        ...
        # -- Figure Dimensions --
        st.markdown("**📐 Figure Dimensions**")
        figC_width = st.slider("Width (inches)", 8, 24, 16, key="fc_w")
        figC_height = st.slider("Height (inches)", 4, 16, 7, key="fc_h")
        panel_ratio = st.slider("Left/Right panel ratio", 0.5, 3.0, 1.4, 0.1, key="pr")
        
        # -- Composition Bar Controls --
        st.markdown("**📊 Composition Bars**")
        barC_height = st.slider("Bar height", 0.2, 1.0, 0.55, 0.05, key="bch")
        barC_edge = st.slider("Edge linewidth", 0.0, 2.0, 0.8, 0.1, key="bce")
        show_pct_labels = st.checkbox("Show percentage labels", value=True, key="spct")
        pct_font_size = st.slider("Percent label size", 6, 16, 10, key="pfs")
        show_total_label = st.checkbox("Show total count", value=True, key="stl")
        total_font_size = st.slider("Total label size", 6, 16, 11, key="tfs")
        ...
        radar_linewidth = st.slider("Radar line width", 0.5, 5.0, 2.5, 0.1, key="rlw")
        radar_markersize = st.slider("Marker size", 2, 12, 6, key="rms")
        radar_fill_alpha = st.slider("Fill alpha", 0.0, 0.5, 0.1, 0.01, key="rfa")
        radar_grid_alpha = st.slider("Grid alpha", 0.0, 1.0, 0.3, 0.05, key="rga")
        radar_legend_bbox = st.slider("Legend X offset", 1.0, 2.0, 1.3, 0.05, key="rlbx")
        
        # -- Font Controls --
        st.markdown("**🔤 Font Controls**")
        comp_ylabel_font = st.slider("Y-label size", 8, 20, 12, key="cylf")
        comp_ytick_font = st.slider("Y-tick size", 8, 20, 12, key="cytf")
        comp_xlabel_font = st.slider("X-label size", 8, 20, 12, key="cxlf")
        comp_title_font = st.slider("Title size", 10, 24, 14, key="ctf")
        comp_legend_font = st.slider("Legend size", 6, 16, 9, key="clegf")
        
        # -- Annotation --
        st.markdown("**💬 Annotation**")
        show_annotation = st.checkbox("Show Mistral annotation", value=True, key="sa")
        annot_fontsize = st.slider("Annotation size", 6, 14, 9, key="afs")
        ...
        comp_grid_alpha = st.slider("Grid opacity", 0.0, 1.0, 0.3, 0.05, key="cga")
...
if show_figB and len(main_models) >= 2:
    ...
    if not consensus_terms:
        ...
        ax_b.legend(loc=legend_pos, fontsize=legend_font, 
                   framealpha=legend_alpha, edgecolor="black" if legend_frame else "none",
                   fancybox=legend_frame)
        ax_b.spines["top"].set_visible(show_top)
        ax_b.spines["right"].set_visible(show_right)
        ax_b.spines["bottom"].set_visible(show_bottom)
        ax_b.spines["left"].set_visible(show_left)
...
if show_figC and len(main_models) >= 2:
    ...
    for i, m in enumerate(main_models):
        ...
        if solo_vals[i] > 0 and show_pct_labels:
            ...
            ax1.text(solo_vals[i]/2, i, f"{solo_vals[i]}\n({pct:.0f}%)", 
                    ...
                    color="white" if solo_color.lower() > "#888888" else "#333333", 
                    fontweight="bold")
    ...
    ax1.set_yticklabels(short_names, fontsize=comp_ytick_font, fontweight="bold")
    ax1.set_xlabel("Number of Extracted Terms", fontsize=comp_xlabel_font, fontweight="bold")
    ax1.legend(loc="lower right", fontsize=comp_legend_font, 
               framealpha=0.95, edgecolor="black")
    ax1.spines["top"].set_visible(comp_show_top)
    ax1.spines["right"].set_visible(comp_show_right)
    if comp_show_grid:
