...
if show_figC and len(main_models) >= 2:
    ...
    short_names = [LEGEND_LABELS_C.get(m.split("(")[1].replace(")", "").strip(), m.split("(")[1].replace(")", "") if "(" in m else m) for m in main_models]
...
if show_upset and len(main_models) >= 2:
    ...
    for combo, terms in intersection_items:
        label = " ∩ ".join([LEGEND_LABELS_C.get(c.split("(")[1].replace(")", "").strip(), c.split("(")[1].replace(")", "") if "(" in c else c) for c in sorted(combo)])
    ...
    ax_u.set_yticklabels([LEGEND_LABELS_C.get(m.split("(")[1].replace(")", "").strip(), m.split("(")[1].replace(")", "") if "(" in m else m) for m in main_models], 
